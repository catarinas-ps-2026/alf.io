...
#show: ieee.with(
  ...
  abstract: [
    This paper presents the implementation of a comprehensive testing strategy for alf.io, an open-source event ticketing platform. The project involved designing and executing tests across four levels: unit, integration, system, and acceptance testing. The testing covered the entire technology stack including a Java/Spring Boot backend, Angular public frontend, and Lit-based administration interface. Automated testing through GitHub Actions CI/CD pipelines achieved 100% pass rates across unit tests (2,290 cases), integration tests (426 cases: 370 JUnit + 56 Playwright), and acceptance tests (22 cases), with an aggregated code coverage of 85.76%. Integration tests validated 30 mandatory API endpoints with deep database-level assertions and identified semantic defects in API responses. Performance testing with K6 validated system behavior under load, fuzzing with Jazzer confirmed input handling robustness, and static analysis with SonarQube identified code quality improvements. The results demonstrate the effectiveness of multi-layered testing approaches for ensuring quality in complex web applications.
  ],
  authors: (
    (
