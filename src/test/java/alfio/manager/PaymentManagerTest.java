...
class PaymentManagerTest {
    ...
    void init() {
        ...
        paymentManager = new PaymentManager(null, null, null, null, null, List.of(stripe, mollie));
    }

    @Test
    void validSelection() {
        doReturn(EnumSet.of(StaticPaymentMethods.CREDIT_CARD).stream()
                        .map(paymentMethod -> (PaymentMethod) paymentMethod)
                        .collect(Collectors.toSet()))
                .when(stripe)
                .getSupportedPaymentMethods(any(), any());

        doReturn(EnumSet.of(StaticPaymentMethods.IDEAL)).when(mollie).getSupportedPaymentMethods(any(), any());
        assertTrue(paymentManager
                .validateSelection(List.of(PaymentProxy.STRIPE, PaymentProxy.MOLLIE), 1)
                .isEmpty());
    }

    @Test
    void selectionConflict() {
        doReturn(EnumSet.of(StaticPaymentMethods.CREDIT_CARD)).when(stripe).getSupportedPaymentMethods(any(), any());
        doReturn(EnumSet.of(StaticPaymentMethods.CREDIT_CARD, StaticPaymentMethods.IDEAL))
                .when(mollie)
                .getSupportedPaymentMethods(any(), any());
        List<Map.Entry<PaymentMethod, Set<PaymentProxy>>> entries =
                paymentManager.validateSelection(List.of(PaymentProxy.STRIPE, PaymentProxy.MOLLIE), 1);
        assertFalse(entries.isEmpty());
        assertEquals(1, entries.size());
        assertSame(StaticPaymentMethods.CREDIT_CARD, entries.get(0).getKey());
        assertEquals(
                EnumSet.of(PaymentProxy.STRIPE, PaymentProxy.MOLLIE),
    ...
    void paymentProxyDeniedListMatchesFullText() {
        var orgId = 1;
        var inputCustomPaymentMethods = List.of(new UserDefinedOfflinePaymentMethod(
        ...
        doReturn(new HashSet<PaymentMethod>(inputCustomPaymentMethods))
                .when(customOffline)
                .getSupportedPaymentMethods(any(), any());
        ...
        doReturn(EnumSet.of(StaticPaymentMethods.BANK_TRANSFER).stream()
                        .map(paymentMethod -> (PaymentMethod) paymentMethod)
                        .collect(Collectors.toSet()))
                .when(bankTransfer)
                .getSupportedPaymentMethods(any(), any());
        ...
        var maybeConfig = MaybeConfigurationBuilder.existing(
                ConfigurationKeys.PAYMENT_METHODS_BLACKLIST, PaymentProxy.CUSTOM_OFFLINE.name());

        when(configurationManager.getFor(eq(ConfigurationKeys.PAYMENT_METHODS_BLACKLIST), any()))
                .thenReturn(maybeConfig);
        var paymentManagerForCustomOffline =
                new PaymentManager(null, configurationManager, null, null, null, List.of(customOffline, bankTransfer));
