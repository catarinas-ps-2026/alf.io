...
class EventManagerIntegrationTest extends BaseIntegrationTest {
    ...
    void testUnboundedTicketsGeneration() {
        ...
        BaseIntegrationTest.testTransferEventToAnotherOrg(
                event.getId(), event.getOrganizationId(), eventAndUsername.getRight(), jdbcTemplate);
    }
    ...
    void testUnboundedEventGeneration() {
        ...
        assertEquals(
                AVAILABLE_SEATS,
                tickets.stream().filter(t -> t.getCategoryId() == null).count());
    ...
    void testAddUnboundedCategoryShrinkBoundedCategory() {
        ...
        int categoryId = ticketCategoryRepository
                .findAllTicketCategories(event.getId())
                .get(0)
                .getId();
        TicketCategoryModification shrink = new TicketCategoryModification(
                categoryId,
                "default",
                TicketCategory.TicketAccessType.INHERIT,
                AVAILABLE_SEATS - 2,
        ...
        assertEquals(
                18,
                tickets.stream()
                        .filter(t -> t.getCategoryId() != null && t.getCategoryId() == categoryId)
                        .count());
    ...
    void testIncreaseEventSeatsWithAnUnboundedCategory() {
        ...
        Pair<Event, String> pair =
                initEvent(categories, organizationRepository, userManager, eventManager, eventRepository);
        Event event = pair.getKey();
        EventModification update = new EventModification(
                event.getId(),
    ...
    void testAddBoundedCategoryToUnboundedEvent() {
        ...
        assertEquals(
                10,
                ticketRepository
                        .countReleasedTicketInCategory(event.getId(), result.getData())
                        .intValue());
    }
    ...
    void testUpdateBoundedFlagToTrue() {
        ...
        Map<String, String> categoryDescription =
                ticketCategoryDescriptionRepository.descriptionForTicketCategory(category.getId());
        TicketCategoryModification tcm = new TicketCategoryModification(
                category.getId(),
                category.getName(),
                TicketCategory.TicketAccessType.INHERIT,
    ...
    void testUpdateBoundedFlagToFalse() {
        ...
        assertEquals(
                AVAILABLE_SEATS,
                ticketRepository.countFreeTicketsForUnbounded(event.getId()).intValue());
    }
    ...
    void testValidationBoundedFailedPendingTickets() {
        ...
        List<Integer> tickets = ticketRepository.selectTicketInCategoryForUpdate(
                event.getId(), category.getId(), 1, Collections.singletonList(Ticket.TicketStatus.FREE.name()));
    ...
    void testIncreaseRestrictedCategory() {
        ...
        assertEquals(
                11,
                ticketRepository
                        .countFreeTickets(event.getId(), category.getId())
                        .intValue());
    }
    ...
    void testDecreaseRestrictedCategory() {
        ...
        assertEquals(
                ...
                ticketRepository
                        ...
                        .intValue());
        assertEquals(
                1, ticketRepository.countReleasedUnboundedTickets(event.getId()).intValue());
    }
    ...
    void testDecreaseRestrictedCategoryWithAlreadySentToken() {
        ...
        TicketCategoryModification tcmOk = new TicketCategoryModification(
                category.getId(),
                category.getName(),
                TicketCategory.TicketAccessType.INHERIT,
                2,
    ...
    void testNewRestrictedCategory() {
        ...
        assertEquals(
                20,
                ticketRepository
                        .countFreeTickets(event.getId(), result.getData())
                        .intValue());
    }
    ...
    void testNewBoundedCategoryWithExistingBoundedAndPendingTicket() {
        ...
        TicketReservationWithOptionalCodeModification reservation =
                new TicketReservationWithOptionalCodeModification(trm, Optional.empty());
        ...
        TicketCategoryModification tcm = new TicketCategoryModification(
                ...
                DateTimeModification.fromZonedDateTime(
                        ZonedDateTime.now(clockProvider.getClock()).plusDays(1)),
                Collections.emptyMap(),
                BigDecimal.TEN,
                false,
                "",
        ...
        tcm = new TicketCategoryModification(
                categoryID,
                tcm.getName(),
                TicketCategory.TicketAccessType.INHERIT,
                AVAILABLE_SEATS,
    ...
    void deleteUnboundedTicketCategorySuccess() {
        ...
        assertEquals(
                1,
                ticketCategoryRepository.findAllTicketCategories(event.getId()).size());
    ...
    void deleteUnboundedTicketCategoryFailure() {
        ...
        Pair<Event, String> pair = initEvent(cat, organizationRepository, userManager, eventManager, eventRepository);
        var event = pair.getLeft();
        var tickets = ticketRepository.selectNotAllocatedTicketsForUpdate(
                event.getId(), 1, List.of(Ticket.TicketStatus.FREE.name()));
        var categories = ticketCategoryRepository.findAllTicketCategories(event.getId());
        assertEquals(2, categories.size());
        int categoryId = categories.get(0).getId();
        ...
        ticketReservationRepository.createNewReservation(
                ...
                event.getOrganizationId(),
                null);
        int result = ticketRepository.reserveTickets(
                reservationId, tickets, categories.get(0), "en", event.getVatStatus(), i -> null);
    ...
    void deleteUnboundedTicketCategoryFailureBecauseSingle() {
        List<TicketCategoryModification> cat = Collections.singletonList(new TicketCategoryModification(
                null,
                "default",
                TicketCategory.TicketAccessType.INHERIT,
                AVAILABLE_SEATS,
    ...
    void deleteBoundedTicketCategoryFailure() {
        ...
        assertEquals(2, categories.size());
        int categoryId = categories.get(0).getId();
        var tickets = ticketRepository.selectTicketInCategoryForUpdate(
                event.getId(), categoryId, 1, List.of(Ticket.TicketStatus.FREE.name()));
        String reservationId = UUID.randomUUID().toString();
        ticketReservationRepository.createNewReservation(
                reservationId,
                ZonedDateTime.now(clockProvider.getClock()),
    ...
    void deletedBoundedCategoriesNotIncludedInStatistics() {
        ...
        var categoryToBeDeleted = categories.stream()
                .filter(c -> c.getName().equals(to_be_deleted))
                .findFirst()
                .orElseThrow();
        ...
        var activeCategory = categories.stream()
                .filter(c -> c.getId() != categoryToBeDeleted.getId())
                .findFirst()
                .orElseThrow();
    ...
    void rearrangeTicketCategories() {
        ...
        var categoryModifications = List.of(
                new CategoryOrdinalModification(first.getId(), first.getName(), second.getOrdinal()),
                new CategoryOrdinalModification(second.getId(), second.getName(), first.getOrdinal()));
    ...
    void updateEventFormat() {
        var categories = List.of(new TicketCategoryModification(
                null,
                "first",
                TicketCategory.TicketAccessType.INHERIT,
                AVAILABLE_SEATS,
        ...
        var onSitePaymentMethodModification = createEventModification(
                AVAILABLE_SEATS, event, Event.EventFormat.IN_PERSON, withOnSite, event.getLocation());
        ...
        try {
            eventManager.updateEventHeader(
                    event,
                    createEventModification(
                            AVAILABLE_SEATS, event, Event.EventFormat.ONLINE, withOnSite, event.getLocation()),
                    username);
            fail();
        ...
        eventManager.updateEventSeatsAndPrices(
                event,
                createEventModification(
                        AVAILABLE_SEATS, event, event.getFormat(), List.of(PaymentProxy.OFFLINE), event.getLocation()),
                username);
        // retry
        event = eventRepository.findById(event.getId());
        eventManager.updateEventHeader(
    ...
    void updateInPersonEventToHybridAndBack() {
        ...
        assertTrue(ticketCategoryRepository.findAllTicketCategories(event.getId()).stream()
                .allMatch(tc -> tc.getTicketAccessType() == TicketCategory.TicketAccessType.IN_PERSON));
        ...
        assertTrue(ticketCategoryRepository.findAllTicketCategories(event.getId()).stream()
                .allMatch(tc -> tc.getTicketAccessType() == TicketCategory.TicketAccessType.INHERIT));
    }
    ...
    void testFailedUpdateOnlineEventToHybrid() {
        var categories = List.of(
                ...
                new TicketCategoryModification(
                        ...
                        null,
                        AlfioMetadata.empty()));
        var eventAndUsername = initEvent(
                categories,
    ...
    void updateOnlineEventToHybridAndBack() {
        ...
        eventManager.updateEventHeader(
                event,
                createEventModification(
                        AVAILABLE_SEATS, event, Event.EventFormat.HYBRID, event.getAllowedPaymentProxies(), "location"),
                username);

        // check that all the categories have been converted to ONLINE
        assertTrue(ticketCategoryRepository.findAllTicketCategories(event.getId()).stream()
                .allMatch(tc -> tc.getTicketAccessType() == TicketCategory.TicketAccessType.ONLINE));
        ...
        eventManager.updateEventHeader(
                event,
                createEventModification(
                        AVAILABLE_SEATS, event, Event.EventFormat.ONLINE, event.getAllowedPaymentProxies(), null),
                username);
        // we expect all the access types to be INHERIT
        assertTrue(ticketCategoryRepository.findAllTicketCategories(event.getId()).stream()
                .allMatch(tc -> tc.getTicketAccessType() == TicketCategory.TicketAccessType.INHERIT));
    ...
    void reallocateCategory() {
        ...
        ticketCategoryRepository.update(
                category1.getId(),
                category1.getName(),
                category1.getUtcInception().minusDays(1),
                category1.getUtcExpiration().minusDays(1),
                category1.getMaxTickets(),
                category1.isAccessRestricted(),
                category1.getSrcPriceCts(),
                category1.getCode(),
                null,
                null,
                null,
                null,
                category1.getTicketCheckInStrategy(),
                category1.getTicketAccessType());
        ...
        assertEquals(
                AVAILABLE_SEATS,
                ticketCategoryRepository.getById(category2.getId()).getMaxTickets());

        // create a reservation, then move tickets back
        List<Integer> tickets = ticketRepository.selectTicketInCategoryForUpdate(
                event.getId(), category2.getId(), 1, Collections.singletonList(Ticket.TicketStatus.FREE.name()));
        String reservationId = "12345678";
        ticketReservationRepository.createNewReservation(
                reservationId,
                ZonedDateTime.now(clockProvider.getClock()),
        ...
        assertEquals(
                AVAILABLE_SEATS - 1,
                ticketCategoryRepository.getById(category1.getId()).getMaxTickets());
    ...
    private Pair<Event, String> generateAndEditEvent(int newEventSize) {
        ...
        if (newEventSize > AVAILABLE_SEATS) {
            assertEquals(
                    newEventSize - AVAILABLE_SEATS,
                    ticketRepository
                            .countReleasedUnboundedTickets(event.getId())
                            .intValue());
        }
        assertEquals(10, tickets.stream().filter(t -> t.getCategoryId() != null).count());
    ...
    void ensureUniqueConstraintApplied() {
        ...
        ticketRepository.bulkTicketInitialization(new MapSqlParameterSource[] {
    ...
    private EventModification createEventModification(int availableSeats, Event event) {
        return createEventModification(
                availableSeats,
                event,
                Event.EventFormat.IN_PERSON,
                event.getAllowedPaymentProxies(),
    ...
    private EventModification createEventModification(
            int availableSeats,
            Event event,
