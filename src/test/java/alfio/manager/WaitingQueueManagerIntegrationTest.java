...
class WaitingQueueManagerIntegrationTest extends BaseIntegrationTest {
    ...
    public void testSubscribeDenied() {
        List<TicketCategoryModification> categories =
                getPreSalesTicketCategoryModifications(false, AVAILABLE_SEATS, true, 10);
        ...
        TicketCategory firstCategory = eventManager.loadTicketCategories(event).stream()
                .filter(t -> t.getName().equals("defaultFirst"))
                .findFirst()
                .orElseThrow(IllegalStateException::new);
        configurationManager.saveCategoryConfiguration(
                firstCategory.getId(),
                event.getId(),
                Collections.singletonList(new ConfigurationModification(
                        null, ConfigurationKeys.MAX_AMOUNT_OF_TICKETS_BY_RESERVATION.getValue(), "1")),
                owner(pair.getRight()));
        ...
        configurationRepository.insertEventLevel(
                ...
                event.getId(),
                ConfigurationKeys.STOP_WAITING_QUEUE_SUBSCRIPTIONS.name(),
                "true",
                "");
    ...
    public void testDistributeSeatsFirstCategoryIsUnbounded() {
        ...
        boolean result =
                waitingQueueManager.subscribe(event, customerJohnDoe(event), "john@doe.com", null, Locale.ENGLISH);
        assertTrue(result);
        List<Triple<WaitingQueueSubscription, TicketReservationWithOptionalCodeModification, ZonedDateTime>>
                subscriptions = waitingQueueManager.distributeSeats(event).collect(Collectors.toList());
        assertEquals(1, subscriptions.size());
        Triple<WaitingQueueSubscription, TicketReservationWithOptionalCodeModification, ZonedDateTime>
                subscriptionDetail = subscriptions.get(0);
    ...
    public void testWaitingQueueForUnboundedCategory() {
        ...
        PaymentResult result = ticketReservationManager.performPayment(
                spec, reservationCost, PaymentProxy.OFFLINE, StaticPaymentMethods.BANK_TRANSFER, null);
    ...
    public void testAssignTicketToWaitingQueueUnboundedCategory() {
        ...
        TicketReservationWithOptionalCodeModification multi =
                new TicketReservationWithOptionalCodeModification(tr, Optional.empty());
        TicketReservationWithOptionalCodeModification single =
        ...
        String reservationIdSingle = ticketReservationManager.createTicketReservation(
                ...
                false,
                null);
        Pair<TotalPrice, Optional<PromoCodeDiscount>> priceAndDiscountSingle =
                ticketReservationManager.totalReservationCostWithVAT(reservationId);
        TotalPrice reservationCostSingle = priceAndDiscountSingle.getLeft();
        assertTrue(priceAndDiscountSingle.getRight().isEmpty());
        specification = new PaymentSpecification(
                reservationIdSingle,
                null,
                StaticPaymentMethods.BANK_TRANSFER,
                reservationCostSingle.getPriceWithVAT(),
                event,
                "email@example.com",
                new CustomerName("full name", "full", "name", event.mustUseFirstAndLastName()),
                "billing address",
                ...
                true,
                false);
        PaymentResult resultSingle = ticketReservationManager.performPayment(
                specification, reservationCostSingle, PaymentProxy.OFFLINE, StaticPaymentMethods.BANK_TRANSFER, null);
    ...
    public void testAssignTicketToWaitingQueueUnboundedCategorySelected() {
        ...
        assertTrue(waitingQueueManager.subscribe(
                event, customerJohnDoe(event), "john@doe.com", first.getId(), Locale.ENGLISH));
        assertTrue(waitingQueueManager.subscribe(
                event,
                new CustomerName("John Doe 2", "John", "Doe 2", event.mustUseFirstAndLastName()),
                "john@doe2.com",
                second.getId(),
                Locale.ENGLISH));
    ...
    private String reserveTickets(Event event, TicketCategory category, int num) {
        ...
        TicketReservationWithOptionalCodeModification tcm =
                new TicketReservationWithOptionalCodeModification(tr, Optional.empty());
        String reservationId = ticketReservationManager.createTicketReservation(
                event,
                Collections.singletonList(tcm),
                Collections.emptyList(),
                DateUtils.addDays(new Date(), 1),
                Optional.empty(),
                Locale.ENGLISH,
    ...
    public void testNoPublicCategoryAvailable() {
        ...
        List<TicketCategory> ticketCategories = ticketCategoryRepository.findAllTicketCategories(event.getId());
        TicketCategory first = ticketCategories.stream()
        ...
        Result<TicketCategory> ticketCategoryResult = eventManager.updateCategory(
                first.getId(),
                event,
                new TicketCategoryModification(
                        first.getId(),
                        ...
                        fromZonedDateTime(first.getExpiration(zoneId)),
        ...
        assertEquals(
                1,
                ticketRepository
                        .countReleasedTicketInCategory(event.getId(), first.getId())
                        .intValue());
    ...
    public void testTicketBelongsToExpiredCategory() {
        ...
        ticketCategoryRepository.update(
                first.getId(),
                first.getName(),
                first.getInception(event.getZoneId()),
                event.now(clockProvider).minusMinutes(1L),
                first.getMaxTickets(),
                first.isAccessRestricted(),
                MonetaryUtil.unitToCents(first.getPrice(), first.getCurrencyCode()),
                first.getCode(),
                null,
                null,
                null,
                null,
                first.getTicketCheckInStrategy(),
                first.getTicketAccessType());

        List<Integer> ticketIds = ticketRepository.findTicketsInReservation(reservationId).stream()
                .map(Ticket::getId)
        ...
        List<TicketInfo> releasedButExpired =
                ticketRepository.findReleasedBelongingToExpiredCategories(event.getId(), event.now(clockProvider));
    ...
    private List<TicketCategoryModification> getPreSalesTicketCategoryModifications(
            boolean firstBounded, int firstSeats, boolean lastBounded, int lastSeats) {
        ...
        return Arrays.asList(
                ...
                new TicketCategoryModification(
                        ...
                        new DateTimeModification(
                                ...
                                LocalTime.now(clockProvider.getClock())),
                        new DateTimeModification(
                                LocalDate.now(clockProvider.getClock()).plusDays(2),
                                LocalTime.now(clockProvider.getClock())),
                        DESCRIPTION,
                        BigDecimal.TEN,
                        false,
