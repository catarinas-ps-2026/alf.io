...
class DataMigratorIntegrationTest extends BaseIntegrationTest {
    ...
    private Pair<Event, String> initEvent(List<TicketCategoryModification> categories, String displayName) {
        ...
        EventModification em = new EventModification(
                ...
                new DateTimeModification(
                        ...
                        LocalTime.now(ClockProvider.clock()).plusHours(1)),
                BigDecimal.TEN,
                "CHF",
                AVAILABLE_SEATS,
                BigDecimal.ONE,
                true,
                List.of(PaymentProxy.ON_SITE),
                categories,
                false,
    ...
    public void testAlreadyMigratedEvent() {
        ...
        try {
            ...
            eventMigrationRepository.insertMigrationData(
                    event.getId(), currentVersion, migrationTs, EventMigration.Status.COMPLETE.toString());
            eventRepository.updatePrices(
                    "CHF",
                    40,
                    false,
    ...
    public void testUpdateTicketReservation() {
        ...
        try {
            ...
            trm.setAmount(1);
            trm.setTicketCategoryId(
                    eventManager.loadTicketCategories(event).get(0).getId());
    ...
    public void testFixCategoriesSize() {
        ...
        Pair<Event, String> eventUsername = initEvent(categories);
        Event event = eventUsername.getKey();
        TicketCategory firstCategory = ticketCategoryRepository.findAllTicketCategories(event.getId()).stream()
        ...
        assertEquals(
                AVAILABLE_SEATS - 1,
                ticketRepository.countAllocatedTicketsForEvent(event.getId()).intValue());
        assertEquals(
                1, ticketRepository.countFreeTicketsForUnbounded(event.getId()).intValue());
        assertEquals(
                AVAILABLE_SEATS - 1,
                ticketRepository
                        .countFreeTickets(event.getId(), firstCategoryID)
                        .intValue());
    ...
    public void testFixStuckTickets() {
        ...
        ticketReservationRepository.updateReservationStatus(
                reservationId, TicketReservation.TicketReservationStatus.CANCELLED.name());
        ...
        assertSame(
                Ticket.TicketStatus.RELEASED, ticketRepository.findByUUID(uuid).getStatus());
    }
}
