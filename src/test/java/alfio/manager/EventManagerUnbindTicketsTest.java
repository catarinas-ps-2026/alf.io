...
public class EventManagerUnbindTicketsTest {
    ...
    void setUp() {
        ...
        when(eventRepository.findOptionalEventAndOrganizationIdByShortName(eq(eventName)))
                .thenReturn(Optional.of(event));
        when(ticketCategory.getId()).thenReturn(categoryId);
        when(organizationRepository.findOrganizationForUser(anyString(), anyInt()))
    ...
    void notUnbindIfNoUnboundCategories() {
        when(ticketCategoryRepository.countUnboundedCategoriesByEventId(eq(eventId)))
                .thenReturn(0);
    ...
    void notUnbindFromAnUnboundedCategory() {
        ...
        when(ticketCategoryRepository.findStatisticWithId(eq(categoryId), eq(eventId)))
                .thenReturn(ticketCategoryStatisticView);
    ...
    void unbindTicketsFromBoundedCategory() {
        ...
        when(ticketRepository.selectTicketInCategoryForUpdate(
                        eq(eventId), eq(categoryId), eq(notSold), eq(singletonList(Ticket.TicketStatus.FREE.name()))))
                .thenReturn(lockedTickets);
        when(ticketRepository.unbindTicketsFromCategory(eq(eventId), eq(categoryId), eq(lockedTickets)))
                .thenReturn(notSold);
        ...
        when(ticketCategoryRepository.findStatisticWithId(eq(categoryId), eq(eventId)))
                .thenReturn(categoryStatisticView);
        ...
        verify(ticketRepository)
                .selectTicketInCategoryForUpdate(
                        eq(eventId), eq(categoryId), eq(notSold), eq(singletonList(Ticket.TicketStatus.FREE.name())));
