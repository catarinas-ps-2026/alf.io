...
public class PassKitManagerTest {
    ...
    private ConfigurationManager.MaybeConfiguration mockMaybeConfig(ConfigurationKeys key, String value, boolean present) {
    ...
    private void setupConfiguration(boolean enablePass, 
                                    String typeId, String keystore, String keystorePwd, 
                                    String teamId, String privateKeyAlias) {
        
        ConfigurationManager.MaybeConfiguration enablePassConf = mockMaybeConfig(ConfigurationKeys.ENABLE_PASS, String.valueOf(enablePass), true);
        ConfigurationManager.MaybeConfiguration typeIdConf = mockMaybeConfig(ConfigurationKeys.PASSBOOK_TYPE_IDENTIFIER, typeId, typeId != null);
        ConfigurationManager.MaybeConfiguration keystoreConf = mockMaybeConfig(ConfigurationKeys.PASSBOOK_KEYSTORE, keystore, keystore != null);
        ConfigurationManager.MaybeConfiguration keystorePwdConf = mockMaybeConfig(ConfigurationKeys.PASSBOOK_KEYSTORE_PASSWORD, keystorePwd, keystorePwd != null);
        ConfigurationManager.MaybeConfiguration teamIdConf = mockMaybeConfig(ConfigurationKeys.PASSBOOK_TEAM_IDENTIFIER, teamId, teamId != null);
        ConfigurationManager.MaybeConfiguration privateKeyAliasConf = mockMaybeConfig(ConfigurationKeys.PASSBOOK_PRIVATE_KEY_ALIAS, privateKeyAlias, privateKeyAlias != null);
        ...
        when(configurationManager.getFor(anySet(), any(ConfigurationLevel.class))).thenReturn(confMap);
    ...
    void testRetrieveTicketDetails_EventNotFound() {
        ...
        Optional<Pair<EventAndOrganizationId, Ticket>> result = passKitManager.retrieveTicketDetails("event-name", UUID.randomUUID().toString());
    ...
    void testRetrieveTicketDetails_TicketNotFound() {
        ...
        when(ticketRepository.findOptionalByPublicUUID(ticketUuid))
            .thenReturn(Optional.empty());
        
        Optional<Pair<EventAndOrganizationId, Ticket>> result = passKitManager.retrieveTicketDetails("event-name", ticketUuid.toString());
        assertFalse(result.isPresent());
    }
    ...
    void testValidateToken_TypeIdentifierNotPresent() {
        ...
        ConfigurationManager.MaybeConfiguration typeIdConf = mockMaybeConfig(ConfigurationKeys.PASSBOOK_TYPE_IDENTIFIER, null, false);
        when(configurationManager.getFor(eq(ConfigurationKeys.PASSBOOK_TYPE_IDENTIFIER), any(ConfigurationLevel.class)))
            .thenReturn(typeIdConf);
    ...
    void testValidateToken_TypeIdentifierMismatch() {
        ...
        ConfigurationManager.MaybeConfiguration typeIdConf = mockMaybeConfig(ConfigurationKeys.PASSBOOK_TYPE_IDENTIFIER, "expected-type-id", true);
        when(configurationManager.getFor(eq(ConfigurationKeys.PASSBOOK_TYPE_IDENTIFIER), any(ConfigurationLevel.class)))
            .thenReturn(typeIdConf);
    ...
    void testValidateToken_TicketNotFound() {
        ...
        ConfigurationManager.MaybeConfiguration typeIdConf = mockMaybeConfig(ConfigurationKeys.PASSBOOK_TYPE_IDENTIFIER, "type-id", true);
        when(configurationManager.getFor(eq(ConfigurationKeys.PASSBOOK_TYPE_IDENTIFIER), any(ConfigurationLevel.class)))
            .thenReturn(typeIdConf);
            
        String ticketUuid = UUID.randomUUID().toString();
        when(ticketRepository.findOptionalByUUID(ticketUuid))
            .thenReturn(Optional.empty());
            
        Optional<Pair<EventAndOrganizationId, Ticket>> result = passKitManager.validateToken(
            "event-name", "type-id", ticketUuid, "ApplePass token"
        );
        assertFalse(result.isPresent());
    }
    ...
    void testValidateToken_TokenMismatch() {
        ...
        Optional<Pair<EventAndOrganizationId, Ticket>> result = passKitManager.validateToken(
            "event-name", "type-id", ticketUuid, "ApplePass mismatched-token"
        );
        assertFalse(result.isPresent());
    }
    ...
    void testValidateToken_Success() {
        ...
        Optional<Pair<EventAndOrganizationId, Ticket>> result = passKitManager.validateToken(
            "event-name", "type-id", ticketUuid, "ApplePass " + correctToken
        );
        assertTrue(result.isPresent());
        assertEquals(event, result.get().getLeft());
        assertEquals(ticket, result.get().getRight());
    ...
    void testWritePass_SuccessWithScaleLogoAndCustomValidity() throws Exception {
        ...
        try (MockedStatic<PassSerializer> passSerializerMock = mockStatic(PassSerializer.class);
             ...
             passSerializerMock.when(() -> PassSerializer.writePkPassArchive(any(Pass.class), any(PassSigner.class), any(OutputStream.class)))
                 .thenAnswer(invocation -> {
    ...
    void testWritePass_SuccessWithLogoScaleExceptionAndNullLocationsAndDefaultValidity() throws Exception {
        ...
        File nonExistentFile = new File(System.getProperty("java.io.tmpdir"), "non-existent-logo-" + UUID.randomUUID() + ".png");
    ...
    void testGetPass_MissingConfigKeys() {
        Map<String, String> model = new HashMap<>();
        model.put("ticket", "{\"id\":1,\"uuid\":\"ticket-uuid\",\"categoryId\":100,\"eventId\":1,\"status\":\"ACQUIRED\",\"ticketsReservationId\":\"res-id\",\"userLanguage\":\"en\",\"email\":\"test@example.com\",\"fullName\":\"Test User\"}");
