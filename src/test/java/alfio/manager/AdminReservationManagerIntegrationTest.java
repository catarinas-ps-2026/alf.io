...
class AdminReservationManagerIntegrationTest extends BaseIntegrationTest {
    ...
    public void testReserveExistingCategoryNotEnoughSeatsNotBoundedSoldOut() {
        List<TicketCategoryModification> categories = Collections.singletonList(new TicketCategoryModification(
                ...
                null,
                AlfioMetadata.empty()));
        performExistingCategoryTest(
                categories, false, Collections.singletonList(1), true, true, AVAILABLE_SEATS, AVAILABLE_SEATS + 1);
    }
    ...
    public void testReserveFromExistingCategoryNotEnoughSeatsNoExtensionAllowedBounded() {
        List<TicketCategoryModification> categories = Collections.singletonList(new TicketCategoryModification(
                ...
                AlfioMetadata.empty()));
        performExistingCategoryTest(
                categories, true, Collections.singletonList(AVAILABLE_SEATS + 1), false, false, 0, AVAILABLE_SEATS);
    }
    ...
    public void testReserveFromNewCategory() {
        List<TicketCategoryModification> categories = Collections.singletonList(new TicketCategoryModification(
                ...
                null,
                AlfioMetadata.empty()));
        Pair<Event, String> eventWithUsername =
        ...
        List<TicketsInfo> ticketsInfoList =
                Collections.singletonList(new TicketsInfo(category, generateAttendees(attendees), true, false));
        ...
        Result<Pair<TicketReservation, List<Ticket>>> result =
                adminReservationManager.createReservation(modification, event.getShortName(), username);
        ...
        assertEquals(
                attendees,
                ticketRepository
                        .findPendingTicketsInCategories(Collections.singletonList(categoryId))
                        .size());
        ...
        adminReservationManager.confirmReservation(
                PurchaseContextType.event, event.getShortName(), data.getLeft().getId(), username, EMPTY);
        ticketCategoryRepository
                .findAllTicketCategories(event.getId())
                .forEach(tc -> assertTrue(specialPriceRepository.findAllByCategoryId(tc.getId()).stream()
                        .allMatch(sp -> sp.getStatus() == SpecialPrice.Status.TAKEN)));
        assertFalse(ticketRepository
                .findAllReservationsConfirmedButNotAssignedForUpdate(event.getId())
    ...
    public void testReserveMixed() {
        ...
        assertTrue(tickets.stream()
                .allMatch(t -> t.getTicketsReservationId().equals(data.getKey().getId())));
        ...
        assertEquals(
                3,
                ticketRepository
                        .findPendingTicketsInCategories(Arrays.asList(resExistingCategoryId, resNewCategoryId))
                        .size());
        assertEquals(
                3,
                ticketRepository
                        .findTicketsInReservation(data.getLeft().getId())
                        .size());
        ...
        assertEquals(
                ticketRepository.findTicketsInReservation(reservationId).stream()
                        ...
                        .getId(),
                ticketRepository
                        .findFirstTicketInReservation(reservationId)
                        .get()
                        .getId());
    ...
    public void testConfirmReservation() {
        List<TicketCategoryModification> categories = Collections.singletonList(new TicketCategoryModification(
                ...
                null,
                AlfioMetadata.empty()));
        Triple<Event, String, TicketReservation> testResult = performExistingCategoryTest(
                categories, true, Collections.singletonList(2), false, true, 0, AVAILABLE_SEATS);
        assertNotNull(testResult);
        var result = adminReservationManager.confirmReservation(
                PurchaseContextType.event,
                testResult.getLeft().getShortName(),
                testResult.getRight().getId(),
                testResult.getMiddle(),
                EMPTY);
        assertTrue(result.isSuccess());
        ...
        ticketCategoryRepository
                .findAllTicketCategories(eventId)
                .forEach(tc -> assertTrue(specialPriceRepository.findAllByCategoryId(tc.getId()).stream()
                        .allMatch(sp -> sp.getStatus() == SpecialPrice.Status.TAKEN)));
        assertFalse(ticketRepository
                .findAllReservationsConfirmedButNotAssignedForUpdate(eventId)
    ...
    public void testConfirmReservationSendConfirmationEmail() {
        ...
        int attendees = 2;
        Triple<Event, String, TicketReservation> testResult = performExistingCategoryTest(
                categories, true, Collections.singletonList(attendees), false, true, 0, AVAILABLE_SEATS);
        assertNotNull(testResult);
        var result = adminReservationManager.confirmReservation(
                PurchaseContextType.event,
                testResult.getLeft().getShortName(),
        ...
        assertEquals(
                attendees + 2,
                emailMessageRepository.findByEventId(eventId, 0, 50, null).size());
        ticketCategoryRepository
                .findAllTicketCategories(eventId)
                .forEach(tc -> assertTrue(specialPriceRepository.findAllByCategoryId(tc.getId()).stream()
                        .allMatch(sp -> sp.getStatus() == SpecialPrice.Status.TAKEN)));
    ...
    private Triple<Event, String, TicketReservation> performExistingCategoryTest(
            ...
            boolean bounded,
            List<Integer> attendeesNr,
            boolean addSeatsIfNotAvailable,
            boolean expectSuccess,
            int reservedTickets,
            int expectedEventSeats) {
        assertEquals(
                categories.size(), attendeesNr.size(), "Test error: categories' size must be equal to attendees' size");
        Pair<Event, String> eventWithUsername =
                initEvent(categories, organizationRepository, userManager, eventManager, eventRepository);
        Event event = eventWithUsername.getKey();
        String username = eventWithUsername.getValue();
        ...
        List<TicketsInfo> ticketsInfoList = existingCategories.stream()
                .map(existingCategory -> {
                    Category category = new Category(
                            existingCategory.getId(), existingCategory.getName(), existingCategory.getPrice(), null);
        ...
        if (reservedTickets > 0) {
            ...
            TicketReservationWithOptionalCodeModification r =
        ...
        return Triple.of(
                eventWithUsername.getLeft(),
                eventWithUsername.getRight(),
                result.getData().getKey());
    }
    ...
    private void validateSuccess(
            ...
            Event event,
            String username,
            List<TicketCategory> existingCategories,
            Result<Pair<TicketReservation, List<Ticket>>> result,
        ...
        Pair<TicketReservation, List<Ticket>> data = result.getData();
        assertEquals(
                data.getRight().size(), attendeesNr.stream().mapToInt(i -> i).sum());
        assertNotNull(data.getLeft());
        ...
        if (bounded) {
            ...
            existingCategories.forEach(existingCategory -> {
                TicketCategory categoryModified =
                        ticketCategoryRepository.getByIdAndActive(existingCategory.getId(), event.getId());
        ...
        ticketCategoryRepository
                .findAllTicketCategories(modified.getId())
                .forEach(tc -> assertTrue(specialPriceRepository.findAllByCategoryId(tc.getId()).stream()
                        .allMatch(sp -> sp.getStatus() == SpecialPrice.Status.PENDING)));
        assertFalse(ticketRepository
                .findAllReservationsConfirmedButNotAssignedForUpdate(event.getId())
