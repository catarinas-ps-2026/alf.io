...
class ExtensionServiceTest {
    ...
    void setUp() {
        extensionService = new ExtensionService(
                ...
                mock(ExternalConfiguration.class),
    ...
    void validationOK() {
        var metadata = generateMetadata(
                EnumSet.of(EVENT_METADATA_UPDATE, ONLINE_CHECK_IN_REDIRECT),
                EnumSet.of(CREATE_VIRTUAL_ROOM, CREATE_GUEST_LINK));
        extensionService.validateCapabilities(metadata);
    }
    ...
    private ExtensionMetadata generateMetadata(
            Collection<ExtensionEvent> events, Collection<ExtensionCapability> capabilities) {
        return new ExtensionMetadata(
                ...
                capabilities.stream()
                        .map(ec -> new ExtensionMetadata.CapabilityDetail(
                                ec.name(), ec.name() + " label", ec.name() + " description", null))
