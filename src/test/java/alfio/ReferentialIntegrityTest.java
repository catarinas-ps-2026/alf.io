...
class ReferentialIntegrityTest {
    ...
    private List<TicketCategoryModification> defaultCategories() {
        return List.of(new TicketCategoryModification(
                null, "default", alfio.model.TicketCategory.TicketAccessType.INHERIT, 20,
    ...
    void cannotInsertReservationWithInvalidEventId() {
        ...
        assertThrows(DataIntegrityViolationException.class, () -> jdbc.update(
                ...
                Map.of("eventId", eventAndUser.getLeft().getId(), "categoryId",
                        ticketCategoryRepository.findAllTicketCategories(eventAndUser.getLeft().getId()).get(0).getId())));
    }
    ...
    void cannotDeleteOrganizationWithEvents() {
        ...
        assertThrows(DataIntegrityViolationException.class, () -> jdbc.update(
                "DELETE FROM organization WHERE id = :orgId", Map.of("orgId", orgId)));
    }
    ...
    void cannotInsertEventWithInvalidOrganizationId() {
        ...
        assertThrows(Exception.class, () -> {
            eventManager.createEvent(new alfio.model.modification.EventModification(
                    null, Event.EventFormat.IN_PERSON, "url", "url", "url", "privacy", "url", null,
                    "test-event", "Test Event", 99999, "location", "0.0", "0.0", "UTC",
                    DESCRIPTION,
                    new DateTimeModification(LocalDate.now(), LocalTime.now()),
                    new DateTimeModification(LocalDate.now().plusDays(1), LocalTime.now()),
                    BigDecimal.TEN, "CHF", 20, BigDecimal.ONE, true,
                    ...
                    7, null, null, AlfioMetadata.empty(), List.of()),
                    "testuser");
        });
    }
