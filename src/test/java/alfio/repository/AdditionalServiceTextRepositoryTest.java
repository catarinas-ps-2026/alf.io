...
class AdditionalServiceTextRepositoryTest {
    ...
    private final AdditionalServiceTextRepository additionalServiceTextRepository =
            mock(AdditionalServiceTextRepository.class, withSettings().defaultAnswer(CALLS_REAL_METHODS));

    @Test
    void testGetDescriptionsByAdditionalServiceIds() {
        int serviceId = 1;
        AdditionalServiceText text =
                new AdditionalServiceText(10, serviceId, "en", AdditionalServiceText.TextType.DESCRIPTION, "Value");
        doReturn(List.of(text)).when(additionalServiceTextRepository).findAllByAdditionalServiceIds(anyCollection());

        Map<Integer, Map<AdditionalServiceText.TextType, Map<String, String>>> result =
                additionalServiceTextRepository.getDescriptionsByAdditionalServiceIds(List.of(serviceId));

        assertEquals(1, result.size());
        assertEquals(
                "Value",
                result.get(serviceId)
                        .get(AdditionalServiceText.TextType.DESCRIPTION)
                        .get("en"));
    }
    ...
    void testFindBestMatchByLocaleAndType() {
        ...
        AdditionalServiceText result =
                additionalServiceTextRepository.findBestMatchByLocaleAndType(serviceId, locale, type);
        ...
        doReturn(List.of(text2))
                .when(additionalServiceTextRepository)
                .findAllByAdditionalServiceIdAndType(serviceId, type);
