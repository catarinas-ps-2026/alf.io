...
public abstract class BaseReservationFlowTest extends BaseIntegrationTest {
    ...
    private void ensureConfiguration(ReservationFlowContext context) {
        ...
        eventManager.addPromoCode(
                ...
                context.event.getEnd().plusDays(2),
                10,
        ...
        hiddenCategoryId = ticketCategoryRepository.findAllTicketCategories(context.event.getId()).stream()
        ...
        eventManager.addPromoCode(
                ...
                context.event.getEnd().plusDays(2),
                0,
                PromoCodeDiscount.DiscountType.NONE,
                null,
                null,
                "hidden",
                "test@test.ch",
        ...
        var af = new AdditionalFieldRequest(
                -1,
                true,
                "field1",
                "text",
                ...
                null,
                null,
                Map.of("en", new EventModification.Description("field en", "", null)),
                null,
                null,
                true);
        purchaseContextFieldManager.addAdditionalField(context.event, af);

        var afId = purchaseContextFieldRepository
                .findAdditionalFieldsForEvent(context.event.getId())
                .get(0)
                .getId();

        purchaseContextFieldRepository.updateFieldOrder(afId, -1);

        var af2 = new AdditionalFieldRequest(
                1,
                true,
                FIELD_2,
                "text",
                ...
                null,
                null,
                Map.of("en", new EventModification.Description("field2 en", "", null)),
                null,
                null,
                null /* old client */);
        ...
        var addServRes = additionalServiceApiController.insert(
                context.event.getId(), addServ, new BeanPropertyBindingResult(addServ, "additionalService"), null);
        assertNotNull(addServRes.getBody());
        additionalServiceId = addServRes.getBody().getId();
        //

        var af3 = new AdditionalFieldRequest(
                2,
                true,
                "field3",
                "text",
                ...
                null,
                null,
                Map.of("en", new EventModification.Description("field3 en", "", null)),
                addServRes.getBody(),
                null,
                null);
        ...
        configurationRepository.insertEventLevel(
                context.event.getOrganizationId(),
                context.event.getId(),
                ConfigurationKeys.ENABLE_WAITING_QUEUE.getValue(),
                "true",
                "");
        configurationRepository.insertEventLevel(
                context.event.getOrganizationId(),
                context.event.getId(),
                ConfigurationKeys.ENABLE_PRE_REGISTRATION.getValue(),
                "true",
                "");
        //
    ...
    private EventModification.AdditionalService buildAdditionalService(
            ReservationFlowContext context, AdditionalService.SupplementPolicy supplementPolicy) {
        return new EventModification.AdditionalService(
                null,
                ...
                new DateTimeModification(
                        ZonedDateTime.now(clockProvider.getClock()).minusDays(2).toLocalDate(),
                        ZonedDateTime.now(clockProvider.getClock()).minusDays(2).toLocalTime()),
                new DateTimeModification(
                        context.event.getEnd().plusDays(2).toLocalDate(),
                        context.event.getEnd().plusDays(2).toLocalTime()),
                context.event.getVat(),
                AdditionalService.VatType.INHERITED,
                null,
                Collections.singletonList(new EventModification.AdditionalServiceText(
                        null, "en", "additional title", AdditionalServiceText.TextType.TITLE)),
                Collections.singletonList(new EventModification.AdditionalServiceText(
                        null, "en", "additional desc", AdditionalServiceText.TextType.DESCRIPTION)),
    ...
    protected void testBasicFlow(Supplier<ReservationFlowContext> contextSupplier) throws Exception {
        ...
        List<BasicEventInfo> body =
                eventApiV2Controller.listEvents(SearchOptions.empty()).getBody();
        assertNotNull(body);
...

        {
            ...
            assertTrue(usersApiController.getAllOrganizations(p).stream()
                    .anyMatch(o -> context.event.getOrganizationId() == o.getId()));
            assertEquals(
                    context.event.getOrganizationId(),
                    usersApiController
                            .getOrganization(context.event.getOrganizationId(), p)
                            .getId());
        }

        //
        assertEquals(
                ContentLanguage.ALL_LANGUAGES.size(),
                translationsApiController.getSupportedLanguages().size());
        assertEquals(
                "or",
                translationsApiController.getPublicTranslations("en", true).get("common.or"));
        assertEquals(
                "o", translationsApiController.getPublicTranslations("it", true).get("common.or"));
        assertEquals(
                "oder",
                translationsApiController.getPublicTranslations("de", true).get("common.or"));

        // check all public translations
        ContentLanguage.ALL_LANGUAGES.forEach(cl -> assertFalse(translationsApiController
                .getPublicTranslations(cl.getLanguage(), true)
                .isEmpty()));
        ...
        assertEquals(
                "Switzerland",
                translationsApiController.getCountries("en").stream()
                        .filter(c -> "CH".equals(c.isoCode()))
                        .findFirst()
                        .orElseThrow()
                        .name());
        ...
        assertEquals(
                "Suisse",
                translationsApiController.getCountries("fr").stream()
                        .filter(c -> "CH".equals(c.isoCode()))
                        .findFirst()
                        .orElseThrow()
                        .name());
        assertEquals(
                "Svizzera",
                translationsApiController.getCountries("it").stream()
                        .filter(c -> "CH".equals(c.isoCode()))
                        .findFirst()
                        .orElseThrow()
                        .name());
        assertEquals(
                "Schweiz",
                translationsApiController.getCountries("de").stream()
                        .filter(c -> "CH".equals(c.isoCode()))
                        .findFirst()
                        .orElseThrow()
                        .name());
        ...
        assertEquals(
                "Greece",
                translationsApiController.getCountriesForVat("en").stream()
                        .filter(c -> "EL".equals(c.isoCode()))
                        .findFirst()
                        .orElseThrow()
                        .name());
        ...
        assertEquals(
                ...
                eventStatisticsManager
                        .getTicketSoldStatistics(context.event.getId(), statisticsFrom, statisticsTo, "day")
                        .stream()
                        .mapToLong(TicketsByDateStatistic::getCount)
                        .sum());
        EventWithAdditionalInfo eventWithAdditionalInfo =
                eventStatisticsManager.getEventWithAdditionalInfo(context.event.getShortName(), context.userId);
        ...
        assertEquals(
                ...
                eventApiV2Controller
                        .getEvent("NOT_EXISTS", new MockHttpSession())
                        .getStatusCode());
        //
        ...
        configurationRepository.insert(
                TRANSLATION_OVERRIDE.name(),
                Json.toJson(Map.of("en", Map.of("show-context.event.tickets.left", "{0} left!"))),
                "");
        configurationRepository.insertEventLevel(
                context.event.getOrganizationId(),
                context.event.getId(),
                "TRANSLATION_OVERRIDE",
                Json.toJson(Map.of("en", Map.of("common.vat", "context.event.vat"))),
                "");
        ...
        assertEquals(
                "context.event.vat", selectedEvent.getI18nOverride().get("en").get("common.vat"));
        ...
        assertEquals(
                selectedEvent.getContentLanguages().stream()
        ...
        assertEquals(
                "redirect:/api/v2/public/event/" + context.event.getShortName() + "/code/MY_CODE",
                indexController.redirectCode(context.event.getShortName(), "MY_CODE", "Normal user agent"));

        // vv social preview should redirect to event page when linking a code page instead of following and creating a
        // reservation
        assertEquals(
                "redirect:/event/" + context.event.getShortName(),
                indexController.redirectCode(context.event.getShortName(), "MY_CODE", "Slackbot"));

        // check open graph & co
        {
            var res = new MockHttpServletResponse();
            indexController.replyToIndex(
            ...
            assertTrue(docWithoutOpenGraph
                    .getAllNodesMatching(Selector.select()
                            .element("meta")
                            .attrValEq("name", "twitter:card")
                            .toMatcher())
                    .isEmpty());
            ...
            var title = (Element) docWithOpenGraph
                    .getAllNodesMatching(
                            Selector.select()
                                    .element("meta")
                                    .attrValEq("property", "og:title")
                                    .toMatcher(),
                            true)
                    .get(0);
        ...
        assertEquals(
                HttpStatus.NOT_FOUND,
                eventApiV2Controller.getTicketCategories("NOT_EXISTING", null).getStatusCode());
        {
            ...
            assertEquals(
                    ...
                    additionalItem
                            .getTitle()
                            .size()); // TODO: check: if there are missing lang, we should at least copy them (?)
            ...
            ticketCategoryRepository.fixDates(
                    visibleCat.getId(),
                    tc.getInception(context.event.getZoneId()).plusDays(2),
                    tc.getExpiration(context.event.getZoneId()));
            //
            items = eventApiV2Controller
                    .getTicketCategories(context.event.getShortName(), null)
                    .getBody();
            assertNotNull(items);
            ...
            var subRes = eventApiV2Controller.subscribeToWaitingList(
                    context.event.getShortName(), subForm, new BeanPropertyBindingResult(subForm, "subForm"));
            ...
            assertEquals(
                    "error.email", subRes.getBody().getValidationErrors().get(0).getCode());
            //

            subForm.setEmail("email@email.com");
            subRes = eventApiV2Controller.subscribeToWaitingList(
                    context.event.getShortName(), subForm, new BeanPropertyBindingResult(subForm, "subForm"));
            ...
            ticketCategoryRepository.fixDates(
                    visibleCat.getId(),
                    tc.getInception(context.event.getZoneId()).minusDays(2),
                    tc.getExpiration(context.event.getZoneId()));
        }
        ...
        // dynamic promo codes can be applied only automatically
        {
            eventManager.addPromoCode(
                    ...
                    "description",
                    "test@test.ch",
                    PromoCodeDiscount.CodeType.DYNAMIC,
                    null,
                    null);
            assertEquals(
                    HttpStatus.UNPROCESSABLE_ENTITY,
                    eventApiV2Controller
                            .validateCode(context.event.getShortName(), "DYNAMIC_CODE")
                            .getStatusCode());
            ...
            ticketReservation.setTicketCategoryId(
                    retrieveCategories(context).get(0).getId());
        ...
        // promo code currency code must match with event's in order to be redeemed
        {
            ...
            assertEquals(
                    HttpStatus.UNPROCESSABLE_ENTITY,
                    eventApiV2Controller
                            .validateCode(context.event.getShortName(), "TEST_TEST_TEST_TEST")
                            .getStatusCode());
        ...
        // hidden category check
        {
            assertEquals(
                    HttpStatus.UNPROCESSABLE_ENTITY,
                    eventApiV2Controller
                            .validateCode(context.event.getShortName(), "NOT_EXISTING")
                            .getStatusCode());
            ...
            var hiddenCat = items2.ticketCategories().stream()
                    .filter(t -> t.isAccessRestricted())
                    .findFirst()
                    .get();
            ...
            var reservationInfo =
                    reservationApiV2Controller.getReservationInfo(res.getBody().getValue(), context.getPublicUser());
            assertEquals(HttpStatus.OK, reservationInfo.getStatusCode());
            assertNotNull(reservationInfo.getBody());
            assertEquals(
                    context.vatIncluded ? "1.00" : "1.01",
                    reservationInfo.getBody().getOrderSummary().getTotalPrice());
            assertEquals(
                    "hidden",
                    reservationInfo
                            .getBody()
            ...
            configurationRepository.insertTicketCategoryLevel(
                    ...
                    context.event.getId(),
                    hiddenCategoryId,
                    ConfigurationKeys.PAYMENT_METHODS_BLACKLIST.name(),
            ...
            configurationRepository.deleteCategoryLevelByKey(
                    ConfigurationKeys.PAYMENT_METHODS_BLACKLIST.name(), context.event.getId(), hiddenCategoryId);
            ...
            assertEquals(
                    ...
                    jdbcTemplate.queryForObject(
                            "select count(*) from ticket where status = 'FREE' and final_price_cts > 0",
                            Map.of(),
                            Integer.class));

            // this is run by a job, but given the fact that it's in another separate transaction, it cannot work in
            // this test (WaitingQueueSubscriptionProcessor.handleWaitingTickets)
        ...
        // check reservation auto creation with code: TODO: will need to check all the flows
        {
            ...
            // code not found
            var notFoundRes = eventApiV2Controller.handleCode(
                    context.event.getShortName(),
                    "NOT_EXIST",
                    new ServletWebRequest(new MockHttpServletRequest(), new MockHttpServletResponse()),
                    context.getPublicUser());
            assertEquals(
                    "/event/" + context.event.getShortName(),
                    notFoundRes.getHeaders().getLocation().getPath());
            assertEquals(
                    "errors=error.STEP_1_CODE_NOT_FOUND",
                    notFoundRes.getHeaders().getLocation().getQuery());
            //

            // promo code, we expect a redirect to event with the code in the query string
            var redirectPromoCodeRes = eventApiV2Controller.handleCode(
                    context.event.getShortName(),
                    PROMO_CODE,
                    new ServletWebRequest(new MockHttpServletRequest(), new MockHttpServletResponse()),
                    context.getPublicUser());
            assertEquals(
                    "/event/" + context.event.getShortName(),
                    redirectPromoCodeRes.getHeaders().getLocation().getPath());
            assertEquals(
                    "code=MYPROMOCODE",
                    redirectPromoCodeRes.getHeaders().getLocation().getQuery());

            // code existing
            assertEquals(
                    2, specialPriceRepository.countFreeTokens(hiddenCategoryId).intValue());
            var res = eventApiV2Controller.handleCode(
                    context.event.getShortName(),
                    URL_CODE_HIDDEN,
                    new ServletWebRequest(new MockHttpServletRequest(), new MockHttpServletResponse()),
                    context.getPublicUser());
            var location = requireNonNull(res.getHeaders().getLocation()).toString();
            var reservationId = location.substring(
                    ("/event/" + context.event.getShortName() + "/reservation/").length(),
                    location.length() - "/book".length());
            ...
            assertEquals(
                    1, specialPriceRepository.countFreeTokens(hiddenCategoryId).intValue());
        ...
        // check reservation auto creation with deletion from the admin side
        {
            ...
            assertTrue(reservationInfo
                    .getBody()
                    .getActivePaymentMethods()
                    .containsKey(StaticPaymentMethods.BANK_TRANSFER.getPaymentMethodId()));
        ...
        // discount check
        {
            ...
            assertEquals(
                    EventCode.EventCodeType.DISCOUNT, discountCode.getValue().getType());
        ...
        // cancel a reservation
        {
            ...
            checkStatus(
                    reservationId, HttpStatus.OK, false, TicketReservation.TicketReservationStatus.PENDING, context);
        ...
        {
            var form = new ReservationForm();
            var categories = eventApiV2Controller
                    .getTicketCategories(context.event.getShortName(), HIDDEN_CODE)
                    .getBody()
                    .ticketCategories();
        ...
        // try to reserve more tickets than available
        {
            ...
            var freeTickets = jdbcTemplate.queryForList(
                    "select id from ticket where event_id = :eventId and status = 'FREE' and ((:categoryFilter::int is null and category_id is null) or category_id = :categoryFilter::int)",
                    new MapSqlParameterSource("eventId", context.event.getId())
                            .addValue("categoryFilter", categoryIdFilter),
                    Integer.class);
            // we leave one ticket
            var lockedTickets = freeTickets.subList(1, freeTickets.size());
            int result = jdbcTemplate.update(
                    "update ticket set status = 'PENDING', category_id = :categoryId where id in (:freeTickets)",
                    new MapSqlParameterSource("freeTickets", lockedTickets).addValue("categoryId", category.getId()));
            assertEquals(result, lockedTickets.size());

            var freeTicketsCount = jdbcTemplate.queryForObject(
                    "select count(*) from ticket where category_id "
                            + (category.isBounded() ? " = :categoryId" : "is null")
                            + " and status = 'FREE' and event_id = :eventId",
                    Map.of("eventId", context.event.getId(), "categoryId", category.getId()),
                    Integer.class);
            ...
            assertEquals(HttpStatus.UNPROCESSABLE_ENTITY, res.getStatusCode());
            assertEquals(
                    ErrorsCode.STEP_1_NOT_ENOUGH_TICKETS,
                    requireNonNull(res.getBody()).getValidationErrors().get(0).getCode());
            // ensure that reservation was not created
            assertEquals(
                    Boolean.FALSE,
                    jdbcTemplate.queryForObject(
                            "select exists(select id from tickets_reservation where status = 'PENDING' and event_id_fk = :eventId)",
                            new MapSqlParameterSource("eventId", context.event.getId()),
                            Boolean.class));
            // restore data
            result = jdbcTemplate.update(
                    "update ticket set status = 'FREE', category_id = :categoryId where id in (:freeTickets)",
                    new MapSqlParameterSource("freeTickets", lockedTickets)
                            .addValue(
                                    "categoryId",
                                    category.isBounded() ? category.getId() : null,
        ...
        // buy 2 ticket, with additional service + field
        {
            ...
            assertEquals(
                    1,
                    ticket1.getTicketFieldConfigurationAfterStandard()
                            .size()); // 1 + 1 additional service related field (appear only on first ticket)
            ...
            contactForm.setTickets(Map.ofEntries(
                    Map.entry(ticket1.getUuid(), ticketForm1), Map.entry(ticket2.getUuid(), ticketForm2)));

            var additionalServiceLinkForm = new AdditionalServiceLinkForm();
            additionalServiceLinkForm.setAdditionalServiceItemId(
                    additionalServiceWithData.get(0).getItemId());
            ...
            var failure = reservationApiV2Controller.validateToOverview(
                    reservationId,
                    "en",
                    false,
                    contactForm,
                    new BeanPropertyBindingResult(contactForm, "paymentForm"),
                    context.getPublicAuthentication());
            assertEquals(HttpStatus.UNPROCESSABLE_ENTITY, failure.getStatusCode());
            assertNotNull(failure.getBody());
            assertEquals(
                    1,
                    failure.getBody().getValidationErrors().stream()
                            .filter(f -> f.getFieldName()
                                    .equals("additionalServices[" + ticket2.getUuid() + "][0].additional[field3][0]"))
                            .count()); // <- missing mandatory

            // check billing errors
            assertEquals(
                    1,
                    failure.getBody().getValidationErrors().stream()
                            .filter(f -> f.getFieldName().equals("billingAddressLine1"))
                            .count()); // <- missing mandatory
            assertEquals(
                    1,
                    failure.getBody().getValidationErrors().stream()
                            .filter(f -> f.getFieldName().equals("billingAddressZip"))
                            .count()); // <- missing mandatory
            assertEquals(
                    1,
                    failure.getBody().getValidationErrors().stream()
                            .filter(f -> f.getFieldName().equals("billingAddressCity"))
                            .count()); // <- missing mandatory
            assertEquals(
                    1,
                    failure.getBody().getValidationErrors().stream()
                            .filter(f -> f.getFieldName().equals("vatCountryCode"))
                            .count()); // <- missing mandatory
            //
            ...
            var values = purchaseContextFieldRepository.findAllValuesByTicketIds(List.of(ticketRepository
                    .findByPublicUUID(UUID.fromString(ticket1.getUuid()))
                    .getId()));
            assertEquals(1, values.size());
            int ticket2Id = ticketRepository
                    .findByPublicUUID(UUID.fromString(ticket2.getUuid()))
                    .getId();
            ...
            assertTrue(values.stream()
                    .anyMatch(f -> f.getName().equals("field3") && f.getValue().equals("value")));
            ...
            assertFalse(requireNonNull(jdbcTemplate.queryForObject(
                    "select exists (select id from additional_service_item where status = 'PENDING' and tickets_reservation_uuid = :reservationId) as res",
        ...
        // buy 2 tickets + additional service with 100% discount
        {
            ...
            additionalServiceLinkForm2.setAdditionalServiceItemId(
                    additionalServiceWithData.get(1).getItemId());
            ...
            jdbcTemplate.update(
                    "delete from additional_service_description where additional_service_id_fk = :id",
                    Map.of("id", addServRes.getBody().getId()));
            assertEquals(
                    1,
                    jdbcTemplate.update(
                            "delete from additional_service where id = :id",
                            Map.of("id", addServRes.getBody().getId())));
        }
        ...
        // buy one ticket
        {
            ...
            assertEquals(
                    1, reservation.getTicketsByCategory().get(0).getTickets().size());

            var selectedTicket =
                    reservation.getTicketsByCategory().get(0).getTickets().get(0);
            assertEquals(
                    "field1",
                    selectedTicket
                            .getTicketFieldConfigurationBeforeStandard()
                            .get(0)
                            .getName());
            assertTrue(selectedTicket
                    .getTicketFieldConfigurationBeforeStandard()
                    .get(0)
                    .isRequired());
            assertEquals(
                    FIELD_2,
                    selectedTicket
                            .getTicketFieldConfigurationAfterStandard()
                            .get(0)
                            .getName());
            assertFalse(selectedTicket
                    .getTicketFieldConfigurationAfterStandard()
                    .get(0)
                    .isRequired());
            ...
            assertEquals(
                    ...
                    validationErrorsRes
                            .getBody()
                            .getErrorCount()); // first name, last name, email + MISSING_ATTENDEE DATA
            ...
            var overviewResFailed = reservationApiV2Controller.validateToOverview(
                    reservationId,
                    "en",
                    false,
                    contactForm,
            ...
            ticketForm.setAdditional(
                    new HashMap<>(Collections.singletonMap("field1", Collections.singletonList("value"))));
            ...
            overviewRes = reservationApiV2Controller.validateToOverview(
                    reservationId,
                    "en",
                    false,
                    contactForm,
            ...
            if (context.publicUserId != null) {
                ...
                assertThrows(
                        ReservationAccessDenied.class,
                        () -> reservationApiV2Controller.getReservationInfo(reservationId, null));
            } else {
                assertTrue(owner.isEmpty());
            }

            int promoCodeId = promoCodeDiscountRepository
                    .findPromoCodeInEventOrOrganization(context.event.getId(), PROMO_CODE)
                    .orElseThrow()
                    .getId();
            ...
            var confRes = reservationApiV2Controller.reSendReservationConfirmationEmail(
                    PurchaseContextType.event,
                    context.event.getShortName(),
                    reservationId,
                    "en",
            ...
            var ticket = reservation.getTicketsByCategory().stream()
                    .findFirst()
                    .orElseThrow()
                    .getTickets()
                    .get(0);
            ...
            var ticketNotFoundRes = ticketApiV2Controller.getTicketInfo(
                    context.event.getShortName(), UUID.fromString("00000000-0000-0000-0000-000000000000"));
            assertEquals(HttpStatus.NOT_FOUND, ticketNotFoundRes.getStatusCode());

            var ticketFoundRes = ticketApiV2Controller.getTicketInfo(
                    context.event.getShortName(), UUID.fromString(ticket.getUuid()));
            ...
            assertTrue(
                    reservationId.startsWith(ticketFoundBody.getReservationId().toLowerCase(Locale.ENGLISH)));

            var sendTicketByEmailRes = ticketApiV2Controller.sendTicketByEmail(
                    context.event.getShortName(), UUID.fromString(ticket.getUuid()));
            ...
            updateTicketOwnerForm.setAdditional(
                    new HashMap<>(Collections.singletonMap("field1", Collections.singletonList("value"))));
            var updateTicketRes = ticketApiV2Controller.updateTicketInfo(
                    context.event.getShortName(),
                    UUID.fromString(ticket.getUuid()),
                    updateTicketOwnerForm,
                    new BeanPropertyBindingResult(updateTicketOwnerForm, "ticket"),
                    context.getPublicAuthentication());
            ...
            ticketFoundRes = ticketApiV2Controller.getTicketInfo(
                    context.event.getShortName(), UUID.fromString(ticket.getUuid()));
            ...
            reservation = reservationApiV2Controller
                    ...
                    .getBody();
            assertNotNull(reservation);
            ticket = reservation.getTicketsByCategory().stream()
                    .findFirst()
                    .orElseThrow()
                    .getTickets()
                    .get(0);
            ...
            ticketApiV2Controller.generateTicketPdf(
                    context.event.getShortName(), UUID.fromString(ticket.getUuid()), ticketPdfMockResp);
            ...
            ticketApiV2Controller.showQrCode(
                    context.event.getShortName(), UUID.fromString(ticket.getUuid()), ticketQRCodeResp);
            ...
            var qrCodeRead = qrCodeReader.decode(
                    new BinaryBitmap(new HybridBinarizer(new BufferedImageLuminanceSource(
                            ImageIO.read(new ByteArrayInputStream(ticketQRCodeResp.getContentAsByteArray()))))),
            ...
            assertEquals(
                    fullTicketInfo.ticketCode(
                            context.event.getPrivateKey(), context.event.supportsQRCodeCaseInsensitive()),
                    text);

            // can only be done for free tickets
            var releaseTicketFailure = ticketApiV2Controller.releaseTicket(
                    context.event.getShortName(), UUID.fromString(ticket.getUuid()));
            assertEquals(HttpStatus.BAD_REQUEST, releaseTicketFailure.getStatusCode());
            assertEquals(
                    HttpStatus.OK,
                    ticketApiV2Controller
                            .getTicketInfo(context.event.getShortName(), UUID.fromString(ticket.getUuid()))
                            .getStatusCode());

            // no invoice, but receipt
            assertEquals(
                    contactForm.isInvoiceRequested() ? HttpStatus.OK : HttpStatus.NOT_FOUND,
                    reservationApiV2Controller
                            ...
                            .getStatusCode());
            assertEquals(
                    contactForm.isInvoiceRequested() ? HttpStatus.NOT_FOUND : HttpStatus.OK,
                    reservationApiV2Controller
                            .getReceipt(
                                    context.event.getShortName(),
                                    reservationId,
...

            {
                ...
                switch (context.event.getFormat()) {
                    case IN_PERSON, HYBRID:
                        ...
                        assertTrue(
                                attendees.stream().anyMatch(sr -> sr.lastName().equals(fullTicketInfo.getLastName())));
                        assertTrue(attendees.stream().allMatch(sr -> sr.additionalInfo() != null));
                        assertEquals(
                                List.of("value"),
                                attendees.get(0).additionalInfo().get("field1"));
                        break;
                    case ONLINE:
                        assertTrue(results.getStatusCode().is2xxSuccessful());
                        assertNotNull(results.getBody());
                ...
                String ticketCode = fullTicketInfo.ticketCode(
                        context.event.getPrivateKey(), context.event.supportsQRCodeCaseInsensitive());
                TicketAndCheckInResult ticketAndCheckInResult = checkInApiController.findTicketWithUUID(
                        context.event.getId(), internalTicketIdentifier, ticketCode, principal);
                assertEquals(
                        CheckInStatus.OK_READY_TO_BE_CHECKED_IN,
                        ticketAndCheckInResult.getResult().getStatus());
                var basicCheckInStatus = checkInApiController.getTicketStatus(
                        context.event.getShortName(), internalTicketIdentifier, principal);
                ...
                assertEquals(
                        ...
                        checkInApiController
                                .checkIn(context.event.getId(), internalTicketIdentifier, tc, principal)
                                .getResult()
                                .getStatus());
                basicCheckInStatus = checkInApiController.getTicketStatus(
                        context.event.getShortName(), internalTicketIdentifier, principal);
                ...
                TicketAndCheckInResult ticketAndCheckInResultOk = checkInApiController.findTicketWithUUID(
                        context.event.getId(), internalTicketIdentifier, ticketCode, principal);
                assertEquals(
                        CheckInStatus.ALREADY_CHECK_IN,
                        ticketAndCheckInResultOk.getResult().getStatus());

                // check stats after check in one ticket
                assertTrue(
                        eventStatisticsManager
                                        .getTicketSoldStatistics(
                                                context.event.getId(), statisticsFrom, statisticsTo, "day")
                                        .stream()
                                        .mapToLong(TicketsByDateStatistic::getCount)
                                        .sum()
                                > 0);
                EventWithAdditionalInfo eventWithAdditionalInfo3 =
                        eventStatisticsManager.getEventWithAdditionalInfo(context.event.getShortName(), context.userId);
                ...
                assertTrue(
                        checkInApiController.revertCheckIn(context.event.getId(), internalTicketIdentifier, principal));

                assertFalse(
                        checkInApiController.revertCheckIn(context.event.getId(), internalTicketIdentifier, principal));
                TicketAndCheckInResult ticketAndCheckInResult2 = checkInApiController.findTicketWithUUID(
                        context.event.getId(), internalTicketIdentifier, ticketCode, principal);
                assertEquals(
                        CheckInStatus.OK_READY_TO_BE_CHECKED_IN,
                        ticketAndCheckInResult2.getResult().getStatus());

                UsersApiController.UserWithPasswordAndQRCode sponsorUser = usersApiController.insertUser(
                ...
                assertEquals(
                        ...
                        attendeeApiController
                                .scanBadge(
                                        new AttendeeApiController.SponsorScanRequest(
                                                "not-existing-event", "not-existing-ticket", null, null, null),
                                        sponsorPrincipal,
                                        null)
                                .getBody()
                                .getResult()
                ...
                assertEquals(
                        ...
                        attendeeApiController
                                .scanBadge(
                                        new AttendeeApiController.SponsorScanRequest(
                                                eventName, "not-existing-ticket", null, null, null),
                                        sponsorPrincipal,
                                        null)
                                .getBody()
                                .getResult()
                                .getStatus());
                assertEquals(
                        CheckInStatus.INVALID_TICKET_STATE,
                        attendeeApiController
                                .scanBadge(
                                        new AttendeeApiController.SponsorScanRequest(
                                                eventName, internalTicketIdentifier, null, null, null),
                                        sponsorPrincipal,
                                        null)
                                .getBody()
                                .getResult()
                ...
                assertTrue(
                        eventStatisticsManager
                                        ...
                                        .sum()
                                > 0);
                EventWithAdditionalInfo eventWithAdditionalInfo4 =
                        eventStatisticsManager.getEventWithAdditionalInfo(context.event.getShortName(), context.userId);
                ...
                TicketAndCheckInResult ticketAndcheckInResult =
                        checkInApiController.checkIn(context.event.getId(), internalTicketIdentifier, tc2, principal);
                assertEquals(
                        CheckInStatus.SUCCESS,
                        ticketAndcheckInResult.getResult().getStatus());
                ...
                var offlineIdentifiers = checkInApiController.getOfflineIdentifiers(
                        context.event.getShortName(), 0L, new MockHttpServletResponse(), principal);
                ...
                if (context.checkInStationsEnabled) {
                    ...
                    // disable Alf.io-PI
                    configurationRepository.insert(
                            ConfigurationKeys.ALFIO_PI_INTEGRATION_ENABLED.name(), "false", null);
                    offlineIdentifiers = checkInApiController.getOfflineIdentifiers(
                            context.event.getShortName(), 0L, new MockHttpServletResponse(), principal);
                    ...
                    configurationRepository.insertEventLevel(
                            context.event.getOrganizationId(),
                            context.event.getId(),
                            ConfigurationKeys.OFFLINE_CHECKIN_ENABLED.name(),
                            "true",
                            null);
                    ...
                    TicketWithCategory ticketwc = testEncryptedCheckInPayload(
                            principal, ticketAndcheckInResult, offlineIdentifiers, false, context);

                    // insert a poll and download again encrypted data. This time we expect a pin to be present because
                    ...
                    assertTrue(attendeeApiController
                            .getScannedBadges(
                                    context.event.getShortName(),
                    ...
                    assertEquals(
                            ...
                            attendeeApiController
                                    .scanBadge(
                                            new AttendeeApiController.SponsorScanRequest(
                                                    eventName, ticketwc.getUuid(), null, null, null),
                                            sponsorPrincipal,
                                            null)
                                    .getBody()
                                    .getResult()
                    ...
                    assertEquals(
                            ...
                            attendeeApiController
                                    .getScannedBadges(
                                            context.event.getShortName(),
                                            EventUtil.JSON_DATETIME_FORMATTER.format(
                    ...
                    eventApiController.downloadSponsorScanExport(
                            context.event.getShortName(), "csv", response, principal);
                    ...
                    MappingIterator<SponsorScanRecord> values = mapper.readerFor(SponsorScanRecord.class)
                            .with(schema)
                            .readValues(BOMInputStream.builder()
                                    .setReader(new StringReader(response.getContentAsString()))
                                    .get());
                    ...
                    var internalUUID = ticketRepository
                            .getInternalUuid(UUID.fromString(ticket.getUuid()), context.event.getId())
                            .orElseThrow();
                    assertEquals(
                            CheckInStatus.SUCCESS,
                            attendeeApiController
                                    .scanBadge(
                                            new AttendeeApiController.SponsorScanRequest(
                                                    eventName, internalUUID, "this is a very good lead!", "HOT", null),
                                            sponsorPrincipal,
                                            null)
                                    .getBody()
                                    .getResult()
                                    .getStatus());
                    var scannedBadges = attendeeApiController
                            .getScannedBadges(
                                    context.event.getShortName(),
                                    EventUtil.JSON_DATETIME_FORMATTER.format(LocalDateTime.of(1970, 1, 1, 0, 0)),
                                    sponsorPrincipal)
                    ...
                    assertEquals(
                            ...
                            attendeeApiController
                                    ...
                                    .getResult()
                                    .getStatus());
                    scannedBadges = attendeeApiController
                            .getScannedBadges(
                                    context.event.getShortName(),
                                    EventUtil.JSON_DATETIME_FORMATTER.format(LocalDateTime.of(1970, 1, 1, 0, 0)),
                                    sponsorPrincipal)
                    ...
                    ticketAndcheckInResult = checkInApiController.checkIn(
                            context.event.getId(), internalTicketIdentifier, badgeScan, principal);
                    // ONCE_PER_DAY is disabled by default, therefore we get an error
                    assertEquals(
                            CheckInStatus.EMPTY_TICKET_CODE,
                            ticketAndcheckInResult.getResult().getStatus());
                    ...
                    ticketCategoryRepository.update(
                            category.getId(),
                            category.getName(),
                            category.getInception(context.event.getZoneId()),
                            category.getExpiration(context.event.getZoneId()),
                            category.getMaxTickets(),
                            category.isAccessRestricted(),
                            MonetaryUtil.unitToCents(category.getPrice(), category.getCurrencyCode()),
                            category.getCode(),
                            category.getValidCheckInFrom(),
                            category.getValidCheckInTo(),
                            category.getTicketValidityStart(),
                            category.getTicketValidityEnd(),
                    ...
                    eventRepository.updateHeader(
                            context.event.getId(),
                            context.event.getDisplayName(),
                            context.event.getWebsiteUrl(),
                            context.event.getExternalUrl(),
                            context.event.getTermsAndConditionsUrl(),
                            context.event.getPrivacyPolicyUrl(),
                            context.event.getImageUrl(),
                            context.event.getFileBlobId(),
                            context.event.getLocation(),
                            context.event.getLatitude(),
                            context.event.getLongitude(),
                            context.event.now(clockProvider).minusSeconds(1),
                            context.event.getEnd(),
                            context.event.getTimeZone(),
                            context.event.getOrganizationId(),
                            context.event.getLocales(),
                            context.event.getFormat());

                    ticketAndcheckInResult = checkInApiController.checkIn(
                            context.event.getId(), internalTicketIdentifier, badgeScan, principal);
                    // we have already scanned the ticket today, so we expect to receive a warning
                    assertEquals(
                            CheckInStatus.BADGE_SCAN_ALREADY_DONE,
                            ticketAndcheckInResult.getResult().getStatus());
                    assertEquals(1, (int) auditingRepository.countAuditsOfTypeForReservation(
                            reservationId, Audit.EventType.BADGE_SCAN));
                    ...
                    assertEquals(
                            3,
                            jdbcTemplate.update(
                                    "update auditing set event_time = event_time - interval '1 day' where reservation_id = :reservationId and event_type in ('BADGE_SCAN', 'CHECK_IN')",
                    ...
                    assertEquals(
                            CheckInStatus.BADGE_SCAN_SUCCESS,
                            ticketAndcheckInResult.getResult().getStatus());
                    assertEquals(2, (int) auditingRepository.countAuditsOfTypeForReservation(
                            reservationId, Audit.EventType.BADGE_SCAN));
                } else {
            ...
            assertTrue(organizationDeleter.deleteOrganization(
                    context.event.getOrganizationId(),
    ...
    private void checkReservationExport(ReservationFlowContext context) {
        ...
        assertEquals(
                1,
                reservationsByEvent.get(0).getReservations().get(0).getTickets().size());
        ...
        assertThrows(
                IllegalArgumentException.class, () -> exportManager.reservationsForInterval(wrongFrom, now, principal));
    }

    static void insertExtension(ExtensionService extensionService, String path, Stream<String> events)
            throws IOException {
        ...
        try (var extensionInputStream = requireNonNull(BaseReservationFlowTest.class.getResourceAsStream(path))) {
            ...
            if (sync) {
                extensionService.createOrUpdate(
                        null,
                        null,
                        new Extension("-", "sync" + name, concatenation.replace("placeHolder", "false"), true));
            }
            if (async) {
                extensionService.createOrUpdate(
                        null,
                        null,
                        new Extension("-", "async" + name, concatenation.replace("placeHolder", "true"), true));
            }
        }
    }
    ...
    protected void performAndValidatePayment(
        ...
        var tStatus = reservationApiV2Controller.getTransactionStatus(
                reservationId, StaticPaymentMethods.BANK_TRANSFER.name());
        ...
        checkStatus(
                reservationId, HttpStatus.OK, true, TicketReservation.TicketReservationStatus.OFFLINE_PAYMENT, context);

        tStatus = reservationApiV2Controller.getTransactionStatus(
                reservationId, StaticPaymentMethods.BANK_TRANSFER.name());
    ...
    private TicketWithCategory testEncryptedCheckInPayload(
            Principal principal,
            TicketAndCheckInResult ticketAndcheckInResult,
            List<Integer> offlineIdentifiers,
            boolean expectPin,
            ReservationFlowContext context) {
        Map<String, String> payload = checkInApiController.getOfflineEncryptedInfo(
                context.event.getShortName(), Collections.emptyList(), offlineIdentifiers, principal);
        ...
        assertEquals(
                TicketCategory.TicketCheckInStrategy.ONCE_PER_EVENT.name(), jsonPayload.get("categoryCheckInStrategy"));
        return ticketwc;
    }
    ...
    protected void testAddSubscription(ReservationFlowContext context, int numberOfTickets, String categoryName) {
        ...
        assertEquals(
                numberOfTickets,
                reservation.getTicketsByCategory().get(0).getTickets().size());
        ...
        reservationCodeForm.setCode(
                Objects.requireNonNullElseGet(context.subscriptionPin, context.subscriptionId::toString));
        ...
        var codeApplicationResult =
                reservationApiV2Controller.applyCode(reservationId, reservationCodeForm, bindingResult);
        ...
        var summaryRowSubscription = reservation.getOrderSummary().getSummary().stream()
                .filter(r -> r.getType() == SummaryRow.SummaryType.APPLIED_SUBSCRIPTION)
                .findFirst();
        ...
        var handleRes = reservationApiV2Controller.confirmOverview(
                reservationId, "en", paymentForm, propertyBindingResult, new MockHttpServletRequest(), null);
    ...
    private void triggerEmailProcessingAndCheck(ReservationFlowContext context, String reservationId) {
        ...
        assertTrue(result > 0);
        assertTrue(notificationManager.loadAllMessagesForReservationId(context.event, reservationId).stream()
                .allMatch(m -> m.getStatus() == EmailMessage.Status.SENT));
    }
    ...
    protected boolean containsOnlineTickets(ReservationFlowContext context, String reservationId) {
        ...
        Integer count = jdbcTemplate.queryForObject(
                "select count(*) from ticket t join ticket_category tc on t.category_id = tc.id where t.tickets_reservation_id = :reservationId and tc.ticket_access_type = 'ONLINE'",
                Map.of("reservationId", reservationId),
                Integer.class);
