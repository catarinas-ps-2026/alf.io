...
class EventUtilTest {
    ...
    void setUp() {
        ...
        when(last.getZonedExpiration())
                .thenReturn(ZonedDateTime.now(clockProvider().getClock()).plusDays(1));
        when(last.getUtcExpiration())
                .thenReturn(ZonedDateTime.now(clockProvider().getClock()).plusDays(1));
        when(last.getZonedInception())
                .thenReturn(ZonedDateTime.now(clockProvider().getClock()).minusDays(1));
        when(last.getUtcInception())
                .thenReturn(ZonedDateTime.now(clockProvider().getClock()).minusDays(1));
        when(last.getAvailableTickets()).thenReturn(0);
        when(first.getZonedInception())
                .thenReturn(ZonedDateTime.now(clockProvider().getClock()).minusDays(2));
        when(first.getUtcInception())
                .thenReturn(ZonedDateTime.now(clockProvider().getClock()).minusDays(2));
        when(first.getUtcExpiration())
                .thenReturn(ZonedDateTime.now(clockProvider().getClock()).plusHours(2));
        // when(configurationManager.getFor(eq(STOP_WAITING_QUEUE_SUBSCRIPTIONS),
        // any())).thenReturn(buildConf(STOP_WAITING_QUEUE_SUBSCRIPTIONS));
    }
    ...
    static ConfigurationManager.MaybeConfiguration buildConf(ConfigurationKeys k, boolean value) {
        return new ConfigurationManager.MaybeConfiguration(
                k, new ConfigurationKeyValuePathLevel(k.getValue(), Boolean.toString(value), null));
    }
    ...
    void displayWaitingQueueFormBeforeSalesStart() {
        ...
        when(last.getZonedExpiration())
                .thenReturn(ZonedDateTime.now(clockProvider().getClock()).plusDays(2));
        when(last.getUtcExpiration())
                .thenReturn(ZonedDateTime.now(clockProvider().getClock()).plusDays(2));
        when(last.getZonedInception())
                .thenReturn(ZonedDateTime.now(clockProvider().getClock()).plusDays(1));
    ...
    void displayWaitingQueueFormBeforeSalesStart2() {
        ...
        when(first.getUtcInception())
                .thenReturn(ZonedDateTime.now(clockProvider().getClock()).plusDays(1));
        when(last.getZonedExpiration())
                .thenReturn(ZonedDateTime.now(clockProvider().getClock()).plusDays(3));
        when(last.getUtcExpiration())
                .thenReturn(ZonedDateTime.now(clockProvider().getClock()).plusDays(3));
        when(last.getZonedInception())
                .thenReturn(ZonedDateTime.now(clockProvider().getClock()).plusDays(2));
