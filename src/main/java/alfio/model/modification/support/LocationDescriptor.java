...
public record LocationDescriptor(String timeZone, String latitude, String longitude, String mapUrl) {
    ...
    public static LocationDescriptor fromGeoData(
            EventFormat format,
            Pair<String, String> coordinates,
            TimeZone timeZone,
            Map<ConfigurationKeys, ConfigurationManager.MaybeConfiguration> geoConf) {
        ...
        return new LocationDescriptor(
                timeZone.getID(), coordinates.getLeft(), coordinates.getRight(), getMapUrl(lat, lng, geoConf));
    }

    public static String getMapUrl(
            String lat, String lng, Map<ConfigurationKeys, ConfigurationManager.MaybeConfiguration> geoConf) {
    ...
    public static ConfigurationKeys.GeoInfoProvider getProvider(
            Map<ConfigurationKeys, ConfigurationManager.MaybeConfiguration> geoConf) {
        if ((!geoConf.containsKey(ConfigurationKeys.MAPS_PROVIDER)
                        || geoConf.get(ConfigurationKeys.MAPS_PROVIDER).isEmpty())
                && (geoConf.containsKey(ConfigurationKeys.MAPS_CLIENT_API_KEY)
                        && geoConf.get(ConfigurationKeys.MAPS_CLIENT_API_KEY).isPresent())) {
            return ConfigurationKeys.GeoInfoProvider.GOOGLE;
        } else if (geoConf.containsKey(ConfigurationKeys.MAPS_PROVIDER)
                && geoConf.get(ConfigurationKeys.MAPS_PROVIDER).isPresent()) {
            return geoConf.get(ConfigurationKeys.MAPS_PROVIDER)
                    .getValue()
                    .map(ConfigurationKeys.GeoInfoProvider::valueOf)
    ...
    private static String mapUrl(ConfigurationKeys.GeoInfoProvider provider) {
        return switch (provider) {
            case GOOGLE -> "https://maps.googleapis.com/maps/api/staticmap?center=${latitude},${longitude}&key=${key}&zoom=16&size=400x400&markers=color:blue%7Clabel:E%7C${latitude},${longitude}";
            case HERE -> "https://image.maps.ls.hereapi.com/mia/1.6/mapview?c=${latitude},${longitude}&z=16&w=400&h=400&poi=${latitude},${longitude}&apikey=${key}";
            default -> "";
        };
    }

    private static void fillParams(
            ConfigurationKeys.GeoInfoProvider provider,
            Map<ConfigurationKeys, ConfigurationManager.MaybeConfiguration> geoConf,
