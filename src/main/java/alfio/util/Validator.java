...
public final class Validator {
    ...
    // source:
    // https://commons.apache.org/proper/commons-validator/apidocs/src-html/org/apache/commons/validator/routines/EmailValidator.html
    private static final Pattern SIMPLE_E_MAIL_PATTERN =
            Pattern.compile("^\\p{javaLetterOrDigit}[^\\s@]*@\\p{javaLetterOrDigit}[^\\s]*$");
    // this regex validates the e-mail to be a canonical address (i.e. test@example.org)
    private static final Pattern CANONICAL_MAIL_VALIDATOR = Pattern.compile(
            "^\\p{javaLetterOrDigit}[^\\s@]*@\\p{javaLetterOrDigit}[^\\s@]*\\.\\p{javaAlphabetic}{2,}$");
    ...
    public static ValidationResult validateEventHeader(
            Optional<Event> event, EventModification ev, int descriptionMaxLength, Errors errors) {
        ...
        if (descriptions == null
                || descriptions.values().stream()
                        .anyMatch(v -> v == null || v.isBlank() || v.length() > descriptionMaxLength)) {
    ...
    private static boolean isLocationMissing(EventModification em) {
        ...
        return descriptor == null || isAnyBlank(descriptor.timeZone());
    }
    ...
    public static ValidationResult validateTicketCategories(EventModification ev, Errors errors) {
        ...
        if (ev.getTicketCategories().stream()
                .anyMatch(tc -> tc.getTicketAccessType() == TicketCategory.TicketAccessType.INHERIT
                        && ev.getFormat() == Event.EventFormat.HYBRID)) {
    ...
    public static ValidationResult validateEventDates(EventModification ev, Errors errors) {
        if (ev.getBegin() == null
                || ev.getBegin().getDate() == null
                || ev.getBegin().getTime() == null) {
    ...
    public static ValidationResult validateCategory(
            TicketCategoryModification category,
            Errors errors,
            String prefix,
        ...
        return validateCategory(
                category,
                prefix,
                eventModification.getBegin().toLocalDateTime(),
                eventModification.getEnd().toLocalDateTime(),
                errors,
    ...
    public static ValidationResult validateCategory(
        ...
        var ticketValidityCheckStatus = checkDateRange(
                category::getTicketValidityStart, category::getTicketValidityEnd, eventBegin, eventEnd, true);
        if (ticketValidityCheckStatus != RangeValidationResultType.OK) {
            String propertyName = ticketValidityCheckStatus == RangeValidationResultType.FAILED_BEGIN
                    ? "ticketValidityStart"
                    : "ticketValidityEnd";
        ...
        // check-in dates
        var checkInDateCheckStatus =
                checkDateRange(category::getValidCheckInFrom, category::getValidCheckInTo, eventBegin, eventEnd, false);
        if (checkInDateCheckStatus != RangeValidationResultType.OK) {
            String propertyName = checkInDateCheckStatus == RangeValidationResultType.FAILED_BEGIN
                    ? "validCheckInFrom"
                    : "validCheckInTo";
    ...
    private static boolean isCategoryExpirationAfterEventEnd(
            TicketCategoryModification category, LocalDateTime eventEnd) {
    ...
    public static ValidationResult evaluateValidationResult(Errors errors) {
        if (errors.hasFieldErrors()) {
            return ValidationResult.failed(errors.getFieldErrors().stream()
                    .map(ValidationResult.ErrorDescriptor::fromFieldError)
                    .toList());
        }
        return ValidationResult.success();
    }

    public static ValidationResult performAdvancedValidation(
            AdvancedTicketAssignmentValidator advancedValidator, AdvancedValidationContext context, Errors errors) {
        if (errors == null) {
            return ValidationResult.success();
        }
    ...
    public static class AdditionalFieldsFilterer {
        ...
        public AdditionalFieldsFilterer(
                List<PurchaseContextFieldConfiguration> purchaseContextFields,
            ...
            this.additionalFieldsForReservation = additionalServiceItems.stream()
        ...
        public List<PurchaseContextFieldConfiguration> getFieldsForTicket(
                UUID publicUuid, Set<PurchaseContextFieldConfiguration.Context> requestedContexts) {
            var ticketOptional = ticketsInReservation.stream()
                    .filter(t -> t.getPublicUuid().equals(publicUuid))
                    .findFirst();
            if (ticketOptional.isEmpty()) {
                return List.of();
            }
            var ticket = ticketOptional.get();
            var isFirstTicket = firstTicketInReservation
                    .map(first -> ticket.getId() == first.getId())
                    .orElse(false);
            return filterFieldsForTicket(
                    purchaseContextFields,
                    ticket,
                    additionalFieldsForReservation,
                    isFirstTicket,
                    eventSupportsAdditionalFieldsLink,
        ...
        private static List<PurchaseContextFieldConfiguration> filterFieldsForTicket(
                List<PurchaseContextFieldConfiguration> additionalFieldsForEvent,
                Ticket ticket,
            ...
            return additionalFieldsForEvent.stream()
                    ...
                    .filter(f -> {
                        ...
                        return included
                                && (!eventSupportsAdditionalFieldsLink
                                        || checkLinked(ticket, additionalServiceItems, f));
                    })
                    .toList();
        }
        ...
        private static boolean checkLinked(
            ...
            return additionalServiceItems.stream()
                    .anyMatch(asi -> tfc.getAdditionalServiceId() == asi.getAdditionalServiceId()
                            && Objects.equals(ticket.getId(), asi.getTicketId()));
        }

        private static boolean isAdditionalServiceIncluded(
                PurchaseContextFieldConfiguration f, Set<Integer> additionalServiceIds) {
    ...
    public static ValidationResult validateTicketAssignment(
        ...
        if (event.mustUseFirstAndLastName()) {
            ...
            validateMaxLength(
                    form.getFirstName(), prefix + FIRST_NAME, ErrorsCode.STEP_2_MAX_LENGTH_FIRSTNAME, 255, errors);

            ValidationUtils.rejectIfEmptyOrWhitespace(errors, prefix + LAST_NAME, ErrorsCode.STEP_2_EMPTY_LASTNAME);
            validateMaxLength(
                    form.getLastName(), prefix + LAST_NAME, ErrorsCode.STEP_2_MAX_LENGTH_LASTNAME, 255, errors);
        } else {
            ValidationUtils.rejectIfEmptyOrWhitespace(errors, prefix + FULL_NAME, ErrorsCode.STEP_2_EMPTY_FULLNAME);
            validateMaxLength(
                    form.getFullName(), prefix + FULL_NAME, ErrorsCode.STEP_2_MAX_LENGTH_FULLNAME, 255, errors);
        }
    ...
    public static ValidationResult validateAdditionalItemFieldsForTicket(
            AdditionalServiceLinkForm form,
            List<PurchaseContextFieldConfiguration> additionalFieldsForTicket,
        ...
        var allKeys = allForms.stream()
                .flatMap(f -> f.getAdditional().keySet().stream())
                .collect(Collectors.toSet());
        ...
        for (PurchaseContextFieldConfiguration fieldConf : additionalFieldsForTicket) {
            validateFieldConfiguration(
                    form, vatValidator, errors, prefix + ".", fieldConf, foundKeys.contains(fieldConf.getName()));
        }
        return evaluateValidationResult(errors);
    }

    public static void validateFieldConfiguration(
            AdditionalFieldsContainer form,
            SameCountryValidator vatValidator,
            Errors errors,
    ...
    private static void validateFieldConfiguration(
            AdditionalFieldsContainer form,
            SameCountryValidator vatValidator,
            Errors errors,
            String prefixForLambda,
        ...
        boolean isFieldPresent =
                form.getAdditional() != null && form.getAdditional().containsKey(fieldConf.getName());

        if (!skipPresenceCheck && !isFieldPresent) {
            if (fieldConf
                    .isRequired()) { // sometimes the field is not propagated, so, if it's required, we need to do some
                // additional work
                ...
                errors.rejectValue(
                        prefixForLambda + ADDITIONAL_PREFIX + fieldConf.getName() + "][0]", ErrorsCode.EMPTY_FIELD);
            }
            return;
        }
        ...
        if (fieldConf.isRequired()
                && fieldConf.getCount() > 1
                && values.stream().allMatch(StringUtils::isBlank)) {
    ...
    private static void validateFieldValue(
        ...
        try {
            if (fieldConf.isEuVat() && !vatValidator.test(formValue)) {
                errors.rejectValue(
                        prefixForLambda + ADDITIONAL_PREFIX + fieldConf.getName() + "][" + i + "]",
                        ErrorsCode.STEP_2_INVALID_VAT);
            }
        } catch (IllegalStateException e) {
            errors.rejectValue(
                    prefixForLambda + ADDITIONAL_PREFIX + fieldConf.getName() + "][" + i + "]",
                    ErrorsCode.VIES_IS_DOWN);
        }
    }

    private static void fieldValueBasicValidation(
            Errors errors,
            String prefixForLambda,
            PurchaseContextFieldConfiguration fieldConf,
            int i,
        ...
        if (!fieldConf.getRestrictedValues().isEmpty()) {
            validateRestrictedValue(
                    formValue,
                    prefixForLambda + ADDITIONAL_PREFIX + fieldConf.getName() + "][" + i + "]",
                    ERROR_RESTRICTED_VALUE,
                    fieldConf.getRestrictedValues(),
                    errors);
        }

        if (fieldConf.isRequired() && fieldConf.getCount() == 1 && StringUtils.isBlank(formValue)) {
            errors.rejectValue(
                    prefixForLambda + ADDITIONAL_PREFIX + fieldConf.getName() + "][" + i + "]", ErrorsCode.EMPTY_FIELD);
        }
    ...
    private static void validateLength(
        ...
        if (isDateOfBirth) {
            ...
            validateDateInThePast(
                    formValue,
                    prefixForLambda + ADDITIONAL_PREFIX + fieldConf.getName() + "][" + fieldIndex + "]",
                    errors);
        }
        ...
        if (fieldConf.isMaxLengthDefined()) {
            validateMaxLength(
                    formValue,
                    prefixForLambda + ADDITIONAL_PREFIX + fieldConf.getName() + "][" + fieldIndex + "]",
                    isDateOfBirth ? "error.tooOld" : "error.tooLong",
                    fieldConf,
                    errors);
        }
        ...
        if (StringUtils.isNotBlank(formValue) && fieldConf.isMinLengthDefined()) {
            validateMinLength(
                    formValue,
                    prefixForLambda + ADDITIONAL_PREFIX + fieldConf.getName() + "][" + fieldIndex + "]",
                    isDateOfBirth ? "error.tooYoung" : "error.tooShort",
                    fieldConf,
                    errors);
        }
    }

    private static void validateRestrictedValue(
            String value, String fieldName, String errorCode, List<String> restrictedValues, Errors errors) {
    ...
    public static boolean isEmailValid(String email) {
        return StringUtils.isNotEmpty(email)
                && !email.strip().endsWith(".")
                && SIMPLE_E_MAIL_PATTERN.matcher(email).matches();
    }

    public static boolean isCanonicalMailAddress(String email) {
        return StringUtils.isNotEmpty(email)
                && CANONICAL_MAIL_VALIDATOR.matcher(email).matches();
    }
    ...
    private static void validateMaxLength(
            String value, String fieldName, String errorCode, int maxLength, Errors errors) {
    ...
    public static ValidationResult validateWaitingQueueSubscription(
            WaitingQueueSubscriptionForm form, Errors errors, Event event) {
    ...
    public static ValidationResult validateAdditionalService(
            EventModification.AdditionalService additionalService, Errors errors) {
        return validateAdditionalService(additionalService, null, errors);
    }

    public static ValidationResult validateAdditionalService(
            EventModification.AdditionalService additionalService, EventModification eventModification, Errors errors) {
    ...
    private static boolean containsAllRequiredTranslations(
            EventModification eventModification, List<EventModification.AdditionalServiceText> descriptions) {
        ...
        return optional.isEmpty()
                || optional.map(e -> ContentLanguage.findAllFor(e.getLocales()))
                        .filter(l -> l.stream().allMatch(l1 -> descriptions.stream()
                                .anyMatch(d -> d.getLocale().equals(l1.getLanguage()))))
                        .isPresent();
    }
    ...
    public static ValidationResult validateAdditionalFields(
            List<PurchaseContextFieldConfiguration> fieldConf, AdditionalFieldRequest field, Errors errors) {
        String duplicateName = fieldConf.stream()
                .map(PurchaseContextFieldConfiguration::getName)
    ...
    public static class AdvancedTicketAssignmentValidator implements Function<AdvancedValidationContext, Result<Void>> {
        ...
        public AdvancedTicketAssignmentValidator(
                SameCountryValidator vatValidator, GroupManager.WhitelistValidator whitelistValidator) {
        ...
        public Result<Void> apply(AdvancedValidationContext context) {
            ...
            Optional<PurchaseContextFieldConfiguration> vatField = context.purchaseContextFieldConfigurations.stream()
                    ...
                    .filter(f -> context.updateTicketOwnerForm.getAdditional() != null
                            && context.updateTicketOwnerForm.getAdditional().containsKey(f.getName()))
                    .findFirst();

            Optional<String> vatNr =
                    vatField.map(c -> Objects.requireNonNull(context.updateTicketOwnerForm.getAdditional())
                            .get(c.getName())
                            .get(0));
            String vatFieldName =
                    vatField.map(PurchaseContextFieldConfiguration::getName).orElse("");

            return new Result.Builder<Void>()
                    .checkPrecondition(
                            () -> vatNr.isEmpty() || vatValidator.test(vatNr.get()),
                            ErrorCode.custom(ErrorsCode.STEP_2_INVALID_VAT, "additional['" + vatFieldName + "']"))
                    .checkPrecondition(
                            () -> whitelistValidator.test(new GroupManager.WhitelistValidationItem(
                                    context.categoryId, context.updateTicketOwnerForm.getEmail())),
                            ErrorCode.custom(ErrorsCode.STEP_2_WHITELIST_CHECK_FAILED, EMAIL_KEY))
                    .build(() -> null);
        }
    }
    ...
    public static class AdvancedValidationContext {
        ...
        public AdvancedValidationContext(
                UpdateTicketOwnerForm updateTicketOwnerForm,
                List<PurchaseContextFieldConfiguration> purchaseContextFieldConfigurations,
                int categoryId,
                String ticketUuid,
                String prefix) {
