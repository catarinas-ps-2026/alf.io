...
public class EventApiV1Controller {
    ...
    public ResponseEntity<String> create(@RequestBody EventCreationRequest request, Principal user) {
        String imageRef =
                Optional.ofNullable(request.getImageUrl()).map(this::fetchImage).orElse(null);
        Organization organization =
                userManager.findUserOrganizations(user.getName()).get(0);
        AtomicReference<Errors> errorsContainer = new AtomicReference<>();
        Result<String> result = new Result.Builder<String>()
                .checkPrecondition(
                        () -> isNotBlank(request.getTitle()), ErrorCode.custom("invalid.title", "Invalid title"))
                .checkPrecondition(
                        () -> isBlank(request.getSlug()) || eventNameManager.isUnique(request.getSlug()),
                        ErrorCode.custom("invalid.slug", "Invalid slug"))
                .checkPrecondition(
                        () -> isNotBlank(request.getWebsiteUrl()),
                        ErrorCode.custom("invalid.websiteUrl", "Invalid Website URL"))
                .checkPrecondition(
                        () -> isNotBlank(request.getTermsAndConditionsUrl()),
                        ErrorCode.custom("invalid.tc", "Invalid Terms and Conditions"))
                .checkPrecondition(
                        () -> isNotBlank(request.getImageUrl()),
                        ErrorCode.custom("invalid.imageUrl", "Invalid Image URL"))
                .checkPrecondition(
                        () -> isNotBlank(request.getTimezone()),
                        ErrorCode.custom("invalid.timezone", "Invalid Timezone"))
                .checkPrecondition(
                        () -> isNotBlank(imageRef),
                        ErrorCode.custom("invalid.image", "Image is either missing or too big (max 200kb)"))
                .checkPrecondition(
                        () -> validateCategoriesSalesPeriod(request),
                        ErrorCode.custom(
                                "invalid.categories",
                                "Ticket categories: sales period not compatible with event dates"))
                .checkPrecondition(
                        () -> validateAdditionalFields(request),
                        ErrorCode.custom("invalid.additionalInfo", "Additional info not valid"))
                .checkPrecondition(
                        () -> {
                            EventModification eventModification = request.toEventModification(
                            ...
                            int descriptionMaxLength = configurationManager
                                    .getFor(ConfigurationKeys.DESCRIPTION_MAXLENGTH, ConfigurationLevel.system())
                                    .getValueAsIntOrDefault(4096);
                            ValidationResult validationResult =
                                    validateEvent(eventModification, errorsContainer.get(), descriptionMaxLength);
    ...
    private boolean validateAdditionalFields(EventCreationRequest request) {
        return CollectionUtils.isEmpty(request.getAdditionalInfo())
                || request.getAdditionalInfo().stream()
    ...
    private boolean validateCategoriesSalesPeriod(EventCreationRequest request) {
        ...
        return request.getTickets().getCategories().stream()
                .allMatch(tc -> tc.getStartSellingDate().isBefore(tc.getEndSellingDate())
                        && tc.getEndSellingDate().isBefore(eventEnd));
    }
    ...
    public ResponseEntity<List<DownloadedAttendeesByCategory>> downloadAttendees(
            @PathVariable String slug, Principal user) {
        accessService.checkEventOwnership(user, slug);
        ...
        var ticketsByCategoryId = eventManager.findAllConfirmedTicketsForCSV(slug, user.getName()).stream()
                .filter(t ->
                        t.getTicket().getCategoryId() != null && t.getTicket().getAssigned())
        ...
        return ResponseEntity.ok(ticketCategories.stream()
                .filter(category -> ticketsByCategoryId.containsKey(category.getId()))
                .map(category -> {
                    ...
                    var downloadedAttendeesData = ticketsInCategory.stream()
                            .map(t -> {
                                ...
                                var additional = Objects.requireNonNullElse(
                                                additionalInfoByTicketId.get(ticket.getId()),
                                                List.<PurchaseContextFieldValue>of())
                                        .stream()
                                        .collect(Collectors.groupingBy(
                                                PurchaseContextFieldValue::getName,
                                                Collectors.mapping(
                                                        PurchaseContextFieldValue::getValue, Collectors.toList())));
                                return new DownloadedAttendeeData(
                                        ticket.getFirstName(),
                                        ticket.getLastName(),
                                        ticket.getEmail(),
                                        Map.of(),
    ...
    public ResponseEntity<String> delete(@PathVariable String slug, Principal user) {
        accessService.checkEventOwnership(user, slug);
        Result<String> result = new Result.Builder<String>().build(() -> {
            eventManager
                    .getOptionalEventAndOrganizationIdByName(slug, user.getName())
                    .ifPresent(e -> eventManager.deleteEvent(e.getId(), user.getName()));
            return "Ok";
        });
        if (result.isSuccess()) {
            return ResponseEntity.ok(result.getData());
    ...
    public ResponseEntity<String> update(
            @PathVariable String slug, @RequestBody EventCreationRequest request, Principal user) {
        accessService.checkEventOwnership(user, slug);
        String imageRef = fetchImage(request.getImageUrl());

        Result<String> result = new Result.Builder<String>().build(() -> updateEvent(slug, request, user, imageRef)
    ...
    public ResponseEntity<LinkedSubscription> updateLinkedSubscriptions(
        ...
        return ResponseEntity.ok(
                retrieveLinkedSubscriptionsForEvent(slug, eventAndOrgId.getId(), eventAndOrgId.getOrganizationId()));
    }
    ...
    private LinkedSubscription retrieveLinkedSubscriptionsForEvent(String slug, int id, int organizationId) {
        ...
        return new LinkedSubscription(
                slug,
                subscriptions.stream()
                        .collect(Collectors.toMap(
                                EventSubscriptionLink::getSubscriptionDescriptorId,
                                EventSubscriptionLink::getCompatibleCategories)));
    }
    ...
    private void handleCategoriesUpdate(
            Principal user, EventModification em, EventWithAdditionalInfo original, Event event) {
        if (em.getTicketCategories() != null && !em.getTicketCategories().isEmpty()) {
            var existingCategories = original.getTicketCategories();
            em.getTicketCategories().stream()
                    .sorted(Comparator.comparing(TicketCategoryModification::getMaxTickets))
                    .forEach(c -> {
                        var existingCategory = findExistingCategory(existingCategories, c.getName(), c.getId());
                        if (existingCategory.isPresent()) {
                            eventManager.updateCategory(
                                    existingCategory.get().getId(), event.getId(), c, user.getName());
                        } else {
    ...
    private Optional<String> insertEvent(EventCreationRequest request, Principal user, String imageRef) {
        try {
            ...
            EventModification em =
                    request.toEventModification(organization, eventNameManager::generateShortName, imageRef);
            eventManager.createEvent(em, user.getName());
            var eventWithStatistics =
                    eventStatisticsManager.getEventWithAdditionalInfo(em.getShortName(), user.getName());
            var event = eventWithStatistics.getEvent();
            Optional.ofNullable(request.getTickets().getPromoCodes())
                    .ifPresent(promoCodes -> promoCodes.forEach(
                            pc -> // TODO add ref to categories
                            eventManager.addPromoCode(
                                            ...
                                            null,
                                            pc.getDiscountType() != PromoCodeDiscount.DiscountType.PERCENTAGE
                                                    ? event.getCurrency()
                                                    : null)));
            ...
            request.getTickets().getCategories().stream()
                    ...
                    .map(cr ->
                            Pair.of(cr, groupManager.findById(cr.getGroupLink().getGroupId(), organization.getId())))
                    .forEach(link -> {
                        if (link.getRight().isPresent()) {
                            ...
                            findExistingCategory(
                                            eventWithStatistics.getTicketCategories(),
                                            categoryRequest.getName(),
                                            categoryRequest.getId())
                                    .ifPresent(category -> {
                                        EventCreationRequest.GroupLinkRequest groupLinkRequest =
                                                categoryRequest.getGroupLink();
            ...
            if (!CollectionUtils.isEmpty(request.getExtensionSettings())) {
                request.getExtensionSettings().stream()
                        ...
                        .forEach((id, settings) -> {
                            List<ExtensionSupport.ExtensionMetadataIdAndName> metadata = extensionService
                                    .getSingle(organization, event, id)
                            ...
                            List<ExtensionMetadataValue> values = settings.stream()
                                    .map(es -> Pair.of(
                                            es,
                                            metadata.stream()
                                                    .filter(mm -> mm.getName().equals(es.getKey()))
                                                    .findFirst()))
                                    .filter(pair -> {
                                        if (pair.getRight().isEmpty()) {
                                            log.warn(
                                                    "ignoring non-existent extension setting key {}",
                                                    pair.getLeft().getKey());
                                        }
                                        return pair.getRight().isPresent();
                                    })
                                    .map(pair -> new ExtensionMetadataValue(
                                            pair.getRight().get().getId(),
