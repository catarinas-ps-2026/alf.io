...
public class ReservationApiV1Controller {
    ...
    public ReservationApiV1Controller(
    ...
    public ResponseEntity<ReservationDetail> retrieveDetail(
        ...
        if (purchaseContext.ofType(PurchaseContextType.event)) {
            var conf = configurationManager.getFor(
                    EnumSet.of(ENABLE_WALLET, ENABLE_PASS, BASE_URL), purchaseContext.getConfigurationLevel());
            var tickets = adminReservationManager.findTicketsWithMetadata(reservationId);
            var valuesByTicketId = purchaseContextFieldManager.findAllValuesByTicketIds(
                    tickets.stream().map(t -> t.getTicket().getId()).toList());
            var ticketsByCategories =
                    tickets.stream().collect(groupingBy(t -> t.getTicket().getCategoryId()));
            attendeesByCategories = ticketsByCategories.keySet().stream()
                    .map(categoryId -> {
                        var ticketsForCategory = ticketsByCategories.get(categoryId);
                        var attendeesData = ticketsForCategory.stream()
                                .map(tfc -> {
                                    var ticket = tfc.getTicket();
                                    var additional = valuesByTicketId.getOrDefault(ticket.getId(), List.of()).stream()
                                            .collect(groupingBy(
                                                    PurchaseContextFieldValue::getName,
                                                    mapping(PurchaseContextFieldValue::getValue, toList())));
            ...
            var subscriptionDetails = ticketReservationManager
                    .findSubscriptionDetails(reservationId)
                    .orElseThrow();
            ...
            var metadata = requireNonNullElseGet(
                    adminReservationManager.findSubscriptionMetadata(subscriptionId), SubscriptionMetadata::empty);
            var fields = purchaseContextFieldManager
                    .findAllValuesBySubscriptionIds(List.of(subscriptionId))
                    .getOrDefault(subscriptionId, List.of())
                    .stream()
                    .collect(groupingBy(
                            PurchaseContextFieldValue::getName,
                            mapping(PurchaseContextFieldValue::getValue, toList())));
            subscriptionOwners = List.of(new Owner(
                    fields,
                    subscriptionId,
        ...
        return ResponseEntity.ok(new ReservationDetail(
                reservationId,
                reservation.getStatus(),
                new ReservationUser(
                        null, reservation.getFirstName(), reservation.getLastName(), reservation.getEmail(), null),
                attendeesByCategories,
                subscriptionOwners));
    }

    @PostMapping("/event/{slug}/reservation")
    @Transactional
    public ResponseEntity<CreationResponse> createTicketsReservation(
        ...
        Optional<String> promoCodeDiscount = ReservationUtil.checkPromoCode(
                reservationCreationRequest, event, promoCodeRequestManager, bindingResult);
        var locale = Locale.forLanguageTag(requireNonNullElseGet(
                reservationCreationRequest.getLanguage(),
                () -> event.getContentLanguages().get(0).getLanguage()));
        var selected = ReservationUtil.validateCreateRequest(
        ...
        if (selected.isPresent() && !bindingResult.hasErrors()) {
            ...
            return handleReservationCreationErrors(
                            () -> ticketReservationManager.createTicketReservation(
                                    event, pair.getLeft(), pair.getRight(), promoCodeDiscount, locale, principal),
                            bindingResult,
                            event.getType())
            ...
            return ResponseEntity.badRequest()
                    .body(CreationResponse.error(bindingResult.getAllErrors().stream()
                            .map(err -> ErrorCode.custom("invalid." + err.getObjectName(), err.getCode()))
                            .collect(toList())));
        }
    }

    @PostMapping("/event/{slug}/reservation/import")
    public ResponseEntity<BulkCreationResponse> bulkCreateEventReservation(
    ...
    public ResponseEntity<AdminReservationRequestStats> getRequestStatus(
            @PathVariable String slug, @PathVariable String requestId, Principal principal) {
    ...
    @Transactional
    public ResponseEntity<CreationResponse> createSubscriptionReservation(
        ...
        var locale = Locale.forLanguageTag(requireNonNullElseGet(
                creationRequest.getLanguage(),
                () -> subscriptionDescriptor.getContentLanguages().get(0).getLanguage()));
        return handleReservationCreationErrors(
                        () -> ticketReservationManager.createSubscriptionReservation(
                                subscriptionDescriptor, locale, principal, creationRequest.getMetadataOrNull()),
                        bindingResult,
                        subscriptionDescriptor.getType())
    ...
    public ResponseEntity<ReservationConfirmationResponse> confirmReservation(
        ...
        if (result.isSuccess()) {
            ...
            if (purchaseContext.ofType(PurchaseContextType.event)) {
                return ResponseEntity.ok(new ReservationConfirmationResponse(data.getMiddle().stream()
                        .map(t -> new HolderDetail(t.getUuid(), t.getFirstName(), t.getLastName(), t.getEmail()))
                        .collect(toList())));
            } else {
                var subscriptions = ticketReservationManager
                        .findSubscriptionDetails(data.getLeft().getId())
                        .map(List::of)
                        .orElseGet(List::of);
                return ResponseEntity.ok(new ReservationConfirmationResponse(subscriptions.stream()
                        .map(s -> {
                            var subscription = s.getSubscription();
                            return new HolderDetail(
    ...
    public ResponseEntity<Boolean> deleteTicket(
            @PathVariable String slug, @PathVariable String ticketUUID, Principal user) {
        accessService.checkEventTicketIdentifierMembership(
                user, slug, ticketUUID, EnumSet.of(Role.OWNER, Role.API_CONSUMER));
        return ResponseEntity.of(adminReservationManager
                .findTicketWithReservationId(ticketUUID, slug, user.getName())
                .map(ticket -> {
                    // make sure that ticket belongs to the same event
                    var result = adminReservationManager.removeTickets(
                            slug,
    ...
    public ResponseEntity<Boolean> deleteSubscription(
            @PathVariable("id") String descriptorId, @PathVariable UUID subscriptionToDelete, Principal user) {
        ...
        return ResponseEntity.of(adminReservationManager
                ...
                .map(descriptorAndReservationId -> {
                    var result = adminReservationManager.removeSubscription(
                            descriptorAndReservationId.getKey(),
                            descriptorAndReservationId.getValue(),
    ...
    private CreationResponse postCreate(
            ReservationAPICreationRequest creationRequest, String id, PurchaseContext purchaseContext, Locale locale) {
        ...
        if (reservationConfiguration != null) {
            var reservationMetadata = new ReservationMetadata(
                    reservationConfiguration.isHideContactData(),
                    false,
                    false,
        ...
        var descriptorId =
                creationRequest instanceof TicketReservationCreationRequest trcr ? trcr.getSubscriptionId() : null;
        if (creationRequest instanceof SubscriptionReservationCreationRequest request && request.hasAdditionalInfo()) {
            var subscriptionId = ticketReservationManager
                    .findSubscriptionDetails(id)
                    .orElseThrow()
                    .getSubscription()
                    .getId();
            purchaseContextFieldManager.updateFieldsForReservation(
                    request.getSubscriptionOwner(), purchaseContext, null, subscriptionId);
        }
        return CreationResponse.success(
                id,
                ticketReservationManager.reservationUrlForExternalClients(
                        id, purchaseContext, locale.getLanguage(), user != null, descriptorId));
    }
