...
public class ConfigurationApiController {
    ...
    public boolean updateConfiguration(@RequestBody ConfigurationModification configuration, Principal principal) {
        ...
        configurationManager.saveSystemConfiguration(
                ConfigurationKeys.fromString(configuration.getKey()), configuration.getValue());
        return true;
    }
    ...
    public boolean updateConfiguration(
            ...
            Principal principal) {
        accessService.ensureAdmin(principal);
        List<ConfigurationModification> list = Objects.requireNonNull(input).values().stream()
    ...
    public Map<ConfigurationKeys.SettingCategory, List<Configuration>> loadOrganizationConfiguration(
    ...
    public boolean updateOrganizationConfiguration(
        ...
        accessService.checkOrganizationOwnership(principal, organizationId);
        //
        configurationManager.saveAllOrganizationConfiguration(
                organizationId,
                input.values().stream().flatMap(Collection::stream).toList(),
                principal.getName());
        return true;
    }
    ...
    public ResponseEntity<String> getSingleConfigForEvent(
            @PathVariable("eventName") String eventShortName, @PathVariable String key, Principal principal) {
        ...
        String singleConfigForEvent =
                configurationManager.getSingleConfigForEvent(event.getId(), key, principal.getName());
    ...
    public ResponseEntity<String> getSingleConfigForOrganization(
            @PathVariable int organizationId, @PathVariable String key, Principal principal) {
        accessService.checkOrganizationOwnership(principal, organizationId);
    ...
    public boolean updateEventConfiguration(
            @PathVariable int organizationId,
    ...
    public boolean updateCategoryConfiguration(
    ...
    public Map<ConfigurationKeys.SettingCategory, List<Configuration>> loadCategoryConfiguration(
    ...
    public boolean deleteOrganizationLevelKey(
            @PathVariable int organizationId, @PathVariable ConfigurationKeys key, Principal principal) {
        accessService.checkOrganizationOwnership(principal, organizationId);
    ...
    public boolean deleteEventLevelKey(
            @PathVariable int eventId, @PathVariable ConfigurationKeys key, Principal principal) {
        accessService.checkEventOwnership(principal, eventId);
    ...
    public boolean deleteCategoryLevelKey(
            @PathVariable int eventId,
            @PathVariable int categoryId,
    ...
    public List<Pair<String, String>> loadEUCountries() {
        return TicketHelper.getLocalizedEUCountriesForVat(
                Locale.ENGLISH,
    ...
    public InstanceSettings loadInstanceSettings() {
        var settings =
                configurationManager.getFor(EnumSet.of(DESCRIPTION_MAXLENGTH, BASE_URL), ConfigurationLevel.system());
        return new InstanceSettings(
                settings.get(DESCRIPTION_MAXLENGTH).getValueAsIntOrDefault(4096),
                settings.get(BASE_URL).getRequiredValue());
    }
    ...
    public Map<String, Boolean> loadPlatformModeStatus(@PathVariable int organizationId, Principal principal) {
        ...
        boolean platformModeEnabled =
                configurationManager.getForSystem(PLATFORM_MODE_ENABLED).getValueAsBooleanOrDefault();
        result.put("enabled", platformModeEnabled);
        if (platformModeEnabled) {
            var options = configurationManager.getFor(
                    List.of(STRIPE_CONNECTED_ID, MOLLIE_CONNECT_REFRESH_TOKEN),
                    ConfigurationLevel.organization(organizationId));
            result.put("stripeConnected", options.get(STRIPE_CONNECTED_ID).isPresent());
            result.put(
                    "mollieConnected", options.get(MOLLIE_CONNECT_REFRESH_TOKEN).isPresent());
        }
        return result;
    }
    ...
    public ResponseEntity<ZonedDateTime> getFirstInvoiceDate(@PathVariable Integer eventId, Principal principal) {
        ...
        return ResponseEntity.of(optionally(() -> eventManager.getSingleEventById(eventId, principal.getName()))
                .map(event -> billingDocumentManager
                        .findFirstInvoiceDate(event.getId())
                        .orElseGet(
                                () -> ZonedDateTime.now(clockProvider.getClock().withZone(event.getZoneId())))));
    }

    @GetMapping(value = "/event/{eventId}/matching-invoices")
    public ResponseEntity<List<Integer>> getMatchingInvoicesForEvent(
    ...
    public ResponseEntity<Boolean> regenerateInvoices(
            @PathVariable Integer eventId, @RequestBody List<Long> documentIds, Principal principal) {
        //
        ...
        return ResponseEntity.ok(adminJobManager.scheduleExecution(
                AdminJobExecutor.JobName.REGENERATE_INVOICES,
                Map.of(
    ...
    public ResponseEntity<Boolean> generateTicketsForSubscriptions(
        ...
        if (!admin
                && (organizationId == null
                        || !userManager.isOwnerOfOrganization(principal.getName(), organizationId))) {
        ...
        if (eventId != null && organizationId != null) {
            accessService.checkEventOwnership(principal, eventId, organizationId);
            jobMetadata = Map.of(
                    AssignTicketToSubscriberJobExecutor.EVENT_ID,
                    eventId,
                    AssignTicketToSubscriberJobExecutor.ORGANIZATION_ID,
                    organizationId);
        ...
        return ResponseEntity.ok(adminJobManager.scheduleExecution(
                ASSIGN_TICKETS_TO_SUBSCRIBERS, requireNonNullElse(jobMetadata, Map.of())));
    }

    @GetMapping(value = "/global-translations-override")
    public Map<String, Map<String, String>> loadTranslations() {
        return configurationManager
                .getFor(TRANSLATION_OVERRIDE, ConfigurationLevel.system())
                .getValue()
    ...
    public Map<String, Map<String, String>> loadTranslationsOverrideForOrganization(
            @PathVariable int organizationId, Principal principal) {
        accessService.checkOrganizationOwnership(principal, organizationId);
        return configurationManager
                .getFor(TRANSLATION_OVERRIDE, ConfigurationLevel.organization(organizationId))
                .getValue()
                .map(v -> Json.fromJson(v, new TypeReference<Map<String, Map<String, String>>>() {}))
                .orElse(Map.of());
    }
    ...
    public Map<String, Map<String, String>> loadTranslationsOverrideForEvent(
            @PathVariable int eventId, Principal principal) {
        var eventAndOrgId = accessService.checkEventOwnership(principal, eventId);
        return configurationManager
                .getFor(TRANSLATION_OVERRIDE, ConfigurationLevel.event(eventAndOrgId))
                .getValue()
                .map(v -> Json.fromJson(v, new TypeReference<Map<String, Map<String, String>>>() {}))
                .orElse(Map.of());
    }
    ...
    public ResponseEntity<List<UserDefinedOfflinePaymentMethod>> getPaymentMethodsForOrganization(
        ...
        if (includeDeleted) {
            paymentMethods =
                    customOfflineConfigurationManager.getOrganizationCustomOfflinePaymentMethodsIncludingDeleted(
                            organizationId);
        } else {
            paymentMethods =
                    customOfflineConfigurationManager.getOrganizationCustomOfflinePaymentMethods(organizationId);
        }
    ...
    public ResponseEntity<String> deletePaymentMethod(
            @PathVariable int organizationId, @PathVariable String paymentMethodId, Principal principal)
            throws CustomOfflinePaymentMethodDoesNotExistException,
                    CustomOfflinePaymentMethodIllegalDeletionStateException {
        accessService.checkOrganizationOwnership(principal, organizationId);
        ...
        var isPaymentMethodActivelyUsed = customOfflineConfigurationManager.isPaymentMethodCurrentlyUsedInActiveEvent(
                organizationId, existingPaymentMethod);
    ...
    public ResponseEntity<List<UserDefinedOfflinePaymentMethod>> getAllowedPaymentMethodsForEvent(
            @PathVariable Integer eventId, Principal principal) throws PassedIdDoesNotExistException {
        var eventAndOrgId = accessService.checkEventOwnership(principal, eventId);
    ...
    public ResponseEntity<Boolean> setEventAllowedPaymentMethods(
            @PathVariable Integer eventId, @RequestBody List<String> paymentMethodIds, Principal principal)
        ...
        try {
            customOfflineConfigurationManager.setAllowedCustomOfflinePaymentMethodsForEvent(event, paymentMethodIds);
        } catch (CustomOfflinePaymentMethodDoesNotExistException e) {
            throw new CustomOfflinePaymentMethodDoesNotExistException(
                    "One or more of the passed payment methods IDs do not exist.", e);
        }
