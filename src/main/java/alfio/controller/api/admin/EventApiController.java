...
public class EventApiController {
    ...
    public List<PaymentManager.PaymentMethodDTO> getPaymentProxies(
        ...
        return userManager.findUserOrganizations(principal.getName()).stream()
    ...
    public List<EventListItem> getAllEventsForExternal(
        ...
        List<Integer> userOrganizations = userManager.findUserOrganizations(authentication.getName()).stream()
        ...
        return eventManager.getEventsByDateRange(dateRangeForEvents).stream()
                .filter(e -> userOrganizations.contains(e.getOrganizationId())
                        && (includeOnline || e.getFormat() != Event.EventFormat.ONLINE))
                .sorted(Comparator.comparing(e -> e.getBegin().withZoneSameInstant(ZoneId.systemDefault())))
                .map(s -> new EventListItem(
                        s, request.getContextPath(), eventDescriptionRepository.findByEventId(s.getId())))
                .collect(toList());
    }
    ...
    public List<EventStatistic> getAllActiveEvents(Principal principal) {
        return eventStatisticsManager.getAllEventsWithStatisticsFilteredBy(
                principal.getName(), event -> !event.expiredSince(14));
    }

    @GetMapping("/expired-events")
    public List<EventStatistic> getAllExpiredEvents(Principal principal) {
        var results = new ArrayList<>(eventStatisticsManager.getAllEventsWithStatisticsFilteredBy(
                principal.getName(), event -> event.expiredSince(14)));
        // items are sorted by start_ts asc; expired events should be sorted by start_ts desc, so we need to reverse the
        // list.
    ...
    public ResponseEntity<EventAndOrganization> getSingleEvent(
        ...
        return optionally(() -> eventStatisticsManager.getEventWithAdditionalInfo(eventName, username))
                .map(event -> {
                    EventAndOrganization out =
                            new EventAndOrganization(event, eventManager.loadOrganizer(event.getEvent(), username));
    ...
    public ValidationResult validateEventRequest(
            @RequestBody EventModification eventModification, Errors errors, Principal principal) {
    ...
    private int getDescriptionLength() {
        return configurationManager
                .getFor(ConfigurationKeys.DESCRIPTION_MAXLENGTH, ConfigurationLevel.system())
                .getValueAsIntOrDefault(4096);
    }

    public static ValidationResult validateEvent(
            EventModification eventModification, Errors errors, int descriptionMaxLength) {
        ...
        return base.or(eventModification.getTicketCategories().stream()
                        .map(c -> validateCategory(
                                c,
                                errors,
                                "ticketCategories[" + counter.getAndIncrement() + "].",
                                eventModification,
    ...
    private static ValidationResult validateAdditionalTicketFields(
            List<AdditionalFieldRequest> ticketFields, Errors errors) {
        // meh
        AtomicInteger cnt = new AtomicInteger();
        return Optional.ofNullable(ticketFields).orElseGet(Collections::emptyList).stream()
                .map(field -> {
    ...
    public Map<Integer, String> getEventNamesByIds(
            @RequestParam("eventIds") List<Integer> eventIds, Principal principal) {
    ...
    public ValidationResult updateHeader(
        ...
        return validateEventHeader(Optional.of(event), eventModification, getDescriptionLength(), errors)
                .ifSuccess(() -> eventManager.updateEventHeader(event, eventModification, principal.getName()));
    }
    ...
    public ValidationResult updatePrices(
        ...
        accessService.checkEventOwnership(principal, id);
        Event event = eventManager.getSingleEventById(id, principal.getName());
        return validateEventPrices(eventModification, errors)
                .ifSuccess(() -> eventManager.updateEventSeatsAndPrices(event, eventModification, principal.getName()));
    }
    ...
    public ValidationResult updateExistingCategory(
        ...
        return validateCategory(
                        category,
                        "",
                        event.getBegin().toLocalDateTime(),
                        event.getEnd().toLocalDateTime(),
                        errors,
    ...
    public ResponseEntity<List<String>> getDeniedCustomPaymentMethods(
            @PathVariable int eventId, @PathVariable int categoryId, Principal principal)
        ...
        var deniedPaymentMethods =
                customOfflineConfigurationManager.getDeniedPaymentMethodsByTicketCategory(event, category);

        return ResponseEntity.ok(
                deniedPaymentMethods.stream().map(pm -> pm.getPaymentMethodId()).toList());
    }
    ...
    public String reallocateTickets(@RequestBody TicketAllocationModification form, Principal principal) {
        var event = accessService.checkCategoryOwnership(
                principal, form.getEventId(), Set.of(form.getSrcCategoryId(), form.getTargetCategoryId()));
    ...
    public ResponseEntity<String> rearrangeCategories(
            @PathVariable String eventName,
            @RequestBody List<CategoryOrdinalModification> categories,
            Principal principal) {
        accessService.checkCategoryOwnership(
                principal,
                eventName,
                categories.stream().map(CategoryOrdinalModification::getId).collect(Collectors.toSet()));
    ...
    static final List<String> FIXED_FIELDS = Arrays.asList(
            "ID",
            "Category",
            "Event",
            "Status",
            ...
            PAYMENT_METHOD,
            EXTERNAL_REFERENCE);
    private static final List<SerializablePair<String, String>> FIXED_PAIRS =
            FIXED_FIELDS.stream().map(f -> SerializablePair.of(f, f)).collect(toList());
    private static final String FISCAL_CODE = "Fiscal Code";
    private static final String REFERENCE_TYPE = "Reference Type";
    private static final List<String> ITALIAN_E_INVOICING_FIELDS =
            List.of(FISCAL_CODE, REFERENCE_TYPE, "Addressee Code", "PEC");
    ...
    private void exportTicketExcel(
            String eventName,
            HttpServletResponse response,
            Principal principal,
            List<String> fields,
        ...
        ExportUtils.exportExcel(
                eventName + "-export.xlsx",
                eventName + " export",
                exportHeader(fields),
                exportLines(eventName, principal, fields, categoriesMap, eventZoneId),
                response);
    }

    private void exportTicketCSV(
            String eventName,
            HttpServletResponse response,
            Principal principal,
            List<String> fields,
    ...
    private Stream<String[]> exportLines(
            String eventName,
            Principal principal,
            List<String> fields,
            Map<Integer, TicketCategory> categoriesMap,
        ...
        var eInvoicingEnabled = configurationManager.isItalianEInvoicingEnabled(
                eventManager.getEventAndOrganizationId(eventName, username));

        return eventManager.findAllConfirmedTicketsForCSV(eventName, username).stream()
                .map(trs -> {
                    ...
                    if (fields.contains("OriginalPrice")) {
                        line.add(MonetaryUtil.centsToUnit(t.getSrcPriceCts(), currencyCode)
                                .toString());
                    }
                    if (fields.contains("PaidPrice")) {
                        line.add(MonetaryUtil.centsToUnit(t.getFinalPriceCts(), currencyCode)
                                .toString());
                    }
                    if (fields.contains("Discount")) {
                        line.add(MonetaryUtil.centsToUnit(t.getDiscountCts(), currencyCode)
                                .toString());
                    }
                    if (fields.contains("VAT")) {
                        line.add(MonetaryUtil.centsToUnit(t.getVatCts(), currencyCode)
                                .toString());
                    }
                    ...
                    if (fields.contains("Promo Code")) {
                        line.add(trs.getPromoCode());
                    }
                    ...
                    if ((paymentIdRequested || paymentGatewayRequested)) {
                        ...
                        if (paymentIdRequested) {
                            line.add(Objects.toString(
                                    transaction.map(Transaction::getPaymentId).orElse(null),
                                    transaction
                        ...
                        if (paymentGatewayRequested) {
                            line.add(transaction
                                    .map(tr -> tr.getPaymentProxy().name())
                                    .orElse(""));
                        }
                    }
                    if (fields.contains(EXTERNAL_REFERENCE)) {
                        line.add(t.getExtReference());
                    }

                    if (eInvoicingEnabled) {
                        var billingDetails = trs.getBillingDetails();
                        var optionalInvoicingData = Optional.ofNullable(billingDetails.getInvoicingAdditionalInfo())
                                .map(TicketReservationInvoicingAdditionalInfo::getItalianEInvoicing);
                        if (fields.contains(FISCAL_CODE)) {
                            line.add(optionalInvoicingData
                                    .map(ItalianEInvoicing::getFiscalCode)
                                    .orElse(""));
                        }
                        if (fields.contains(REFERENCE_TYPE)) {
                            line.add(optionalInvoicingData
                                    .map(ItalianEInvoicing::getReferenceTypeAsString)
                                    .orElse(""));
                        }
                        if (fields.contains("Addressee Code")) {
                            line.add(optionalInvoicingData
                                    .map(ItalianEInvoicing::getAddresseeCode)
                                    .orElse(""));
                        }
                        if (fields.contains("PEC")) {
                            line.add(optionalInvoicingData
                                    .map(ItalianEInvoicing::getPec)
                                    .orElse(""));
                        }
                    }
                    ...
                    Map<String, List<String>> additionalValues =
                            purchaseContextFieldRepository.findAllValuesForTicketId(t.getId());

                    Predicate<String> contains = FIXED_FIELDS::contains;

                    fields.stream()
                            .filter(contains.negate())
                            .filter(f -> f.startsWith(CUSTOM_FIELDS_PREFIX))
                            .forEachOrdered(field -> {
                                String customFieldName = field.substring(CUSTOM_FIELDS_PREFIX.length());
                                line.add(String.join("; ", additionalValues.getOrDefault(customFieldName, List.of()))
                                        .replace("\"", ""));
                            });
    ...
    public void downloadSponsorScanExport(
        ...
        List<PurchaseContextFieldConfiguration> fields =
                purchaseContextFieldRepository.findAdditionalFieldsForEvent(event.getId());
        ...
        header.add("Email");
        header.addAll(
                fields.stream().map(PurchaseContextFieldConfiguration::getName).toList());
        ...
        Stream<String[]> sponsorScans = userManager.findAllEnabledUsers(principal.getName()).stream()
                ...
                .flatMap(
                        p -> sponsorScanRepository
                                .loadSponsorData(
                                        event.getId(), p.getKey().getId(), SponsorScanRepository.DEFAULT_TIMESTAMP)
                                .stream()
                                .map(v -> Pair.of(
                                        v,
                                        purchaseContextFieldRepository.findAllValuesForTicketId(
                                                v.getTicket().getId()))))
                .map(p -> {
                    ...
                    return Pair.of(
                            data,
                            fields.stream()
                                    .map(x -> String.join("; ", descriptions.getOrDefault(x.getName(), List.of())))
                                    .collect(toList()));
                })
                .map(p -> {
                    List<String> line = new ArrayList<>();
    ...
    private void exportSponsorScanExcel(
            String eventName, List<String> header, Stream<String[]> sponsorScans, HttpServletResponse response)
            throws IOException {
        ExportUtils.exportExcel(
                eventName + "-sponsor-scan.xlsx",
    ...
    private void exportSponsorScanCSV(
            String eventName, List<String> header, Stream<String[]> sponsorScans, HttpServletResponse response)
            throws IOException {
    ...
    public List<SerializablePair<String, String>> getAllFields(@PathVariable String eventName, Principal principal) {
        ...
        if (configurationManager.isItalianEInvoicingEnabled(eventAndOrganizationId)) {
            fields.addAll(ITALIAN_E_INVOICING_FIELDS.stream()
                    .map(f -> SerializablePair.of(f, f))
                    .collect(toList()));
        }
        fields.addAll(purchaseContextFieldRepository.findFieldsForEvent(eventName).stream()
                .map(f -> SerializablePair.of(CUSTOM_FIELDS_PREFIX + f, f))
                .collect(toList()));
        return fields;
    }

    @GetMapping("/events/{eventName}/pending-payments")
    public List<TicketReservationWithTransaction> getPendingPayments(
            @PathVariable String eventName, Principal principal) {
        accessService.checkEventOwnership(principal, eventName);
    ...
    public Integer getPendingPaymentsCount(@PathVariable String eventName, Principal principal) {
        accessService.checkEventMembership(principal, eventName, AccessService.MEMBERSHIP_ROLES);
        return eventManager
                .getOptionalEventAndOrganizationIdByName(eventName, principal.getName())
    ...
    public String confirmPayment(
        ...
        ticketReservationManager.confirmOfflinePayment(
                event, reservationId, transactionMetadataModification, principal.getName());
    ...
    public String deletePendingPayment(
        ...
        ticketReservationManager.deleteOfflinePayment(
                loadEvent(eventName, principal),
                reservationId,
                false,
                Boolean.TRUE.equals(creditReservation),
                notify,
                principal.getName());
        return OK;
    }
    ...
    public List<Triple<Boolean, String, String>> bulkConfirmation(
            @PathVariable String eventName, Principal principal, @RequestBody UploadBase64FileModification file)
            throws IOException {
        ...
        try (InputStreamReader isr = new InputStreamReader(file.getInputStream(), UTF_8)) {
            ...
            var reservationIds = all.stream().map(Transaction::reservationId).collect(Collectors.toSet());
                    ...
                    .map(line -> {
                        try {
                            ticketReservationManager.validateAndConfirmOfflinePayment(
                                    line.reservationId, event, line.price, principal.getName());
                            return Triple.of(Boolean.TRUE, line.reservationId, "");
                        } catch (Exception e) {
                            return Triple.of(
                                    Boolean.FALSE,
                                    Optional.ofNullable(line.reservationId).orElse(""),
                                    e.getMessage());
                        }
                    })
                    .collect(toList());
    ...
    public void getAllInvoices(@PathVariable String eventName, HttpServletResponse response, Principal principal)
            throws IOException {
        accessService.checkEventOwnership(principal, eventName);
        ...
        try (OutputStream os = response.getOutputStream();
            ...
            ticketReservationManager.streamAllDocumentsFor(event.getId()).forEach(pair -> {
    ...
    private void addPdfToZip(
            Event event, ZipOutputStream zipOS, TicketReservation reservation, BillingDocument document) {
        ...
        pdf = switch (document.getType()) {
            case CREDIT_NOTE -> TemplateProcessor.buildCreditNotePdf(
                    event, fileUploadManager, language, templateManager, reservationModel, extensionManager);
            case RECEIPT -> TemplateProcessor.buildReceiptPdf(
                    event, fileUploadManager, language, templateManager, reservationModel, extensionManager);
            default -> TemplateProcessor.buildInvoicePdf(
                    event, fileUploadManager, language, templateManager, reservationModel, extensionManager);};

        if (pdf.isPresent()) {
            String fileName = FileUtil.getBillingDocumentFileName(event.getShortName(), reservation.getId(), document);
            var entry = new ZipEntry(fileName);
            entry.setTimeLocal(document.getGenerationTimestamp()
    ...
    public void getAllDocumentsXls(@PathVariable String eventName, HttpServletResponse response, Principal principal)
            throws IOException {
        accessService.checkEventOwnership(principal, eventName);
        ...
        ExportUtils.exportExcel(
                event.getShortName() + "-billing-documents.xlsx",
                "Documents",
                header.toArray(String[]::new),
                ticketReservationManager.streamAllDocumentsFor(event.getId()).flatMap(entry -> {
                    ...
                    return entry.getValue().stream().map(bd -> {
                        ...
                        fields.add(reservation.getLineSplittedBillingAddress().stream()
                                .findFirst()
                                .orElse(""));
                        fields.add(reservation.getVatNr());
                        if (italianEInvoicingEnabled) {
                            var additionalInfo = reservationWithTransaction
                                    .getBillingDetails()
                                    .getInvoicingAdditionalInfo();
                        ...
                        fields.add(Objects.requireNonNullElse(reservation.getPaymentMethod(), PaymentProxy.NONE)
                                .name());
                        fields.add(bd.getGenerationTimestamp()
                                .withZoneSameInstant(event.getZoneId())
                                .format(formatter));
    ...
    public PageAndContent<List<TicketWithStatistic>> getTicketsInCategory(
            @PathVariable String eventName,
            @PathVariable int categoryId,
        ...
        return new PageAndContent<>(
                eventStatisticsManager.loadModifiedTickets(event.getId(), categoryId, page == null ? 0 : page, search),
                eventStatisticsManager.countModifiedTicket(event.getId(), categoryId, search));
    }

    @GetMapping("/events/{eventName}/ticket-sold-statistics")
    public ResponseEntity<TicketsStatistics> getTicketsStatistics(
            @PathVariable String eventName,
        ...
        return ResponseEntity.of(
                eventManager.getOptionalByName(eventName, principal.getName()).map(event -> {
                    var eventId = event.getId();
                    var zoneId = event.getZoneId();
                    var from = parseDate(
                            f,
                            zoneId,
                            () -> eventStatisticsManager.getFirstReservationConfirmedTimestamp(event.getId()),
                            () -> ZonedDateTime.now(clockProvider.getClock().withZone(zoneId))
                                    .minusDays(1));
                    var reservedFrom = parseDate(
                            f,
                            zoneId,
                            () -> eventStatisticsManager.getFirstReservationCreatedTimestamp(event.getId()),
                            () -> ZonedDateTime.now(clockProvider.getClock().withZone(zoneId))
                                    .minusDays(1));
                    var to = parseDate(
                                    t,
                                    ...
                                    Optional::empty,
                                    () -> ZonedDateTime.now(
                                            clockProvider.getClock().withZone(zoneId)))
                            .plusDays(1L);

                    var granularity = getGranularity(reservedFrom, to);
                    var ticketSoldStatistics =
                            eventStatisticsManager.getTicketSoldStatistics(eventId, from, to, granularity);
                    var ticketReservedStatistics =
                            eventStatisticsManager.getTicketReservedStatistics(eventId, reservedFrom, to, granularity);
    ...
    private ZonedDateTime parseDate(
        ...
        return Optional.ofNullable(dateToParse)
                .map(p -> LocalDate.parse(p, dateFormatter).atTime(23, 59, 59).atZone(zoneId))
                .or(dateLoader)
    ...
    public ResponseEntity<String> discardMatchingPayment(
            @PathVariable String eventName,
            @PathVariable String reservationId,
            @PathVariable int transactionId,
            Principal principal) {
        accessService.checkEventAndReservationAndTransactionOwnership(
                principal, eventName, reservationId, transactionId);
    ...
    public ResponseEntity<Boolean> updateMetadata(
            @PathVariable String eventName,
        ...
        return ResponseEntity.of(eventManager
                .getOptionalByName(eventName, principal.getName())
    ...
    public ResponseEntity<Boolean> updateCategoryMetadata(
            @PathVariable String eventName,
            @PathVariable int categoryId,
            @RequestBody MetadataModification metadataModification,
            Principal principal) {
        ...
        return ResponseEntity.of(eventManager
                .getOptionalEventAndOrganizationIdByName(eventName, principal.getName())
                .map(event ->
                        eventManager.updateCategoryMetadata(event, categoryId, metadataModification.toMetadataObj())));
    }

    @GetMapping("/events/{eventName}/category/{categoryId}/metadata")
    public ResponseEntity<AlfioMetadata> loadCategoryMetadata(
    ...
    public ResponseEntity<String> executeCapabilityForEvent(
            @PathVariable String eventName,
        ...
        try {
            return ResponseEntity.of(
                    eventManager.executeCapability(eventName, principal.getName(), capability, params));
    ...
    record TicketsStatistics(
            String granularity, List<TicketsByDateStatistic> sold, List<TicketsByDateStatistic> reserved) {}
    ...
    @ExceptionHandler({PassedIdDoesNotExistException.class})
