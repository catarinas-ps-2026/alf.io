...
public class AdminPaymentsApiController {
    ...
    private static final String[] EXPORT_COLUMNS =
            new String[] {"ID", "Name", "Email", "Type", "Amount", "Currency", "Payment Date/Time", "Notes"};
    ...
    PageAndContent<List<ReservationPaymentDetail>> getPaymentsForPurchaseContext(
    ...
    ResponseEntity<String> updateTransactionData(
            @PathVariable PurchaseContext.PurchaseContextType purchaseContextType,
            @PathVariable String publicIdentifier,
        ...
        try {
            return ResponseEntity.of(purchaseContextManager
                    ...
                    .map(purchaseContext -> {
                        ...
                        var timestamp = timestampModification != null
                                ? timestampModification.toZonedDateTime(purchaseContext.getZoneId())
                                : null;
                        paymentManager.updateTransactionDetails(
                                reservationId, transactionMetadataModification.getNotes(), timestamp, principal);
                        return "OK";
                    }));
    ...
    void exportPayments(
        ...
        if (purchaseContextOptional.isPresent()) {
            ...
            accessService.checkOrganizationOwnership(principal, purchaseContext.getOrganizationId());
            boolean useInvoiceNumber = configurationManager
                    .getFor(ConfigurationKeys.USE_INVOICE_NUMBER_AS_ID, purchaseContext.getConfigurationLevel())
                    .getValueAsBooleanOrDefault();
            var data = purchaseContextSearchManager.findAllPaymentsForExport(purchaseContext, search).stream()
                    .map(d -> new String[] {
            ...
            exportExcel(
                    purchaseContext.getDisplayName() + " payments.xlsx", "Payments", EXPORT_COLUMNS, data, response);
        } else {
