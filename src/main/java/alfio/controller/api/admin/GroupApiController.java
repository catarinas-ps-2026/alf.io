...
public class GroupApiController {
    ...
    public ResponseEntity<List<Group>> loadAllGroupsForOrganization(
            @PathVariable int organizationId,
            @RequestParam(name = "showAll", defaultValue = "false", required = false) boolean showAll,
            Principal principal) {
    ...
    public ResponseEntity<GroupModification> loadDetail(
            @PathVariable int organizationId, @PathVariable int listId, Principal principal) {
        accessService.checkGroupOwnership(principal, listId, organizationId);
        return groupManager.loadComplete(listId).map(ResponseEntity::ok).orElseGet(() -> ResponseEntity.notFound()
                .build());
    }

    @PostMapping("/for/{organizationId}/update/{groupId}")
    public ResponseEntity<GroupModification> updateGroup(
            @PathVariable int organizationId,
    ...
    public ResponseEntity<String> createNew(
            @PathVariable int organizationId, @RequestBody GroupModification request, Principal principal) {
    ...
    public ResponseEntity<LinkedGroup> findActiveGroup(
        ...
        return eventManager
                .getOptionalEventAndOrganizationIdByName(eventName, principal.getName())
                .map(event -> {
                    Optional<LinkedGroup> configuration = groupManager.findLinks(event.getId(), categoryId).stream()
    ...
    public ResponseEntity<Integer> linkGroup(
            @PathVariable int groupId, @RequestBody LinkedGroupModification body, Principal principal) {
        ...
        Optional<LinkedGroup> existing = groupManager.getLinksForEvent(event.getId()).stream()
    ...
    public ResponseEntity<String> unlinkGroup(
            @PathVariable int organizationId,
    ...
    public ResponseEntity<Boolean> deactivateMember(
    ...
    public ResponseEntity<Boolean> deactivateGroup(
            @PathVariable int groupId, @PathVariable int organizationId, Principal principal) {
        accessService.checkGroupOwnership(principal, groupId, organizationId);
