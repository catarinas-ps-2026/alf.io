...
public class AdminReservationApiController {
    ...
    public AdminReservationApiController(
    ...
    public Result<String> createNew(
        ...
        accessService.checkEventMembership(
                principal,
                publicIdentifier,
                AccessService.MEMBERSHIP_ROLES); // ADMIN, OWNER _and_ SUPERVISOR can create a new reservation
        return adminReservationManager
                .createReservation(reservation, publicIdentifier, principal.getName())
                .map(r -> r.getLeft().getId());
    }

    @GetMapping("/{purchaseContextType}/{publicIdentifier}/reservations/all-status")
    public TicketReservation.TicketReservationStatus[] getAllStatus(
    ...
    public PageAndContent<List<TicketReservation>> findAll(
            @PathVariable PurchaseContextType purchaseContextType,
            @PathVariable String publicIdentifier,
        ...
        return purchaseContextManager
                ...
                .map(purchaseContext -> {
                    ...
                    Pair<List<TicketReservation>, Integer> res =
                            purchaseContextSearchManager.findAllReservationsFor(purchaseContext, page, search, status);
    ...
    public Result<TicketReservationDescriptor> confirmReservation(
            @PathVariable PurchaseContextType purchaseContextType,
            @PathVariable String publicIdentifier,
    ...
    public Result<Boolean> updateReservation(
            ...
            Principal principal) {
        accessService.checkReservationMembership(principal, purchaseContextType, publicIdentifier, reservationId);
        return adminReservationManager.updateReservation(
                purchaseContextType, publicIdentifier, reservationId, arm, principal.getName());
    }

    @PutMapping("/{purchaseContextType}/{publicIdentifier}/{reservationId}/notify")
    public Result<Boolean> notifyReservation(
            @PathVariable PurchaseContextType purchaseContextType,
            @PathVariable String publicIdentifier,
            @PathVariable String reservationId,
            @RequestBody AdminReservationModification arm,
            Principal principal) {
        accessService.checkReservationMembership(principal, purchaseContextType, publicIdentifier, reservationId);
        return adminReservationManager.notify(
                purchaseContextType, publicIdentifier, reservationId, arm, principal.getName());
    }

    @PutMapping("/event/{publicIdentifier}/{reservationId}/notify-attendees")
    public Result<Boolean> notifyAttendees(
            @PathVariable String publicIdentifier,
            @PathVariable String reservationId,
    ...
    public Result<List<Audit>> getAudit(
            ...
            Principal principal) {
        accessService.checkReservationMembership(principal, purchaseContextType, publicIdentifier, reservationId);
        return adminReservationManager.getAudit(
                purchaseContextType, publicIdentifier, reservationId, principal.getName());
    }

    @GetMapping("/{purchaseContextType}/{publicIdentifier}/{reservationId}/billing-documents")
    public Result<List<BillingDocument>> getBillingDocuments(
            @PathVariable PurchaseContextType purchaseContextType,
            @PathVariable String publicIdentifier,
            @PathVariable String reservationId,
            Principal principal) {
    ...
    public ResponseEntity<Boolean> invalidateBillingDocument(
            @PathVariable PurchaseContextType purchaseContextType,
            @PathVariable String publicIdentifier,
            @PathVariable String reservationId,
        ...
        accessService.checkBillingDocumentOwnership(
                principal, purchaseContextType, publicIdentifier, reservationId, documentId);
        Result<Boolean> invalidateResult =
                adminReservationManager.invalidateBillingDocument(reservationId, documentId, principal.getName());
    ...
    public ResponseEntity<Boolean> restoreBillingDocument(
            @PathVariable PurchaseContextType purchaseContextType,
            @PathVariable String publicIdentifier,
            @PathVariable String reservationId,
            @PathVariable long documentId,
            Principal principal) {
        accessService.checkReservationOwnership(principal, purchaseContextType, publicIdentifier, reservationId);
        Result<Boolean> restoreResult =
                adminReservationManager.restoreBillingDocument(reservationId, documentId, principal.getName());
    ...
    public ResponseEntity<Void> getBillingDocument(
            @PathVariable PurchaseContextType purchaseContextType,
            @PathVariable String publicIdentifier,
            @PathVariable String reservationId,
            @PathVariable long documentId,
        ...
        accessService.checkReservationOwnership(principal, purchaseContextType, publicIdentifier, reservationId);
        Result<Boolean> result = adminReservationManager
                .getSingleBillingDocumentAsPdf(
                        purchaseContextType, publicIdentifier, reservationId, documentId, principal.getName())
    ...
    public Result<TicketReservationDescriptor> loadReservation(
            @PathVariable PurchaseContextType purchaseContextType,
            @PathVariable String publicIdentifier,
            @PathVariable String reservationId,
            Principal principal) {
        accessService.checkReservationMembership(principal, purchaseContextType, publicIdentifier, reservationId);
        return adminReservationManager
                .loadReservation(purchaseContextType, publicIdentifier, reservationId, principal.getName())
                .map(triple -> toReservationDescriptor(reservationId, triple));
    }

    @GetMapping("/{purchaseContextType}/{publicIdentifier}/{reservationId}/ticket/{ticketId}")
    public Result<Ticket> loadTicket(
            @PathVariable PurchaseContextType purchaseContextType,
            @PathVariable String publicIdentifier,
            @PathVariable String reservationId,
    ...
    public List<Integer> ticketsWithAdditionalData(
            @PathVariable PurchaseContextType purchaseContextType,
            @PathVariable String publicIdentifier,
            @PathVariable String reservationId,
            Principal principal) {
        ...
        accessService.checkReservationMembership(principal, purchaseContextType, publicIdentifier, reservationId);
        return adminReservationManager.getTicketIdsWithAdditionalData(
                purchaseContextType, publicIdentifier, reservationId);
    }

    @PostMapping("/event/{publicIdentifier}/{reservationId}/remove-tickets")
    public Result<RemoveResult> removeTickets(
            @PathVariable String publicIdentifier,
            @PathVariable String reservationId,
    ...
    public Result<TransactionAndPaymentInfo> getPaymentInfo(
            @PathVariable PurchaseContextType purchaseContextType,
            @PathVariable String publicIdentifier,
            @PathVariable String reservationId,
            Principal principal) {
    ...
    public Result<Boolean> removeReservation(
            ...
            Principal principal) {
        accessService.checkReservationMembership(principal, purchaseContextType, publicIdentifier, reservationId);
        return adminReservationManager.removeReservation(
                purchaseContextType,
                publicIdentifier,
                reservationId,
                refund,
    ...
    public Result<Boolean> creditReservation(
            @PathVariable PurchaseContextType purchaseContextType,
            @PathVariable String publicIdentifier,
            @PathVariable String reservationId,
            @RequestParam("refund") boolean refund,
        ...
        adminReservationManager.creditReservation(
                purchaseContextType, publicIdentifier, reservationId, refund, notify, principal.getName());
    ...
    public Result<Boolean> regenerateBillingDocument(
            ...
            Principal principal) {
        accessService.checkReservationOwnership(principal, purchaseContextType, publicIdentifier, reservationId);
        return adminReservationManager.regenerateBillingDocument(
                purchaseContextType, publicIdentifier, reservationId, principal.getName());
    }
    ...
    public Result<Boolean> refund(
            ...
            Principal principal) {
        accessService.checkReservationOwnership(principal, purchaseContextType, publicIdentifier, reservationId);
        return adminReservationManager.refund(
                purchaseContextType,
                publicIdentifier,
                reservationId,
    ...
    public Result<List<LightweightMailMessage>> getEmailList(
            @PathVariable PurchaseContextType purchaseContextType,
            @PathVariable String publicIdentifier,
            @PathVariable String reservationId,
            Principal principal) {
        accessService.checkReservationOwnership(principal, purchaseContextType, publicIdentifier, reservationId);
        return adminReservationManager.getEmailsForReservation(
                purchaseContextType, publicIdentifier, reservationId, principal.getName());
    }

    @GetMapping("/{purchaseContextType}/{publicIdentifier}/{reservationId}/ticket/{ticketId}/full-data")
    public ResponseEntity<BookingInfoTicket> loadFullTicketData(
            @PathVariable PurchaseContextType purchaseContextType,
            @PathVariable String publicIdentifier,
            @PathVariable String reservationId,
        ...
        return ResponseEntity.of(adminReservationManager
                .loadFullTicketInfo(reservationId, publicIdentifier, ticketUUID)
                .map(eventAndTicket -> bookingInfoTicketLoader.toBookingInfoTicket(
                        eventAndTicket.getRight(),
                        eventAndTicket.getLeft(),
                        PurchaseContextFieldConfiguration.EVENT_RELATED_CONTEXTS)));
    }

    private TicketReservationDescriptor toReservationDescriptor(
            String reservationId, Triple<TicketReservation, List<Ticket>, PurchaseContext> triple) {
        List<SerializablePair<TicketCategory, List<Ticket>>> tickets =
                triple.getMiddle().stream().collect(Collectors.groupingBy(Ticket::getCategoryId)).entrySet().stream()
                        .map(entry -> SerializablePair.of(
                                eventManager.getTicketCategoryById(
                                        entry.getKey(),
                                        triple.getRight().event().orElseThrow().getId()),
                                entry.getValue()))
                        .collect(Collectors.toList());
        TicketReservation reservation = triple.getLeft();
        return new TicketReservationDescriptor(
                reservation,
    ...
    private SubscriptionWithUsageDetails buildSubscriptionDetails(
        ...
        if (purchaseContext.ofType(PurchaseContextType.subscription)) {
            ...
            return ticketReservationManager
                    .findSubscriptionDetails(reservation.getId())
                    .orElse(null);
        }
        return null;
    }
    ...
    public static class TicketReservationDescriptor {
        ...
        public TicketReservationDescriptor(
    ...
    public static class RemoveTicketsModification {
        ...
        public RemoveTicketsModification(
                List<Integer> ticketIds, Map<Integer, Boolean> refundTo, Boolean notify, Boolean issueCreditNote) {
