...
public class EventApiV2Controller {
    ...
    public ResponseEntity<List<BasicEventInfo>> listEvents(SearchOptions searchOptions) {
        ...
        var events = eventManager.getPublishedEvents(searchOptions).stream()
                .map(e -> {
    ...
    public ResponseEntity<EventWithAdditionalInfo> getEvent(@PathVariable String eventName, HttpSession session) {
        return eventLoader
                .loadEventInfo(eventName, session)
                .map(eventWithAdditionalInfo ->
                        new ResponseEntity<>(eventWithAdditionalInfo, getCorsHeaders(), HttpStatus.OK))
                .orElseGet(() ->
                        ResponseEntity.notFound().headers(getCorsHeaders()).build());
    }

    @PostMapping("event/{eventName}/waiting-list/subscribe")
    public ResponseEntity<ValidatedResponse<Boolean>> subscribeToWaitingList(
            @PathVariable String eventName,
            @RequestBody WaitingQueueSubscriptionForm subscription,
            BindingResult bindingResult) {

        Optional<ResponseEntity<ValidatedResponse<Boolean>>> res = eventRepository
                ...
                .map(event -> {
                    ...
                    if (bindingResult.hasErrors()) {
                        return ResponseEntity.status(HttpStatus.UNPROCESSABLE_ENTITY)
                                .body(ValidatedResponse.toResponse(bindingResult, null));
                    } else {
                        var subscriptionResult = waitingQueueManager.subscribe(
                                event,
                                subscription.toCustomerName(event),
                                subscription.getEmail(),
                                subscription.getSelectedCategory(),
                                subscription.getUserLanguage());
                        return ResponseEntity.ok(
                                new ValidatedResponse<>(ValidationResult.success(), subscriptionResult));
                    }
                });
    ...
    public ResponseEntity<ItemsByCategory> getTicketCategories(
            @PathVariable String eventName, @RequestParam(value = "code", required = false) String code) {

        //
        return ticketCategoryAvailabilityManager
                .getTicketCategories(eventName, code)
                .map(i -> new ResponseEntity<>(i, getCorsHeaders(), HttpStatus.OK))
                .orElseGet(() ->
                        ResponseEntity.notFound().headers(getCorsHeaders()).build());
    }
    ...
    public void getCalendar(
        ...
        eventRepository
                ...
                .ifPresentOrElse(
                        ev -> {
                            var description = eventDescriptionRepository
                                    .findDescriptionByEventIdTypeAndLocale(
                                            ev.getId(), EventDescription.EventDescriptionType.DESCRIPTION, locale)
                                    .orElse("");
                            var category = ticketRepository
                                    .findOptionalByUUID(ticketId)
                                    .map(t -> ticketCategoryRepository.getById(t.getCategoryId()))
                                    .orElse(null);
                            if ("google".equals(calendarType)) {
                                ...
                                EventUtil.getIcalForEvent(ev, category, description)
                                        .ifPresentOrElse(
                                                ical -> {
    ...
    public ResponseEntity<ValidatedResponse<String>> reserveTickets(
            @PathVariable String eventName,
        ...
        Optional<ResponseEntity<ValidatedResponse<String>>> r = eventRepository
                .findOptionalByShortName(eventName)
                .map(event -> {
                    Locale locale = LocaleUtil.forLanguageTag(lang, event);

                    Optional<String> promoCodeDiscount =
                            ReservationUtil.checkPromoCode(reservation, event, promoCodeRequestManager, bindingResult);
                    var configurationValues = configurationManager.getFor(
                            List.of(ENABLE_CAPTCHA_FOR_TICKET_SELECTION, RECAPTCHA_API_KEY),
                    ...
                    Optional<String> reservationIdRes = createTicketReservation(
                            reservation, bindingResult, event, locale, promoCodeDiscount, principal);

                    if (bindingResult.hasErrors()) {
                        return new ResponseEntity<>(
                                ValidatedResponse.toResponse(bindingResult, null),
                                getCorsHeaders(),
                                HttpStatus.UNPROCESSABLE_ENTITY);
                    } else {
                        var reservationIdentifier = reservationIdRes.orElseThrow(IllegalStateException::new);
                        return ResponseEntity.ok(
                                new ValidatedResponse<>(ValidationResult.success(), reservationIdentifier));
                    }
                });
    ...
    public ResponseEntity<ValidatedResponse<EventCode>> validateCode(
            @PathVariable String eventName, @RequestParam("code") String code) {

        var res = promoCodeRequestManager.checkCode(eventName, code);
        if (res.isSuccess()) {
            var value = res.getValue();
            var eventCode = value.getLeft()
                    .map(sp -> new EventCode(
                            code, EventCode.EventCodeType.SPECIAL_PRICE, PromoCodeDiscount.DiscountType.NONE, null))
                    .orElseGet(() -> {
                        var promoCodeDiscount = value.getRight().orElseThrow();
                        var type = promoCodeDiscount.getCodeType() == PromoCodeDiscount.CodeType.ACCESS
                                ? EventCode.EventCodeType.ACCESS
                                : EventCode.EventCodeType.DISCOUNT;
                        String formattedDiscountAmount = PromoCodeDiscount.format(
                                promoCodeDiscount, value.getMiddle().getCurrency());
            ...
            return new ResponseEntity<>(
                    res.withValue(new EventCode(code, null, null, null)), HttpStatus.UNPROCESSABLE_ENTITY);
        }
    }

    @PostMapping("event/{eventName}/check-discount")
    public ResponseEntity<DynamicDiscount> checkDiscount(
            @PathVariable String eventName, @RequestBody ReservationForm reservation) {
        return eventRepository
                ...
                .flatMap(event -> {
                    Map<Integer, Long> quantityByCategory = reservation.getReservation().stream()
                            ...
                            .collect(groupingBy(
                                    TicketReservationModification::getTicketCategoryId,
                                    summingLong(TicketReservationModification::getQuantity)));
                    if (quantityByCategory.isEmpty()
                            || ticketCategoryRepository.countPaidCategoriesInReservation(quantityByCategory.keySet())
                                    == 0) {
                        return Optional.empty();
                    }
                    return extensionManager
                            .handleDynamicDiscount(event, quantityByCategory, null)
                            ...
                            .map(d -> {
                                ...
                                if (d.getDiscountType() == PromoCodeDiscount.DiscountType.PERCENTAGE) {
                                    ...
                                    formattedDiscount =
                                            MonetaryUtil.formatCents(d.getDiscountAmount(), event.getCurrency());
                                }
                                return new DynamicDiscount(
                                        formattedDiscount, d.getDiscountType(), formatDynamicCodeMessage(event, d));
                            });
                })
                .filter(d -> d.getDiscountType() != PromoCodeDiscount.DiscountType.NONE)
    ...
    public ResponseEntity<Void> handleCode(
            @PathVariable String eventName, @PathVariable String code, ServletWebRequest request, Principal principal) {
        ...
        var url = promoCodeRequestManager
                .createReservationFromPromoCode(
                        eventName, trimmedCode, queryStrings::put, handleErrors, request, principal)
                .map(reservationId -> UriComponentsBuilder.fromPath(
                ...
                .orElseGet(() -> {
                    ...
                    return backToEvent
                            .build(Map.of("eventShortName", eventName))
                            .toString();
                });
    ...
    private boolean isCaptchaInvalid(
            ...
            Map<ConfigurationKeys, ConfigurationManager.MaybeConfiguration> configurationValues) {
    ...
    private Map<String, String> formatDynamicCodeMessage(Event event, PromoCodeDiscount promoCodeDiscount) {
        Validate.isTrue(promoCodeDiscount != null
                && promoCodeDiscount.getDiscountType() != PromoCodeDiscount.DiscountType.NONE);
