...
public class ReservationApiV2Controller {
    ...
    public ResponseEntity<ReservationInfo> getReservationInfo(@PathVariable String reservationId, Principal principal) {
        ...
        Optional<ReservationInfo> res = purchaseContextManager
                .findByReservationId(reservationId)
                .flatMap(purchaseContext -> ticketReservationManager
                        .findById(reservationId)
                        .flatMap(reservation -> {
                            validateAccessToReservation(principal, reservation);

                            var orderSummary = ticketReservationManager.orderSummaryForReservationId(
                                    reservationId, purchaseContext);
                            ...
                            if (purchaseContext.ofType(PurchaseContextType.event)) {
                                ...
                                if (!ticketIds.isEmpty()) {
                                    var valuesByTicketIds =
                                            purchaseContextFieldRepository.findAllValuesByTicketIds(ticketIds).stream()
                                                    .collect(Collectors.groupingBy(
                                                            PurchaseContextFieldValue::getTicketId));

                                    var ticketFieldsFilterer =
                                            bookingInfoTicketLoader.getTicketFieldsFilterer(reservationId, event);
                                    var ticketsByCategory =
                                            tickets.stream().collect(Collectors.groupingBy(Ticket::getCategoryId));
                                    var hasPaidSupplement =
                                            ticketReservationManager.hasPaidSupplements(event.getId(), reservationId);
                                    var categories =
                                            ticketCategoryRepository.findCategoriesInReservation(reservationId);
                                    ticketsInReservation = ticketsByCategory.entrySet().stream()
                                            .map(e -> {
                                                var tc = categories.stream()
                                                        .filter(t -> t.getId() == e.getKey())
                                                        .findFirst()
                                                        .orElseThrow();
                                                var context = event.supportsLinkedAdditionalServices()
                                                        ? EnumSet.of(PurchaseContextFieldConfiguration.Context.ATTENDEE)
                                                        : EVENT_RELATED_CONTEXTS;
                                                var ts = e.getValue().stream()
                                                        .map(t -> bookingInfoTicketLoader.toBookingInfoTicket(
                                                                t,
                                                                hasPaidSupplement,
                                                                event,
                                                                ticketFieldsFilterer,
                                                ...
                                                return new TicketsByTicketCategory(
                                                        tc.getName(), tc.getTicketAccessType(), ts);
                                            })
                                            .collect(Collectors.toList());
                                    containsCategoriesLinkedToGroups =
                                            ticketReservationManager.containsCategoriesLinkedToGroups(
                                                    reservationId, event.getId());
                                    categoryIds = ticketsByCategory.keySet();
                                    var additionalServiceItems = additionalServiceManager.findItemsInReservation(
                                            event.getId(), reservationId);
                                    Map<Integer, List<AdditionalServiceFieldValue>> additionalServicesByItemId =
                                            additionalServiceItems.isEmpty()
                                                    ? Map.of()
                                                    : purchaseContextFieldRepository
                                                            .findAdditionalServicesValueByItemIds(
                                                                    additionalServiceItems.stream()
                                                                            .map(AdditionalServiceItem::getId)
                                                                            .collect(Collectors.toList()))
                                                            .stream()
                                                            .collect(groupingBy(
                                                                    AdditionalServiceFieldValue
                                                                            ::getAdditionalServiceItemId));
                                    additionalServices = additionalServiceHelper.getAdditionalServicesWithData(
                                            event,
                            ...
                            var formattedExpirationDate = reservation.getValidity() != null
                                    ? formatDateForLocales(
                                            purchaseContext,
                                            ZonedDateTime.ofInstant(
                                                    reservation.getValidity().toInstant(), purchaseContext.getZoneId()),
                                            "datetime.pattern")
                                    : null;

                            var paymentToken = paymentManager.getPaymentToken(reservationId);
                            boolean tokenAcquired = paymentToken.isPresent();
                            PaymentProxy selectedPaymentProxy = paymentToken
                                    .map(PaymentToken::getPaymentProvider)
                                    .orElse(null);
                            ...
                            if (purchaseContext.ofType(PurchaseContextType.subscription)) {
                                subscriptionInfos =
                                        subscriptionRepository.findSubscriptionsByReservationId(reservationId).stream()
                                                .limit(1) // since we support only one subscription for now, it makes
                                                // sense to limit the result to avoid N+1
                                                .map(s -> {
                                                    int usageCount =
                                                            ticketRepository.countSubscriptionUsage(s.getId(), null);
                                                    var metadata = requireNonNullElseGet(
                                                            subscriptionRepository.getSubscriptionMetadata(s.getId()),
                                                            SubscriptionMetadata::empty);
                                                    var fields = purchaseContextFieldManager.findAdditionalFields(
                                                            purchaseContext);
                                                    var valuesById =
                                                            purchaseContextFieldRepository
                                                                    .findAllValuesBySubscriptionIds(List.of(s.getId()))
                                                                    .stream()
                                                                    .collect(Collectors.groupingBy(
                                                                            PurchaseContextFieldValue
                                                                                    ::getFieldConfigurationId));
                                                    var additionalFields = fields.stream()
                                                            .sorted(Comparator.comparing(
                                                                    PurchaseContextFieldConfiguration::getOrder))
                                                            .flatMap(tfc -> toAdditionalFieldsStream(
                                                                    descriptionsByFieldId, tfc, valuesById))
                                                            .collect(Collectors.toList());
                                                    return new ReservationInfo.SubscriptionInfo(
                                                            s.getStatus() == AllocationStatus.ACQUIRED
                                                                    ? s.getId()
                                                                    : null,
                                                            s.getStatus() == AllocationStatus.ACQUIRED
                                                                    ? s.getPin()
                                                            ...
                                                            new ReservationInfo.SubscriptionOwner(
                            ...
                            return Optional.of(new ReservationInfo(
                                    reservation.getId(),
                                    shortReservationId,
                                    ...
                                    new ReservationInfo.ReservationInfoOrderSummary(orderSummary),
                                    ...
                                    tokenAcquired,
                                    selectedPaymentProxy != null
                                            ? selectedPaymentProxy
                                    ...
                                    getActivePaymentMethods(
                                            purchaseContext,
                                            requireNonNullElse(categoryIds, Set.of()),
                                            orderSummary,
                                            reservationId),
                                    subscriptionInfos,
    ...
    private Map<String, PaymentProxyWithParameters> getActivePaymentMethods(
            PurchaseContext purchaseContext,
            Collection<Integer> categoryIds,
            OrderSummary orderSummary,
            String reservationId) {
        if (!purchaseContext.isFreeOfCharge()) {
            var blacklistedMethodsForReservation =
                    configurationManager.getBlacklistedMethodsForReservation(purchaseContext, categoryIds);
            return paymentManager
                    ...
                    .stream()
                    .filter(p -> blacklistedMethodsForReservation.stream().noneMatch(p2 -> p2.getPaymentMethodId()
                            .equals(p.getPaymentMethod().getPaymentMethodId())))
                    ...
                    .collect(toMap(
                            ...
                            pm -> new PaymentProxyWithParameters(
                                    pm.getPaymentProxy(),
                                    paymentManager.loadModelOptionsFor(
                                            List.of(pm.getPaymentProxy()), purchaseContext))));
        } else {
            return Map.of();
        }
    }
    ...
    public ResponseEntity<ReservationStatusInfo> getReservationStatus(@PathVariable String reservationId) {
        ...
        Optional<ReservationStatusInfo> res = ticketReservationRepository
                .findOptionalStatusAndValidationById(reservationId)
                .map(status ->
                        new ReservationStatusInfo(status.getStatus(), Boolean.TRUE.equals(status.getValidated())));
        return res.map(ResponseEntity::ok)
                .orElseGet(() -> ResponseEntity.notFound().build());
    }
    ...
    public ResponseEntity<Boolean> cancelPendingReservation(@PathVariable String reservationId) {
        getReservationWithPendingStatus(reservationId)
    ...
    public ResponseEntity<ValidatedResponse<ReservationPaymentResult>> confirmOverview(
        ...
        return getReservation(reservationId)
                .map(er -> {
                    ...
                    if (paymentForm.getPaymentProxy() == PaymentProxy.CUSTOM_OFFLINE
                            && event.event().isPresent()
                            && !isCustomPaymentMethodValidForEvent(
                                    event.event().get(), paymentForm.getSelectedPaymentMethod())) {
                    ...
                    OrderSummary orderSummary =
                            ticketReservationManager.orderSummaryForReservationId(reservationId, event);

                    PaymentToken paymentToken =
                            paymentManager.getPaymentToken(reservationId).orElse(null);
                    if (paymentToken == null && StringUtils.isNotEmpty(paymentForm.getGatewayToken())) {
                        paymentToken = paymentManager.buildPaymentToken(
                                paymentForm.getGatewayToken(),
                                paymentForm.getPaymentProxy(),
                    ...
                    PaymentSpecification spec = new PaymentSpecification(
                            reservationId,
                            paymentToken,
                            paymentForm.getSelectedPaymentMethod(),
                            reservationCost.getPriceWithVAT(),
                            event,
                            reservation.getEmail(),
                            customerName,
                            ...
                            Boolean.TRUE.equals(paymentForm.getTermAndConditionsAccepted()),
                            Boolean.TRUE.equals(paymentForm.getPrivacyPolicyAccepted()));

                    final PaymentResult status = ticketReservationManager.performPayment(
                            spec,
                            reservationCost,
                            paymentForm.getPaymentProxy(),
                            paymentForm.getSelectedPaymentMethod(),
                    ...
                    if (status.isRedirect()) {
                        var body = ValidatedResponse.toResponse(
                                bindingResult,
                                new ReservationPaymentResult(
                                        !bindingResult.hasErrors(),
                                        true,
                                        status.getRedirectUrl(),
                                        status.isFailed(),
                                        status.getGatewayIdOrNull()));
                        return ResponseEntity.ok(body);
                    }

                    if (!status.isSuccessful()) {
                        String errorMessageCode =
                                status.getErrorCode().orElse(StripeCreditCardManager.STRIPE_UNEXPECTED);
                        MessageSourceResolvable message = new DefaultMessageSourceResolvable(
                                new String[] {errorMessageCode, StripeCreditCardManager.STRIPE_UNEXPECTED});
                        bindingResult.reject(
                                ErrorsCode.STEP_2_PAYMENT_PROCESSING_ERROR,
                                new Object[] {
    ...
    private static ResponseEntity<ValidatedResponse<ReservationPaymentResult>> buildReservationPaymentStatus(
            BindingResult bindingResult) {
        var body = ValidatedResponse.toResponse(
                bindingResult, new ReservationPaymentResult(!bindingResult.hasErrors(), false, null, true, null));
        return ResponseEntity.status(bindingResult.hasErrors() ? HttpStatus.UNPROCESSABLE_ENTITY : HttpStatus.OK)
                .body(body);
    }

    @PostMapping("/reservation/{reservationId}/validate-to-overview")
    public ResponseEntity<ValidatedResponse<Boolean>> validateToOverview(
            @PathVariable String reservationId,
            @RequestParam("lang") String lang,
        ...
        return getPurchaseContextAndReservationWithPendingStatus(reservationId)
                .map(er -> {
                    ...
                    final TotalPrice reservationCost = ticketReservationManager
                            .totalReservationCostWithVAT(reservation.withVatStatus(purchaseContext.getVatStatus()))
                            .getLeft();

                    purchaseContext.event().ifPresent(event -> {
                        boolean forceAssignment = configurationManager
                                .getFor(
                    ...
                    CustomerName customerName = new CustomerName(
                            contactAndTicketsForm.getFullName(),
                            contactAndTicketsForm.getFirstName(),
                            contactAndTicketsForm.getLastName(),
                            true,
                            false);
                    ...
                    ticketReservationRepository.resetVat(
                            ...
                            reservationCost.getPriceWithVAT(),
                            reservationCost.getVAT(),
                            Math.abs(reservationCost.getDiscount()),
                            reservation.getCurrencyCode());

                    var optionalCustomTaxPolicy = extensionManager.handleCustomTaxPolicy(
                            purchaseContext, reservationId, contactAndTicketsForm, reservationCost);
                    if (optionalCustomTaxPolicy.isPresent()) {
                        ...
                        reverseChargeManager.applyCustomTaxPolicy(
                            ...
                            && (contactAndTicketsForm.isBusiness()
                                    || configurationManager.noTaxesFlagDefinedFor(
                                            ticketCategoryRepository.findCategoriesInReservation(reservationId)))) {
                        reverseChargeManager.checkAndApplyVATRules(
                                purchaseContext, reservationId, contactAndTicketsForm, bindingResult);
                    } else if (reservationCost.getPriceWithVAT() > 0) {
                        reverseChargeManager.resetVat(purchaseContext, reservationId);
                    }

                    // persist data
                    ticketReservationManager.updateReservation(
                            reservationId,
                            customerName,
                            contactAndTicketsForm.getEmail(),
                            contactAndTicketsForm.getBillingAddressCompany(),
                            contactAndTicketsForm.getBillingAddressLine1(),
                            contactAndTicketsForm.getBillingAddressLine2(),
                            contactAndTicketsForm.getBillingAddressZip(),
                            contactAndTicketsForm.getBillingAddressCity(),
                            contactAndTicketsForm.getBillingAddressState(),
                            contactAndTicketsForm.getVatCountryCode(),
                            contactAndTicketsForm.getCustomerReference(),
                            contactAndTicketsForm.getVatNr(),
                            contactAndTicketsForm.isInvoiceRequested(),
                            contactAndTicketsForm.getAddCompanyBillingDetails(),
                            contactAndTicketsForm.canSkipVatNrCheck(),
                            false,
                            locale,
                            principal);

                    boolean italyEInvoicing = configurationManager
                            .getFor(ENABLE_ITALY_E_INVOICING, purchaseContext.getConfigurationLevel())
                            .getValueAsBooleanOrDefault();

                    if (italyEInvoicing) {
                        ticketReservationManager.updateReservationInvoicingAdditionalInformation(
                                reservationId,
                                purchaseContext,
                                new TicketReservationInvoicingAdditionalInfo(
                                        getItalianInvoicingInfo(contactAndTicketsForm)));
                    }
                    ...
                    if (purchaseContext instanceof Event event) {
                        if (event.supportsLinkedAdditionalServices() && contactAndTicketsForm.hasAdditionalServices()) {
                            var tickets = ticketReservationManager.findTicketsInReservation(reservationId);
                            additionalServiceManager.linkItemsToTickets(
                                    reservationId, contactAndTicketsForm.getAdditionalServices(), tickets);
                            additionalServiceManager.persistFieldsForAdditionalItems(
                                    event.getId(),
                                    event.getOrganizationId(),
                                    contactAndTicketsForm.getAdditionalServices(),
                                    tickets);
                        }
                    ...
                    if (purchaseContext.ofType(PurchaseContextType.subscription)) {
                        ...
                        if (contactAndTicketsForm.isDifferentSubscriptionOwner()) {
                            Validate.isTrue(subscriptionRepository.assignSubscription(
                                            reservationId, owner.getFirstName(), owner.getLastName(), owner.getEmail())
                                    == 1);
                        } else {
                            // cleanup data
                            Validate.isTrue(
                                    subscriptionRepository.assignSubscription(reservationId, null, null, null) == 1);
                        }
                        if (MapUtils.isNotEmpty(owner.getAdditional())) {
                            purchaseContextFieldManager.updateFieldsForReservation(
                                    owner,
                                    purchaseContext,
                                    null,
                    ...
                    Map<ConfigurationKeys, Boolean> formValidationParameters =
                            Collections.singletonMap(ENABLE_ITALY_E_INVOICING, italyEInvoicing);

                    var fieldsFilterer = purchaseContext
                            .event()
                            .map(event -> bookingInfoTicketLoader.getTicketFieldsFilterer(reservationId, event))
                            .or(() -> Optional.of(bookingInfoTicketLoader.getSubscriptionFieldsFilterer(
                                    reservationId, (SubscriptionDescriptor) purchaseContext)));

                    //
                    contactAndTicketsForm.validate(
                            bindingResult,
                            purchaseContext,
                            new SameCountryValidator(
                                    configurationManager, extensionManager, purchaseContext, reservationId, vatChecker),
                            formValidationParameters,
                            fieldsFilterer,
                            reservationCost.requiresPayment(),
                            extensionManager,
                    ...
                    if (!bindingResult.hasErrors()) {
                        extensionManager.handleReservationValidation(
                                purchaseContext, reservation, contactAndTicketsForm, bindingResult);
                    }

                    if (!bindingResult.hasErrors() && (!bindingResult.hasWarnings() || ignoreWarnings)) {
                        ticketReservationManager.flagAsValidated(
                                reservationId, purchaseContext, bindingResult.getWarnings());
                        // save customer data
                        if (principal != null && configurationManager.isPublicOpenIdEnabled()) {
                            var additionalData = contactAndTicketsForm.getTickets().values().stream()
                                    .filter(f -> principal.getName().equals(f.getEmail())
                                            && !f.getAdditional().isEmpty())
                                    .limit(1)
                                    .map(form -> publicUserManager.buildAdditionalInfoWithLabels(
                                            principal, purchaseContext, form))
                                    .findFirst()
                                    .orElse(Map.of());

                            publicUserManager.persistProfileForPublicUser(
                                    principal,
                                    contactAndTicketsForm,
                                    bindingResult,
                    ...
                    return ResponseEntity.status(
                                    bindingResult.hasErrors() ? HttpStatus.UNPROCESSABLE_ENTITY : HttpStatus.OK)
                            .body(body);
                })
                .orElseGet(() -> ResponseEntity.notFound().build());
    }
    ...
    private TicketReservationInvoicingAdditionalInfo.ItalianEInvoicing getItalianInvoicingInfo(
    ...
    private void assignTickets(
        ...
        if (!contactAndTicketsForm.isPostponeAssignment()) {
            contactAndTicketsForm.getTickets().forEach((ticketPublicId, owner) -> {
                ...
                if (preAssign) {
                    Optional<BindingResult> bindingResultOptional =
                            skipValidation ? Optional.empty() : Optional.of(bindingResult);
    ...
    private Optional<Pair<PurchaseContext, TicketReservation>> getPurchaseContextAndReservationWithPendingStatus(
            String reservationId) {
        return purchaseContextManager.findByReservationId(reservationId).flatMap(event -> ticketReservationManager
                .findById(reservationId)
                .filter(reservation -> reservation.getStatus() == TicketReservation.TicketReservationStatus.PENDING)
    ...
    public ResponseEntity<Boolean> reSendReservationConfirmationEmail(
            @PathVariable PurchaseContextType purchaseContextType,
            @PathVariable String publicIdentifier,
            @PathVariable String reservationId,
            @RequestParam("lang") String lang,
            Principal principal) {

        return ResponseEntity.of(purchaseContextManager
                .findBy(purchaseContextType, publicIdentifier)
                .map(purchaseContext -> ticketReservationManager
                        .findById(reservationId)
                        .map(ticketReservation -> {
                            ...
                            ticketReservationManager.sendConfirmationEmail(
                                    purchaseContext,
                                    ticketReservation,
                                    LocaleUtil.forLanguageTag(lang, purchaseContext),
    ...
    public ResponseEntity<Void> getReceipt(
    ...
    private ResponseEntity<Void> handleReservationWith(
            String eventName,
            String reservationId,
            Authentication authentication,
        ...
        return eventRepository
                ...
                .map(event -> {
                    if (canAccessReceiptOrInvoice(event, authentication)) {
                        return ticketReservationManager
                                .findById(reservationId)
                                .map(ticketReservation -> with.apply(event, ticketReservation))
                                .orElse(notFound);
                    } else {
                        return badRequest;
                    }
    ...
    private BiFunction<Event, TicketReservation, ResponseEntity<Void>> generatePdfFunction(
            boolean forInvoice, HttpServletResponse response) {
        return (event, reservation) -> {
            if ((forInvoice ^ reservation.getInvoiceNumber() != null) || reservation.isCancelled()) {
                return ResponseEntity.notFound().build();
            }

            BillingDocument billingDocument = billingDocumentManager.getOrCreateBillingDocument(
                    event, reservation, null, ticketReservationManager.orderSummaryForReservation(reservation, event));

            try {
                FileUtil.sendHeaders(response, event.getShortName(), reservation.getId(), billingDocument);
                TemplateProcessor.buildReceiptOrInvoicePdf(
                        event,
                        fileUploadManager,
                        LocaleUtil.forLanguageTag(reservation.getUserLanguage()),
                        templateManager,
                        billingDocument.getModel(),
                        forInvoice ? TemplateResource.INVOICE_PDF : TemplateResource.RECEIPT_PDF,
                        extensionManager,
    ...
    public ResponseEntity<TransactionInitializationToken> initTransaction(
        ...
        Optional<ResponseEntity<TransactionInitializationToken>> responseEntity = purchaseContextManager
                .getReservationWithPurchaseContext(reservationId)
                .map(pair -> {
                    var event = pair.getLeft();
                    return ticketReservationManager
                            .initTransaction(event, reservationId, paymentMethod, allParams)
    ...
    public ResponseEntity<Boolean> removeToken(@PathVariable String reservationId) {
        ...
        var res = purchaseContextManager
                .getReservationWithPurchaseContext(reservationId)
                .map(et -> paymentManager.removePaymentTokenReservation(
                        et.getRight().getId()))
                .orElse(false);
    ...
    public ResponseEntity<Boolean> deletePaymentAttempt(@PathVariable String reservationId) {
        ...
        var res = purchaseContextManager
                .getReservationWithPurchaseContext(reservationId)
                .map(et -> ticketReservationManager.cancelPendingPayment(
                        et.getRight().getId(), et.getLeft()))
                .orElse(false);
        return ResponseEntity.ok(res);
    }
    ...
    public ResponseEntity<ReservationPaymentResult> getTransactionStatus(
            @PathVariable String reservationId, @PathVariable("method") String paymentMethodAsString) {
     ...
     */
    public ResponseEntity<ReservationPaymentResult> getTransactionStatus(
            @NonNull String reservationId, @NonNull PaymentMethod paymentMethod) {
        return purchaseContextManager
                .getReservationWithPurchaseContext(reservationId)
                .flatMap(pair -> paymentManager.getTransactionStatus(pair.getRight(), paymentMethod))
                .map(pr -> ResponseEntity.ok(new ReservationPaymentResult(
                        pr.isSuccessful(),
                        pr.isRedirect(),
                        pr.getRedirectUrl(),
                        pr.isFailed(),
                        pr.getGatewayIdOrNull())))
                .orElseGet(() -> ResponseEntity.notFound().build());
    }

    @PostMapping("/reservation/{reservationId}/apply-code")
    public ResponseEntity<ValidatedResponse<Boolean>> applyCode(
            @PathVariable String reservationId,
            @RequestBody ReservationCodeForm reservationCodeForm,
            BindingResult bindingResult) {
        ...
        boolean res = purchaseContextManager
                .getReservationWithPurchaseContext(reservationId)
                .map(et -> ticketReservationManager.validateAndApplySubscriptionCode(
                        et.getLeft(),
                        et.getRight(),
    ...
    public ResponseEntity<Boolean> removeCode(
            @PathVariable String reservationId, @RequestParam("type") ReservationCodeForm.ReservationCodeType type) {
        boolean res = false;
        if (type == ReservationCodeForm.ReservationCodeType.SUBSCRIPTION) {
            res = purchaseContextManager
                    .getReservationWithPurchaseContext(reservationId)
                    .map(et -> ticketReservationManager.removeSubscription(et.getRight()))
                    .orElse(false);
        }
        return ResponseEntity.ok(res);
    }
    ...
    public ResponseEntity<List<UserDefinedOfflinePaymentMethod>> getApplicableCustomPaymentMethodDetails(
            @PathVariable String reservationId, Principal principal) throws PassedIdDoesNotExistException {
        ...
        var allowedPaymentMethods =
                customOfflineConfigurationManager.getAllowedCustomOfflinePaymentMethodsForEvent(event);

        allowedPaymentMethods.forEach(
                paymentMethod -> paymentMethod.getLocalizations().forEach((key, locale) -> {
                    ...
                    var updatedLocale = new UserDefinedOfflinePaymentMethod.Localization(
                            locale.paymentName(), localeTexts.get("description"), localeTexts.get("instructions"));
    ...
    public ResponseEntity<UserDefinedOfflinePaymentMethod> getSelectedCustomPaymentMethodDetails(
        ...
        var event = eventRepository.findByReservationId(reservationId);
        var paymentMethods =
                customOfflineConfigurationManager.getOrganizationCustomOfflinePaymentMethods(event.getOrganizationId());
        ...
        UserDefinedOfflinePaymentMethod respPaymentMethod = paymentMethods.stream()
        ...
        respPaymentMethod.getLocalizations().forEach((key, locale) -> {
            Map<String, String> localeTexts = new HashMap<>();
            localeTexts.put("description", locale.paymentDescription());
            localeTexts.put("instructions", locale.paymentInstructions());
            localeTexts = Formatters.applyCommonMark(localeTexts);
    ...
    @ExceptionHandler({PassedIdDoesNotExistException.class, CustomOfflinePaymentMethodDoesNotExistException.class})
    ...
    @ResponseStatus(
            ...
            reason = "There is no transaction associated with the passed reservation.")
    public class NoCustomPaymentTransactionForReservationException extends RuntimeException {}

    private Map<String, String> formatDateForLocales(
            PurchaseContext purchaseContext, ZonedDateTime date, String formattingCode) {
        ...
        for (ContentLanguage cl : purchaseContext.getContentLanguages()) {
            ...
            res.put(
                    cl.locale().getLanguage(),
                    DateTimeFormatter.ofPattern(formatter, cl.locale()).format(date));
        }
        return res;
    }

    private boolean isCaptchaInvalid(
            int cost,
            PaymentProxy paymentMethod,
    ...
    private boolean isCustomPaymentMethodValidForEvent(Event event, PaymentMethod paymentMethod) {
        var eventSelectedMethods =
                customOfflineConfigurationManager.getAllowedCustomOfflinePaymentMethodsForEvent(event);
        return eventSelectedMethods.stream()
