...
public class SubscriptionsApiController {
    ...
    public ResponseEntity<List<BasicSubscriptionDescriptorInfo>> listSubscriptions(SearchOptions searchOptions) {
        ...
        var activeSubscriptions =
                subscriptionManager.getActivePublicSubscriptionsDescriptor(now, searchOptions).stream()
    ...
    private static BasicSubscriptionDescriptorInfo subscriptionDescriptorMapper(
            SubscriptionDescriptor s, MessageSource messageSource) {
        var currencyUnit = CurrencyUnit.of(s.getCurrency());
        var currencyDescriptor = new CurrencyDescriptor(
        ...
        return new BasicSubscriptionDescriptorInfo(
                ...
                s.getContentLanguages().stream()
                        .map(cl -> new Language(cl.locale().getLanguage(), cl.getDisplayLanguage()))
                        .collect(toList()));
    }

    @GetMapping("subscription/{id}")
    public ResponseEntity<SubscriptionDescriptorWithAdditionalInfo> getSubscriptionInfo(
        ...
        return res.map(s -> {
                    ...
                    var bankAccountOwner = Arrays.asList(configurationsValues
                    ...
                    return new SubscriptionDescriptorWithAdditionalInfo(
                            s,
                            invoicingInfo,
                            analyticsConf,
                            captchaConf,
    ...
    public ResponseEntity<ValidatedResponse<String>> reserveSubscription(
            @PathVariable String id, Locale locale, Principal principal) {
        var bindingResult = new MapBindingResult(new HashMap<>(), "request");
        return subscriptionManager
                .getSubscriptionById(UUID.fromString(id))
                .map(subscriptionDescriptor -> {
                    var reservationOptional = ReservationUtil.handleReservationCreationErrors(
                            () -> reservationManager.createSubscriptionReservation(
                                    subscriptionDescriptor, locale, principal),
                            bindingResult,
                            subscriptionDescriptor.getType());
                    if (bindingResult.hasErrors()) {
                        return new ResponseEntity<ValidatedResponse<String>>(
                                ValidatedResponse.toResponse(bindingResult, null), HttpStatus.UNPROCESSABLE_ENTITY);
                    } else {
                        return reservationOptional
                                .map(reservationId -> ResponseEntity.ok(
                                        new ValidatedResponse<>(ValidationResult.success(), reservationId)))
                                .orElseGet(() ->
