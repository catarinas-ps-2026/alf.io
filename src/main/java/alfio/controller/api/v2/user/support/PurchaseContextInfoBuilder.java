...
public class PurchaseContextInfoBuilder {
    ...
    public static Map<ConfigurationKeys, ConfigurationManager.MaybeConfiguration> configurationsValues(
            PurchaseContext purchaseContext, ConfigurationManager configurationManager) {
    ...
    public static InvoicingConfiguration invoicingInfo(
            ConfigurationManager configurationManager,
            Map<ConfigurationKeys, ConfigurationManager.MaybeConfiguration> configurationsValues) {
        boolean canGenerateReceiptOrInvoiceToCustomer =
                configurationManager.canGenerateReceiptOrInvoiceToCustomer(configurationsValues);
        ...
        boolean customerReferenceEnabled =
                configurationsValues.get(ENABLE_CUSTOMER_REFERENCE).getValueAsBooleanOrDefault();
        boolean enabledItalyEInvoicing =
                configurationsValues.get(ENABLE_ITALY_E_INVOICING).getValueAsBooleanOrDefault();
        boolean vatNumberStrictlyRequired =
                configurationsValues.get(VAT_NUMBER_IS_REQUIRED).getValueAsBooleanOrDefault();

        return new InvoicingConfiguration(
    ...
    public static EventWithAdditionalInfo.CaptchaConfiguration captchaConfiguration(
        ...
        //
        boolean captchaForOfflinePaymentAndFreeEnabled =
                configurationManager.isRecaptchaForOfflinePaymentAndFreeEnabled(configurationsValues);
        return new EventWithAdditionalInfo.CaptchaConfiguration(
                captchaForTicketSelection, captchaForOfflinePaymentAndFreeEnabled, recaptchaApiKey);
    }

    public static boolean isRecaptchaForTicketSelectionEnabled(
