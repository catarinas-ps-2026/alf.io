...
public class ExtensionManager {
    ...
    Set<ExtensionCapabilitySummary> getSupportedCapabilities(
            Set<ExtensionCapability> requested, PurchaseContext purchaseContext) {
    ...
    void handleEventSeatsUpdateValidation(Event event, int seatsNumber) {
        var request = new EventModification(
                event.getId(),
                null,
                null,
                null,
    ...
    Optional<AlfioMetadata> handleGenerateMeetingLinkCapability(
            Event event, Organization organization, AlfioMetadata existingMetadata, Map<String, String> requestParams) {
        Map<String, Object> context = buildMetadataUpdatePayload(organization, existingMetadata);
        context.put(REQUEST, requestParams);
        return internalExecuteCapability(
                ExtensionCapability.GENERATE_MEETING_LINK, context, event, AlfioMetadata.class);
    }

    Optional<String> handleGenerateLinkCapability(
            Event event, AlfioMetadata existingMetadata, Map<String, String> requestParams) {
    ...
    private Map<String, Object> buildMetadataUpdatePayload(Organization organization, AlfioMetadata metadata) {
        ...
        payload.put(
                ...
                configurationManager
                        .getFor(ConfigurationKeys.BASE_URL, ConfigurationLevel.organization(organization.getId()))
                        .getRequiredValue());
        return payload;
    }

    void handleReservationConfirmation(
            TicketReservation reservation, BillingDetails billingDetails, PurchaseContext purchaseContext) {
        Map<String, Object> payload = new HashMap<>();
        payload.put(RESERVATION, reservation);
        payload.put(BILLING_DETAILS, billingDetails);
        payload.put(ADDITIONAL_INFO, Map.of());
        transactionRepository
                .loadOptionalByReservationId(reservation.getId())
                .ifPresent(tr -> payload.put("transaction", tr));
        asyncCall(ExtensionEvent.RESERVATION_CONFIRMED, purchaseContext, payload);
    }

    public void handleTicketAssignment(
            Ticket ticket, TicketCategory category, Map<String, List<String>> additionalInfo) {
        ...
        asyncCall(
                ExtensionEvent.TICKET_ASSIGNED,
                event,
                Map.of(
                        TICKET,
                        ticket,
                        ADDITIONAL_INFO,
                        Objects.requireNonNullElse(additionalInfo, Map.of()),
                        EVENT_METADATA,
                        Objects.requireNonNullElseGet(
                                eventRepository.getMetadataForEvent(event.getId()), AlfioMetadata::empty),
                        "onlineAccessTicket",
    ...
    void handleWaitingQueueSubscription(WaitingQueueSubscription waitingQueueSubscription) {
        ...
        asyncCall(
                ExtensionEvent.WAITING_QUEUE_SUBSCRIBED,
                event,
    ...
    public Optional<CustomEmailText> handleReservationEmailCustomText(
            PurchaseContext purchaseContext,
            TicketReservation reservation,
            TicketReservationAdditionalInfo additionalInfo) {
        Map<String, Object> payload =
                Map.of(RESERVATION, reservation, "purchaseContext", purchaseContext, "billingData", additionalInfo);
        try {
            return Optional.ofNullable(syncCall(
                    ExtensionEvent.CONFIRMATION_MAIL_CUSTOM_TEXT, purchaseContext, payload, CustomEmailText.class));
        } catch (Exception ex) {
    ...
    public Optional<CustomEmailText> handleTicketEmailCustomText(
            Event event,
            TicketReservation reservation,
            TicketReservationAdditionalInfo additionalInfo,
            List<PurchaseContextFieldValue> fields) {
        Map<String, Object> payload = Map.of(
                RESERVATION, reservation, EVENT, event, "billingData", additionalInfo, "additionalFields", fields);
        try {
            return Optional.ofNullable(
                    syncCall(ExtensionEvent.TICKET_MAIL_CUSTOM_TEXT, event, payload, CustomEmailText.class));
        } catch (Exception ex) {
    ...
    private void handleReservationRemoval(
            PurchaseContext purchaseContext, Collection<String> reservationIds, ExtensionEvent extensionEvent) {
        Map<String, Object> payload = new HashMap<>();
    ...
    public void handleCreditNoteGenerated(
            TicketReservation reservation,
            PurchaseContext purchaseContext,
            TotalPrice cost,
    ...
    public Optional<InvoiceGeneration> handleInvoiceGeneration(
        ...
        return Optional.ofNullable(syncCall(
                ExtensionEvent.INVOICE_GENERATION, spec.getPurchaseContext(), payload, InvoiceGeneration.class, false));
    }

    public Optional<CreditNoteGeneration> handleCreditNoteGeneration(
            PurchaseContext purchaseContext, String reservationId, String invoiceNumber, Organization organization) {
        return Optional.ofNullable(syncCall(
                ExtensionEvent.CREDIT_NOTE_GENERATION,
                purchaseContext,
                Map.of(RESERVATION_ID, reservationId, "invoiceNumber", invoiceNumber, ORGANIZATION, organization),
                CreditNoteGeneration.class));
    }

    public Optional<String> handleOnlineCheckInLink(
            String originalUrl, Ticket ticket, EventWithCheckInInfo event, Map<String, List<String>> additionalInfo) {
        Map<String, Object> payload = new HashMap<>();
        ...
        return Optional.ofNullable(extensionService.executeScriptsForEvent(
                ONLINE_CHECK_IN_REDIRECT.name(), toPath(event), payload, String.class));
    }
    ...
    boolean handleTaxIdValidation(PurchaseContext purchaseContext, String taxIdNumber, String countryCode) {
        ...
        return Optional.ofNullable(
                        syncCall(ExtensionEvent.TAX_ID_NUMBER_VALIDATION, purchaseContext, payload, Boolean.class))
                .orElse(false);
    }
    ...
    public void handleReservationValidation(
            PurchaseContext purchaseContext,
            TicketReservation reservation,
            Object clientForm,
    ...
    public void handleTicketUpdateValidation(
            PurchaseContext purchaseContext,
            UpdateTicketOwnerForm form,
            BindingResult bindingResult,
            String keyPrefix) {
        Map<String, Object> payload =
                Map.of("form", form, "validationErrorNotifier", new ValidationErrorNotifier(bindingResult, keyPrefix));
    ...
    void handleEventHeaderUpdate(Event event, Organization organization) {
        Map<String, Object> payload =
                Map.of(EVENT_METADATA, eventRepository.getMetadataForEvent(event.getId()), ORGANIZATION, organization);
     ...
     */
    public Optional<List<AdditionalInfoItem>> filterAdditionalInfoToSave(
            PurchaseContext purchaseContext,
        ...
        var result = syncCall(
                ExtensionEvent.USER_ADDITIONAL_INFO_FILTER, purchaseContext, payload, AdditionalInfoFilterResult.class);
        if (result != null) {
    ...
    public boolean handlePdfTransformation(String html, PurchaseContext purchaseContext, OutputStream outputStream) {
        ...
        try {
            PdfGenerationResult response =
                    syncCall(ExtensionEvent.PDF_GENERATION, purchaseContext, payload, PdfGenerationResult.class);
    ...
    public Optional<String> generateOAuth2StateParam(int organizationId) {
        try {
            return Optional.ofNullable(extensionService.executeScriptsForEvent(
                    ExtensionEvent.OAUTH2_STATE_GENERATION.name(),
                    "-" + organizationId,
                    Map.of(
                            "baseUrl",
                            configurationManager
                                    .getFor(ConfigurationKeys.BASE_URL, ConfigurationLevel.organization(organizationId))
                                    .getRequiredValue(),
                            ORGANIZATION_ID,
                            organizationId),
                    String.class));
    ...
    public Optional<PromoCodeDiscount> handleDynamicDiscount(
            Event event, Map<Integer, Long> quantityByCategory, String reservationId) {
        try {
            var values = new HashMap<String, Object>();
            values.put(
                    "quantityByCategory",
                    quantityByCategory.entrySet().stream()
                            .map(entry -> new QuantityByCategoryId(
                                    entry.getKey(), entry.getValue().intValue()))
                            .collect(Collectors.toList()));
            values.put(RESERVATION_ID, reservationId);
            var dynamicDiscountResult =
                    syncCall(ExtensionEvent.DYNAMIC_DISCOUNT_APPLICATION, event, values, DynamicDiscount.class);
            if (dynamicDiscountResult == null
                    || dynamicDiscountResult.getDiscountType() == PromoCodeDiscount.DiscountType.NONE) {
                return Optional.empty();
            }
            ...
            int discountAmountInCents = dynamicDiscountResult.getDiscountType() == PERCENTAGE
                    ? discountAmount.intValue()
                    : MonetaryUtil.unitToCents(discountAmount, event.getCurrency());
            return Optional.of(new PromoCodeDiscount(
                    Integer.MIN_VALUE,
                    dynamicDiscountResult.getCode(),
                    event.getId(),
                    event.getOrganizationId(),
                    now.minusSeconds(1),
                    event.getBegin().withZoneSameInstant(now.getZone()),
                    discountAmountInCents,
                    dynamicDiscountResult.getDiscountType(),
                    null,
                    null,
                    null,
                    null,
    ...
    private void asyncCall(ExtensionEvent extensionEvent, PurchaseContext event, Map<String, Object> payload) {
        extensionService.executeScriptAsync(extensionEvent.name(), toPath(event), fillWithBasicInfo(payload, event));
    }

    private <T> T syncCall(
    ...
    private <T> T syncCall(
        ...
        try {
            return extensionService.executeScriptsForEvent(
                    extensionEvent.name(), toPath(purchaseContext), fillWithBasicInfo(payload, purchaseContext), clazz);
    ...
    public <T> Optional<T> executeCapability(
    ...
    private <T> Optional<T> internalExecuteCapability(
            ExtensionCapability capability,
            Map<String, Object> contextParams,
            PurchaseContext purchaseContext,
            Class<T> resultType) {
        return extensionService.executeCapability(
                capability, toPath(purchaseContext), fillWithBasicInfo(contextParams, purchaseContext), resultType);
    }
    ...
    public Optional<TicketMetadata> handleCustomOnlineJoinUrl(
            Event event, Ticket ticket, Map<String, List<String>> ticketAdditionalInfo) {
        var ticketMetadataContainer = Objects.requireNonNullElseGet(
                ticketRepository.getTicketMetadata(ticket.getId()), TicketMetadataContainer::empty);
        ...
        var result = Optional.ofNullable(
                syncCall(ExtensionEvent.CUSTOM_ONLINE_JOIN_URL, event, context, TicketMetadata.class, false));
        result.ifPresent(m -> {
            // we update the value only if it's changed
            boolean changed =
                    existingMetadata.isEmpty() || !existingMetadata.get().equals(m);
    ...
    public Optional<TicketMetadata> handleTicketAssignmentMetadata(
            TicketWithMetadataAttributes ticketWithMetadata, Event event, Map<String, List<String>> additionalInfo) {
        ...
        context.put(
                "attendeeResources",
                new AttendeeResourcesContainer(AttendeeResources.fromTicket(
                        ticketWithMetadata.getTicket(),
                        event,
                        configurationManager.getFor(
                                EnumSet.of(ENABLE_WALLET, ENABLE_PASS, BASE_URL), event.getConfigurationLevel()),
                        true)));
        return Optional.ofNullable(syncCall(
                ExtensionEvent.TICKET_ASSIGNED_GENERATE_METADATA, event, context, TicketMetadata.class, false));
    }

    public Optional<SubscriptionMetadata> handleSubscriptionAssignmentMetadata(
        ...
        return Optional.ofNullable(syncCall(
                ExtensionEvent.SUBSCRIPTION_ASSIGNED_GENERATE_METADATA,
                descriptor,
                context,
    ...
    public Optional<CustomTaxPolicy> handleCustomTaxPolicy(
            PurchaseContext purchaseContext,
        ...
        return Optional.ofNullable(
                syncCall(CUSTOM_TAX_POLICY_APPLICATION, event, context, CustomTaxPolicy.class, false));
    }
}
