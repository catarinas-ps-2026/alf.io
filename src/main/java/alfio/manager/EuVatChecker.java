...
public class EuVatChecker {
    ...
    private static final Set<EUVatCheckResponse.FaultType> REMOTE_FAULTS =
            EnumSet.of(GLOBAL_MAX_CONCURRENT_REQ, TIMEOUT, MS_MAX_CONCURRENT_REQ, SERVICE_UNAVAILABLE, MS_UNAVAILABLE);
    private final ConfigurationManager configurationManager;
    private final AuditingRepository auditingRepository;
    private final EUVatChecker client = new EUVatChecker();
    private final ExtensionManager extensionManager;

    private static final Cache<Pair<String, String>, EUVatCheckResponse> validationCache =
    ...
    public Optional<VatDetail> checkVat(String vatNr, String countryCode, PurchaseContext purchaseContext) {
        Optional<VatDetail> res =
                performCheck(vatNr, countryCode, purchaseContext).apply(configurationManager, client);
        return res.map(detail -> {
            if (!detail.isValid()) {
                String organizerCountry = organizerCountry(configurationManager, purchaseContext);
                boolean valid = ("IT".equals(organizerCountry)
                                && "IT".equals(countryCode)
                                && ItalianTaxIdValidator.validateVatId(vatNr))
    ...
    static BiFunction<ConfigurationManager, EUVatChecker, Optional<VatDetail>> performCheck(
            String vatNr, String countryCode, Configurable configurable) {
        return (configurationManager, client) -> {
            ...
            boolean euCountryCode = configurationManager
            ...
            if (euCountryCode && validationEnabled) {
                ...
                return Optional.ofNullable(validationResult)
                        .map(r -> getVatDetail(
                                reverseChargeEnabled(configurationManager, configurable),
                                r,
                                vatNr,
                                countryCode,
                                organizerCountry(configurationManager, configurable)));
            }
            ...
            BooleanSupplier applyVatToForeignBusiness = () -> configurationManager
                    .getFor(APPLY_VAT_FOREIGN_BUSINESS, configurable.getConfigurationLevel())
                    .getValueAsBooleanOrDefault();
            boolean vatExempt = !organizerCountry.equals(countryCode)
                    && (euCountryCode || !applyVatToForeignBusiness.getAsBoolean());
            return Optional.of(new VatDetail(
                    vatNr,
                    countryCode,
                    true,
                    "",
                    "",
                    euCountryCode ? VatDetail.Type.SKIPPED : VatDetail.Type.EXTRA_EU,
                    vatExempt));
        };
    }

    public void logSuccessfulValidation(VatDetail detail, String reservationId, PurchaseContext purchaseContext) {
        List<Map<String, Object>> modifications = List.of(Map.of(
                "vatNumber", detail.getVatNr(), "country", detail.getCountry(), "validationType", detail.getType()));
        ...
        auditingRepository.insert(
                reservationId, null, purchaseContext, eventType, new Date(), RESERVATION, reservationId, modifications);
    }
    ...
    private static VatDetail getVatDetail(
            boolean reverseChargeEnabled,
            EUVatCheckResponse response,
            String vatNr,
            String countryCode,
            String organizerCountryCode) {
        ...
        return new VatDetail(
                ...
                response.getAddress(),
                VatDetail.Type.VIES,
                isValid && reverseChargeEnabled && !organizerCountryCode.equals(countryCode));
    }

    private static void checkServerErrors(EUVatCheckResponse response, String countryCode) {
        if (response.isError()
                && response.getFault() != null
                && REMOTE_FAULTS.contains(response.getFault().getFaultType())) {
    ...
    static String organizerCountry(ConfigurationManager configurationManager, Configurable configurable) {
        return configurationManager
                .getFor(COUNTRY_OF_BUSINESS, configurable.getConfigurationLevel())
                .getValueOrNull();
    }
    ...
    public static Map<ConfigurationKeys, ConfigurationManager.MaybeConfiguration>
            loadConfigurationForReverseChargeCheck(
                    ConfigurationManager configurationManager, Configurable configurable) {
    ...
    public static boolean reverseChargeEnabled(Map<ConfigurationKeys, ConfigurationManager.MaybeConfiguration> res) {
        ...
        return (res.get(ENABLE_EU_VAT_DIRECTIVE).getValueAsBooleanOrDefault()
                        && (res.get(ENABLE_REVERSE_CHARGE_IN_PERSON).getValueAsBooleanOrDefault()
                                || res.get(ENABLE_REVERSE_CHARGE_ONLINE).getValueAsBooleanOrDefault()))
    ...
    static boolean validationEnabled(ConfigurationManager configurationManager, Configurable configurable) {
        return configurationManager
                .getFor(ConfigurationKeys.ENABLE_VIES_VALIDATION, configurable.getConfigurationLevel())
                .getValueAsBooleanOrDefault();
    }
}
