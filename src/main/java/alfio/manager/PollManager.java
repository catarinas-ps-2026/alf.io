...
public class PollManager {
    ...
    public Result<List<Poll>> getActiveForEvent(String eventName, String pin) {
        return validatePinAndEvent(pin, eventName)
                .flatMap(eventAndTicket -> Result.success(pollRepository.findActiveForEvent(
                        eventAndTicket.getLeft().getId())));
    }

    public Result<PollWithOptions> getSingleActiveForEvent(String eventName, Long id, String pin) {
        return validatePinAndEvent(pin, eventName).flatMap(eventAndTicket -> {
            var optionalPoll = pollRepository.findSingleActiveForEvent(
                    eventAndTicket.getLeft().getId(), id);
    ...
    public Result<Boolean> registerAnswer(String eventName, Long pollId, Long optionId, String pin) {
        ...
        return validatePinAndEvent(pin, eventName).flatMap(eventAndTicket -> {
            ...
            Validate.isTrue(
                    pollRepository.registerAnswer(pollId, optionId, ticket.getId(), event.getOrganizationId()) == 1,
                    "Unexpected error while inserting answer");
    ...
    private Optional<PollWithOptions> getSingleForEvent(Long pollId, EventAndOrganizationId event) {
        return pollRepository
                .findSingleForEvent(event.getId(), Objects.requireNonNull(pollId))
    ...
    public Optional<Long> createNewPoll(String eventName, PollModification form) {
        ...
        return eventRepository
                .findOptionalEventAndOrganizationIdByShortName(eventName)
                .map(event -> {
                    List<String> tags = form.isAccessRestricted()
    ...
    public Optional<PollWithOptions> updatePoll(String eventName, PollModification form) {
        ...
        return eventRepository
                ...
                .flatMap(event -> {
                    ...
                    var existingPollWithOptions =
                            getSingleForEvent(pollId, event).orElseThrow();
                    ...
                    if (form.isAccessRestricted() == existingPoll.allowedTags().isEmpty()) {
                        tags = form.isAccessRestricted()
                                ? List.of(UUID.randomUUID().toString())
                                : List.of();
                    }
                    Validate.isTrue(pollRepository.update(
                                    form.getTitle(),
                                    form.getDescription(),
                                    tags,
                                    form.getOrder(),
                    ...
                    var newOptions = form.getOptions().stream()
                    ...
                    var existingOptions = form.getOptions().stream()
                            .filter(pm -> pm.getId() != null)
                            .collect(Collectors.toList());
    ...
    public Optional<PollWithOptions> updateStatus(
            Long pollId, EventAndOrganizationId event, Poll.PollStatus newStatus) {
        Validate.isTrue(newStatus != Poll.PollStatus.DRAFT, "can't revert to draft");
        Validate.isTrue(
                pollRepository.updateStatus(newStatus, pollId, event.getId()) == 1, "Error while updating status");
        return getSingleForEvent(pollId, event);
    }

    public Optional<List<PollParticipant>> searchTicketsToAllow(
            EventAndOrganizationId event, Long pollId, String filter) {
        Validate.isTrue(StringUtils.isNotBlank(filter));
        return pollRepository
                .findSingleForEvent(event.getId(), pollId)
                .map(p -> ticketSearchRepository.filterConfirmedTicketsInEventForPoll(
                        event.getId(), 20, "%" + filter + "%", p.allowedTags()));
    }
    ...
    public List<PollParticipant> removeParticipants(
            EventAndOrganizationId event, List<Integer> ticketIds, long pollId) {
        var poll = pollRepository.findSingleForEvent(event.getId(), pollId).orElseThrow();
        Validate.isTrue(CollectionUtils.isNotEmpty(poll.allowedTags()));
        var tag = poll.allowedTags().get(0);
    ...
    public Optional<PollStatistics> getStatisticsFor(EventAndOrganizationId event, long pollId) {
        return pollRepository.findSingleForEvent(event.getId(), pollId).map(p -> {
            int allowedParticipants;
            if (p.allowedTags().isEmpty()) {
                allowedParticipants =
                        eventRepository.findStatisticsFor(event.getId()).getCheckedInTickets();
            } else {
                allowedParticipants = ticketRepository.countTicketsMatchingTagsAndStatus(
                        event.getId(), p.allowedTags(), List.of(Ticket.TicketStatus.CHECKED_IN.name()));
            }
            var statistics = pollRepository.getStatisticsFor(p.id(), event.getId());
            return new PollStatistics(
                    statistics.stream().mapToInt(PollOptionStatistics::getVotes).sum(),
                    allowedParticipants,
                    statistics);
        });
    }
    ...
    private void updateOptions(
            List<PollOptionModification> existingOptions, EventAndOrganizationId event, Long pollId) {
    ...
    private Result<Pair<EventAndOrganizationId, Ticket>> validatePinAndEvent(String pin, String eventName) {
        ...
        return new Result.Builder<EventAndOrganizationId>()
                ...
                .flatMap(event -> {
                    ...
                    var tickets = ticketRepository.findByEventIdAndPartialUUIDForUpdate(
                            event.getId(), partialUuid + "%", Ticket.TicketStatus.CHECKED_IN);
