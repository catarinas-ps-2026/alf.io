...
public class ReservationEmailContentHelper {
    ...
    public ReservationEmailContentHelper(
    ...
    public void sendConfirmationEmail(
        ...
        if (configurationManager.canAttachBillingDocumentToConfirmationEmail(
                purchaseContext)) { // https://github.com/alfio-event/alf.io/issues/573
            attachments = generateAttachmentForConfirmationEmail(
                    purchaseContext, ticketReservation, language, summary, username);
        } else {
            attachments = List.of();
        }
        ...
        if (purchaseContext.ofType(PurchaseContext.PurchaseContextType.subscription)) {
            var firstSubscription = subscriptionRepository.findSubscriptionsByReservationId(reservationId).stream()
                    .findFirst()
                    .orElseThrow();
            boolean sendSeparateEmailToOwner =
                    !Objects.equals(firstSubscription.getEmail(), ticketReservation.getEmail());
            var metadata = Objects.requireNonNullElseGet(
                    subscriptionRepository.getSubscriptionMetadata(firstSubscription.getId()),
                    SubscriptionMetadata::empty);
            Map<String, Object> initialModel = Map.of(
                    "pin",
                    firstSubscription.getPin(),
                    "subscriptionId",
                    firstSubscription.getId(),
                    "includePin",
                    metadata.getConfiguration().isDisplayPin(),
                    "fullName",
                    firstSubscription.getFirstName() + " " + firstSubscription.getLastName(),
                    METADATA_ATTRIBUTES_KEY,
                    metadata.getProperties());
            var model = prepareModelForReservationEmail(
                    purchaseContext, ticketReservation, vat, summary, List.of(), initialModel);
            ...
            configurations.add(new ConfirmationEmailConfiguration(
                    TemplateResource.CONFIRMATION_EMAIL_SUBSCRIPTION,
                    firstSubscription.getEmail(),
                    model,
                    sendSeparateEmailToOwner ? List.of(subscriptionAttachment) : subscriptionAttachments));
            ...
            var model = prepareModelForReservationEmail(
                    ...
                    Map.of());
            configurations.add(new ConfirmationEmailConfiguration(
                    TemplateResource.CONFIRMATION_EMAIL, ticketReservation.getEmail(), model, attachments));
        }
        ...
        configurations.forEach(configuration -> {
            notificationManager.sendSimpleEmail(
                    ...
                    messageSource.getMessage(
                            "reservation-email-subject",
                            new Object[] {
                                configurationManager.getShortReservationID(purchaseContext, ticketReservation),
                                purchaseContext.getTitle().get(language.getLanguage()),
                                localizedType
                            },
                            language),
                    () -> templateManager.renderTemplate(
                            purchaseContext, configuration.getTemplateResource(), configuration.getModel(), language),
    ...
    private Mailer.Attachment generateSubscriptionAttachment(Subscription subscription) {
        ...
        return new Mailer.Attachment(
                "subscription_" + subscription.getId() + ".pdf",
                null,
    ...
    private List<Mailer.Attachment> generateAttachmentForConfirmationEmail(
            PurchaseContext purchaseContext,
            TicketReservation ticketReservation,
            Locale language,
            OrderSummary summary,
            String username) {
        if (mustGenerateBillingDocument(
                summary, ticketReservation)) { // #459 - include PDF invoice in reservation email
            BillingDocument.Type type = ticketReservation.getHasInvoiceNumber() ? INVOICE : RECEIPT;
            return billingDocumentManager.generateBillingDocumentAttachment(
                    purchaseContext, ticketReservation, language, type, username, summary);
        }
        return List.of();
    }

    public void sendReservationCompleteEmailToOrganizer(
            PurchaseContext purchaseContext, TicketReservation ticketReservation, Locale language, String username) {
        String reservationId = ticketReservation.getId();
        ...
        if (!configurationManager.canGenerateReceiptOrInvoiceToCustomer(purchaseContext)
                || configurationManager.isInvoiceOnly(
                        purchaseContext)) { // https://github.com/alfio-event/alf.io/issues/573
            attachments = generateAttachmentForConfirmationEmail(
                    purchaseContext, ticketReservation, language, summary, username);
        }
        ...
        if (configurationManager.isNotifyOrganizerOnReservationEnabled(purchaseContext)) {
            ...
            notificationManager.sendSimpleEmail(
                    ...
                    cc,
                    "Reservation complete " + shortReservationID,
                    () -> templateManager.renderTemplate(
                            purchaseContext,
                            TemplateResource.CONFIRMATION_EMAIL_FOR_ORGANIZER,
    ...
    public List<Mailer.Attachment> generateBillingDocumentAttachment(
        ...
        model.put(
                "reservationEmailModel", Json.toJson(billingDocumentModel)); // ticketReservation.getHasInvoiceNumber()
        return switch (documentType) {
            case INVOICE -> Collections.singletonList(new Mailer.Attachment(
                    "invoice.pdf", null, APPLICATION_PDF.getType(), model, Mailer.AttachmentIdentifier.INVOICE_PDF));
            case RECEIPT -> Collections.singletonList(new Mailer.Attachment(
                    "receipt.pdf", null, APPLICATION_PDF.getType(), model, Mailer.AttachmentIdentifier.RECEIPT_PDF));
            case CREDIT_NOTE -> Collections.singletonList(new Mailer.Attachment(
                    "credit-note.pdf",
                    null,
                    APPLICATION_PDF.getType(),
                    model,
    ...
    public String getReservationEmailSubject(
            PurchaseContext purchaseContext, Locale reservationLanguage, String key, String id) {
        return messageSourceManager
    ...
    public Map<String, Object> prepareModelForReservationEmail(
        ...
        var bankingInfo = configurationManager.getFor(
                Set.of(INVOICE_ADDRESS, BANK_ACCOUNT_NR, BANK_ACCOUNT_OWNER),
        ...
        Map<Integer, List<Ticket>> ticketsByCategory =
                ticketsToInclude.stream().collect(groupingBy(Ticket::getCategoryId));
        final List<TicketWithCategory> ticketsWithCategory =
                ReservationUtil.collectTicketsWithCategory(ticketsByCategory, ticketCategoryRepository);
        Map<String, Object> baseModel = new HashMap<>();
        baseModel.putAll(initialOptions);
        baseModel.putAll(extensionManager
                .handleReservationEmailCustomText(
                        purchaseContext, reservation, ticketReservationRepository.getAdditionalInfo(reservationId))
        ...
        model.put(
                ...
                ticketReservationRepository.getAdditionalInfo(reservationId).getInvoicingAdditionalInfo());
        if (purchaseContext.getType() == PurchaseContext.PurchaseContextType.event) {
            var event = purchaseContext.event().orElseThrow();
            model.put(
                    "displayLocation",
                    ticketsWithCategory.stream().noneMatch(tc -> EventUtil.isAccessOnline(tc.category(), event)));
        } else {
        ...
        if (ticketReservationRepository.hasSubscriptionApplied(reservationId)) {
            ...
            var subscription = subscriptionRepository
                    .findAppliedSubscriptionByReservationId(reservationId)
                    .orElseThrow();
            if (subscription.getMaxEntries() > -1) {
                var subscriptionUsageDetails = UsageDetails.fromSubscription(
                        subscription, ticketRepository.countSubscriptionUsage(subscription.getId(), null));
    ...
    public void sendTicketByEmail(
            Ticket ticket, Locale locale, Event event, PartialTicketTextGenerator confirmationTextBuilder) {
        TicketReservation reservation =
        ...
        notificationManager.sendTicketByEmail(
                ...
                reservation,
                ticketCategory,
                () -> retrieveAttendeeAdditionalInfoForTicket(ticket));
    }
    ...
    private void checkIfFinalized(String reservationId) {
        if (!Boolean.TRUE.equals(ticketReservationRepository.checkIfFinalized(reservationId))) {
            throw new IncompatibleStateException(
                    "Reservation was confirmed but not finalized yet. Cannot send emails.");
        }
    }

    public Map<String, List<String>> retrieveAttendeeAdditionalInfoForTicket(Ticket ticket) {
        return purchaseContextFieldRepository.findNameAndValue(ticket.getId()).stream()
    ...
    public Map<String, List<String>> retrieveAttendeeAdditionalInfoForSubscription(UUID subscriptionId) {
        return purchaseContextFieldRepository.findNameAndValue(subscriptionId).stream()
                .collect(groupingBy(FieldNameAndValue::getName, mapping(FieldNameAndValue::getValue, toList())));
    }
    ...
    public PartialTicketTextGenerator getTicketEmailGenerator(
        ...
        return ticket -> {
            ...
            var initialModel = new HashMap<>(extensionManager
                    .handleTicketEmailCustomText(
                            event,
                            ticketReservation,
                            ticketReservationRepository.getAdditionalInfo(ticketReservation.getId()),
                            purchaseContextFieldRepository.findAllByTicketId(ticket.getId()))
                    .map(CustomEmailText::toMap)
                    .orElse(Map.of()));
            ...
            var baseUrl = StringUtils.removeEnd(
                    configurationManager
                            .getFor(BASE_URL, ConfigurationLevel.event(event))
                            .getRequiredValue(),
                    "/");
            var calendarUrl = UriComponentsBuilder.fromUriString(
                            baseUrl + "/api/v2/public/event/{eventShortName}/calendar/{currentLang}")
                    .queryParam("type", "google")
                    .build(Map.of("eventShortName", event.getShortName(), "currentLang", ticketLanguage.getLanguage()))
                    .toString();
            return TemplateProcessor.buildPartialEmail(
                            event,
                            organization,
                            ticketReservation,
                            ticketCategory,
    ...
    public Optional<String> getVAT(PurchaseContext purchaseContext) {
        return configurationManager
                .getFor(VAT_NR, purchaseContext.getConfigurationLevel())
                .getValue();
    }
