...
public class OrderSummaryGenerator {
    ...
    public OrderSummary orderSummaryForReservation(TicketReservation reservation, PurchaseContext context) {
        ...
        boolean hasRefund =
                auditingRepository.countAuditsOfTypeForReservation(reservation.getId(), Audit.EventType.REFUND) > 0;

        if (hasRefund) {
            refundedAmount = paymentManager
                    .getInfo(reservation, context)
        ...
        return new OrderSummary(
                ...
                Optional.ofNullable(context.getVat())
                        .map(p -> MonetaryUtil.formatCents(MonetaryUtil.unitToCents(p, currencyCode), currencyCode))
                        .orElse(null),
                reservation.getVatStatus(),
                refundedAmount);
    }
    ...
    private boolean isCashPayment(TicketReservation reservation) {
        return reservation.getPaymentMethod() == PaymentProxy.ON_SITE
                && !transactionRepository.transactionExists(reservation.getId());
    }

    public OrderSummary orderSummaryForCreditNote(
            TicketReservation reservation, PurchaseContext purchaseContext, List<Ticket> removedTickets) {
        var totalPriceAndDiscount = totalReservationCostWithVAT(
                null, purchaseContext, reservation, removedTickets, List.of(), List.of(), Optional.empty());
        TotalPrice reservationCost = totalPriceAndDiscount.getLeft();
        //
        boolean free = reservationCost.getPriceWithVAT() == 0;
        ...
        return new OrderSummary(
                ...
                extractSummary(
                        ...
                        removedTickets,
                        Stream.empty(),
                        subscriptionRepository.findSubscriptionsByReservationId(reservation.getId())),
                free,
                formatCents(reservationCost.getPriceWithVAT(), currencyCode),
                formatCents(reservationCost.getVAT(), currencyCode),
                reservation.getStatus() == TicketReservation.TicketReservationStatus.OFFLINE_PAYMENT,
                reservation.getStatus() == DEFERRED_OFFLINE_PAYMENT,
                isCashPayment(reservation),
                Optional.ofNullable(purchaseContext.getVat())
                        .map(p -> MonetaryUtil.formatCents(MonetaryUtil.unitToCents(p, currencyCode), currencyCode))
                        .orElse(null),
                reservation.getVatStatus(),
                null);
    ...
    List<SummaryRow> extractSummary(
        ...
        purchaseContext.event().ifPresent(event -> {
            boolean multipleTaxRates = tickets.stream()
                            .map(TicketPriceContainer::getVatStatus)
                            .collect(Collectors.toSet())
                            .size()
                    > 1;
            var ticketsByCategory =
                    tickets.stream().collect(Collectors.groupingBy(TicketPriceContainer::getCategoryId));
            List<Map.Entry<Integer, List<TicketPriceContainer>>> sorted;
            if (multipleTaxRates) {
                sorted = ticketsByCategory.entrySet().stream()
                        .sorted(Comparator.comparing((Map.Entry<Integer, List<TicketPriceContainer>> e) ->
                                        e.getValue().get(0).getVatStatus())
                                .reversed())
                        .toList();
            } else {
            ...
            if (ticketsByCategory.isEmpty()) {
                ...
                categoriesById =
                        ticketCategoryRepository.getByIdsAndActive(ticketsByCategory.keySet(), event.getId()).stream()
            ...
            for (var categoryWithTickets : sorted) {
                ...
                final int subTotal = categoryTickets.stream()
                        .mapToInt(TicketPriceContainer::getSummarySrcPriceCts)
                        .sum();
                ...
                final int priceBeforeVat =
                        SummaryPriceContainer.getSummaryPriceBeforeVatCts(singletonList(firstTicket));
                String categoryName =
                        categoriesById.get(categoryWithTickets.getKey()).getName();
        ...
        summary.addAll(additionalServicesToInclude
                .map(entry -> {
                    ...
                    AdditionalServiceText title = additionalServiceTextRepository.findBestMatchByLocaleAndType(
                            entry.getKey().id(), language, AdditionalServiceText.TextType.TITLE);
                    if (!title.getLocale().equals(language) || title.getId() == -1) {
                        log.debug(
                                "additional service {}: title not found for locale {}",
                                title.getAdditionalServiceId(),
                                language);
                    }
                    List<AdditionalServiceItemPriceContainer> prices = generateASIPriceContainers(purchaseContext, null)
                            .apply(entry)
                            .collect(toList());
                    AdditionalServiceItemPriceContainer first = prices.get(0);
                    final int subtotal = prices.stream()
                            .mapToInt(AdditionalServiceItemPriceContainer::getSrcPriceCts)
                            .sum();
                    ...
                    return new SummaryRow(
                            ...
                            formatCents(
                                    SummaryPriceContainer.getSummaryPriceBeforeVatCts(singletonList(first)),
                                    currencyCode),
                            prices.size(),
                            formatCents(subtotal, currencyCode),
                            formatCents(subtotalBeforeVat, currencyCode),
        ...
        Optional.ofNullable(promoCodeDiscount).ifPresent(promo -> {
            String formattedSingleAmount = "-"
                    + (PromoCodeDiscount.DiscountType.isFixedAmount(promo.getDiscountType())
                            ? formatCents(promo.getDiscountAmount(), currencyCode)
                            : (promo.getDiscountAmount() + "%"));
            summary.add(new SummaryRow(
                    formatPromoCode(promo, ticketsToInclude, locale, purchaseContext),
                    formattedSingleAmount,
                    formattedSingleAmount,
                    reservationCost.getDiscountAppliedCount(),
                    formatCents(reservationCost.getDiscount(), currencyCode),
                    formatCents(reservationCost.getDiscount(), currencyCode),
                    reservationCost.getDiscount(),
        ...
        if (purchaseContext instanceof SubscriptionDescriptor descriptor) {
            if (!subscriptionsToInclude.isEmpty()) {
                ...
                summary.add(new SummaryRow(
                        purchaseContext.getTitle().get(locale.getLanguage()),
                        formatCents(priceContainer.getSummarySrcPriceCts(), currencyCode),
                        priceBeforeVat,
                        subscriptionsToInclude.size(),
                        formatCents(
                                priceContainer.getSummarySrcPriceCts() * subscriptionsToInclude.size(), currencyCode),
                        formatUnit(
                                priceContainer.getNetPrice().multiply(new BigDecimal(subscriptionsToInclude.size())),
                                currencyCode),
            ...
            subscriptionRepository
                    .findOne(subscription.getSubscriptionDescriptorId(), subscription.getOrganizationId())
                    .ifPresent(subscriptionDescriptor -> {
                        log.trace("found subscriptionDescriptor with ID {}", subscriptionDescriptor.getId());
                        // find tickets with subscription applied
                        var ticketsSubscription = tickets.stream()
                                .filter(t -> Objects.equals(subscription.getId(), t.getSubscriptionId()))
                                .toList();
                        final int ticketPriceCts = ticketsSubscription.stream()
                                .mapToInt(TicketPriceContainer::getSummarySrcPriceCts)
                                .sum();
                        final int priceBeforeVat =
                                SummaryPriceContainer.getSummaryPriceBeforeVatCts(ticketsSubscription);
                        summary.add(new SummaryRow(
                                subscriptionDescriptor.getLocalizedTitle(locale),
                                "-" + formatCents(ticketPriceCts, currencyCode),
                                "-" + formatCents(priceBeforeVat, currencyCode),
    ...
    public List<SummaryRow> extractSummary(
        ...
        if (purchaseContext.ofType(PurchaseContext.PurchaseContextType.event)) {
            subscriptionsToInclude = subscriptionRepository
    ...
    private String formatPromoCode(
            PromoCodeDiscount promoCodeDiscount, List<Ticket> tickets, Locale locale, PurchaseContext purchaseContext) {
        ...
        if (promoCodeDiscount.getCodeType() == PromoCodeDiscount.CodeType.DYNAMIC) {
            return messageSourceManager
                    ...
                    .getMessage(
                            ...
                            null,
                            locale); // we don't expose the internal promo code
        }

        List<Ticket> filteredTickets = tickets.stream()
                .filter(ticket -> promoCodeDiscount.getCategories().contains(ticket.getCategoryId()))
                .toList();
        ...
        String formattedDiscountedCategories =
                filteredTickets.stream().map(Ticket::getCategoryId).collect(toSet()).stream()
                        .map(categoryId -> ticketCategoryRepository
                                .getByIdAndActive(categoryId, promoCodeDiscount.getEventId())
                                .getName())
    ...
    private static Function<
                    Pair<AdditionalService, List<AdditionalServiceItem>>,
                    Stream<? extends AdditionalServiceItemPriceContainer>>
            generateASIPriceContainers(PurchaseContext purchaseContext, PromoCodeDiscount discount) {
        return p -> p.getValue().stream()
                .map(asi -> AdditionalServiceItemPriceContainer.from(asi, p.getKey(), purchaseContext, discount));
    }
}
