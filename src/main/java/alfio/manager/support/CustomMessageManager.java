...
public class CustomMessageManager {
    ...
    public Map<String, Object> generatePreview(
            String eventName, Optional<Integer> categoryId, List<MessageModification> input, String username) {
        Map<String, Object> result = new HashMap<>();
        Event event = eventManager.getSingleEvent(eventName, username);
        result.put(
                "affectedUsers",
                categoryId
                        .map(id -> ticketRepository.countAssignedTickets(event.getId(), id))
    ...
    public void sendMessages(
        ...
        Map<String, List<MessageModification>> byLanguage =
                input.stream().collect(Collectors.groupingBy(m -> m.getLocale().getLanguage()));
        ...
        var configuration = configurationManager.getFor(
                EnumSet.of(
                        ...
                        ConfigurationKeys.ENABLE_HTML_EMAILS),
        ...
        sendMessagesExecutor.execute(() -> {
            ...
            categoryId
                    .map(id -> ticketRepository.findConfirmedByCategoryId(event.getId(), id))
                    ...
                    .map(t -> {
                        ...
                        model.addAttribute(
                                "ticketURL", ReservationUtil.ticketUpdateUrl(event, t, configurationManager));
                    ...
                    .forEach(triple -> {
                        ...
                        MessageModification m = Optional.ofNullable(byLanguage.get(ticket.getUserLanguage()))
                                .orElseGet(() -> byLanguage.get(byLanguage.keySet().stream()
                                        .findFirst()
                        ...
                        StringBuilder text = new StringBuilder(
                                renderResource(m.getText(), event, model, m.getLocale(), templateManager));
                        ...
                        if (m.isAttachTicket()) {
                            ...
                            boolean onlineTicket = optionalTicketCategory.isPresent()
                                    && EventUtil.isAccessOnline(optionalTicketCategory.get(), event);
                            ...
                            if (optionalReservation.isPresent() && onlineTicket) {
                                ...
                                onlineCheckInModel.putAll(getModelForTicket(
                                        ticket, optionalReservation.get(), optionalTicketCategory.get(), organization));
                                ...
                                text.append(notificationManager.buildOnlineCheckInText(
                                        onlineCheckInModel,
                                        Locale.forLanguageTag(ticket.getUserLanguage()),
                                        messageSource));
                                templateModel.putAll(onlineCheckInModel);
                            } else if (optionalReservation.isPresent() && optionalTicketCategory.isPresent()) {
                                boolean htmlEmailsEnabled = configuration
                                        .get(ConfigurationKeys.ENABLE_HTML_EMAILS)
                                        .getValueAsBooleanOrDefault();
                                attachments.add(generateTicketAttachment(
                                        ticket,
                                        optionalReservation.get(),
                                        optionalTicketCategory.get(),
                                        organization,
                                        htmlEmailsEnabled));
                            }
                            ...
                            templateModel.put(
                                    "walletEnabled", (googleWalletEnabled || appleWalletEnabled) && !onlineTicket);
                        } else {
                        ...
                        notificationManager.sendSimpleEmail(
                                ...
                                () -> templateManager.renderTemplate(
                                        event,
                                        TemplateResource.CUSTOM_MESSAGE,
                                        templateModel,
                                        Locale.forLanguageTag(ticket.getUserLanguage())),
                                attachments);
                    });
    ...
    private List<MessageModification> preview(Event event, List<MessageModification> input, String username) {
        ...
        return input.stream()
                .map(m -> MessageModification.preview(
                        m,
                        renderResource(m.getSubject(), event, model, m.getLocale(), templateManager),
                        renderResource(m.getText(), event, model, m.getLocale(), templateManager),
                        m.isAttachTicket()))
                .collect(Collectors.toList());
    }

    public static Mailer.Attachment generateTicketAttachment(
            Ticket ticket,
        ...
        return new Mailer.Attachment(
                "ticket-" + ticket.getPublicUuid() + ".pdf",
                null,
                "application/pdf",
                model,
    ...
    private static Map<String, String> getModelForTicket(
            Ticket ticket, TicketReservation reservation, TicketCategory ticketCategory, Organization organization) {
    ...
    public static Mailer.Attachment generateCalendarAttachmentForOnlineEvent(
            Event event,
            Ticket ticket,
    ...
    public static Mailer.Attachment generateCalendarAttachmentForOnlineEvent(Map<String, String> model) {
        return new Mailer.Attachment(
                CALENDAR_ICS.fileName(""), null, CALENDAR_ICS.contentType(""), model, CALENDAR_ICS);
    }

    private static String renderResource(
            String template,
            PurchaseContext purchaseContext,
            Model model,
            Locale locale,
            TemplateManager templateManager) {
        return templateManager.renderString(
                purchaseContext, template, model.asMap(), locale, TemplateManager.TemplateOutput.TEXT);
    }
}
