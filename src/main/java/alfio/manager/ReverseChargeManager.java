...
public class ReverseChargeManager {
    ...
    public void checkAndApplyVATRules(
        ...
        var reverseChargeConfiguration =
                EuVatChecker.loadConfigurationForReverseChargeCheck(configurationManager, purchaseContext);

        if (EuVatChecker.reverseChargeEnabled(reverseChargeConfiguration)
                && (country == null || isEUCountry(country))) {
        ...
        boolean reverseChargeInPerson = !isEvent
                || reverseChargeConfiguration
                        .get(ENABLE_REVERSE_CHARGE_IN_PERSON)
                        .getValueAsBooleanOrDefault();
        boolean reverseChargeOnline = !isEvent
                || reverseChargeConfiguration.get(ENABLE_REVERSE_CHARGE_ONLINE).getValueAsBooleanOrDefault();
        ...
        try {
            ...
            if (!categoriesList.isEmpty()) {
                noTaxCategories.addAll(configurationManager.getCategoriesWithNoTaxes(
                        categoriesList.stream().map(TicketCategory::getId).collect(Collectors.toList())));
            }

            Optional<VatDetail> vatDetail = performReverseChargeCheck(
                    purchaseContext,
                    contactAndTicketsForm,
                    country,
                    reverseChargeInPerson,
                    reverseChargeOnline,
                    optionalReservation,
                    categoriesList);
            ...
            if (vatDetail.isPresent()) {
                ...
                updateTicketsWithNoTaxSetting(
                        purchaseContext, reservationId, reservation, categoriesList, noTaxCategories);
                updateBillingData(
                        contactAndTicketsForm,
                        purchaseContext,
                        country,
                        trimToNull(contactAndTicketsForm.getVatNr()),
                        reservation,
    ...
    private Optional<VatDetail> performReverseChargeCheck(
            PurchaseContext purchaseContext,
            ContactAndTicketsForm contactAndTicketsForm,
            String country,
            boolean reverseChargeInPerson,
            boolean reverseChargeOnline,
            Optional<TicketReservation> optionalReservation,
            List<TicketCategory> categoriesList) {
        return optionalReservation
                .filter(e -> {
                    if (purchaseContext.ofType(PurchaseContextType.event)
                            && (!reverseChargeInPerson || !reverseChargeOnline)) {
                        ...
                        return categoriesList.stream()
                                .anyMatch(findReverseChargeCategory(
                                        reverseChargeInPerson, reverseChargeOnline, eventFormat));
                    }
    ...
    private void updateTicketsWithNoTaxSetting(
            PurchaseContext purchaseContext,
            String reservationId,
        ...
        if (purchaseContext.ofType(PurchaseContextType.event)) {
            ...
            var priceContainers = mapPriceContainersByCategoryId(
                    categoriesList,
    ...
    private void handleSplitPayment(
            PurchaseContext purchaseContext,
            String reservationId,
            ContactAndTicketsForm contactAndTicketsForm,
            String country,
    ...
    private void handleValidationResult(
        ...
        if (!vatValidation.isValid()) {
            ...
            PriceContainer.VatStatus vatStatus =
                    determineVatStatus(purchaseContext.getVatStatus(), vatValidation.isVatExempt());
            ...
            if (!isEvent || (reverseChargeOnline && reverseChargeInPerson)) {
                if (isEvent) {
                    var event = purchaseContext.event().orElseThrow();
                    var priceContainers = mapPriceContainersByCategoryId(
                            categoriesList, a -> true, currencyCode, vatStatus, discount, event, noTaxCategories);
                ...
                updateBillingData(
                        ...
                        purchaseContext,
                        country,
                        trimToNull(vatValidation.getVatNr()),
                        reservation,
                        vatStatus);
            } else {
                var event = purchaseContext.event().orElseThrow();
                var eventFormat = event.getFormat();
                var matchingCategories = mapPriceContainersByCategoryId(
                        categoriesList,
    ...
    public void applyCustomTaxPolicy(
            PurchaseContext purchaseContext,
        ...
        if (customTaxPolicy.getTicketPolicies().stream().anyMatch(tp -> !ticketIds.contains(tp.getUuid()))) {
            ...
            auditingRepository.insert(
                    ...
                    RESERVATION,
                    reservationId,
                    List.of(Map.of("policy", customTaxPolicy)));
            var priceMapping = customTaxPolicy.getTicketPolicies().stream()
                    .map(tcp -> toTicketPriceContainer(
                            ticketsInReservation.get(tcp.getUuid()), tcp, getDiscountOrNull(reservation), event))
            ...
            updateBillingData(
                    ...
                    trimToNull(reservation.getVatNr()),
                    reservation,
                    reservation.getVatStatus());
        }
    }

    private static TicketPriceContainer toTicketPriceContainer(
            Ticket ticket, CustomTaxPolicy.TicketTaxPolicy categoryTaxPolicy, PromoCodeDiscount discount, Event event) {
    ...
    private Map<Integer, TicketCategoryPriceContainer> mapPriceContainersByCategoryId(
            List<TicketCategory> categoriesList,
        ...
        return categoriesList.stream()
                ...
                .map(c -> {
                    ...
                    return new TicketCategoryPriceContainer(
                            c.getId(), c.getSrcPriceCts(), currencyCode, event.getVat(), categoryVatStatus, discount);
                })
                .collect(toMap(o -> o.categoryId, Function.identity()));
    }

    private void updateTicketPricesByCategory(
            String reservationId,
            String currencyCode,
            Event event,
        ...
        MapSqlParameterSource[] parameterSources = matchingCategories.entrySet().stream()
                .map(entry -> buildParameterSourceForPriceUpdate(
                        entry.getValue(),
                        event.getId(),
                        entry.getKey(),
                        currencyCode,
    ...
    private static MapSqlParameterSource buildParameterSourceForPriceUpdate(
    ...
    private Predicate<TicketCategory> findReverseChargeCategory(
            boolean reverseChargeInPerson, boolean reverseChargeOnline, Event.EventFormat eventFormat) {
        return tc -> {
            if (eventFormat == Event.EventFormat.HYBRID) {
                return tc.getTicketAccessType() == TicketCategory.TicketAccessType.IN_PERSON
                        ? reverseChargeInPerson
                        : reverseChargeOnline;
            } else {
    ...
    private void updateBillingData(
        ...
        List<AdditionalServiceItem> additionalServiceItems = purchaseContext.ofType(PurchaseContextType.event)
                ? additionalServiceItemRepository.findByReservationUuid(
                        purchaseContext.event().orElseThrow().getId(), reservation.getId())
                : List.of();
        var tickets = ticketReservationManager.findTicketsInReservation(reservation.getId());
        var additionalServices = purchaseContext
                .event()
                .map(event -> additionalServiceRepository.loadAllForEvent(event.getId()))
                .orElse(List.of());
        ...
        var calculator = new ReservationPriceCalculator(
                reservation.withVatStatus(vatStatus),
                discount,
                tickets,
    ...
    private boolean isEUCountry(String countryCode) {
        return configurationManager
                .getForSystem(EU_COUNTRIES_LIST)
                .getRequiredValue()
    ...
    private static class TicketCategoryPriceContainer implements PriceContainer {
        ...
        private TicketCategoryPriceContainer(
                int categoryId,
                int srcPriceCts,
                String currencyCode,
