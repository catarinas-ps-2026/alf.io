...
public class PublicUserManager {
    ...
    public boolean deleteUserProfile(OAuth2AuthenticationToken authentication) {
        return userManager
                .findOptionalEnabledUserByUsername(authentication.getName())
    ...
    public Optional<PublicUserProfile> updateProfile(
            User original, UpdateProfileForm update, boolean italianEInvoicingEnabled) {
        // update user
        ...
        var currentAdditionalData = userRepository
                .loadUserProfile(userId)
                .map(PublicUserProfile::getAdditionalData)
                .orElse(Map.of());
        Validate.isTrue(1
                == userRepository.persistUserProfile(
                        userId,
    ...
    private Map<String, AdditionalInfoWithLabel> updateExistingAdditionalData(
            Map<String, AdditionalInfoWithLabel> existingData, UpdateProfileForm form) {
        ...
        return existingData.entrySet().stream()
                .map(entry -> {
                    ...
                    var newVal = additionalInfoMap.containsKey(entry.getKey())
                            ? List.of(additionalInfoMap.get(entry.getKey()))
                            : existingAdditionalInfoWithLabel.getValues();
                    return Map.entry(
                            entry.getKey(),
                            new AdditionalInfoWithLabel(existingAdditionalInfoWithLabel.getLabel(), newVal));
                })
                .collect(toMap(Map.Entry::getKey, Map.Entry::getValue));
    }

    private Map<String, AdditionalInfoWithLabel> buildAdditionalInfoWithLabels(
            PublicUserProfile existingProfile, PurchaseContext purchaseContext, UpdateTicketOwnerForm form) {
        ...
        var filteredItems =
                extensionManager.filterAdditionalInfoToSave(purchaseContext, form.getAdditional(), existingProfile);
        final Map<String, AdditionalInfoItem> filteredItemsByKey;
        filteredItemsByKey = filteredItems
                .map(additionalInfoItems ->
                        additionalInfoItems.stream().collect(toMap(AdditionalInfoItem::getKey, Function.identity())))
                .orElse(null);
        var labels = purchaseContextFieldRepository.findDescriptionsForLocale(event.getId(), userLanguage).stream()
                ...
                .map(e -> {
                    if (filteredItemsByKey != null) {
                        return Map.entry(
                                e.getKey(), filteredItemsByKey.get(e.getKey()).getLabel());
                    }
                    return Map.entry(e.getKey(), e.getValue().getLabelDescription());
                })
                .collect(toMap(Map.Entry::getKey, Map.Entry::getValue));
        Map<String, AdditionalInfoWithLabel> result = new HashMap<>(Optional.ofNullable(existingProfile)
                .map(PublicUserProfile::getAdditionalData)
                .orElse(Map.of()));
        ...
        result.putAll(form.getAdditional().entrySet().stream()
                ...
                .map(e -> {
                    ...
                    return Map.entry(
                            e.getKey(), new AdditionalInfoWithLabel(Map.of(userLanguage, label), e.getValue()));
                })
                .collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue)));
        return result;
    }

    public Map<String, AdditionalInfoWithLabel> buildAdditionalInfoWithLabels(
            Authentication principal, PurchaseContext purchaseContext, UpdateTicketOwnerForm form) {
    ...
    private TicketReservationInvoicingAdditionalInfo getAdditionalInfo(
            ContactAndTicketsForm update, boolean italianEInvoicingEnabled) {
    ...
    public void persistProfileForPublicUser(
            Principal principal,
            Object form,
        ...
        if (!bindingResult.hasErrors()) {
            userRepository
                    ...
                    .ifPresent(id -> userRepository.persistUserProfile(
                            id,
