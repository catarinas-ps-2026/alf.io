...
public class GoogleWalletManager {
    ...
    public Optional<Pair<EventAndOrganizationId, Ticket>> validateTicket(String eventName, String ticketUuid) {
        ...
        return ticketRepository
                .findOptionalByPublicUUID(UUID.fromString(ticketUuid))
        ...
        try {
            ...
            if (!passConf.isEmpty()) {
                invalidateAccess
                        ...
                        .ifPresent(
                                s -> invalidateObject(invalidateAccess.ticket().getUuid(), s, passConf));
            }
        } catch (Exception e) {
            log.warn(
                    "Error while invalidating access for ticket "
                            + invalidateAccess.ticket().getUuid(),
                    e);
        }
    }
    ...
    private Map<ConfigurationKeys, String> getConfigurationKeys(EventAndOrganizationId event) {
        ...
        var configValues = Map.of(
                ...
                WALLET_OVERWRITE_PREVIOUS_CLASSES_AND_EVENTS,
                        conf.get(WALLET_OVERWRITE_PREVIOUS_CLASSES_AND_EVENTS).getValue(),
    ...
    private String buildWalletPassUrl(Ticket ticket, Event event, Map<ConfigurationKeys, String> config) {
        ...
        boolean overwritePreviousClassesAndEvents =
                Boolean.parseBoolean(config.get(WALLET_OVERWRITE_PREVIOUS_CLASSES_AND_EVENTS));

        String eventDescription = MustacheCustomTag.renderToTextCommonmark(eventDescriptionRepository
                .findDescriptionByEventIdTypeAndLocale(
                        event.getId(), EventDescription.EventDescriptionType.DESCRIPTION, ticket.getUserLanguage())
                .orElse(""));
        TicketCategory category = ticketCategoryRepository.getById(ticket.getCategoryId());
        var ticketValidityStart = Optional.ofNullable(category.getTicketValidityStart(event.getZoneId()))
                .orElse(event.getBegin());
        var ticketValidityEnd = Optional.ofNullable(category.getTicketValidityEnd(event.getZoneId()))
                .orElse(event.getEnd());

        EventTicketClass.LatitudeLongitudePoint latitudeLongitudePoint = null;
        if (event.getLatitude() != null && event.getLongitude() != null) {
            latitudeLongitudePoint = EventTicketClass.LatitudeLongitudePoint.of(
                    Double.parseDouble(event.getLatitude()), Double.parseDouble(event.getLongitude()));
        }
    ...
    private void persistPassId(Ticket ticket, String eventObjectId) {
        var metadataContainer = Objects.requireNonNullElseGet(
        ...
        var attributesMap = existingMetadata
                .map(ticketMetadata -> new HashMap<>(ticketMetadata.getAttributes()))
                .orElseGet(HashMap::new);
        attributesMap.put(WALLET_OBJECT_ID, eventObjectId);
        metadataContainer.putMetadata(
                TicketMetadataContainer.GENERAL,
                existingMetadata
                        .map(tm -> tm.withAttributes(attributesMap))
                        .orElseGet(() -> new TicketMetadata(null, null, attributesMap)));
    ...
    private String formatEventTicketObjectId(Ticket ticket, Event event, String issuerId, String host) {
        return String.format(
                ...
                // if the attendee gives their ticket to somebody else, the new ticket holder must be able to add their
                // ticket to the wallet
                // therefore we add count(audit(UPDATE_TICKET)) as suffix for the ticket UUID
                ticket.getUuid() + "_"
                        + auditingRepository.countAuditsOfTypeForTicket(
                                ticket.getTicketsReservationId(),
                                ticket.getId(),
                                Audit.EventType.TICKET_HOLDER_CHANGED));
    }

    private String formatEventTicketClassId(Event event, String issuerId, TicketCategory category, String host) {
        return String.format(
                "%s.%s-%s-class.%s-%d",
                issuerId, walletIdPrefix(), host, event.getShortName().replaceAll("[^\\w.-]", "_"), category.getId());
    }
    ...
    private String generateWalletPassUrl(GoogleCredentials credentials, String eventObjectId, String url) {
        ...
        Algorithm algorithm =
                Algorithm.RSA256(null, (RSAPrivateKey) ((ServiceAccountCredentials) credentials).getPrivateKey());
    ...
    private String createEventClass(
            ...
            boolean overwritePreviousClassesAndEvents) {
        return createOnWallet(
                EventTicketClass.WALLET_URL, credentials, eventTicketClass, overwritePreviousClassesAndEvents);
    }

    private String createEventObject(
            GoogleCredentials credentials,
            EventTicketObject eventTicketObject,
            boolean overwritePreviousClassesAndEvents) {
        return createOnWallet(
                EventTicketObject.WALLET_URL, credentials, eventTicketObject, overwritePreviousClassesAndEvents);
    }

    private String createOnWallet(
            String uri, GoogleCredentials credentials, WalletEntity entity, boolean overwritePreviousClassesAndEvents) {
        try {
            ...
            if (getResponse.statusCode() != 200 && getResponse.statusCode() != 404) {
                logIfWarnEnabled(() -> log.warn(
                        "Received {} status code when creating entity but 200 or 404 were expected: {}",
                        getResponse.statusCode(),
                        getResponse.body()));
                throw new GoogleWalletException(
                        "Cannot create Wallet class. Response status: " + getResponse.statusCode());
            }
            ...
            if (getResponse.statusCode() == 404 || overwritePreviousClassesAndEvents) {
                ...
                if (getResponse.statusCode() == 404) {
                    ...
                    builder =
                            builder.uri(uriWithId).PUT(HttpRequest.BodyPublishers.ofString(entity.build(objectMapper)));
                }
                HttpRequest postOrPutRequest = builder.build();
                log.debug("POST or PUT Request: {}", postOrPutRequest);
                HttpResponse<String> postOrPutResponse =
                        httpClient.send(postOrPutRequest, HttpResponse.BodyHandlers.ofString());
                log.debug("POST or PUT Response: {}", postOrPutResponse);
                if (postOrPutResponse.statusCode() != 200) {
                    logIfWarnEnabled(() -> log.warn(
                            "Received {} status code when creating entity: {}",
                            postOrPutResponse.statusCode(),
                            postOrPutResponse.body()));
                    throw new GoogleWalletException(
                            "Cannot create wallet. Response status: " + postOrPutResponse.statusCode());
                }
            }
            return entity.getId();
        } catch (IOException e) {
        ...
        var profile = Stream.of(PROFILE_DEMO, PROFILE_DEV, PROFILE_LIVE)
                ...
                .orElseThrow(
                        () -> new NoSuchElementException(
                                "No suitable Spring Profile found to create a Wallet ID prefix for classes and objects. Must have one of PROFILE_DEMO, PROFILE_DEV, or PROFILE_LIVE"));
