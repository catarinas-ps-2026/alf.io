...
public class SpecialPriceManager {
    ...
    private static final Predicate<SendCodeModification> IS_CODE_PRESENT =
            v -> Optional.ofNullable(v.code()).isPresent();
    ...
    private List<String> checkCodeAssignment(
            Set<SendCodeModification> input, int categoryId, EventAndOrganizationId event, String username) {
        final TicketCategory category = checkOwnership(categoryId, event, username);
        List<String> availableCodes =
                specialPriceRepository.findActiveByCategoryIdForUpdate(category.getId(), input.size()).stream()
                        .map(SpecialPrice::getCode)
                        .collect(toList());
        Validate.isTrue(
                input.size() <= availableCodes.size(),
                "Requested codes: " + input.size() + ", available: " + availableCodes.size() + ".");
        List<String> requestedCodes = input.stream()
                .filter(IS_CODE_PRESENT)
                .map(SendCodeModification::code)
                .toList();
        Validate.isTrue(
                requestedCodes.stream().distinct().count() == requestedCodes.size(),
                "Cannot assign the same code twice. Please fix the input file.");
    ...
    private TicketCategory checkOwnership(int categoryId, EventAndOrganizationId event, String username) {
        ...
        final TicketCategory category = categories.stream()
    ...
    public List<SendCodeModification> linkAssigneeToCode(
            List<SendCodeModification> input, String eventName, int categoryId, String username) {
        ...
        return Stream.concat(
                        set.stream().filter(IS_CODE_PRESENT),
                        input.stream()
                                .filter(IS_CODE_PRESENT.negate())
                                .map(p ->
                                        new SendCodeModification(codes.next(), p.assignee(), p.email(), p.language())))
                .collect(toList());
    }
    ...
    public List<SpecialPrice> loadSentCodes(String eventName, int categoryId, String username) {
        ...
        return specialPriceRepository.findAllByCategoryId(categoryId).stream()
                .filter(p.negate())
                .collect(toList());
    }
    ...
    public boolean sendCodeToAssignee(
            List<SendCodeModification> input, String eventName, int categoryId, String username) {
        ...
        Validate.isTrue(
                !eventLanguages.isEmpty(),
                "No locales have been defined for the event. Please check the configuration");
        ContentLanguage defaultLocale =
                eventLanguages.contains(ContentLanguage.ENGLISH) ? ContentLanguage.ENGLISH : eventLanguages.get(0);
        set.forEach(m -> {
            var messageSource = messageSourceManager.getMessageSourceFor(event);
            Locale locale = LocaleUtil.forLanguageTag(
                    Objects.toString(StringUtils.trimToNull(m.language()), defaultLocale.getLanguage()));
            var usePartnerCode = configurationManager
                    .getFor(USE_PARTNER_CODE_INSTEAD_OF_PROMOTIONAL, event.getConfigurationLevel())
                    .getValueAsBooleanOrDefault();
            var promoCodeDescription = messageSource.getMessage(
                    "show-event.promo-code-type." + (usePartnerCode ? "partner" : "promotional"), null, null, locale);
            Map<String, Object> model = TemplateResource.prepareModelForSendReservedCode(
                    organization, event, m, eventManager.getEventUrl(event), promoCodeDescription);
            notificationManager.sendSimpleEmail(
                    event,
                    null,
                    m.email(),
                    messageSource.getMessage(
                            "email-code.subject", new Object[] {event.getDisplayName(), promoCodeDescription}, locale),
                    () -> templateManager.renderTemplate(event, TemplateResource.SEND_RESERVED_CODE, model, locale));
            int marked = specialPriceRepository.markAsSent(
