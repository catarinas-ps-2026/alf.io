...
public class EventManager {
    ...
    static final String ERROR_ONLINE_ON_SITE_NOT_COMPATIBLE =
            "Cannot switch to Online. Please remove On-Site payment method first.";
    ...
    public static Predicate<EventAndOrganizationId> checkOwnership(
            String username, OrganizationRepository organizationRepository) {
    ...
    private static boolean checkOwnership(
            OrganizationRepository organizationRepository, String username, Integer orgId) {
        return orgId != null
                && organizationRepository
                        .findOrganizationForUser(username, orgId)
                        .isPresent();
    }
    ...
    public void createEvent(EventModification em, String username) {
        ...
        var organization = organizationRepository.findAllForUser(username).stream()
    ...
    private void copySettings(Event event, EventAndOrganizationId srcEvent) {
        int count = configurationRepository.copyEventConfiguration(
                event.getId(), event.getOrganizationId(), srcEvent.getId(), srcEvent.getOrganizationId());
        log.info("copied {} settings from source event", count);
    }

    private void createSubscriptionLinks(
            int eventId, int organizationId, List<LinkSubscriptionsToEventRequest> linkedSubscriptions) {
        ...
        Optional.ofNullable(em.getDescription())
                .ifPresent(descriptions -> descriptions.forEach((locale, description) -> {
                    if (validLocales.contains(locale)) {
                        eventDescriptionRepository.insert(
                                eventId, locale, EventDescription.EventDescriptionType.DESCRIPTION, description);
                    }
                }));
    }
    ...
    private void createAdditionalFields(Event event, EventModification em) {
        if (!CollectionUtils.isEmpty(em.getTicketFields())) {
            em.getTicketFields()
                    .forEach(f -> purchaseContextFieldManager.insertAdditionalField(event, f, f.getOrder()));
        }
    }
    ...
    public void updateEventHeader(Event original, EventModification em, String username) {
        ...
        Validate.isTrue(
                ownershipChecker.test(original.getOrganizationId())
                        && (sameOrganization || ownershipChecker.test(em.getOrganizationId())),
                "Invalid organizationId");
        int eventId = original.getId();
        Validate.isTrue(
                sameOrganization || groupRepository.countByEventId(eventId) == 0,
                "Cannot change organization because there is a group linked to this event.");
        Validate.isTrue(
                sameOrganization || !subscriptionRepository.hasLinkedSubscription(eventId),
                "Cannot change organization because there are one or more subscriptions linked.");

        boolean formatUpdated = em.getFormat() != original.getFormat();
        if (em.getFormat() == EventFormat.ONLINE && formatUpdated) {
            Validate.isTrue(
                    original.getAllowedPaymentProxies().stream().allMatch(p -> p != PaymentProxy.ON_SITE),
                    ERROR_ONLINE_ON_SITE_NOT_COMPATIBLE);
        }

        String timeZone = ObjectUtils.firstNonNull(
                em.getZoneId(),
                em.getGeolocation() != null ? em.getGeolocation().timeZone() : null);
        String latitude = ObjectUtils.firstNonNull(
                em.getLatitude(),
                em.getGeolocation() != null ? em.getGeolocation().latitude() : null);
        String longitude = ObjectUtils.firstNonNull(
                em.getLongitude(),
                em.getGeolocation() != null ? em.getGeolocation().longitude() : null);
        ...
        eventRepository.updateHeader(
                ...
                em.getWebsiteUrl(),
                em.getExternalUrl(),
                em.getTermsAndConditionsUrl(),
                em.getPrivacyPolicyUrl(),
                em.getImageUrl(),
                em.getFileBlobId(),
                em.getLocation(),
                latitude,
        ...
        extensionManager.handleEventHeaderUpdate(
                eventRepository.findById(eventId),
                organizationRepository
                        .findOrganizationForUser(username, em.getOrganizationId())
                        .orElseThrow());
    }
    ...
    public void updateEventSeatsAndPrices(
            Event original, EventModification em, String username, boolean updateSubscriptions) {
        ...
        eventRepository.updatePrices(
                em.getCurrency(),
                em.getAvailableSeats(),
                em.isVatIncluded(),
                vat,
                paymentProxies,
                eventId,
        ...
        if (seatsDifference != 0) {
            ...
            if (seatsDifference > 0) {
                final MapSqlParameterSource[] params = generateEmptyTickets(
                                modified,
                                Date.from(ZonedDateTime.now(clockProvider.withZone(modified.getZoneId()))
                                        .toInstant()),
                                seatsDifference,
                ...
                List<Integer> ids = ticketRepository.selectNotAllocatedTicketsForUpdate(
                        eventId, Math.abs(seatsDifference), singletonList(TicketStatus.FREE.name()));
                ...
                Validate.isTrue(
                        ids.size() == invalidatedTickets,
                        "error during ticket invalidation: expected %d, got %d"
                                .formatted(ids.size(), invalidatedTickets));
            }
        }
    ...
    private static List<LinkSubscriptionsToEventRequest> toSubscriptionLinkRequests(List<UUID> subscriptionIds) {
        return requireNonNullElse(subscriptionIds, List.<UUID>of()).stream()
                .map(s -> new LinkSubscriptionsToEventRequest(s, List.of()))
                .collect(Collectors.toList());
    }

    public void updateLinkedSubscriptions(
            List<LinkSubscriptionsToEventRequest> linkedSubscriptions, int eventId, int organizationId) {
        if (CollectionUtils.isNotEmpty(linkedSubscriptions)) {
            var descriptorIds = linkedSubscriptions.stream()
                    .map(LinkSubscriptionsToEventRequest::getDescriptorId)
                    .collect(Collectors.toList());
        ...
        return optionally(() -> {
                ...
                .map(b -> {
                    ...
                    return new Result.Builder<Integer>()
                            ...
                            .checkPrecondition(
                                    () -> requestedTickets <= notAllocated,
                                    ErrorCode.CategoryError.ALL_TICKETS_ASSIGNED)
                            .checkPrecondition(
                                    () -> tcm.getExpiration()
                                            .toZonedDateTime(event.getZoneId())
    ...
    Result<TicketCategory> updateCategory(
            int categoryId, Event event, TicketCategoryModification tcm, String username, boolean resetTicketsToFree) {
        checkOwnership(event, username, event.getOrganizationId());
        int eventId = event.getId();
        return Optional.of(ticketCategoryRepository.getById(categoryId))
                .filter(tc -> tc.getId() == categoryId)
                .map(existing -> new Result.Builder<TicketCategory>()
                        ...
                        .checkPrecondition(
                                () -> !existing.isBounded()
                                        || tcm.getMaxTickets()
                                                        - existing.getMaxTickets()
                                                        + ticketRepository.countAllocatedTicketsForEvent(eventId)
                                                <= eventRepository.countExistingTickets(eventId),
                                ErrorCode.CategoryError.NOT_ENOUGH_SEATS)
                        .checkPrecondition(
                                () -> tcm.isTokenGenerationRequested() == existing.isAccessRestricted()
                                        || ticketRepository.countConfirmedAndPendingTickets(eventId, categoryId) == 0,
                                ErrorCode.custom("", "cannot update category: there are tickets already sold."))
                        .checkPrecondition(
                                () -> tcm.isBounded() == existing.isBounded()
                                        || ticketRepository.countPendingOrReleasedForCategory(eventId, existing.getId())
                                                == 0,
                                ErrorCode.custom(
                                        "",
                        ...
                        .checkPrecondition(
                                ...
                                ErrorCode.custom("", "Dynamic allocation is not compatible with restricted access"))
                        .checkPrecondition(
                                () -> {
                                    // see https://github.com/exteso/alf.io/issues/335
                                    // handle the case when the user try to shrink a category with tokens that are
                                    // already sent
                                    ...
                                    return addedTicket >= 0
                                            || !existing.isAccessRestricted()
                                            || specialPriceRepository.countNotSentToken(categoryId)
                                                    >= Math.abs(addedTicket);
                                },
                        ...
                        .checkPrecondition(
                                () -> {
                                    if (tcm.isBounded() && !existing.isBounded()) {
                                        ...
                                        int confirmed =
                                                ticketRepository.countConfirmedForCategory(eventId, existing.getId());
    ...
    Result<TicketCategory> updateCategory(
            int categoryId, Event event, TicketCategoryModification tcm, String username) {
    ...
    private void fixTicketCategoryDates(
            ZonedDateTime end, TicketCategory tc, ZonedDateTime inception, ZonedDateTime expiration) {
        final ZonedDateTime newExpiration = ObjectUtils.min(end, expiration);
        Objects.requireNonNull(newExpiration);
        Validate.isTrue(
                inception.isBefore(newExpiration),
                format(
                        "Cannot fix dates for category \"%s\" (id: %d), try updating that category first.",
                        tc.getName(), tc.getId()));
    ...
    public void reallocateTickets(int srcCategoryId, int targetCategoryId, EventAndOrganizationId event) {
        reallocateTickets(
                ticketCategoryRepository.findStatisticWithId(srcCategoryId, event.getId()),
                Optional.of(ticketCategoryRepository.getByIdAndActive(targetCategoryId, event.getId())),
                event);
    }

    void reallocateTickets(
            TicketCategoryStatisticView src, Optional<TicketCategory> target, EventAndOrganizationId event) {
        ...
        List<Integer> lockedTickets = ticketRepository.selectTicketInCategoryForUpdate(
                event.getId(), src.getId(), notSoldTickets, singletonList(TicketStatus.FREE.name()));
        ...
        if (target.isPresent()) {
            ...
            ticketCategoryRepository.updateSeatsAvailability(
                    targetCategory.getId(), targetCategory.getMaxTickets() + locked);
            ticketRepository.moveToAnotherCategory(
                    lockedTickets, targetCategory.getId(), targetCategory.getSrcPriceCts());
    ...
    public void unbindTickets(String eventName, int categoryId, String username) {
        ...
        Validate.isTrue(
                ticketCategoryRepository.countUnboundedCategoriesByEventId(event.getId()) > 0,
                "cannot unbind tickets: there aren't any unbounded categories");
        TicketCategoryStatisticView ticketCategory =
                ticketCategoryRepository.findStatisticWithId(categoryId, event.getId());
    ...
    MapSqlParameterSource[] prepareTicketsBulkInsertParameters(
            ZonedDateTime creation, Event event, int requestedTickets, TicketStatus ticketStatus) {
    ...
    private Stream<MapSqlParameterSource> generateTicketsForCategory(
            TicketCategory tc, Event event, Date creationDate, int existing) {
        Optional<TicketCategory> filteredTC = Optional.of(tc).filter(TicketCategory::isBounded);
        int missingTickets = filteredTC
                .map(c -> Math.abs(c.getMaxTickets() - existing))
    ...
    private void createCategoriesForEvent(
            EventModification em, Event event, Optional<EventAndOrganizationId> srcEventOptional) {
        ...
        em.getTicketCategories().forEach(tc -> {
            ...
            final AffectedRowCountAndKey<Integer> category = ticketCategoryRepository.insert(
                    tc.getInception().toZonedDateTime(zoneId),
                    tc.getExpiration().toZonedDateTime(zoneId),
                    tc.getName(),
                    maxTickets,
                    tc.isTokenGenerationRequested(),
                    eventId,
                    tc.isBounded(),
                    price,
                    StringUtils.trimToNull(tc.getCode()),
            ...
            if (tc.isTokenGenerationRequested()) {
                final TicketCategory ticketCategory =
                        ticketCategoryRepository.getByIdAndActive(category.getKey(), event.getId());
                specialPriceRepository.bulkInsert(ticketCategory, ticketCategory.getMaxTickets());
            }

            if (srcEventOptional.isPresent()
                    && tc.getMetadata() != null
                    && StringUtils.isNumeric(tc.getMetadata().getCopiedFrom())) {
                int count = configurationRepository.copyCategoryConfiguration(
                        event.getId(),
    ...
    private Integer insertCategory(TicketCategoryModification tc, Event event) {
        ...
        final AffectedRowCountAndKey<Integer> category = ticketCategoryRepository.insert(
                ...
                tc.getExpiration().toZonedDateTime(zoneId),
                tc.getName(),
                tc.isBounded() ? tc.getMaxTickets() : 0,
                tc.isTokenGenerationRequested(),
                eventId,
                tc.isBounded(),
                price,
        ...
        if (tc.isBounded()) {
            List<Integer> lockedTickets = ticketRepository.selectNotAllocatedTicketsForUpdate(
                    eventId,
                    ticketCategory.getMaxTickets(),
                    asList(TicketStatus.FREE.name(), TicketStatus.RELEASED.name()));
    ...
    void saveBadgeColorConfiguration(String badgeColor, Event event, Integer categoryId) {
        if (StringUtils.isNotBlank(badgeColor)) {
            ...
            if (!existingConfiguration) {
                colorConfiguration = new CheckInOutputColorConfiguration(
                        "success", List.of(new ColorConfiguration(chosenColor, List.of(categoryId))));
            } else {
                var configurationWithoutCategory = colorConfiguration.getConfigurations().stream()
                        .map(cc -> new ColorConfiguration(
                                cc.getColorName(),
                                cc.getCategories().stream()
                                        .filter(c -> !c.equals(categoryId))
                                        .collect(toUnmodifiableList())))
                        .filter(cc -> !cc.getCategories().isEmpty())
                        .collect(toList());
                boolean colorExists = configurationWithoutCategory.stream()
                        .anyMatch(cc -> cc.getColorName().equals(chosenColor));
                if (colorExists) {
                    colorConfiguration = new CheckInOutputColorConfiguration(
                            colorConfiguration.getDefaultColorName(),
                            configurationWithoutCategory.stream()
                                    .map(cc -> {
                    ...
                    colorConfiguration =
                            new CheckInOutputColorConfiguration(colorConfiguration.getDefaultColorName(), newList);
                }
            }
            if (existingConfiguration) {
                configurationRepository.updateEventLevel(
                        event.getId(),
                        event.getOrganizationId(),
                        CHECK_IN_COLOR_CONFIGURATION.name(),
                        Json.toJson(colorConfiguration));
            } else {
    ...
    private void insertOrUpdateTicketCategoryDescription(int tcId, TicketCategoryModification tc, Event event) {
        ...
        Optional.ofNullable(tc.getDescription())
                .ifPresent(descriptions -> descriptions.forEach((locale, desc) -> {
    ...
    private void handleTicketAllocationStrategyChange(
            EventAndOrganizationId event, TicketCategory original, TicketCategoryModification updated) {
        if (updated.isBounded()) {
            ...
            List<Integer> ids = ticketRepository.selectNotAllocatedTicketsForUpdate(
                    eventId, addedTickets, singletonList(TicketStatus.FREE.name()));
            Validate.isTrue(
                    addedTickets >= 0,
                    "Cannot reduce capacity to " + newSize + ". Minimum size allowed is " + confirmed);
            ...
            Validate.isTrue(
                    ids.isEmpty()
                            || ticketRepository.moveToAnotherCategory(
                                            ids,
                                            original.getId(),
                                            MonetaryUtil.unitToCents(updated.getPrice(), original.getCurrencyCode()))
                                    == ids.size(),
                    "not enough tickets");
        } else {
            reallocateTickets(
                    ticketCategoryRepository.findStatisticWithId(original.getId(), event.getId()),
                    Optional.empty(),
                    event);
        }
    ...
    void handlePriceChange(EventAndOrganizationId event, TicketCategory original, TicketCategory updated) {
        ...
        final List<Integer> ids = ticketRepository.selectTicketInCategoryForUpdate(
                event.getId(), updated.getId(), updated.getMaxTickets(), singletonList(TicketStatus.FREE.name()));
        ...
        // there's no need to calculate final price, vat etc, since these values will be updated at the time of
        // reservation
        ticketRepository.updateTicketPrice(
                updated.getId(), event.getId(), updated.getSrcPriceCts(), 0, 0, 0, original.getCurrencyCode());
    }
    ...
    void handleTicketNumberModification(Event event, TicketCategory updated, int addedTickets, boolean resetToFree) {
        ...
        if (addedTickets > 0) {
            ...
            List<Integer> lockedTickets = ticketRepository.selectNotAllocatedTicketsForUpdate(
                    event.getId(), addedTickets, asList(TicketStatus.FREE.name(), TicketStatus.RELEASED.name()));
            Validate.isTrue(
                    addedTickets == lockedTickets.size(),
                    "Cannot add %d tickets. There are only %d free tickets.",
                    addedTickets,
                    lockedTickets.size());
            ...
            final List<Integer> ids =
                    ticketRepository.lockTicketsToInvalidate(event.getId(), updated.getId(), absDifference);
            ...
            final MapSqlParameterSource[] params = generateEmptyTickets(
                            event,
                            Date.from(event.now(clockProvider).toInstant()),
                            absDifference,
                            resetToFree ? TicketStatus.FREE : TicketStatus.RELEASED)
                    .toArray(MapSqlParameterSource[]::new);
            ticketRepository.bulkTicketInitialization(params);
        }
    }
    ...
    private void createAllTicketsForEvent(Event event, EventModification em) {
        ...
        final MapSqlParameterSource[] params = prepareTicketsBulkInsertParameters(
                event.now(clockProvider), event, em.getAvailableSeats(), TicketStatus.FREE);
        ticketRepository.bulkTicketInitialization(params);
    }
    ...
    private int insertEvent(EventModification em) {
        ...
        return eventRepository
                .insert(
                        ...
                        em.getLatitude(),
                        em.getLongitude(),
                        em.getBegin().toZonedDateTime(zoneId),
                        em.getEnd().toZonedDateTime(zoneId),
                        em.getZoneId(),
                        em.getCurrency(),
                        em.getAvailableSeats(),
                        em.isVatIncluded(),
    ...
    private String collectPaymentProxies(EventModification em) {
        return em.getAllowedPaymentProxies().stream()
    ...
    public boolean toggleTicketLocking(String eventName, int categoryId, int ticketId, String username) {
        ...
        var existingCategory = ticketCategoryRepository.findAllTicketCategories(event.getId()).stream()
                .filter(tc -> tc.getId() == categoryId)
                .findFirst();
    ...
    public void addPromoCode(
        ...
        Validate.isTrue(
                (eventId != null && organizationId == null) || (eventId == null && organizationId != null),
        ...
        Validate.isTrue(
                !PromoCodeDiscount.supportsCurrencyCode(codeType, discountType)
                        || StringUtils.length(currencyCode) == 3,
        ...
        promoCodeRepository.addPromoCode(
                promoCode,
                eventId,
                organizationId,
                start,
    ...
    public void updatePromoCode(
        ...
        promoCodeRepository.updateEventPromoCode(
                promoCodeId, start, end, maxUsage, categoriesJson, description, emailReference, hiddenCategoryId);
    }
    ...
    public List<PromoCodeDiscountWithFormattedTimeAndAmount> findPromoCodesInEvent(int eventId) {
        ...
        return promoCodeRepository.findAllInEvent(eventId).stream()
                .map(p -> new PromoCodeDiscountWithFormattedTimeAndAmount(p, event.getZoneId(), event.getCurrency()))
                .collect(toList());
    }
    ...
    public List<PromoCodeDiscountWithFormattedTimeAndAmount> findPromoCodesInOrganization(int organizationId) {
        ...
        return promoCodeRepository.findAllInOrganization(organizationId).stream()
                .map(p -> new PromoCodeDiscountWithFormattedTimeAndAmount(p, zoneId, null))
                .collect(toList());
    }

    public String getEventUrl(Event event) {
        var baseUrl = configurationManager
                .getFor(ConfigurationKeys.BASE_URL, event.getConfigurationLevel())
                .getRequiredValue();
    ...
    public List<Event> getPublishedEvents(SearchOptions searchOptions) {
        return eventRepository.findVisibleBySearchOptions(
                searchOptions.getSubscriptionCodeUUIDOrNull(),
    ...
    private Stream<Event> getActiveEventsStream(int days) {
        return eventRepository.findAll().stream().filter(e -> e.getEnd()
                .truncatedTo(ChronoUnit.DAYS)
                .plusDays(days)
                .isAfter(
                        ZonedDateTime.now(clockProvider.withZone(e.getZoneId())).truncatedTo(ChronoUnit.DAYS)));
    }
    ...
    public void deleteCategory(String eventName, int categoryId, String username) {
        ...
        if (category.isBounded()) {
            ...
            var ticketIds = ticketRepository.selectTicketInCategoryForUpdate(
                    eventId, categoryId, ticketsCount, List.of(TicketStatus.FREE.name(), TicketStatus.RELEASED.name()));
            Validate.isTrue(
                    ticketIds.size() == ticketsCount,
                    "Error while deleting category. Please ensure that there is no pending reservation.");
            ticketRepository.resetTickets(ticketIds);
            Validate.isTrue(
                    ticketsCount == ticketRepository.unbindTicketsFromCategory(eventId, categoryId, ticketIds),
                    "Cannot remove tickets from category.");
        }
    ...
    public Map<Integer, String> getEventNamesByIds(List<Integer> eventIds, Principal principal) {
        ...
        return eventRepository.getEventNamesByIds(eventIds).stream()
                .collect(Collectors.toMap(EventIdShortName::getId, EventIdShortName::getShortName));
    }
    ...
    public Map<Integer, String> getEventsNameInOrganization(int orgId, Principal principal) {
        ...
        return eventRepository.getEventsNameInOrganization(orgId).stream()
                .collect(Collectors.toMap(EventIdShortName::getId, EventIdShortName::getShortName));
    }
    ...
    public boolean updateMetadata(Event event, AlfioMetadata metadata) {
        ...
        var updatedMetadata = extensionManager.handleMetadataUpdate(
                event, organizationRepository.getById(event.getOrganizationId()), metadata);
        eventRepository.updateMetadata(
                existing.merge(Objects.requireNonNullElse(updatedMetadata, metadata)), event.getId());
        return true;
    }
    ...
    public Optional<String> executeCapability(
            String eventName, String username, ExtensionCapability capability, Map<String, String> requestParams) {
        return getOptionalByName(eventName, username).flatMap(event -> {
            if (capability == ExtensionCapability.GENERATE_MEETING_LINK) {
                ...
                return extensionManager
                        .handleGenerateMeetingLinkCapability(
                                event, organization, getMetadataForEvent(event), requestParams)
                        .map(metadata -> {
                            eventRepository.updateMetadata(
                                    requireNonNullElseGet(metadata, AlfioMetadata::empty), event.getId());
