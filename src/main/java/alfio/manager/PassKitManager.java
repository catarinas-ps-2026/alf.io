...
public class PassKitManager {
    ...
    public boolean writePass(Ticket ticket, EventAndOrganizationId event, OutputStream out)
            throws IOException, PassSigningException {
    ...
    private void buildPass(
        ...
        // from example:
        // https://github.com/ryantenney/passkit4j/blob/master/src/test/java/com/ryantenney/passkit4j/EventTicketExample.java
        // specification:
        ...
        Pass pass = new Pass()
                ...
                .expirationDate(Date.from(Optional.ofNullable(category.getTicketValidityEnd(event.getZoneId()))
                        .orElse(event.getEnd())
                        .toInstant()))
                .barcode(new Barcode(
                        BarcodeFormat.QR,
                ...
                .passInformation(new EventTicket()
                        .headerFields(List.of(new TextField(
                                "eventStartDate",
                                "Date",
                                ticketValidityStart.format(DateTimeFormatter.ofLocalizedDateTime(FormatStyle.MEDIUM)
                                        .withLocale(ticketLocale)))))
                        .primaryFields(List.of(new TextField("categoryId", event.getDisplayName(), category.getName())))
                        .secondaryFields(new TextField("location", "Venue", event.getLocation()))
                        .auxiliaryFields(getAuxiliaryFields(ticket))
                        .backFields(
        ...
        passResources.add(
                new PassResource("icon.png", () -> new ClassPathResource("/alfio/icon/icon.png").getInputStream()));
        passResources.add(new PassResource(
                "icon@2x.png", () -> new ClassPathResource("/alfio/icon/icon@2x.png").getInputStream()));
        passResources.add(new PassResource(
                "icon@3x.png", () -> new ClassPathResource("/alfio/icon/icon@3x.png").getInputStream()));

        fileUploadManager.findMetadata(event.getFileBlobId()).ifPresent(metadata -> {
            if (metadata.getContentType().equals("image/png")
                ...
                try {
                    var logo1x = fileBlobCacheManager.getFile(
                            PASSKIT_SECTION, id + "scale-1", () -> scaleLogo(fullSizeLogo, 1));
                    var logo2x = fileBlobCacheManager.getFile(
                            PASSKIT_SECTION, id + "scale-2", () -> scaleLogo(fullSizeLogo, 2));
                    var logo3x = fileBlobCacheManager.getFile(
                            PASSKIT_SECTION, id + "scale-3", () -> scaleLogo(fullSizeLogo, 3));
                    ...
                    log.warn(
                            "was not able to add the logo to the passkit for ticket {} in event {}",
                            ticket.getUuid(),
                            event.getShortName(),
                            e);
                }
    ...
    public Optional<Pair<EventAndOrganizationId, Ticket>> retrieveTicketDetails(String eventName, String ticketUuid) {
        return eventRepository
                ...
                .flatMap(e -> ticketRepository
                        .findOptionalByPublicUUID(UUID.fromString(ticketUuid))
                        .filter(t -> e.getId() == t.getEventId())
                        .map(t -> Pair.of(e, t)));
    }

    public Optional<Pair<EventAndOrganizationId, Ticket>> validateToken(
            String eventName, String typeIdentifier, String ticketUuid, String authorizationHeader) {
        String token;
        ...
        var typeIdentifierOptional =
                configurationManager.getFor(PASSBOOK_TYPE_IDENTIFIER, event.getConfigurationLevel());
        if (!typeIdentifierOptional.isPresent() || !typeIdentifier.equals(typeIdentifierOptional.getValueOrNull())) {
            if (log.isTraceEnabled()) {
                log.trace(
                        "typeIdentifier does not match. Expected {}, got {}",
                        typeIdentifierOptional.getValueOrDefault("not-found"),
                        removeTabsAndNewlines(typeIdentifier));
            }
            return Optional.empty();
        }
        return ticketRepository
                ...
                .filter(t -> buildAuthenticationToken(t, event, eventRepository.getPrivateKey(event.getId()))
                        .equals(token))
    ...
    private static File scaleLogo(File sourceImage, int factor) {
        try {
            ...
            var thumbImg = Scalr.resize(
                    ImageIO.read(sourceImage),
            ...
            throw new IllegalStateException(
                    "was not able to scale logo from file " + sourceImage + " with factor " + factor, e);
        }
    }
}
