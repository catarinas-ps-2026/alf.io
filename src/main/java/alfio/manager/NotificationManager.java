...
public class NotificationManager {
    ...
    public NotificationManager(
        ...
        attachmentTransformer.put(
                Mailer.AttachmentIdentifier.RECEIPT_PDF,
                receiptOrInvoiceFactory(
                        purchaseContextManager,
                        eventRepository,
                        payload -> TemplateProcessor.buildReceiptPdf(
                                payload.getLeft(),
                                fileUploadManager,
                                payload.getMiddle(),
                                templateManager,
                                ...
                                extensionManager)));
        attachmentTransformer.put(
                Mailer.AttachmentIdentifier.INVOICE_PDF,
                receiptOrInvoiceFactory(
                        purchaseContextManager,
                        eventRepository,
                        payload -> TemplateProcessor.buildInvoicePdf(
                                ...
                                extensionManager)));
        attachmentTransformer.put(
                Mailer.AttachmentIdentifier.CREDIT_NOTE_PDF,
                receiptOrInvoiceFactory(
                        purchaseContextManager,
                        eventRepository,
                        payload -> TemplateProcessor.buildCreditNotePdf(
                                payload.getLeft(),
                                fileUploadManager,
                                payload.getMiddle(),
                                templateManager,
        ...
        var retrieveFieldValues = EventUtil.retrieveFieldValues(
                ticketRepository, purchaseContextFieldManager, additionalServiceItemRepository, true);
        attachmentTransformer.put(
                Mailer.AttachmentIdentifier.TICKET_PDF,
                generateTicketPDF(
                        eventRepository,
                        organizationRepository,
                        configurationManager,
                        ...
                        additionalServiceHelper));
        attachmentTransformer.put(
                Mailer.AttachmentIdentifier.SUBSCRIPTION_PDF,
        ...
        return model -> {
            ...
            try {
                TicketReservation reservation =
                        ticketReservationRepository.findReservationById(ticket.getTicketsReservationId());
                TicketCategory ticketCategory = Json.fromJson(model.get("ticketCategory"), TicketCategory.class);
                Event event = eventRepository.findById(ticket.getEventId());
                Organization organization =
                        organizationRepository.getById(Integer.valueOf(model.get("organizationId"), 10));
                var ticketWithMetadata =
                        TicketWithMetadataAttributes.build(ticket, ticketRepository.getTicketMetadata(ticket.getId()));
                ...
                TemplateProcessor.renderPDFTicket(
                        ...
                        TemplateProcessor.getSubscriptionDetailsModelForTicket(
                                ticket, subscriptionRepository::findDescriptorBySubscriptionId, locale),
    ...
    private static Function<Map<String, String>, byte[]> generateICS(
            EventRepository eventRepository,
            EventDescriptionRepository eventDescriptionRepository,
            TicketCategoryRepository ticketCategoryRepository,
            OrganizationRepository organizationRepository,
        ...
        return model -> {
            ...
            String description = eventDescriptionRepository
                    .findDescriptionByEventIdTypeAndLocale(
                            event.getId(), EventDescription.EventDescriptionType.DESCRIPTION, locale.getLanguage())
                    .orElse("");
            if (model.containsKey("onlineCheckInUrl")
                    && configurationManager
                            .getFor(INCLUDE_CHECK_IN_URL_ICAL, event.getConfigurationLevel())
                            .getValueAsBooleanOrDefault()) { // special case: online event
    ...
    private static BiFunction<Map<String, String>, Locale, String> buildOnlineCheckInInformation(
    ...
    private static Function<Map<String, String>, byte[]> receiptOrInvoiceFactory(
            PurchaseContextManager purchaseContextManager,
            EventRepository eventRepository,
        ...
        return model -> {
            ...
            Map<String, Object> reservationEmailModel =
                    Json.fromJson(model.get("reservationEmailModel"), new TypeReference<>() {});
            if (reservationEmailModel.get("purchaseContext") != null) {
                ...
                var purchaseContextType =
                        model.get(EVENT_ID) != null ? PurchaseContextType.event : PurchaseContextType.subscription;
                purchaseContext = purchaseContextManager
    ...
    public void sendTicketByEmail(
        ...
        boolean htmlEmailEnabled = configurationManager
                .getFor(ConfigurationKeys.ENABLE_HTML_EMAILS, event.getConfigurationLevel())
        ...
        if (EventUtil.isAccessOnline(ticketCategory, event)) { // generate only calendar invitation
            ...
            attachments.add(CustomMessageManager.generateTicketAttachment(
                    ticket, reservation, ticketCategory, organization, htmlEmailEnabled));
        }

        String displayName = event.getDisplayName();
        String subject = messageSourceManager
                .getMessageSourceFor(event)
                .getMessage("ticket-email-subject", new Object[] {displayName}, locale);
        ...
        tx.execute(status -> {
            emailMessageRepository
                    .findIdByEventIdAndChecksum(event.getId(), checksum)
                    .ifPresentOrElse(
                            // see issue #967
                            id -> emailMessageRepository.updateStatusToWaitingWithHtml(
                                    id, renderedTemplate.getHtmlPart()),
                            () -> emailMessageRepository.insert(
                                    event.getId(),
                                    null,
                                    reservation.getId(),
                                    recipient,
                                    cc,
                                    subject,
                                    renderedTemplate.getTextPart(),
                                    renderedTemplate.getHtmlPart(),
                                    encodedAttachments,
                                    checksum,
    ...
    public List<String> getCCForEventOrganizer(PurchaseContext purchaseContext) {
        var systemNotificationCC = configurationManager
                .getFor(ConfigurationKeys.MAIL_SYSTEM_NOTIFICATION_CC, purchaseContext.getConfigurationLevel())
                .getValueOrDefault("");
    ...
    public void sendSimpleEmail(
        ...
        sendSimpleEmail(
                purchaseContext, reservationId, recipient, Collections.emptyList(), subject, textBuilder, attachments);
    }
    ...
    public void sendSimpleEmail(
        ...
        if (recipient == null) {
            log.warn(
                    "No recipient provided, mail for reservation id {} with subject {} will be ignored",
                    removeTabsAndNewlines(reservationId),
                    removeTabsAndNewlines(subject));
            return;
        }

        String encodedAttachments =
                attachments.isEmpty() ? null : encodeAttachments(attachments.toArray(new Mailer.Attachment[0]));
        ...
        Optional<Integer> existing =
                emailMessageRepository.findIdByPurchaseContextAndChecksum(purchaseContext, checksum);
    ...
    public Pair<Integer, List<LightweightMailMessage>> loadAllMessagesForPurchaseContext(
            PurchaseContext purchaseContext, Integer page, String search) {
        ...
        if (purchaseContext.ofType(PurchaseContextType.event)) {
            ...
            return Pair.of(
                    emailMessageRepository.countFindByEventId(eventId, toSearch),
            ...
            return Pair.of(
                    emailMessageRepository.countFindBySubscriptionDescriptorId(subscriptionDescriptorId, toSearch),
                    emailMessageRepository.findBySubscriptionDescriptorId(
                            subscriptionDescriptorId, offset, pageSize, toSearch));
        }
    }

    public List<LightweightMailMessage> loadAllMessagesForReservationId(
            PurchaseContext purchaseContext, String reservationId) {
        return emailMessageRepository.findByPurchaseContextAndReservationId(purchaseContext, reservationId);
    }

    public Optional<LightweightMailMessage> loadSingleMessageForPurchaseContext(
            PurchaseContext purchaseContext, int messageId) {
        if (purchaseContext.ofType(PurchaseContextType.event)) {
            return emailMessageRepository.findByEventIdAndMessageId(((Event) purchaseContext).getId(), messageId);
        } else {
            return emailMessageRepository.findBySubscriptionDescriptorIdAndMessageId(
                    ((SubscriptionDescriptor) purchaseContext).getId(), messageId);
        }
    }

    @Transactional
    public int sendWaitingMessages() {
        emailMessageRepository.setToRetryOldInProcess(
                ZonedDateTime.now(clockProvider.getClock()).minusHours(1));
        return emailMessageRepository.loadAllWaitingForProcessing().stream()
                ...
                .flatMapToInt(entry -> {
                    ...
                    PurchaseContext purchaseContext = purchaseContextManager
                            .findById(PurchaseContextType.from(splitKey[0]), splitKey[1])
                            .orElseThrow();
    ...
    private int processMessage(EmailMessage message, PurchaseContext purchaseContext) {
        ...
        if (message.getAttempts()
                >= configurationManager
                        .getFor(ConfigurationKeys.MAIL_ATTEMPTS_COUNT, configurationLevel)
                        .getValueAsIntOrDefault(10)) {
            tx.execute(status -> emailMessageRepository.updateStatusAndAttempts(
                    messageId,
                    ERROR.name(),
                    message.getAttempts(),
        ...
        try {
            int result = Optional.ofNullable(tx.execute(status -> emailMessageRepository.updateStatus(
                            messageId,
                            message.getChecksum(),
                            IN_PROCESS.name(),
                            Arrays.asList(WAITING.name(), RETRY.name()))))
                    .orElse(0);
            if (result > 0) {
                ...
                log.debug(
                        "no messages have been updated on DB for the following criteria: id: {}, checksum: {}",
                        messageId,
                        message.getChecksum());
            }
        } catch (Exception e) {
            tx.execute(status -> emailMessageRepository.updateStatusAndAttempts(
                    message.getId(),
                    RETRY.name(),
                    ZonedDateTime.now(clockProvider.getClock()).plusMinutes(message.getAttempts() + 1L),
    ...
    private void sendMessage(PurchaseContext purchaseContext, EmailMessage message) {
        ...
        mailer.send(
                ...
                message.getSubject(),
                message.getMessage(),
                Optional.ofNullable(message.getHtmlMessage()),
                decodeAttachments(message.getAttachments()));
        emailMessageRepository.updateStatusToSent(
                message.getId(),
                message.getChecksum(),
                ZonedDateTime.now(clockProvider.getClock()),
                Collections.singletonList(IN_PROCESS.name()));
    }
    ...
    private Mailer.Attachment[] decodeAttachments(String input) {
        ...
        Set<Mailer.AttachmentIdentifier> alreadyPresents = Arrays.stream(attachments)
                .map(Mailer.Attachment::getIdentifier)
                .filter(Objects::nonNull)
                .collect(Collectors.toSet());
        //
        List<Mailer.Attachment> toReinterpret = Arrays.stream(attachments)
                .filter(attachment -> attachment.getIdentifier() != null
                        && !attachment.getIdentifier().reinterpretAs().isEmpty())
                .collect(Collectors.toList());
        ...
        toReinterpret.forEach(attachment -> attachment.getIdentifier().reinterpretAs().stream()
    ...
    private Mailer.Attachment transformAttachment(
            Mailer.Attachment attachment, Mailer.AttachmentIdentifier identifier) {
        if (identifier != null) {
    ...
    private static String calculateChecksum(
            String recipient, String attachments, String subject, RenderedTemplate renderedTemplate) {
        try {
    ...
    private static final class AttachmentConverter
        ...
        public JsonElement serialize(Mailer.Attachment src, Type typeOfSrc, JsonSerializationContext context) {
            ...
            obj.addProperty(
                    SOURCE, src.getSource() != null ? Base64.getEncoder().encodeToString(src.getSource()) : null);
            obj.addProperty("contentType", src.getContentType());
            obj.addProperty(
                    IDENTIFIER,
                    src.getIdentifier() != null ? src.getIdentifier().name() : null);
        ...
        public Mailer.Attachment deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) {
            ...
            byte[] source = jsonObject.has(SOURCE)
                    ? Base64.getDecoder()
                            .decode(jsonObject.getAsJsonPrimitive(SOURCE).getAsString())
                    : null;
            ...
            Mailer.AttachmentIdentifier identifier = jsonObject.has(IDENTIFIER)
                    ? Mailer.AttachmentIdentifier.valueOf(
                            jsonObject.getAsJsonPrimitive(IDENTIFIER).getAsString())
                    : null;
            Map<String, String> model = jsonObject.has(MODEL)
                    ? Json.fromJson(jsonObject.getAsJsonPrimitive(MODEL).getAsString(), new TypeReference<>() {})
                    : null;
            return new Mailer.Attachment(filename, source, contentType, model, identifier);
        }
    }

    private static Function<Map<String, String>, byte[]> generateSubscriptionPDF(
            OrganizationRepository organizationRepository,
            ConfigurationManager configurationManager,
            FileUploadManager fileUploadManager,
            TemplateManager templateManager,
