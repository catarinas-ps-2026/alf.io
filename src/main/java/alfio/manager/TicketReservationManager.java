...
public class TicketReservationManager {
    ...
    private static final String STUCK_TICKETS_MSG =
            "there are stuck tickets for the event %s. Please check admin area.";
    ...
    public TicketReservationManager(
        ...
        this.requiresNewTransactionTemplate = new TransactionTemplate(
                transactionManager, new DefaultTransactionDefinition(TransactionDefinition.PROPAGATION_REQUIRES_NEW));
        DefaultTransactionDefinition serialized =
    ...
    private String createSubscriptionReservation(
            SubscriptionDescriptor subscriptionDescriptor,
            Date reservationExpiration,
            Locale locale,
            Integer userId,
            SubscriptionMetadata metadata)
            throws CannotProceedWithPayment, NotEnoughTicketsException {
        ...
        if (subscriptionDescriptor.getMaxAvailable() > 0) {
            ...
            Validate.isTrue(subscriptionRepository.bindSubscriptionToReservation(
                            reservationId, subscriptionDescriptor.getPrice(), AllocationStatus.PENDING, subscription)
                    == 1);
            ...
            subscriptionRepository.createSubscription(
                    ...
                    subscriptionDescriptor.getPrice(),
                    subscriptionDescriptor.getCurrency(),
                    subscriptionDescriptor.getOrganizationId(),
        ...
        ticketReservationRepository.updateBillingData(
                subscriptionDescriptor.getVatStatus(),
                calculateSrcPrice(vatStatus, totalPrice),
        ...
        auditingRepository.insert(
                ...
                null,
                subscriptionDescriptor.event().map(Event::getId).orElse(null),
                Audit.EventType.RESERVATION_CREATE,
                new Date(),
        ...
        if (!canProceedWithPayment(subscriptionDescriptor, totalPrice, reservationId)) {
            throw new CannotProceedWithPayment(
                    "No payment method applicable for purchase context  " + subscriptionDescriptor.getType()
                            + " with public id " + subscriptionDescriptor.getPublicIdentifier());
        }
     ...
     * @return
     */
    public String createTicketReservation(
            Event event,
        ...
        Optional<PromoCodeDiscount> discount = promotionCodeDiscount.flatMap(promoCodeDiscount ->
                promoCodeDiscountRepository.findPromoCodeInEventOrOrganization(event.getId(), promoCodeDiscount));
        ...
        discount.ifPresent(d -> {
            if (d.getMaxUsage() != null) {
                ...
                var appliedDiscountCount = CollectionUtils.isEmpty(discountedCategories)
                        ? list.size()
                        : list.stream()
                                .filter(t -> discountedCategories.contains(t.getTicketCategoryId()))
                                .count();
                if (d.getMaxUsage()
                        < promoCodeDiscountRepository.countUsedPromoCode(d.getId()) + appliedDiscountCount) {
        ...
        ticketReservationRepository.createNewReservation(
                ...
                reservationExpiration,
                dynamicDiscount.or(() -> discount).map(PromoCodeDiscount::getId).orElse(null),
                locale.getLanguage(),
                event.getId(),
                event.getVat(),
                event.isVatIncluded(),
        ...
        list.forEach(t -> reserveTicketsForCategory(
                event, reservationId, t, locale, forWaitingQueue, discount.orElse(null), dynamicDiscount.orElse(null)));

        // add all additional services
        additionalServiceManager.bookAdditionalServicesForReservation(
                event, reservationId, additionalServices, discount);
        ...
        if (!canProceedWithPayment(event, totalPrice, reservationId)) {
            throw new CannotProceedWithPayment("No payment method applicable for categories "
                    + list.stream()
                            .map(t -> String.valueOf(t.getTicketCategoryId()))
    ...
    private Optional<PromoCodeDiscount> createDynamicPromoCode(
            Optional<PromoCodeDiscount> existingDiscount,
            Event event,
            List<TicketReservationWithOptionalCodeModification> list,
        ...
        if (existingDiscount.filter(dt -> dt.getCodeType() != CodeType.ACCESS).isEmpty()) {
            return createDynamicPromoCodeIfNeeded(event, list, reservationId)
                    .flatMap(promoCodeDiscount -> promoCodeDiscountRepository.findPromoCodeInEventOrOrganization(
                            event.getId(), promoCodeDiscount));
        }
        return existingDiscount;
    }

    Optional<String> createDynamicPromoCodeIfNeeded(
            Event event, List<TicketReservationWithOptionalCodeModification> list, String reservationId) {

        var paidCategories = ticketCategoryRepository.countPaidCategoriesInReservation(list.stream()
                .map(TicketReservationWithOptionalCodeModification::getTicketCategoryId)
                .collect(Collectors.toSet()));
        if (paidCategories == null || paidCategories == 0) {
            return Optional.empty();
        }

        var newCodeOptional = extensionManager.handleDynamicDiscount(
                event,
                list.stream()
                        .collect(groupingBy(
                                TicketReservationWithOptionalCodeModification::getTicketCategoryId,
                                summingLong(TicketReservationWithOptionalCodeModification::getQuantity))),
                reservationId);
        if (newCodeOptional.isPresent()) {
            var newCode = newCodeOptional.get();
            int result = promoCodeDiscountRepository.addPromoCodeIfNotExists(
                    newCode.getPromoCode(),
                    event.getId(),
                    event.getOrganizationId(),
                    newCode.getUtcStart(),
                    newCode.getUtcEnd(),
                    newCode.getDiscountAmount(),
                    newCode.getDiscountType(),
                    "[]",
                    null,
                    null,
                    null,
            ...
            if (result > 0) {
                auditingRepository.insert(
                        ...
                        null,
                        event.getId(),
                        Audit.EventType.DYNAMIC_DISCOUNT_CODE_CREATED,
                        new Date(),
                        Audit.EntityType.RESERVATION,
                        reservationId);
            }
    ...
    private int calculateSrcPrice(VatStatus vatStatus, TotalPrice totalPrice) {
        return (vatStatus == VatStatus.INCLUDED
                        ? totalPrice.getPriceWithVAT()
                        : totalPrice.getPriceWithVAT() - totalPrice.getVAT())
                + Math.abs(totalPrice.getDiscount());
    }

    void reserveTicketsForCategory(
            Event event,
            String reservationId,
        ...
        if (accessCodeOrDiscount != null
                && accessCodeOrDiscount.getCodeType() == CodeType.ACCESS
            ...
            Optional<SpecialPrice> specialPrice = fixToken(
                    ticketReservation.getSpecialPrice(),
                    ticketReservation.getTicketCategoryId(),
                    event.getId(),
                    ticketReservation);
            specialPrices = specialPrice.stream().collect(toList());
        }

        List<Integer> reservedForUpdate = reserveTickets(
                event.getId(),
                ticketReservation,
                forWaitingQueue
                        ? asList(TicketStatus.RELEASED, TicketStatus.PRE_RESERVED)
                        : singletonList(TicketStatus.FREE));
        ...
        TicketCategory category =
                ticketCategoryRepository.getByIdAndActive(ticketReservation.getTicketCategoryId(), event.getId());
        initTicketsForReservation(
                event,
                reservationId,
                locale,
        ...
        TicketPriceContainer priceContainer =
                TicketPriceContainer.from(ticket, null, event.getVat(), event.getVatStatus(), discountToApply);
        var currencyCode = priceContainer.getCurrencyCode();
        ticketRepository.updateTicketPrice(
                reservedForUpdate,
                category.getId(),
                event.getId(),
    ...
    private void initTicketsForReservation(
            Event event,
            String reservationId,
            Locale locale,
            PromoCodeDiscount accessCodeOrDiscount,
        ...
        if (!specialPrices.isEmpty()) {
            ...
            if (specialPrices.size() == 1) {
                ...
                var accessCodeId = accessCodeOrDiscount != null && accessCodeOrDiscount.getHiddenCategoryId() != null
                        ? accessCodeOrDiscount.getId()
                        : null;
                ...
                ticketRepository.reserveTicket(
                        reservationId,
                        ticketId,
                        sp.getId(),
                        locale.getLanguage(),
                ...
                jdbcTemplate.batchUpdate(
                        ticketRepository.batchReserveTicketsForSpecialPrice(),
                        ticketsAndSpecialPrices.stream()
                                .map(triple -> {
                                    String metadata = null;
                                    var attendee = triple.getRight();
                                    if (attendee.hasMetadata()) {
                                        metadata = json.asJsonString(TicketMetadataContainer.fromMetadata(
                                                new TicketMetadata(null, null, attendee.getMetadata())));
                                    }
                ...
                ticketsAndSpecialPrices.stream()
                        ...
                        .forEach(tsp -> purchaseContextFieldRepository.updateOrInsert(
                                tsp.getRight().getAdditional(), event, tsp.getLeft(), null));
            }
        } else {
            ...
            for (int i = 0; i < reservedForUpdate.size(); i++) {
                ...
                if (attendee.hasAdditionalData()) {
                    purchaseContextFieldRepository.updateOrInsert(
                            attendee.getAdditional(), event, reservedForUpdate.get(i), null);
                }
            }
        }
    }
    ...
    List<SpecialPrice> reserveTokensForAccessCode(
            TicketReservationWithOptionalCodeModification ticketReservation, PromoCodeDiscount accessCode) {
        try {
            ...
            Validate.isTrue(promoCodeDiscountRepository
                    .lockAccessCodeForUpdate(accessCode.getId())
                    .equals(accessCode.getId()));
            List<SpecialPrice> boundSpecialPrices = specialPriceRepository.bindToAccessCode(
                    ticketReservation.getTicketCategoryId(), accessCode.getId(), ticketReservation.getQuantity());
    ...
    List<Integer> reserveTickets(
            int eventId,
            TicketReservationWithOptionalCodeModification ticketReservation,
            List<TicketStatus> requiredStatuses) {
        return reserveTickets(
                eventId, ticketReservation.getTicketCategoryId(), ticketReservation.getQuantity(), requiredStatuses);
    }
    ...
    List<Integer> reserveTickets(int eventId, int categoryId, int qty, List<TicketStatus> requiredStatuses) {
        ...
        List<String> statusesAsString =
                requiredStatuses.stream().map(TicketStatus::name).collect(toList());
        if (category.isBounded()) {
            return ticketRepository.selectTicketInCategoryForUpdateSkipLocked(
                    eventId, categoryId, qty, statusesAsString);
        }
        return ticketRepository.selectNotAllocatedTicketsForUpdateSkipLocked(eventId, qty, statusesAsString);
    }

    Optional<SpecialPrice> fixToken(
            Optional<SpecialPrice> token,
        ...
        Optional<SpecialPrice> specialPrice =
                token.map(SpecialPrice::getCode).flatMap(specialPriceRepository::getForUpdateByCode);
    ...
    public PaymentResult performPayment(
        ...
        try {
            ...
            if (reservationCost.requiresPayment()) {
                var transactionRequest = new TransactionRequest(
                        reservationCost,
                        ticketReservationRepository.getBillingDetailsForReservation(spec.getReservationId()));
                PaymentContext paymentContext = spec.getPaymentContext();
                paymentResult = paymentManager
                        .streamActiveProvidersByProxy(paymentProxy, paymentContext)
                        .filter(paymentProvider ->
                                paymentProvider.accept(paymentMethod, paymentContext, transactionRequest))
                        .findFirst()
    ...
    private boolean paymentMethodIsBlacklisted(PaymentMethod paymentMethod, PaymentSpecification spec) {
        return configurationManager
                .getBlacklistedMethodsForReservation(
                        spec.getPurchaseContext(), findCategoryIdsInReservation(spec.getReservationId()))
                .stream()
                .anyMatch(m -> m == paymentMethod);
    }

    public Collection<Integer> findCategoryIdsInReservation(String reservationId) {
        return findTicketsInReservation(reservationId).stream()
    ...
    public boolean cancelPendingPayment(String reservationId, PurchaseContext purchaseContext) {
        ...
        boolean remoteDeleteResult = paymentManager
                .lookupProviderByTransactionAndCapabilities(transaction, List.of(ServerInitiatedTransaction.class))
                .map(provider -> ((ServerInitiatedTransaction) provider)
                        .discardTransaction(optionalTransaction.get(), purchaseContext))
                .orElse(true);
    ...
    private void transitionToComplete(PaymentSpecification spec, PaymentProxy paymentProxy, String username) {
        var status = ticketReservationRepository
                .findOptionalStatusAndValidationById(spec.getReservationId())
                .orElseThrow()
                .getStatus();
        if (status != COMPLETE) {
    ...
    private boolean isDiscountCodeUsageExceeded(String reservationId) {
        ...
        if (reservation.getPromoCodeDiscountId() != null) {
            final PromoCodeDiscount promoCode =
                    promoCodeDiscountRepository.findById(reservation.getPromoCodeDiscountId());
            if (promoCode.getMaxUsage() == null) {
                return false;
            }
            int currentTickets = ticketReservationRepository.countTicketsInReservationForCategories(
                    reservationId, categoriesOrNull(promoCode));
    ...
    public boolean containsCategoriesLinkedToGroups(String reservationId, int eventId) {
        ...
        return ticketRepository.findTicketsInReservation(reservationId).stream().anyMatch(t -> allLinks.stream()
                .anyMatch(lg -> lg.getTicketCategoryId() == null
                        || lg.getTicketCategoryId().equals(t.getCategoryId())));
    }
    ...
    private boolean initPaymentProcess(
            TotalPrice reservationCost, PaymentProxy paymentProxy, PaymentSpecification spec, Principal principal) {
    ...
    private boolean acquireGroupMembers(String reservationId, PurchaseContext purchaseContext) {
        List<LinkedGroup> linkedGroups = purchaseContext
                .event()
                .map(event -> groupManager.getLinksForEvent(event.getId()))
                .orElse(List.of());
        if (!linkedGroups.isEmpty()) {
            List<Ticket> ticketsInReservation = ticketRepository.findTicketsInReservation(reservationId);
            return Boolean.TRUE.equals(requiresNewTransactionTemplate.execute(status -> ticketsInReservation.stream()
                    .filter(ticket -> linkedGroups.stream()
                            .anyMatch(c -> c.getTicketCategoryId() == null
                                    || c.getTicketCategoryId().equals(ticket.getCategoryId())))
    ...
    public void sendConfirmationEmail(
            PurchaseContext purchaseContext, TicketReservation ticketReservation, Locale language, String username) {
    ...
    private Locale findReservationLanguage(String reservationId) {
        return ticketReservationRepository
                .findOptionalReservationById(reservationId)
                .map(ReservationUtil::getReservationLocale)
                .orElse(Locale.ENGLISH);
    }

    public void deleteOfflinePayment(
            Event event, String reservationId, boolean expired, boolean credit, boolean notify, String username) {
        TicketReservation reservation = findById(reservationId).orElseThrow(IllegalArgumentException::new);
        Validate.isTrue(
                EnumSet.of(OFFLINE_PAYMENT, DEFERRED_OFFLINE_PAYMENT, CUSTOM_OFFLINE_PAYMENT)
                        .contains(reservation.getStatus()),
                "Invalid reservation status");
        Validate.isTrue(
                !(credit && reservation.getStatus() == DEFERRED_OFFLINE_PAYMENT), "Cannot credit deferred payment");
        if (credit) {
            ...
            if (notify) {
                ...
                String subject = reservationHelper.getReservationEmailSubject(
                        event, reservationLanguage, "reservation-email-expired-subject", reservation.getId());
                notificationManager.sendSimpleEmail(
                        ...
                        () -> templateManager.renderTemplate(
                                event,
                                TemplateResource.OFFLINE_RESERVATION_EXPIRED_EMAIL,
                                emailModel,
                                reservationLanguage));
            }
    ...
    @Transactional
    public void issueCreditNoteForReservation(
            PurchaseContext purchaseContext, TicketReservation reservation, String username, boolean sendEmail) {
        var reservationId = reservation.getId();
        ticketReservationRepository.updateReservationStatus(
                reservationId, TicketReservationStatus.CREDIT_NOTE_ISSUED.toString());
        auditingRepository.insert(
                reservationId,
                userRepository.nullSafeFindIdByUserName(username).orElse(null),
                purchaseContext,
                Audit.EventType.CREDIT_NOTE_ISSUED,
                new Date(),
                RESERVATION,
                reservationId);
        var model = prepareModelForReservationEmail(
                purchaseContext,
                reservation,
                reservationHelper.getVAT(purchaseContext),
                orderSummaryForReservation(reservation, purchaseContext),
                ticketRepository.findTicketsInReservation(reservation.getId()),
                Map.of());
        BillingDocument billingDocument = billingDocumentManager.createBillingDocument(
                purchaseContext,
                reservation,
                username,
                BillingDocument.Type.CREDIT_NOTE,
                orderSummaryForReservation(reservation, purchaseContext));
        var organization = organizationRepository.getById(purchaseContext.getOrganizationId());
        extensionManager.handleCreditNoteGenerated(
                reservation,
                purchaseContext,
                ((OrderSummary) model.get("orderSummary")).getOriginalTotalPrice(),
        ...
        if (sendEmail) {
            ...
            notificationManager.sendSimpleEmail(
                    ...
                    reservationId,
                    reservation.getEmail(),
                    reservationHelper.getReservationEmailSubject(
                            purchaseContext,
                            reservationLocale,
                            "credit-note-issued-email-subject",
                            reservation.getId()),
                    () -> templateManager.renderTemplate(
                            purchaseContext, TemplateResource.CREDIT_NOTE_ISSUED_EMAIL, model, reservationLocale),
                    reservationHelper.generateBillingDocumentAttachment(
                            purchaseContext, reservation, reservationLocale, billingDocument.getModel(), CREDIT_NOTE));
        }
    }

    void issuePartialCreditNoteForReservation(
            Event event, TicketReservation reservation, String username, List<Integer> ticketsId) {
        Validate.isTrue(
                TransactionSynchronizationManager.isActualTransactionActive(),
                "issuePartialCreditNoteForReservation() needs to be called within an active transaction");
        ...
        Map<String, Object> model =
                prepareModelForPartialCreditNote(event, reservation, ticketRepository.findByIds(ticketsId));
        log.trace("model for partial credit note created");
        var orderSummary = (OrderSummary) model.get("orderSummary");
        var billingDocument = billingDocumentManager.createBillingDocument(
                event, reservation, username, BillingDocument.Type.CREDIT_NOTE, orderSummary);
        var organization = organizationRepository.getById(event.getOrganizationId());
        extensionManager.handleCreditNoteGenerated(
                reservation,
                event,
                orderSummary.getOriginalTotalPrice(),
                billingDocument.getId(),
                Map.of(ORGANIZATION, organization));
    }

    void issueCreditNoteForRefund(
            PurchaseContext purchaseContext, TicketReservation reservation, BigDecimal refundAmount, String username) {
        Validate.isTrue(
                TransactionSynchronizationManager.isActualTransactionActive(),
                "issueCreditNoteForRefund() needs to be called within an active transaction");
        var currencyCode = reservation.getCurrencyCode();
        var priceContainer = new RefundPriceContainer(
                unitToCents(refundAmount, currencyCode),
                currencyCode,
                reservation.getVatStatus(),
                reservation.getVatPercentageOrZero());
        var summaryRowTitle = messageSourceManager
                .getMessageSourceFor(purchaseContext)
                .getMessage("invoice.refund.line-item", null, Locale.forLanguageTag(reservation.getUserLanguage()));
        ...
        var cost = new TotalPrice(
                priceContainer.getSrcPriceCts(),
        ...
        log.trace("model for partial credit note created");
        var billingDocument = billingDocumentManager.createBillingDocument(
                purchaseContext, reservation, username, BillingDocument.Type.CREDIT_NOTE, orderSummary);
        var organization = organizationRepository.getById(purchaseContext.getOrganizationId());
        extensionManager.handleCreditNoteGenerated(
                reservation, purchaseContext, cost, billingDocument.getId(), Map.of(ORGANIZATION, organization));
    }

    @Transactional(readOnly = true)
    public Map<String, Object> prepareModelForReservationEmail(
        ...
        return reservationHelper.prepareModelForReservationEmail(
                purchaseContext, reservation, vat, summary, ticketsToInclude, initialOptions);
    }
    ...
    private Map<String, Object> prepareModelForPartialCreditNote(
            Event event, TicketReservation reservation, List<Ticket> removedTickets) {
    ...
    private void transitionToInPayment(PaymentSpecification spec, Principal principal) {
        requiresNewTransactionTemplate.execute(status -> {
            var optionalStatusAndValidation =
                    ticketReservationRepository.findOptionalStatusAndValidationById(spec.getReservationId());
            if (optionalStatusAndValidation.isPresent()
                    && optionalStatusAndValidation.get().getStatus() == COMPLETE) {
                // reservation has been already completed. Let's check if there is a corresponding audit event
                Validate.isTrue(
                        auditingRepository.countAuditsOfTypeForReservation(spec.getReservationId(), PAYMENT_CONFIRMED)
                                == 1,
                        "Trying to confirm an already paid reservation, but can't find autiting event");
            } else if (optionalStatusAndValidation.isPresent()
                    && optionalStatusAndValidation.get().getStatus() == PENDING) {
                int updatedReservation = ticketReservationRepository.updateTicketReservation(
                        ...
                        null,
                        PaymentProxy.STRIPE.toString(),
                        spec.getCustomerReference());
                Validate.isTrue(
                        updatedReservation == 1, "expected exactly one updated reservation, got " + updatedReservation);
                if (principal != null && configurationManager.isPublicOpenIdEnabled()) {
                    ticketReservationRepository.setReservationOwner(
                            spec.getReservationId(), retrievePublicUserId(principal));
                }
            }
            return null;
        });
    ...
    public static boolean hasValidOfflinePaymentWaitingPeriod(
            PaymentContext context, ConfigurationManager configurationManager) {
    ...
    public boolean isValidPaymentMethod(
            PaymentManager.PaymentMethodDTO paymentMethodDTO, PurchaseContext purchaseContext) {
        boolean ifCustomOfflineIsApplicableForEvent = true;
        if (paymentMethodDTO.getPaymentProxy() == PaymentProxy.CUSTOM_OFFLINE
                && purchaseContext.event().isPresent()) {
            var event = purchaseContext.event().get();
            ifCustomOfflineIsApplicableForEvent =
                    customOfflineConfigurationManager.getAllowedCustomOfflinePaymentMethodsForEvent(event).stream()
        ...
        return paymentMethodDTO.isActive()
                ...
                && (!paymentMethodDTO.getPaymentProxy().equals(PaymentProxy.OFFLINE)
                        || hasValidOfflinePaymentWaitingPeriod(
    ...
    private void reTransitionToPending(String reservationId, boolean deleteTransactions) {
        int updatedReservation = ticketReservationRepository.updateReservationStatus(
                reservationId, TicketReservationStatus.PENDING.toString());
    ...
    public Optional<Triple<Event, TicketReservation, Ticket>> from(
            String eventName, String reservationId, Ticket ticket) {
        return eventRepository
                .findOptionalByShortName(eventName)
                .flatMap(event -> ticketReservationRepository
                        .findOptionalReservationById(reservationId)
    ...
    public Optional<Triple<Event, TicketReservation, Ticket>> from(
            String eventName, String reservationId, String ticketIdentifier) {
        return ticketRepository
                .findOptionalByUUID(ticketIdentifier)
                .flatMap(ticket -> from(eventName, reservationId, ticket));
    }

    public Optional<Triple<Event, TicketReservation, Ticket>> from(
            String eventName, String reservationId, UUID publicTicketUUID) {
        return ticketRepository
                .findOptionalByPublicUUID(publicTicketUUID)
                .flatMap(ticket -> from(eventName, reservationId, ticket));
    }
    ...
    void completeReservation(
        ...
        // set by the payment provider. This is useful especially in case a single "PaymentProxy" can produce different
        // statuses
        // like OFFLINE_PAYMENT and DEFERRED_OFFLINE_PAYMENT
        var currentStatus = ticketReservationRepository
                .findOptionalStatusAndValidationById(spec.getReservationId())
                .orElseThrow()
                .getStatus();
        ...
        this.applicationEventPublisher.publishEvent(new FinalizeReservation(
                spec, paymentProxy, sendReservationConfirmationEmail, sendTickets, username, currentStatus));
    }

    public PartialTicketTextGenerator getTicketEmailGenerator(
            Event event,
    ...
    public void cleanupExpiredReservations(Date expirationDate) {
        List<String> expiredReservationIds =
                ticketReservationRepository.findExpiredReservationForUpdate(expirationDate);
        ...
        ticketReservationRepository
                .findReservationsWithPendingTransaction(expiredReservationIds)
                .forEach(reservation -> {
                    ...
                    if (purchaseContextOptional.isPresent()) {
                        ...
                        if (resultOptional.isPresent()) {
                            ...
                            if (result.isSuccessful()) {
                                ...
                                log.debug(
                                        "Force check for expired reservation ID {} revealed a completed transaction. Will not delete.",
                                        reservationId);
                                reservationsToIgnore.add(reservationId);
                            } else {
                                // we need to cancel the pending payment, otherwise we could end up with a mismatch
                                boolean cancelPendingPaymentResult =
                                        cancelPendingPayment(reservationId, purchaseContext);
                                log.warn(
                                        "Trying to force pending payment cancellation for reservation ID {}. Successful: {}",
                                        reservationId,
                                        cancelPendingPaymentResult);
                            }
                        } else {
    ...
    public void cleanupExpiredOfflineReservations(Date expirationDate) {
        ticketReservationRepository
                .findExpiredOfflineReservationsForUpdate(expirationDate)
    ...
    private void cleanupOfflinePayment(String reservationId) {
        try {
            nestedTransactionTemplate.execute(tc -> {
                ...
                boolean enabled = configurationManager
                        .getFor(AUTOMATIC_REMOVAL_EXPIRED_OFFLINE_PAYMENT, ConfigurationLevel.event(event))
                        .getValueAsBooleanOrDefault();
                if (enabled) {
                    deleteOfflinePayment(event, reservationId, true, false, true, null);
                } else {
                    log.trace(
                            "Will not cleanup reservation with id {} because the automatic removal has been disabled",
                            reservationId);
                }
                return null;
            });
    ...
    public void markExpiredInPaymentReservationAsStuck(Date expirationDate) {
        ...
        if (!stuckReservations.isEmpty()) {
            List<String> ids =
                    stuckReservations.stream().map(p -> p.getLeft().getId()).collect(toList());
            ticketReservationRepository.updateReservationsStatus(ids, TicketReservationStatus.STUCK.name());

            Map<Event, List<Pair<TicketReservation, Event>>> reservationsGroupedByEvent =
                    stuckReservations.stream().collect(Collectors.groupingBy(Pair::getRight));
            ...
            reservationsGroupedByEvent.forEach((event, reservations) -> {
                ...
                notificationManager.sendSimpleEmail(
                        ...
                        () -> RenderedTemplate.plaintext(
                                STUCK_TICKETS_MSG.formatted(event.getDisplayName()), Map.of()));

                extensionManager.handleStuckReservations(
                        event,
                        reservations.stream().map(p -> p.getLeft().getId()).collect(toList()));
            });
        }
    }

    private List<Pair<TicketReservation, Event>> findStuckPaymentsToBeNotified(Date expirationDate) {
        List<ReservationIdAndEventId> stuckReservations =
                ticketReservationRepository.findStuckReservationsForUpdate(expirationDate);
        Map<Integer, Event> events;
        if (!stuckReservations.isEmpty()) {
            events =
                    eventRepository
                            .findByIds(stuckReservations.stream()
                                    .map(ReservationIdAndEventId::getEventId)
                                    .collect(toSet()))
                            .stream()
        ...
        return stuckReservations.stream()
                .map(id -> Pair.of(
                        ticketReservationRepository.findReservationById(id.getId()), events.get(id.getEventId())))
                .filter(reservationAndEvent -> {
                    var event = reservationAndEvent.getRight();
                    var reservation = reservationAndEvent.getLeft();
                    var optionalTransaction = transactionRepository.loadOptionalByReservationIdAndStatusForUpdate(
                            reservation.getId(), Transaction.Status.PENDING);
                    ...
                    handlePaymentWebhookResult(
                            event,
                            providerAndWebhookResult.getLeft(),
    ...
    public String reservationUrlForExternalClients(
            String reservationId,
            PurchaseContext purchaseContext,
            String userLanguage,
            boolean userLoggedIn,
            String subscriptionId) {
        var configMap = configurationManager.getFor(
                EnumSet.of(BASE_URL, OPENID_PUBLIC_ENABLED), purchaseContext.getConfigurationLevel());
        ...
        if (userLoggedIn && configMap.get(OPENID_PUBLIC_ENABLED).getValueAsBooleanOrDefault()) {
            return baseUrl + "/openid/" + purchaseContext.getType() + "/" + purchaseContext.getPublicIdentifier()
                    + "/reservation/" + reservationId;
        } else {
            var cleanSubscriptionId = StringUtils.trimToNull(subscriptionId);
            return ReservationUtil.reservationUrl(
                    baseUrl,
                    reservationId,
                    purchaseContext,
                    userLanguage,
                    cleanSubscriptionId != null ? "subscription=" + cleanSubscriptionId : null);
        }
    }
    ...
    public int maxAmountOfTicketsForCategory(
            EventAndOrganizationId eventAndOrganizationId, int ticketCategoryId, String promoCode) {
        // verify if the promo code is present and if it's actually an access code
        if (StringUtils.isNotBlank(promoCode)) {
            Integer maxTicketsPerAccessCode = promoCodeDiscountRepository
                    .findPromoCodeInEventOrOrganization(eventAndOrganizationId.getId(), promoCode)
                    .filter(d -> d.getCodeType() == CodeType.ACCESS)
                    .map(PromoCodeDiscount::getMaxUsage)
                    .orElse(null);
        ...
        return configurationManager
                .getFor(
                        ...
                        ConfigurationLevel.ticketCategory(eventAndOrganizationId, ticketCategoryId))
                .getValueAsIntOrDefault(5);
    }
    ...
    private static Predicate<TicketReservation> notificationNotSent(Clock clock, int quietPeriod) {
        return r -> r.latestNotificationTimestamp(clock.getZone())
                .map(t -> t.truncatedTo(ChronoUnit.DAYS)
                        .plusDays(quietPeriod)
                        .isBefore(ZonedDateTime.now(clock).truncatedTo(ChronoUnit.DAYS)))
                .orElse(true);
    }
    ...
    private void creditReservation(TicketReservation reservation, String username, boolean sendEmail) {
        ...
        billingDocumentManager.ensureBillingDocumentIsPresent(
                event, reservation, username, () -> orderSummaryForReservationId(reservation.getId(), event));
    ...
    private void cleanupReferencesToReservation(
            boolean expired,
            String username,
            String reservationId,
            PurchaseContext purchaseContext,
            boolean afterTicketReleased,
        ...
        applicationEventPublisher.publishEvent(new CleanupReservations(
                purchaseContext, List.of(reservationId), expired, afterTicketReleased, creditNoteIssued));
        ...
        auditingRepository.insert(
                ...
                userRepository.nullSafeFindIdByUserName(username).orElse(null),
                purchaseContext.event().map(Event::getId).orElse(null),
                expired ? Audit.EventType.CANCEL_RESERVATION_EXPIRED : Audit.EventType.CANCEL_RESERVATION,
                new Date(),
                Audit.EntityType.RESERVATION,
                reservationId);
    }
    ...
    private void removeReservation(
            PurchaseContext purchaseContext, TicketReservation reservation, boolean expired, String username) {
    ...
    public void updateTicketOwner(
        ...
        var ticketFieldValueMapCollector =
                groupingBy(PurchaseContextFieldValue::getName, mapping(PurchaseContextFieldValue::getValue, toList()));
        Map<String, List<String>> preUpdateTicketFields =
                purchaseContextFieldRepository.findAllByTicketId(ticket.getId()).stream()
                        .collect(ticketFieldValueMapCollector);

        String newEmail = StringUtils.trim(updateTicketOwner.getEmail());
        CustomerName customerName = new CustomerName(
                updateTicketOwner.getFullName(),
                updateTicketOwner.getFirstName(),
                updateTicketOwner.getLastName(),
                event.mustUseFirstAndLastName(),
                false);
        ticketRepository.updateTicketOwner(
                ticket.getUuid(),
                newEmail,
        ...
        if (MapUtils.isNotEmpty(updateTicketOwner.getAdditionalServices())) {
            additionalServiceManager.persistFieldsForAdditionalItems(
                    event.getId(),
                    event.getOrganizationId(),
                    updateTicketOwner.getAdditionalServices(),
                    List.of(ticket));
        }

        Ticket newTicket = ticketRepository.findByUUID(ticket.getUuid());
        boolean sendTicketAllowed = configurationManager
                .getFor(SEND_TICKETS_AUTOMATICALLY, ConfigurationLevel.event(event))
                .getValueAsBooleanOrDefault();
        if (sendTicketAllowed
                && (newTicket.getStatus() == TicketStatus.ACQUIRED || newTicket.getStatus() == TicketStatus.TO_BE_PAID)
                && (!equalsIgnoreCase(newEmail, ticket.getEmail())
                        || !equalsIgnoreCase(customerName.getFullName(), ticket.getFullName()))) {
        ...
        if (!admin
            ...
            String subject = messageSourceManager
                    .getMessageSourceFor(event)
                    .getMessage(
                            "ticket-has-changed-owner-subject", new Object[] {event.getDisplayName()}, oldUserLocale);
            notificationManager.sendSimpleEmail(
                    event,
        ...
        if (admin) {
            TicketReservation reservation =
                    findById(ticket.getTicketsReservationId()).orElseThrow(IllegalStateException::new);
            ...
            log.warn(
                    "Reservation {}: forced assignee replacement old: {} new: {}",
                    reservation.getId(),
        ...
        extensionManager.handleTicketAssignment(
                newTicket, ticketCategoryRepository.getById(ticket.getCategoryId()), updateTicketOwner.getAdditional());
        ...
        auditingHelper.auditUpdateTicket(
                preUpdateTicket, preUpdateTicketFields, postUpdateTicket, postUpdateTicketFields, event.getId());
    }
    ...
    boolean isTicketBeingReassigned(Ticket original, UpdateTicketOwnerForm updated, Event event) {
        ...
        CustomerName customerName = new CustomerName(
                updated.getFullName(), updated.getFirstName(), updated.getLastName(), event.mustUseFirstAndLastName());
        return StringUtils.isNotBlank(original.getEmail())
                && StringUtils.isNotBlank(original.getFullName())
                && (!equalsIgnoreCase(original.getEmail(), updated.getEmail())
                        || !equalsIgnoreCase(original.getFullName(), customerName.getFullName()));
    }

    private boolean isAdmin(Optional<UserDetails> userDetails) {
        return userDetails
                .flatMap(u -> u.getAuthorities().stream()
                        .map(a -> Role.fromRoleName(a.getAuthority()))
    ...
    public Optional<Triple<Event, TicketReservation, Ticket>> fetchComplete(String eventName, UUID ticketPublicUUID) {
        return ticketRepository.findOptionalByPublicUUID(ticketPublicUUID).flatMap(ticket -> from(
                        eventName, ticket.getTicketsReservationId(), ticket)
                .flatMap(triple -> {
     ...
     * @return
     */
    public Optional<Triple<Event, TicketReservation, Ticket>> fetchCompleteAndAssigned(
            String eventName, UUID ticketIdentifier) {
    ...
    public void sendReminderForOfflinePayments() {
        Date expiration = truncate(
                addHours(
                        new Date(),
                        configurationManager
                                .getForSystem(OFFLINE_REMINDER_HOURS)
                                .getValueAsIntOrDefault(24)),
                Calendar.DATE);
                    ...
                    return truncate(
                                            addHours(
                                                    ...
                                                    configurationManager
                                                            ...
                                                            .getValueAsIntOrDefault(24)),
                                            Calendar.DATE)
                                    .compareTo(p.getLeft().getValidity())
                            >= 0;
                })
                .map(p -> Triple.of(
                        p.getLeft(), p.getMiddle().orElseThrow(), p.getRight().orElseThrow()))
                .forEach(p -> {
                    ...
                    notificationManager.sendSimpleEmail(
                            ...
                            messageSourceManager
                                    .getMessageSourceFor(event)
                                    .getMessage(
                                            "reservation.reminder.mail.subject",
                                            new Object[] {configurationManager.getShortReservationID(event, reservation)
                                            },
                                            locale),
                            () -> templateManager.renderTemplate(
                                    event, TemplateResource.REMINDER_EMAIL, model, locale));
                });
    }

    // called each hour
    public void sendReminderForOfflinePaymentsToEventManagers() {
        eventRepository.findAllActives(ZonedDateTime.now(clockProvider.getClock())).stream()
                .filter(event -> {
                    ZonedDateTime dateTimeForEvent = event.now(clockProvider);
                    return dateTimeForEvent.truncatedTo(ChronoUnit.HOURS).getHour()
                            == 5; // only for the events at 5:00 local time
                })
                .forEachOrdered(event -> {
                    ZonedDateTime dateTimeForEvent = event.now(clockProvider)
                            .truncatedTo(ChronoUnit.DAYS)
                            .plusDays(1);
                    List<TicketReservationInfo> reservations =
                            ticketReservationRepository.findAllOfflinePaymentReservationWithExpirationBeforeForUpdate(
                                    dateTimeForEvent, event.getId());
                    log.info(
                            "for event {} there are {} pending offline payments to handle",
                            event.getId(),
                            reservations.size());
                    if (!reservations.isEmpty()) {
                        Organization organization = organizationRepository.getById(event.getOrganizationId());
                        List<String> cc = notificationManager.getCCForEventOrganizer(event);
                        String subject = "There are %d pending offline payments that will expire in event: %s"
                                .formatted(reservations.size(), event.getDisplayName());
                        String baseUrl = configurationManager
                                .getFor(BASE_URL, ConfigurationLevel.event(event))
                                .getRequiredValue();
                        Map<String, Object> model =
                                TemplateResource.prepareModelForOfflineReservationExpiringEmailForOrganizer(
                                        event, reservations, baseUrl);
                        notificationManager.sendSimpleEmail(
                                event,
                                null,
                                organization.getEmail(),
                                ...
                                () -> templateManager.renderTemplate(
                                        event,
                                        TemplateResource.OFFLINE_RESERVATION_EXPIRING_EMAIL_FOR_ORGANIZER,
                                        model,
                                        Locale.ENGLISH));
    ...
    public void sendReminderForOptionalData() {
        getNotifiableEventsStream()
                .filter(e -> configurationManager
                        .getFor(OPTIONAL_DATA_REMINDER_ENABLED, ConfigurationLevel.event(e))
                        .getValueAsBooleanOrDefault())
    ...
    private void sendOptionalDataReminder(Pair<Event, List<Ticket>> eventAndTickets) {
        nestedTransactionTemplate.execute(ts -> {
            ...
            int daysBeforeStart = configurationManager
                    .getFor(ASSIGNMENT_REMINDER_START, ConfigurationLevel.event(event))
                    .getValueAsIntOrDefault(10);
            List<Ticket> tickets = eventAndTickets.getRight().stream()
                    .filter(t -> !purchaseContextFieldRepository.hasOptionalData(t.getId()))
                    .collect(toList());
            Set<String> notYetNotifiedReservations = tickets.stream()
                    .map(Ticket::getTicketsReservationId)
                    .distinct()
                    .filter(rid -> findByIdForNotification(
                                    rid, clockProvider.withZone(event.getZoneId()), daysBeforeStart)
                            .isPresent())
                    .collect(toSet());
            tickets.stream()
                    ...
                    .forEach(t -> {
                        ...
                        Map<String, Object> model = TemplateResource.prepareModelForReminderTicketAdditionalInfo(
                                organizationRepository.getById(event.getOrganizationId()),
                                event,
                                t,
                                ReservationUtil.ticketUpdateUrl(event, t, configurationManager));
                        Locale locale = Optional.ofNullable(t.getUserLanguage())
                                .map(LocaleUtil::forLanguageTag)
                                .orElseGet(() -> findReservationLanguage(t.getTicketsReservationId()));
                        notificationManager.sendSimpleEmail(
                                event,
                                t.getTicketsReservationId(),
                                t.getEmail(),
                                messageSource.getMessage(
                                        "reminder.ticket-additional-info.subject",
                                        new Object[] {event.getDisplayName()},
                                        locale),
                                () -> templateManager.renderTemplate(
                                        event, TemplateResource.REMINDER_TICKET_ADDITIONAL_INFO, model, locale));
                    });
            return null;
        });
    }
    ...
    Stream<Event> getNotifiableEventsStream() {
        return eventRepository.findAll().stream().filter(e -> {
            int daysBeforeStart = configurationManager
                    .getFor(ASSIGNMENT_REMINDER_START, ConfigurationLevel.event(e))
                    .getValueAsIntOrDefault(10);
            int days = (int) ChronoUnit.DAYS.between(
                    ZonedDateTime.now(clockProvider.withZone(e.getZoneId())).toLocalDate(),
        ...
        try {
            nestedTransactionTemplate.execute(ts -> {
                ...
                int quietPeriod = configurationManager
                        ...
                        .getValueAsIntOrDefault(3);
                p.getRight().stream()
                        .map(id -> findByIdForNotification(id, clockProvider.withZone(eventZoneId), quietPeriod))
                        .flatMap(Optional::stream)
                        .forEach(reservation -> {
                            Map<String, Object> model =
                                    reservationHelper.prepareModelForReservationEmail(event, reservation);
                            ticketReservationRepository.updateLatestReminderTimestamp(
                                    reservation.getId(), ZonedDateTime.now(clockProvider.withZone(eventZoneId)));
                            ...
                            notificationManager.sendSimpleEmail(
                                    ...
                                    reservation.getEmail(),
                                    messageSource.getMessage(
                                            "reminder.ticket-not-assigned.subject",
                                            new Object[] {event.getDisplayName()},
                                            locale),
                                    () -> templateManager.renderTemplate(
                                            event, TemplateResource.REMINDER_TICKETS_ASSIGNMENT_EMAIL, model, locale));
                        });
                return null;
            });
        } catch (Exception ex) {
    ...
    public TicketReservation findByPartialID(String reservationId) {
        ...
        List<TicketReservation> results = ticketReservationRepository.findByPartialID(
                trimToEmpty(reservationId).toLowerCase() + "%");
    ...
    public String getShortReservationID(Configurable event, String reservationId) {
        return configurationManager.getShortReservationID(
                event, findById(reservationId).orElseThrow());
    }
    ...
    public void releaseTicket(Event event, TicketReservation ticketReservation, final Ticket ticket) {
        ...
        var configurationLevel = ticket.getCategoryId() != null
                ? ConfigurationLevel.ticketCategory(event, ticket.getCategoryId())
                : ConfigurationLevel.event(event);
        var enableFreeCancellation = configurationManager
                .getFor(ALLOW_FREE_TICKETS_CANCELLATION, configurationLevel)
                .getValueAsBooleanOrDefault();
        ...
        int result = ticketRepository.releaseTicket(
                reservationId, UUID.randomUUID().toString(), UUID.randomUUID(), event.getId(), ticket.getId());
        ...
        Locale locale = LocaleUtil.forLanguageTag(
                Optional.ofNullable(ticket.getUserLanguage()).orElse("en"));
        notificationManager.sendSimpleEmail(
                event,
                reservationId,
                ticket.getEmail(),
                messageSource.getMessage(
                        "email-ticket-released.subject", new Object[] {event.getDisplayName()}, locale),
        ...
        String ticketCategoryDescription = ticketCategoryDescriptionRepository
                .findByTicketCategoryIdAndLocale(category.getId(), ticket.getUserLanguage())
                .orElse("");

        List<AdditionalServiceItem> additionalServiceItems =
                additionalServiceManager.findItemsInReservation(event.getId(), reservationId);
        Map<String, Object> adminModel = TemplateResource.buildModelForTicketHasBeenCancelledAdmin(
                organization,
                event,
                ticket,
                ticketCategoryDescription,
                additionalServiceItems,
                asi -> additionalServiceManager.loadItemTitle(asi, locale));
        notificationManager.sendSimpleEmail(
                event,
                null,
                organization.getEmail(),
                messageSource.getMessage(
                        "email-ticket-released.admin.subject",
                        new Object[] {ticket.getId(), event.getDisplayName()},
                        locale),
                () -> templateManager.renderTemplate(
                        event, TemplateResource.TICKET_HAS_BEEN_CANCELLED_ADMIN, adminModel, locale));
        ...
        if (ticketRepository.countTicketsInReservation(reservationId) == 0
                && transactionRepository
                        .loadOptionalByReservationId(reservationId)
                        .isEmpty()) {
            ...
            auditingRepository.insert(
                    ...
                    null,
                    event.getId(),
                    Audit.EventType.CANCEL_RESERVATION,
                    new Date(),
                    Audit.EntityType.RESERVATION,
                    reservationId);
        }
    ...
    private void invalidateAccess(Event event, Ticket ticket, boolean reassigned) {
        ...
        applicationEventPublisher.publishEvent(
                new InvalidateAccess(ticket, ticketRepository.getTicketMetadata(ticket.getId()), event));
    }

    int getReservationTimeout(Configurable configurable) {
        return configurationManager
                .getFor(RESERVATION_TIMEOUT, configurable.getConfigurationLevel())
                .getValueAsIntOrDefault(25);
    }

    public void validateAndConfirmOfflinePayment(
            String reservationId, Event event, BigDecimal paidAmount, String username) {
        TicketReservation reservation = findByPartialID(reservationId);
        Optional<OrderSummary> optionalOrderSummary =
                optionally(() -> orderSummaryForReservationId(reservation.getId(), event));
        ...
        Validate.isTrue(
                MonetaryUtil.centsToUnit(orderSummary.getOriginalTotalPrice().getPriceWithVAT(), currencyCode)
                                .compareTo(paidAmount)
                        == 0,
                "paid price differs from due price");
    ...
    public void updateReservation(
        ...
        String completeBillingAddress = buildCompleteBillingAddress(
                customerName,
        ...
        ticketReservationRepository.updateTicketReservationWithValidation(
                reservationId,
                customerName.getFullName(),
                customerName.getFirstName(),
                customerName.getLastName(),
    ...
    public void setReservationOwner(
            String reservationId,
            String username,
            String email,
            String firstName,
    ...
    public void setReservationMetadata(String reservationId, ReservationMetadata metadata) {
        Validate.isTrue(
                ticketReservationRepository.setMetadata(reservationId, metadata) == 1, "Error while updating metadata");
    }

    public static String buildCompleteBillingAddress(
            CustomerName customerName,
            String billingAddressCompany,
            String billingAddressLine1,
            String billingAddressLine2,
        ...
        return Arrays.stream(stripAll(
                        Objects.requireNonNullElse(companyName, fullName),
                        billingAddressLine1,
                        billingAddressLine2,
                        stripToEmpty(billingAddressZip) + " " + stripToEmpty(billingAddressCity) + " "
                                + stripToEmpty(billingAddressState),
                        stripToNull(country)))
                .filter(Predicate.not(StringUtils::isEmpty))
                .collect(joining("\n"));
    }

    public void updateReservationInvoicingAdditionalInformation(
            String reservationId,
            PurchaseContext purchaseContext,
            TicketReservationInvoicingAdditionalInfo ticketReservationInvoicingAdditionalInfo) {
        auditingRepository.insert(
                reservationId,
                null,
                purchaseContext.event().map(Event::getId).orElse(null),
                ...
                RESERVATION,
                reservationId,
                json.asJsonString(List.of(ticketReservationInvoicingAdditionalInfo)));
        ticketReservationRepository.updateInvoicingAdditionalInformation(
                reservationId, json.asJsonString(ticketReservationInvoicingAdditionalInfo));
    }

    public PaymentWebhookResult processTransactionWebhook(
            String body, String signature, PaymentProxy paymentProxy, Map<String, String> additionalInfo) {
    ...
    public PaymentWebhookResult processTransactionWebhook(
        ...
        var paymentProviderOptional = paymentManager
                .streamActiveProvidersByProxyAndCapabilities(paymentProxy, pc, List.of(WebhookHandler.class))
                .findFirst();
        ...
        if (pc.getConfigurationLevel().isSystem()) {
            ...
            paymentContext = ((WebhookHandler) paymentProvider)
                    .detectPaymentContext(body)
                    .orElse(pc);
        } else {
            paymentContext = pc;
        }

        var optionalTransactionWebhookPayload = ((WebhookHandler) paymentProvider)
                .parseTransactionPayload(body, signature, additionalInfo, paymentContext);
        ...
        var optionalReservation =
                ticketReservationRepository.findOptionalReservationById(transactionPayload.getReservationId());
        if (optionalReservation.isEmpty()) {
        ...
        if (reservationStatusNotCompatible(reservation) || transaction.getStatus() != Transaction.Status.PENDING) {
            log.warn(
                    "discarding transaction webhook {} for reservation id {} ({}). Transaction status is: {}",
                    transactionPayload.getType(),
                    reservation.getId(),
        ...
        return paymentManager
                .lookupProviderByTransactionAndCapabilities(transaction, List.of(WebhookHandler.class))
                .map(provider -> {
                    var paymentWebhookResult = ((WebhookHandler) provider)
                            .processWebhook(transactionPayload, transaction, paymentContextReloaded);
    ...
    private PaymentWebhookResult handlePaymentWebhookResult(
            PurchaseContext purchaseContext,
        ...
        switch (paymentWebhookResult.getType()) {
            ...
            case TRANSACTION_INITIATED: {
                if (reservation.getStatus() == EXTERNAL_PROCESSING_PAYMENT && moveToWatingExternalConfirmationAllowed) {
                    ...
                    log.trace(
                            "Event {} received. Setting status {} for reservation {}",
                            operationType,
                            status,
                            reservation.getId());
                    ...
                    log.trace(
                            "Ignoring Event {}, as it cannot be applied for reservation {} ({})",
                            operationType,
                            reservation.getId(),
                            reservation.getStatus());
                }
            ...
            case SUCCESSFUL: {
                log.trace(
                        "Event {} for reservation {} has been successfully processed.",
                        operationType,
                        reservation.getId());
                ...
                var paymentSpecification = new PaymentSpecification(
                        ...
                        purchaseContext,
                        paymentToken,
                        paymentProvider.getPaymentMethodForTransaction(transaction),
                        orderSummaryForReservation(reservation, purchaseContext),
                        true,
                        hasPrivacyPolicy(purchaseContext));
            ...
            case FAILED: {
                ...
                //
                //      1) the user is still waiting on the payment page. In this case, there's no harm in reverting the
                // reservation status to PENDING
                ...
                //
                //      - if the reservation has expired, we cancel it and keep its data for reference, and we notify
                // also the organizer.
                //      - if the reservation is still valid, we can ensure that the user has at least 10 min left to
                // retry

                log.debug(
                        "Event {} for reservation {} has failed with reason: {}",
                        operationType,
                        reservation.getId(),
                        paymentWebhookResult.getReason());

                Date expiration = reservation.getValidity();
                Date now = new Date();
                int slackTime = configurationManager
                        .getFor(RESERVATION_MIN_TIMEOUT_AFTER_FAILED_PAYMENT, paymentContext.getConfigurationLevel())
                        .getValueAsIntOrDefault(10);
                PaymentMethod paymentMethodForTransaction = paymentProvider.getPaymentMethodForTransaction(transaction);
                if (expiration.before(now)) {
                    sendTransactionFailedEmail(
                            purchaseContext, reservation, paymentMethodForTransaction, paymentWebhookResult, true);
                    ...
                    ticketReservationRepository.updateValidity(
                            reservation.getId(), DateUtils.addMinutes(now, slackTime));
                }
                reTransitionToPending(reservation.getId(), false);
                purchaseContext
                        .event()
                        .ifPresent(event -> sendTransactionFailedEmail(
                                event, reservation, paymentMethodForTransaction, paymentWebhookResult, false));
                break;
            }
            case CANCELLED: {
                reTransitionToPending(reservation.getId(), false);
    ...
    public Optional<PaymentResult> forceTransactionCheck(
            PurchaseContext purchaseContext, TicketReservation reservation) {
        var optionalTransaction = transactionRepository.loadOptionalByReservationIdAndStatusForUpdate(
                reservation.getId(), Transaction.Status.PENDING);
        if (optionalTransaction.isEmpty()) {
            return Optional.empty();
        ...
        return checkTransactionStatus(purchaseContext, reservation).map(providerAndWebhookResult -> {
            var paymentWebhookResult = providerAndWebhookResult.getRight();
            handlePaymentWebhookResult(
                    purchaseContext,
                    providerAndWebhookResult.getLeft(),
            ...
            return switch (paymentWebhookResult.getType()) {
                ...
                case TRANSACTION_INITIATED -> StringUtils.isNotEmpty(paymentWebhookResult.getRedirectUrl())
                        ? PaymentResult.redirect(paymentWebhookResult.getRedirectUrl())
                        : PaymentResult.pending(transaction.getPaymentId());
                default -> PaymentResult.successful(
                        paymentWebhookResult.getPaymentToken().getToken());
            };
        });
    }

    private Optional<Pair<PaymentProvider, PaymentWebhookResult>> checkTransactionStatus(
            PurchaseContext purchaseContext, TicketReservation reservation) {
        var optionalTransaction = transactionRepository.loadOptionalByReservationIdAndStatusForUpdate(
                reservation.getId(), Transaction.Status.PENDING);
        if (optionalTransaction.isEmpty()) {
        ...
        return paymentManager
                .lookupProviderByTransactionAndCapabilities(transaction, List.of(WebhookHandler.class))
                .map(provider -> Pair.of(
                        provider,
                        ((WebhookHandler) provider).forceTransactionCheck(reservation, transaction, paymentContext)));
    }
    ...
    private void sendTransactionFailedEmail(
            PurchaseContext purchaseContext,
            TicketReservation reservation,
            PaymentMethod paymentMethod,
            PaymentWebhookResult paymentWebhookResult,
        ...
        Map<String, Object> model = Map.of(
                ...
                "provider",
                requireNonNullElse(paymentMethod, StaticPaymentMethods.NONE).name(),
                "reason",
                paymentWebhookResult.getReason(),
                "reservationUrl",
                reservationUrl(reservation, purchaseContext));

        Locale locale = LocaleUtil.forLanguageTag(reservation.getUserLanguage());
        if (cancelReservation
                || configurationManager
                        .getFor(NOTIFY_ALL_FAILED_PAYMENT_ATTEMPTS, purchaseContext.getConfigurationLevel())
                        .getValueAsBooleanOrDefault()) {
            notificationManager.sendSimpleEmail(
                    purchaseContext,
                    reservation.getId(),
                    reservation.getEmail(),
                    messageSource.getMessage(
                            "email-transaction-failed.subject",
                            new Object[] {shortReservationID, purchaseContext.getDisplayName()},
                            locale),
                    () -> templateManager.renderTemplate(
                            purchaseContext, TemplateResource.CHARGE_ATTEMPT_FAILED_EMAIL_FOR_ORGANIZER, model, locale),
                    List.of());
        }
        ...
        notificationManager.sendSimpleEmail(
                ...
                messageSource.getMessage(
                        ...
                        locale),
                () -> templateManager.renderTemplate(
                        purchaseContext, TemplateResource.CHARGE_ATTEMPT_FAILED_EMAIL, model, locale),
                List.of());
    }

    public Optional<TransactionInitializationToken> initTransaction(
            PurchaseContext purchaseContext,
            String reservationId,
            PaymentMethod paymentMethod,
        ...
        var transactionRequest = new TransactionRequest(
                totalReservationCostWithVAT(reservation).getLeft(),
                ticketReservationRepository.getBillingDetailsForReservation(reservationId));
        var optionalProvider = paymentManager.lookupProviderByMethodAndCapabilities(
                paymentMethod,
                new PaymentContext(purchaseContext),
                transactionRequest,
                List.of(WebhookHandler.class, ServerInitiatedTransaction.class));
        ...
        if (!acquireGroupMembers(reservationId, purchaseContext)) {
            ...
            var errorMessage = messageSource.getMessage(
                    "error.STEP2_WHITELIST", null, LocaleUtil.forLanguageTag(reservation.getUserLanguage()));
        ...
        if (transitionToExternalProcessingPayment(reservation)) {
            auditingRepository.insert(
                    reservationId, null, purchaseContext, INIT_PAYMENT, new Date(), RESERVATION, reservationId);
        }
    ...
    private boolean transitionToExternalProcessingPayment(TicketReservation reservation) {
        ...
        if (optionalTransaction
                .filter(ot -> ot.getStatus() == Transaction.Status.PENDING)
                .isEmpty()) {
            log.warn(
                    "trying to transition reservation {} to EXTERNAL_PROCESSING_PAYMENT but the current transaction is not PENDING. Ignoring the request...",
                    reservationId);
            return false;
        }
        if (reservation.getStatus() != PENDING) {
            log.warn(
                    "trying to transition reservation {} to EXTERNAL_PROCESSING_PAYMENT but the current status is {}. Ignoring the request...",
                    reservationId,
                    reservation.getStatus());
            return false;
        }
    ...
    public void checkOfflinePaymentsStatus() {
        eventRepository
                .findAllActives(ZonedDateTime.now(clockProvider.getClock()))
                .forEach(this::checkOfflinePaymentsForEvent);
    }

    public Optional<String> createSubscriptionReservation(
            SubscriptionDescriptor subscriptionDescriptor, Locale locale, Principal principal) {
    ...
    public Optional<String> createSubscriptionReservation(
        ...
        return Optional.of(createSubscriptionReservation(
                subscriptionDescriptor, expiration, locale, retrievePublicUserId(principal), metadata));
    }

    public Optional<String> createTicketReservation(
            Event event,
            List<TicketReservationWithOptionalCodeModification> list,
            List<ASReservationWithOptionalCodeModification> additionalServices,
        ...
        var reservationId = createTicketReservation(
                event, list, additionalServices, expiration, promoCodeDiscount, locale, false, principal);
    ...
    boolean canProceedWithPayment(PurchaseContext purchaseContext, TotalPrice totalPrice, String reservationId) {
        ...
        var blacklistedPaymentMethods =
                configurationManager.getBlacklistedMethodsForReservation(purchaseContext, categoriesInReservation);
        var transactionRequest = new TransactionRequest(
                totalPrice, ticketReservationRepository.getBillingDetailsForReservation(reservationId));
        var availableMethods = paymentManager.getPaymentMethods(purchaseContext, transactionRequest).stream()
                .filter(pm -> pm.getStatus() == PaymentMethodStatus.ACTIVE
                        && pm.getPaymentMethod() != StaticPaymentMethods.NONE)
                .toList();
        var areAllMethodsBlacklisted = availableMethods.stream().allMatch(pm -> blacklistedPaymentMethods.stream()
                .anyMatch(blItem -> blItem.getPaymentMethodId().equals(pm.getPaymentMethodId())));
    ...
    public Result<Boolean> discardMatchingPayment(String eventName, String reservationId, int transactionId) {
        return eventRepository
                .findOptionalByShortName(eventName)
                .flatMap(e -> ticketReservationRepository
                        .findOptionalReservationById(reservationId)
                        .map(r -> Pair.of(e, r)))
                .flatMap(pair -> transactionRepository
                        ...
                        .map(transaction -> {
                            auditingRepository.insert(
                                    ...
                                    null,
                                    pair.getLeft().getId(),
                                    Audit.EventType.MATCHING_PAYMENT_DISCARDED,
                                    new Date(),
                                    RESERVATION,
                                    reservationId);
                            Validate.isTrue(
                                    transactionRepository.discardMatchingPayment(transactionId) == 1,
                                    "Transaction is in an incompatible state.");
    ...
    public void flagAsValidated(String reservationId, PurchaseContext purchaseContext, List<WarningMessage> warnings) {
        ...
        if (!warnings.isEmpty()) {
            auditingRepository.insert(
                    ...
                    RESERVATION,
                    reservationId,
                    List.of(Map.of("warnings", warnings)));
        }
    }
    ...
    private void checkOfflinePaymentsForEvent(Event event) {
        ...
        var providers = paymentManager
                .streamActiveProvidersByProxyAndCapabilities(
                        PaymentProxy.OFFLINE, paymentContext, List.of(OfflineProcessor.class))
                .collect(toList());
        if (providers.isEmpty()) {
            log.trace("No active offline provider has been found. Exiting...");
            return;
        }
        var pendingReservationsMap =
                ticketSearchRepository.findOfflineReservationsWithPendingTransaction(event.getId()).stream()
        ...
        for (int i = 0; !pendingReservationsMap.isEmpty() && i < providers.size(); i++) {
            ...
            Result<List<String>> matching =
                    offlineProcessor.checkPendingReservations(pendingReservationsMap.values(), paymentContext, null);
            if (matching.isSuccess()) {
                int resultSize = matching.getData().size();
                matchingCount += resultSize;
                log.trace(
                        "found {} matches for provider {}",
                        matchingCount,
                        offlineProcessor.getClass().getName());
                if (resultSize > 0) {
        ...
        if (matchingCount > 0) {
            ...
            notificationManager.sendSimpleEmail(
                    ...
                    subject,
                    () -> templateManager.renderTemplate(
                            event, TemplateResource.OFFLINE_PAYMENT_MATCHES_FOUND, model, Locale.ENGLISH));
        }
    }

    private void processResults(
            Event event,
            Map<String, TicketReservationWithTransaction> pendingReservationsMap,
            ArrayList<String> errors,
            ArrayList<String> confirmed,
            ArrayList<String> pendingReview,
            Result<List<String>> matching) {
        ...
        reservations.forEach(tr -> {
            ...
            auditingRepository.insert(
                    ...
                    RESERVATION,
                    reservationId,
                    json.asJsonString(List.of(tr.getTransaction().getMetadata())));
        ...
        byStatus.getOrDefault(Transaction.Status.OFFLINE_MATCHING_PAYMENT_FOUND, List.of())
                .forEach(tr -> {
                    var reservationId = tr.getTicketReservation().getId();
                    if (automaticConfirmOfflinePayment(event, reservationId)) {
                        ...
                        auditingRepository.insert(
                                ...
                                null,
                                event.getId(),
                                Audit.EventType.AUTOMATIC_PAYMENT_CONFIRMATION,
                                new Date(),
                                RESERVATION,
                                reservationId);
                        ...
                        auditingRepository.insert(
                                ...
                                null,
                                event.getId(),
                                Audit.EventType.AUTOMATIC_PAYMENT_CONFIRMATION_FAILED,
                                new Date(),
                                RESERVATION,
                                reservationId);
        ...
        pendingReview.addAll(byStatus.getOrDefault(Transaction.Status.OFFLINE_PENDING_REVIEW, List.of()).stream()
                .map(tr -> tr.getTicketReservation().getId())
                .collect(toList()));
    }
    ...
    public boolean validateAndApplySubscriptionCode(
            PurchaseContext purchaseContext,
            TicketReservation reservation,
        ...
        var subscriptionId = isUUID
                ? UUID.fromString(pin)
                : requireEmail
                        ? subscriptionRepository.getSubscriptionIdByPartialUuidAndEmail(partialUuid, email)
                        : subscriptionRepository.getSubscriptionIdByPartialUuid(partialUuid);
        ...
        try {
            return applySubscriptionCode(
                    ((Event) purchaseContext).getId(), reservation, subscriptionDescriptor, subscriptionId);
        } catch (SubscriptionUsageExceeded | SubscriptionUsageExceededForEvent ex) {
            bindingResult.reject(
                    ex instanceof SubscriptionUsageExceeded
                            ? "subscription.max-usage-reached"
                            : "subscription.max-usage-reached-per-event");
            return false;
        }
    }

    boolean applySubscriptionCode(
            int eventId,
            TicketReservation reservation,
            SubscriptionDescriptor subscriptionDescriptor,
            UUID subscriptionId)
            throws SubscriptionUsageExceeded, SubscriptionUsageExceededForEvent {
        ...
        var linkOptional = subscriptionRepository.findLink(
                subscription.getOrganizationId(), subscription.getSubscriptionDescriptorId(), eventId);

        if (linkOptional.isEmpty()) {
            log.warn(
                    "Attempt to use subscription {} - descriptor {} for event {}",
                    subscription.getId(),
                    subscriptionDescriptor.getId(),
                    eventId);
            return false;
        ...
        if (CollectionUtils.isNotEmpty(link.getCompatibleCategories())) {
            var bookedCategories = ticketCategoryRepository.findCategoriesInReservation(reservation.getId()).stream()
                    .map(TicketCategory::getId)
                    .collect(toSet());
            if (CollectionUtils.intersection(link.getCompatibleCategories(), bookedCategories)
                    .isEmpty()) {
                log.warn(
                        "Attempt to use subscription {} - descriptor {} unsuccessful for event {}. No compatible categories found (booked: {}).",
                        subscription.getId(),
                        subscriptionDescriptor.getId(),
                        eventId,
                        bookedCategories);
        ...
        try {
            ...
            if (countExisting >= limit) {
                throw oncePerEvent
                        ? new SubscriptionUsageExceededForEvent(limit, countExisting + 1)
                        : new SubscriptionUsageExceeded(limit, countExisting + 1);
            }
            ...
            int count;
            if (CollectionUtils.isNotEmpty(link.getCompatibleCategories())) {
                count = ticketRepository.applySubscriptionToTicketsInReservation(
                        reservation.getId(), subscriptionId, link.getCompatibleCategories(), limit - countExisting);
            } else {
                count = ticketRepository.applySubscriptionToTicketsInReservation(
                        reservation.getId(), subscriptionId, limit - countExisting);
            }
            log.trace(
                    "Applied subscription {} to {} tickets for reservation {}",
                    subscriptionId,
                    count,
                    reservation.getId());
        } catch (UncategorizedSQLException sqlException) {
            log.trace(
                    "got exception while trying to apply SubscriptionID {} to ReservationID {}",
                    subscriptionId,
                    reservation.getId());
            throw SqlUtils.findServerError(sqlException)
                    .map(serverError -> {
                        if (serverError.getMessage() == null || serverError.getDetail() == null) {
                            log.warn(
                                    "Cannot retrieve ErrorDetails for SubscriptionID {} and ReservationID {}",
                                    subscriptionId,
                                    reservation.getId());
                            return sqlException;
                        }
                        ...
                        if (Objects.equals(serverError.getMessage(), SubscriptionUsageExceeded.ERROR)) {
                            return new SubscriptionUsageExceeded(
                                    errorDetails.getAllowed(), errorDetails.getRequested());
                        }
                        return new SubscriptionUsageExceededForEvent(
                                errorDetails.getAllowed(), errorDetails.getRequested());
                    })
                    .orElse(sqlException);
        }
    ...
    public boolean validateAccessToReservation(TicketReservation reservation, Principal principal) {
        return ticketReservationRepository
                .getReservationOwnerAndOrganizationId(reservation.getId())
                .map(owner -> {
    ...
    public Optional<SubscriptionWithUsageDetails> findSubscriptionDetails(String reservationId) {
        return subscriptionRepository.findSubscriptionsByReservationId(reservationId).stream()
                .findFirst()
                .map(s -> {
                    int usageCount = ticketRepository.countSubscriptionUsage(s.getId(), null);
                    return new SubscriptionWithUsageDetails(
                            s,
    ...
    private Integer retrievePublicUserId(Principal principal) {
        ...
        if (configurationManager.isPublicOpenIdEnabled() && principal != null) {
            userId = userRepository
                    .findPublicUserIdByUsername(principal.getName())
                    .orElse(null);
        }
        return userId;
    }
