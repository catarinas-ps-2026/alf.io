...
public class AdminReservationManager {
    ...
    private static final ErrorCode ERROR_CANNOT_CANCEL_CHECKED_IN_TICKETS = ErrorCode.custom(
            "remove-reservation.failed", "This reservation contains checked-in tickets. Unable to cancel it.");
    ...
    public AdminReservationManager(
    ...
    // the following methods have an explicit transaction handling, therefore the @Transactional annotation is not
    // helpful here
    Result<Triple<TicketReservation, List<Ticket>, PurchaseContext>> confirmReservation(
        ...
        Result<String> result = template.execute(status -> {
            try {
                Result<String> confirmationResult = purchaseContextManager
                        .findBy(purchaseContextType, eventName)
                        .map(purchaseContext -> ticketReservationRepository
                ...
                if (!confirmationResult.isSuccess()) {
                    log.debug(
                            "Reservation confirmation failed for eventName: {} reservationId: {}, username: {}",
                            eventName,
                            reservationId,
                            username);
    ...
    public Result<Triple<TicketReservation, List<Ticket>, PurchaseContext>> confirmReservation(
            PurchaseContextType purchaseContextType,
            String eventName,
            String reservationId,
            String username,
    ...
    public Result<Boolean> updateReservation(
            ...
            String reservationId,
            AdminReservationModification adminReservationModification,
            String username) {
        DefaultTransactionDefinition definition = new DefaultTransactionDefinition();
        TransactionTemplate template = new TransactionTemplate(transactionManager, definition);
        return template.execute(status -> {
            try {
                Result<Boolean> result = purchaseContextManager
                        .findBy(purchaseContextType, publicIdentifier)
                        .map(event -> ticketReservationRepository
                                .findOptionalReservationById(reservationId)
                                .map(r ->
                                        performUpdate(reservationId, event, r, adminReservationModification, username))
                                .orElseGet(() -> Result.error(ErrorCode.ReservationError.UPDATE_FAILED)))
                        .orElseGet(() -> Result.error(ErrorCode.ReservationError.NOT_FOUND));
                if (!result.isSuccess()) {
                    log.debug(
    ...
    public Result<Pair<TicketReservation, List<Ticket>>> createReservation(
            AdminReservationModification input, String eventName, String username) {
        ...
        return template.execute(status -> {
            ...
            try {
                Result<Pair<TicketReservation, List<Ticket>>> result = eventRepository
                        .findOptionalByShortNameForUpdate(eventName)
                ...
                if (!result.isSuccess()) {
                    log.warn(
                            ...
                            eventName,
                            username,
                            AdminReservationModification.summary(input));
        ...
        return getEventTicketReservationPair(purchaseContextType, publicIdentifier, reservationId, username)
                .map(pair -> {
                    ...
                    if (notification.isCustomer()) {
                        ticketReservationManager.sendConfirmationEmail(
                                purchaseContext,
                                reservation,
    ...
    private Optional<Pair<PurchaseContext, TicketReservation>> getEventTicketReservationPair(
            PurchaseContextType purchaseContextType, String publicIdentifier, String reservationId, String username) {
        return purchaseContextManager
                .findBy(purchaseContextType, publicIdentifier)
                .flatMap(ev -> ticketReservationRepository
                        .findOptionalReservationById(reservationId)
                        .map(r -> Pair.of(ev, r)));
    }
    ...
    private void sendTicketToAttendees(Event event, TicketReservation reservation, Predicate<Ticket> matcher) {
        ticketRepository.findTicketsInReservation(reservation.getId()).stream()
                ...
                .forEach(t -> {
                    ...
                    reservationEmailContentHelper.sendTicketByEmail(
                            t,
                            locale,
                            event,
                            ticketReservationManager.getTicketEmailGenerator(
                                    event, reservation, locale, additionalInfo));
                });
    }

    private Result<Boolean> performUpdate(
            String reservationId,
        ...
        billingDocumentManager.ensureBillingDocumentIsPresent(
                ...
                r,
                username,
                () -> ticketReservationManager.orderSummaryForReservationId(reservationId, purchaseContext));
        ticketReservationRepository.updateValidity(
                reservationId,
                Date.from(arm.getExpiration()
                        .toZonedDateTime(purchaseContext.getZoneId())
                        .toInstant()));
        if (arm.isUpdateContactData()) {
            ...
            ticketReservationRepository.updateTicketReservation(
                    ...
                    customerData.getUserLanguage(),
                    customerData.getBillingAddress(),
                    r.getConfirmationTimestamp(),
                    Optional.ofNullable(r.getPaymentMethod())
                            .map(PaymentProxy::name)
                            .orElse(null),
                    customerData.getCustomerReference());

            if (StringUtils.isNotBlank(customerData.getVatNr())
                    || StringUtils.isNotBlank(customerData.getVatCountryCode())) {
                ticketReservationRepository.updateBillingData(
                        ...
                        r.getDiscountCts(),
                        r.getCurrencyCode(),
                        customerData.getVatNr(),
                        customerData.getVatCountryCode(),
                        r.isInvoiceRequested(),
                        reservationId);
            }

            ticketReservationRepository.updateInvoicingAdditionalInformation(
                    reservationId, Json.toJson(arm.getCustomerData().getInvoicingAdditionalInfo()));
        }
        ...
        if (arm.isUpdateAdvancedBillingOptions() && purchaseContext.getVatStatus() != PriceContainer.VatStatus.NONE) {
            ...
            if (vatApplicationRequested) {
                ...
                newVatStatus = purchaseContext.getVatStatus() == PriceContainer.VatStatus.INCLUDED
                        ? PriceContainer.VatStatus.INCLUDED_EXEMPT
                        : PriceContainer.VatStatus.NOT_INCLUDED_EXEMPT;
            }

            if (newVatStatus != ObjectUtils.firstNonNull(r.getVatStatus(), purchaseContext.getVatStatus())) {
                auditingRepository.insert(
                        reservationId,
                        userRepository.getByUsername(username).getId(),
                        purchaseContext,
                        Audit.EventType.FORCE_VAT_APPLICATION,
                        new Date(),
                        Audit.EntityType.RESERVATION,
                        reservationId,
                        singletonList(singletonMap("vatStatus", newVatStatus)));
                ticketReservationRepository.addReservationInvoiceOrReceiptModel(reservationId, null);
                var newPrice = ticketReservationManager
                        .totalReservationCostWithVAT(r.withVatStatus(newVatStatus))
                        .getLeft();
                ticketReservationRepository.resetVat(
                        reservationId,
                        r.isInvoiceRequested(),
                        newVatStatus,
                        r.getSrcPriceCts(),
                        newPrice.getPriceWithVAT(),
                        newPrice.getVAT(),
                        Math.abs(newPrice.getDiscount()),
                        r.getCurrencyCode());
        ...
        arm.getTicketsInfo().stream()
                ...
                .forEach(a -> {
                    ...
                    auditingRepository.insert(
                            ...
                            d,
                            TICKET,
                            Integer.toString(a.getTicketId()),
                            singletonList(modifications));
                });
        ...
        if (purchaseContext.getType() == PurchaseContextType.subscription && arm.getSubscriptionDetails() != null) {
            ...
            var subscription = subscriptionRepository
                    .findFirstSubscriptionByReservationIdForUpdate(reservationId)
                    .orElseThrow();
            subscriptionRepository.updateSubscription(
                    ...
                    subscriptionDetailsModification.getValidityFrom() != null
                            ? subscriptionDetailsModification
                                    .getValidityFrom()
                                    .toZonedDateTime(subscription.getZoneId())
                            : null,
                    subscriptionDetailsModification.getValidityTo() != null
                            ? subscriptionDetailsModification.getValidityTo().toZonedDateTime(subscription.getZoneId())
                            : null);
        }
    ...
    @Transactional
    public Result<Triple<TicketReservation, List<Ticket>, PurchaseContext>> loadReservation(
            PurchaseContextType purchaseContextType, String publicIdentifier, String reservationId, String username) {
        return purchaseContextManager
                .findBy(purchaseContextType, publicIdentifier)
    ...
    @Transactional
    public List<Integer> getTicketIdsWithAdditionalData(
            PurchaseContextType purchaseContextType, String publicIdentifier, String reservationId) {
    ...
    public Optional<Pair<Event, Ticket>> loadFullTicketInfo(
            String reservationId, String eventShortName, String ticketUUID) {
        return purchaseContextManager
                .findBy(PurchaseContextType.event, eventShortName)
                .flatMap(event -> ticketRepository
                        .findOptionalByUUID(ticketUUID)
    ...
    private Result<String> performConfirmation(
            String reservationId,
            PurchaseContext purchaseContext,
        ...
        try {
            ...
            if (subscriptionId != null && purchaseContext.ofType(PurchaseContextType.event)) {
                if (reservation.getVatStatus() == null && purchaseContext.getVatStatus() != null) {
                    ...
                    reservation =
                            ticketReservationManager.findById(reservationId).orElseThrow();
                }
                ...
                boolean result = ticketReservationManager.validateAndApplySubscriptionCode(
                        purchaseContext,
                        reservation,
                ...
                if (!result) {
                    ...
                    log.warn(
                            "Unable to apply subscription {}: {}",
                            subscriptionId,
                            bindingResult.getAllErrors().stream()
                                    .map(ObjectError::getCode)
                                    .collect(joining(", ")));
                    return Result.error(ErrorCode.custom(
                            message,
                            "Cannot assign subscription %s to Reservation %s"
                                    .formatted(subscriptionId, reservationId)));
                }
            ...
            PaymentSpecification spec = new PaymentSpecification(
                    ...
                    new CustomerName(
                            ...
                            purchaseContext.mustUseFirstAndLastName()),
            ...
            if (transactionDetails.getPaymentProvider() != PaymentProxy.ADMIN) {
                var timestamp = Objects.requireNonNullElseGet(
                                transactionDetails.getTimestamp(), () -> LocalDateTime.now(clockProvider.getClock()))
                        .atZone(purchaseContext.getZoneId());
                if (transactionRepository.transactionExists(reservationId)) {
                    paymentManager.updateTransactionDetails(
                            reservationId, transactionDetails.getNotes(), timestamp, null);
                } else {
            ...
            ticketReservationManager.completeReservation(
                    spec,
    ...
    Result<Pair<Event, AdminReservationModification>> validateTickets(AdminReservationModification input, Event event) {
        Set<String> keys = input.getTicketsInfo().stream()
                .flatMap(ti -> ti.getAttendees().stream())
    ...
    private Result<Pair<TicketReservation, List<Ticket>>> transactionalCreateReservation(
            AdminReservationModification input, Event event, String username) {
        return optionally(() -> {
    ...
    private Result<Pair<TicketReservation, List<Ticket>>> processReservation(
            AdminReservationModification input, String username, Event event) {
    ...
    private Result<Pair<TicketReservation, List<Ticket>>> createReservation(
            Result<List<TicketsInfo>> input, Event event, AdminReservationModification arm) {
        final TicketsInfo empty = new TicketsInfo(null, null, false, false);
        return input.flatMap(t -> {
            String reservationId = UUID.randomUUID().toString();
            Date validity = Date.from(
                    arm.getExpiration().toZonedDateTime(event.getZoneId()).toInstant());
            ...
            ticketReservationRepository.updateTicketReservation(
                    reservationId,
                    TicketReservationStatus.PENDING.name(),
                    customerData.getEmailAddress(),
                    customerData.getFullName(),
                    customerData.getFirstName(),
                    customerData.getLastName(),
    ...
    private Result<List<Ticket>> reserveForTicketsInfo(
            Event event,
            AdminReservationModification arm,
            String reservationId,
            Pair<TicketCategory, TicketsInfo> pair) {
        ...
        List<Integer> reservedForUpdate = ticketReservationManager.reserveTickets(
                event.getId(), categoryId, attendees.size(), singletonList(Ticket.TicketStatus.FREE));
        ...
        ticketRepository.reserveTickets(
                reservationId, reservedForUpdate, category, arm.getLanguage(), event.getVatStatus(), i -> null);
        Ticket ticket = ticketRepository.findById(reservedForUpdate.get(0), categoryId);
        TicketPriceContainer priceContainer =
                TicketPriceContainer.from(ticket, null, event.getVat(), event.getVatStatus(), null);
        ...
        List<SpecialPrice> codes =
                category.isAccessRestricted() ? bindSpecialPriceTokens(categoryId, attendees) : Collections.emptyList();
        ...
        assignTickets(
                ...
                arm.getLanguage(),
                category.getSrcPriceCts());
        List<Ticket> tickets = reservedForUpdate.stream()
                .map(id -> ticketRepository.findById(id, categoryId))
                .collect(toList());
    ...
    private Stream<Pair<TicketCategory, TicketsInfo>> flattenTicketsInfo(
            Event event, TicketsInfo empty, List<TicketsInfo> t) {
        return t.stream().collect(groupingBy(ti -> ti.getCategory().getExistingCategoryId())).entrySet().stream()
                .map(entry -> {
                    TicketsInfo ticketsInfo = entry.getValue().stream()
                            .reduce((ti1, ti2) -> {
                                ...
                                return new TicketsInfo(
                                        ti1.getCategory(),
                                        attendees,
                                        ti1.isAddSeatsIfNotAvailable() && ti2.isAddSeatsIfNotAvailable(),
                                        ti1.isUpdateAttendees() && ti2.isUpdateAttendees());
                            })
                            .orElse(empty);
                    return Pair.of(
                            ticketCategoryRepository.getByIdAndActive(entry.getKey(), event.getId()), ticketsInfo);
                });
    }
    ...
    private List<SpecialPrice> bindSpecialPriceTokens(int categoryId, List<Attendee> attendees) {
        ...
        List<SpecialPrice> codes =
                specialPriceRepository.findActiveNotAssignedByCategoryId(categoryId, attendees.size());

        codes.forEach(c ->
                specialPriceRepository.updateStatus(c.getId(), SpecialPrice.Status.PENDING.toString(), null, null));
        return codes;
    }
    ...
    private void assignTickets(
        ...
        Optional<Iterator<SpecialPrice>> specialPriceIterator =
                Optional.of(codes).filter(c -> !c.isEmpty()).map(Collection::iterator);
        for (int i = 0; i < reservedForUpdate.size(); i++) {
            ...
            if (!attendee.isEmpty()) {
                ...
                if (!attendee.getMetadata().isEmpty()) {
                    ...
                    ticketRepository.updateTicketMetadata(
                            ticketId, TicketMetadataContainer.fromMetadata(ticketMetadata));
                }
            }
            specialPriceIterator
                    .map(Iterator::next)
                    .ifPresent(code -> ticketRepository.reserveTicket(
                            reservationId,
                            ticketId,
                            code.getId(),
                            userLanguage,
    ...
    private void updateExtRefAndLocking(int categoryId, Attendee attendee, Integer ticketId) {
        try {
            ticketRepository.updateExternalReferenceAndLocking(
                    ticketId,
                    categoryId,
                    StringUtils.trimToNull(attendee.getReference()),
                    attendee.isReassignmentForbidden());
        } catch (DataIntegrityViolationException ex) {
            log.warn("Duplicate found for external reference: " + attendee.getReference() + " and ticketID: "
                    + ticketId);
            throw new DuplicateReferenceException("Duplicated Reference: " + attendee.getReference(), ex);
        }
    }

    private Result<List<TicketsInfo>> checkCategoryCapacity(
            TicketsInfo ti, Event event, AdminReservationModification reservation, String username) {
        Result<TicketCategory> ticketCategoryResult = ti.getCategory().isExisting()
                ? checkExistingCategory(ti, event, username)
                : createCategory(ti, event, reservation, username);
        return ticketCategoryResult.map(tc -> List.of(new TicketsInfo(
                new Category(tc.getId(), tc.getName(), tc.getPrice(), tc.getTicketAccessType()),
                ti.getAttendees(),
                ti.isAddSeatsIfNotAvailable(),
                ti.isUpdateAttendees())));
    }

    private Result<TicketCategory> createCategory(
            TicketsInfo ti, Event event, AdminReservationModification reservation, String username) {
        ...
        var accessType = event.getFormat() != Event.EventFormat.HYBRID
                ? TicketCategory.TicketAccessType.INHERIT
                : Objects.requireNonNull(category.getTicketAccessType());
        TicketCategoryModification tcm = new TicketCategoryModification(
                category.getExistingCategoryId(),
                category.getName(),
                accessType,
                tickets,
                inception,
        ...
        return eventManager
                .insertCategory(modified, tcm, username)
    ...
    private Result<TicketCategory> checkExistingCategory(TicketsInfo ti, Event event, String username) {
        ...
        if (freeTicketsInCategory < tickets && existing.isBounded()) {
            ...
            TicketCategoryModification tcm = new TicketCategoryModification(
                    existingCategoryId,
                    existing.getName(),
                    existing.getTicketAccessType(),
                    maxTickets,
                    fromZonedDateTime(existing.getInception(modified.getZoneId())),
                    fromZonedDateTime(existing.getExpiration(event.getZoneId())),
                    Collections.emptyMap(),
                    existing.getPrice(),
    ...
    private void createMissingTickets(Event event, int tickets) {
        final MapSqlParameterSource[] params = generateEmptyTickets(
                        event, Date.from(event.now(clockProvider).toInstant()), tickets, Ticket.TicketStatus.FREE)
    ...
    public Optional<Ticket> findTicketWithReservationId(String ticketUUID, String eventSlug, String username) {
        return eventManager
                .getOptionalEventAndOrganizationIdByName(eventSlug, username)
                .flatMap(eventAndOrganizationId -> {
                    var ticket = ticketRepository.findByUUID(ticketUUID);
                    // check that ticket belongs to the event
                    if (ticket.getEventId() != eventAndOrganizationId.getId()
                            || StringUtils.isEmpty(ticket.getTicketsReservationId())) {
    ...
    @Transactional
    public Result<Boolean> removeTickets(
            String publicIdentifier,
            String reservationId,
            List<Integer> ticketIds,
            List<Integer> toRefund,
            boolean notify,
            boolean creditNoteRequested,
            String username) {
        return loadReservation(PurchaseContextType.event, publicIdentifier, reservationId, username)
                .map(res -> {
                    ...
                    Set<Integer> ticketIdsInReservation =
                            tickets.stream().map(Ticket::getId).collect(toSet());
                    // ensure that all the tickets ids are present in tickets
                    Assert.isTrue(
                            ticketIdsInReservation.containsAll(ticketIds),
                            "Some ticket ids are not contained in the reservation");
                    Assert.isTrue(
                            ticketIdsInReservation.containsAll(toRefund),
                            "Some ticket ids to refund are not contained in the reservation");
                    if (!ticketsStatusIsCompatibleWithCancellation(
                            tickets.stream().filter(t -> ticketIds.contains(t.getId())))) {
                    ...
                    boolean issueCreditNote = (creditNoteRequested
                            &&
                            // if payment method supports refund we require that the user has selected at least one
                            // ticket
                            (!toRefund.isEmpty()
                                    || !reservation.getPaymentMethod().isSupportRefund()));
                    removeTicketsFromReservation(
                            reservation, e, ticketIds, notify, username, removeReservation, issueCreditNote);
                    //

                    if (removeReservation) {
                        markAsCancelled(reservation, username, e);
                        additionalServiceItemRepository.updateItemsStatusWithReservationUUID(
                                e.getId(),
                                reservation.getId(),
                        ...
                        var discount = reservation.getPromoCodeDiscountId() != null
                                ? promoCodeDiscountRepository.findById(reservation.getPromoCodeDiscountId())
                                : null;
                        List<AdditionalServiceItem> additionalServiceItems =
                                additionalServiceItemRepository.findByReservationUuid(e.getId(), reservationId);
                        var calculator = new ReservationPriceCalculator(
                                reservation,
                                discount,
                                updatedTickets,
                                additionalServiceItems,
                                additionalServiceRepository.loadAllForEvent(e.getId()),
                                e,
                                List.of(),
                                Optional.empty());
                        ticketReservationRepository.updateBillingData(
                                calculator.getVatStatus(),
                                calculator.getSrcPriceCts(),
                                unitToCents(calculator.getFinalPrice(), currencyCode),
                                unitToCents(calculator.getVAT(), currencyCode),
                                unitToCents(calculator.getAppliedDiscount(), currencyCode),
                                calculator.getCurrencyCode(),
                                reservation.getVatNr(),
                                reservation.getVatCountryCode(),
    ...
    @Transactional
    public Optional<Pair<SubscriptionDescriptor, String>> findReservationIdForSubscription(
            String subscriptionDescriptorId, UUID subscriptionId, Principal principal) {
        return purchaseContextManager
                .findBy(PurchaseContextType.subscription, subscriptionDescriptorId)
                .flatMap(purchaseContext -> {
                    ...
                    if (subscription.getSubscriptionDescriptorId().equals(descriptor.getId())
                            && StringUtils.isNotBlank(subscription.getReservationId())) {
    ...
    public Result<Boolean> removeSubscription(
            SubscriptionDescriptor descriptor, String reservationId, UUID subscriptionId, String username) {
        ...
        return result == 1
                ? Result.success(true)
                : Result.error(ErrorCode.custom("cannot-cancel-subscription", "Cannot cancel subscription"));
    }

    @Transactional(readOnly = true)
    public Result<List<Audit>> getAudit(
            PurchaseContextType purchaseContextType, String publicIdentifier, String reservationId, String username) {
    ...
    @Transactional(readOnly = true)
    public Result<Pair<BillingDocument, byte[]>> getSingleBillingDocumentAsPdf(
            PurchaseContextType purchaseContextType,
            String publicIdentifier,
            String reservationId,
            long documentId,
        ...
        return loadReservation(purchaseContextType, publicIdentifier, reservationId, username)
                .map(res -> {
                    BillingDocument billingDocument = billingDocumentRepository
                            .findByIdAndReservationId(documentId, reservationId)
                            .orElseThrow(IllegalArgumentException::new);
                    Function<Map<String, Object>, Optional<byte[]>> pdfGenerator =
                            model -> TemplateProcessor.buildBillingDocumentPdf(
                                    billingDocument.getType(),
                                    res.getRight(),
                                    fileUploadManager,
                                    LocaleUtil.forLanguageTag(res.getLeft().getUserLanguage()),
                                    templateManager,
                                    model,
                                    extensionManager);
                    Map<String, Object> billingModel = billingDocument.getModel();
                    return Pair.of(
                            billingDocument, pdfGenerator.apply(billingModel).orElse(null));
                });
    }

    @Transactional
    public Result<Boolean> invalidateBillingDocument(String reservationId, long documentId, String username) {
        return updateBillingDocumentStatus(
                reservationId,
                documentId,
                username,
                BillingDocument.Status.NOT_VALID,
                Audit.EventType.BILLING_DOCUMENT_INVALIDATED);
    }
    ...
    public Result<Boolean> restoreBillingDocument(String reservationId, long documentId, String username) {
        return updateBillingDocumentStatus(
                ...
                Audit.EventType.BILLING_DOCUMENT_RESTORED);
    }

    private Result<Boolean> updateBillingDocumentStatus(
            String reservationId,
            long documentId,
            String username,
    ...
    public Result<TransactionAndPaymentInfo> getPaymentInfo(String reservationId) {
        return loadReservation(reservationId).map(res -> paymentManager.getInfo(res.getLeft(), res.getRight()));
    }

    @Transactional
    public Result<Boolean> removeReservation(
            PurchaseContextType purchaseContextType,
            String eventName,
            String reservationId,
            boolean refund,
        ...
        return loadReservation(purchaseContextType, eventName, reservationId, username)
                .flatMap(result -> new Result.Builder<Pair<PurchaseContext, TicketReservation>>()
                        .checkPrecondition(
                                () -> ticketsStatusIsCompatibleWithCancellation(result.getMiddle()),
                                ERROR_CANNOT_CANCEL_CHECKED_IN_TICKETS)
                        .buildAndEvaluate(() -> {
                            ...
                            if (creditNoteRequested && reservation.getHasInvoiceNumber()) {
                                ticketReservationManager.issueCreditNoteForReservation(
                                        result.getRight(), reservation, username, false);
                            }
        ...
        loadReservation(purchaseContextType, publicIdentifier, reservationId, username)
                .ifSuccess(res -> {
                    var offlinePaymentEnumValues = EnumSet.of(
                            TicketReservationStatus.OFFLINE_PAYMENT, TicketReservationStatus.CUSTOM_OFFLINE_PAYMENT);
                    if (offlinePaymentEnumValues.contains(res.getLeft().getStatus())) {
                        ticketReservationManager.deleteOfflinePayment(
                                res.getRight().event().orElseThrow(), reservationId, false, true, notify, username);
                    } else {
    ...
    private Result<Pair<PurchaseContext, TicketReservation>> removeReservation(
        ...
        return new Result.Builder<Triple<TicketReservation, List<Ticket>, PurchaseContext>>()
                .checkPrecondition(
                        () -> ticketsStatusIsCompatibleWithCancellation(triple.getMiddle()),
                        ERROR_CANNOT_CANCEL_CHECKED_IN_TICKETS)
                .buildAndEvaluate(() -> {
                ...
                .map(t -> {
                    ...
                    if (purchaseContext.ofType(PurchaseContextType.event)) {
                        ...
                        removeTicketsFromReservation(
                                reservation,
                                event,
                                tickets.stream().map(Ticket::getId).collect(toList()),
                                notify,
                                username,
                                removeReservation,
                                issueCreditNote);
    ...
    private ErrorCode refundIfRequested(
            TicketReservation reservation, PurchaseContext purchaseContext, String username, boolean requested) {
        if (requested
                && reservation.getPaymentMethod() != null
                && reservation.getPaymentMethod().isSupportRefund()) {
            // fully refund
        ...
        return loadReservation(purchaseContextType, publicIdentifier, reservationId, username)
                .map(res -> {
                    ...
                    if (reservation.getHasInvoiceNumber()) {
                        ticketReservationManager.issueCreditNoteForRefund(
                                purchaseContext, reservation, refundAmount, username);
                    }
                    return reservation.getPaymentMethod() != null
                            ...
                            && paymentManager.refund(
                                    reservation,
                                    purchaseContext,
                                    unitToCents(refundAmount, reservation.getCurrencyCode()),
                                    username);
                });
    }

    @Transactional
    public Result<Boolean> regenerateBillingDocument(
            PurchaseContextType purchaseContextType, String publicIdentifier, String reservationId, String username) {
        return loadReservation(purchaseContextType, publicIdentifier, reservationId, username)
                .map(res -> {
                    ...
                    if (billingDocument.getType() != BillingDocument.Type.CREDIT_NOTE) {
                        billingDocumentRepository.invalidateAllPreviousDocumentsOfType(
                                billingDocument.getType(), billingDocument.getId(), reservationId);
                    }
                    return true;
                });
    }
    ...
    @Transactional
    public Result<List<LightweightMailMessage>> getEmailsForReservation(
            PurchaseContextType purchaseContextType, String publicIdentifier, String reservationId, String username) {
        return loadReservation(purchaseContextType, publicIdentifier, reservationId, username)
    ...
    private void removeTicketsFromReservation(
        ...
        if (!ticketIds.isEmpty() && issueCreditNote && reservation.getHasInvoiceNumber()) {
            ticketReservationManager.issuePartialCreditNoteForReservation(
                    purchaseContext, reservation, username, ticketIds);
        }
        ...
        if (!issueCreditNote || !reservation.getHasInvoiceNumber()) {
            billingDocumentManager.ensureBillingDocumentIsPresent(
                    ...
                    reservation,
                    username,
                    () -> ticketReservationManager.orderSummaryForReservation(reservation, purchaseContext));
        }

        Integer userId = userRepository.findIdByUserName(username).orElse(null);
        Date date = new Date();

        ticketIds.forEach(id -> auditingRepository.insert(
                reservationId, userId, purchaseContext.getId(), CANCEL_TICKET, date, TICKET, id.toString()));
    ...
    private void sendTicketHasBeenRemoved(Event event, Organization organization, Ticket ticket) {
        ...
        notificationManager.sendSimpleEmail(
                ...
                messageSourceManager
                        ...
                        .getMessage("email-ticket-released.subject", new Object[] {event.getDisplayName()}, locale),
                () -> templateManager.renderTemplate(event, TemplateResource.TICKET_HAS_BEEN_CANCELLED, model, locale));
    }

    private void markAsCancelled(
            TicketReservation ticketReservation, String username, PurchaseContext purchaseContext) {
    ...
    private void markAsCancelled(String reservationId, String username, PurchaseContext purchaseContext) {
        ticketReservationRepository.updateReservationStatus(
                reservationId, TicketReservationStatus.CANCELLED.toString());
        auditingRepository.insert(
                reservationId,
    ...
    private void handleTicketsRefund(
            List<Integer> toRefund,
            Event e,
            TicketReservation reservation,
            Map<Integer, Ticket> ticketsById,
