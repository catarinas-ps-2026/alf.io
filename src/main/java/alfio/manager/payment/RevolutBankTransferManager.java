...
public class RevolutBankTransferManager implements PaymentProvider, OfflineProcessor, PaymentInfo {
    ...
    private static final Cache<String, List<String>> accountsCache =
    ...
    @Override
    public Result<List<String>> checkPendingReservations(
        ...
        var host = options.get(REVOLUT_LIVE_MODE).getValueAsBooleanOrDefault()
        ...
        if (revolutKeyOptional.isEmpty()) {
            return Result.error(
                    ErrorCode.custom("unavailable", "cannot retrieve Revolut API KEY. Please fix configuration"));
        }
        ...
        return loadAccounts(revolutKey, host)
                ...
                .flatMap(revolutTransactions -> matchTransactions(
                        reservations,
                        revolutTransactions,
                        context,
                        options.get(REVOLUT_MANUAL_REVIEW).getValueAsBooleanOrDefault()));
    }

    Result<List<String>> matchTransactions(
            Collection<TicketReservationWithTransaction> pendingReservations,
            List<RevolutTransactionDescriptor> transactions,
            PaymentContext context,
            boolean manualReviewRequired) {
        List<Pair<TicketReservationWithTransaction, RevolutTransactionDescriptor>> matched =
                pendingReservations.stream()
                        .map(reservation -> Pair.of(
                                reservation,
                                transactions.stream()
                                        .filter(transactionMatches(reservation, context))
                                        .findFirst()))
                        .filter(pair -> pair.getRight().isPresent())
                        .map(pair -> Pair.of(pair.getLeft(), pair.getRight().orElseThrow()))
                        .collect(Collectors.toList());

        return Result.success(matched.stream()
                .map(pair -> {
                    ...
                    if (latestTransaction.isEmpty()
                            || latestTransaction.get().getId() != transaction.getId()
                            || latestTransaction.get().getStatus() != transaction.getStatus()) {
                        log.trace(
                                "transaction {} has been modified while processing. Current status is: {}",
                                transaction.getId(),
                                latestTransaction.map(t -> t.getStatus().name()).orElse("N/A"));
                        return null;
                    }
    ...
    private Predicate<RevolutTransactionDescriptor> transactionMatches(
            TicketReservationWithTransaction reservationWithTransaction, PaymentContext context) {
        ...
        var shortReservationId = configurationManager
                .getShortReservationID(context.getPurchaseContext(), reservation)
                .toLowerCase();
        String[] terms;
        ...
        return revolutTransaction -> revolutTransaction.getTransactionBalance().compareTo(BigDecimal.ZERO) > 0
                && Arrays.stream(terms)
                        .anyMatch(s ->
                                revolutTransaction.getReference().toLowerCase().contains(s))
                && transaction
                        .getCurrency()
                        .equals(revolutTransaction.getLegs().get(0).getCurrency())
                && transaction.getPriceInCents()
                        == MonetaryUtil.unitToCents(
                                revolutTransaction.getTransactionBalance(), transaction.getCurrency());
    }

    private Result<List<RevolutTransactionDescriptor>> loadTransactions(
            ZonedDateTime lastCheck, String revolutKey, String revolutUrl, List<String> accounts) {
        if (accounts.isEmpty()) {
            return Result.error(ErrorCode.custom("no-account", "No active accounts found."));
        }
        try {
            var from = lastCheck != null
                    ? lastCheck.withZoneSameInstant(clockProvider.getClock().getZone())
                    : ZonedDateTime.now(clockProvider.getClock()).minusDays(1); // defaults to now - 24h
            var request = HttpRequest.newBuilder(URI.create(
                            revolutUrl + "/api/1.0/transactions?from=" + from.format(JSON_DATETIME_FORMATTER)))
                    .GET()
            ...
            if (response.statusCode() == HttpStatus.OK.value()) {
                ...
                return Result.success(result.stream()
                        .filter(t -> "completed".equals(t.getState())
                                && t.getLegs().size() == 1
                                && accounts.contains(t.getLegs().get(0).getAccountId()))
                        .collect(Collectors.toList()));
            }
            return Result.error(ErrorCode.custom(
                    "no data received", "No data received from Revolut. Status code is " + response.statusCode()));
        ...
        try {
            ...
            if (response.statusCode() == HttpStatus.OK.value()) {
                ...
                return result.stream()
                        .filter(m -> "active".equals(m.get("state")))
    ...
    public Optional<PaymentInformation> getInfo(Transaction transaction, PurchaseContext purchaseContext) {
        ...
        if (metadata != null && metadata.containsKey("counterpartyAccountId")) {
            return Optional.of(new PaymentInformation(
                    MonetaryUtil.formatCents(transaction.getPriceInCents(), transaction.getCurrency()),
                    null,
