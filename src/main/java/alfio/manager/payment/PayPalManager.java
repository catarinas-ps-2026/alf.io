...
public class PayPalManager implements PaymentProvider, RefundRequest, PaymentInfo, ExtractPaymentTokenFromTransaction {
    ...
    private final Cache<String, PayPalHttpClient> cachedClients =
    ...
    private String generateKey(PayPalEnvironment environment) {
        return DigestUtils.sha256Hex(
                environment.baseUrl() + "::" + environment.clientId() + "::" + environment.clientSecret());
    }

    private PayPalEnvironment getApiContext(Configurable configurable) {
        var paypalConf = configurationManager.getFor(
                Set.of(
                        ConfigurationKeys.PAYPAL_LIVE_MODE,
        ...
        String clientSecret =
                paypalConf.get(ConfigurationKeys.PAYPAL_CLIENT_SECRET).getRequiredValue();
        return isLive
    ...
    private String createCheckoutRequest(PaymentSpecification spec) throws Exception {
        ...
        String hmac = computeHMAC(
                spec.getCustomerName(), spec.getEmail(), spec.getBillingAddress(), spec.getPurchaseContext());
        ...
        OrderRequest orderRequest = new OrderRequest()
                ...
                .purchaseUnits(List.of(new PurchaseUnitRequest()
                        .amountWithBreakdown(new AmountWithBreakdown()
                                .currencyCode(spec.getCurrencyCode())
                                .value(spec.getOrderSummary().getTotalPrice()))));
            ...
            if ("APPROVED".equals(status) || "COMPLETED".equals(status)) {
                if ("APPROVED".equals(status)) {
                    saveToken(
                            ...
                            new PayPalToken(order.payer().payerId(), order.id(), hmac));
                }
                ...
                ticketReservationRepository.updateValidity(
                        spec.getReservationId(), DateUtils.addMinutes(reservation.getValidity(), 15));
    ...
    private static String computeHMAC(
            CustomerName customerName, String email, String billingAddress, PurchaseContext purchaseContext) {
        return new HmacUtils(HmacAlgorithms.HMAC_SHA_256, purchaseContext.getPrivateKey())
                .hmacHex(StringUtils.trimToEmpty(customerName.getFullName())
                        + StringUtils.trimToEmpty(email)
                        + StringUtils.trimToEmpty(billingAddress));
    }

    private static boolean isValidHMAC(
        ...
        return MessageDigest.isEqual(
                hmac.getBytes(StandardCharsets.UTF_8), computedHmac.getBytes(StandardCharsets.UTF_8));
    }
    ...
    private static final Set<String> MAPPED_ERROR =
            Set.of("FAILED_TO_CHARGE_CC", "INSUFFICIENT_FUNDS", "EXPIRED_CREDIT_CARD", "INSTRUMENT_DECLINED");
    ...
    private PayPalChargeDetails commitPayment(
            String reservationId, PayPalToken payPalToken, PurchaseContext purchaseContext) throws HttpException {

        try {
            OrdersCaptureRequest request =
                    new OrdersCaptureRequest(payPalToken.getPaymentId()).payPalRequestId(reservationId);
            ...
            if (HttpUtils.statusCodeIsSuccessful(response.statusCode())) {
                ...
                // if we are at this stage, the only possible options are approved or failed, thus it's safe to re
                // transition the reservation to a pending status: no payment has been made!
                if (!"COMPLETED".equals(result.status())) {
                    log.warn(
                            "error in state for reservationId {}, expected 'approved' state, but got '{}'",
                            reservationId,
                            result.status());
                    throw new IllegalStateException();
                }
                ...
                var payPalFee = captureOptional
                        .map(Capture::sellerReceivableBreakdown)
    ...
    private Optional<PaymentInformation> getInfo(
            Transaction transaction, PurchaseContext purchaseContext, Supplier<String> platformFeeSupplier) {
        ...
        try {
            if (paymentId != null) {
                ...
                if (HttpUtils.statusCodeIsSuccessful(orderResponse.statusCode()) && orderResponse.result() != null) {
                    ...
                    var refund = payments.stream()
                            .flatMap(p -> emptyIfNull(p.refunds()).stream())
                            .map(r -> new BigDecimal(r.amount().value()))
                            .reduce(BigDecimal.ZERO, BigDecimal::add);
                    var gatewayFee = payments.stream()
                            .flatMap(p -> emptyIfNull(p.captures()).stream())
                            .map(c -> {
                            ...
                            .reduce(BigDecimal.ZERO, BigDecimal::add);
                    var paidAmount = payments.stream()
                            .flatMap(p -> emptyIfNull(p.captures()).stream())
                            .map(r -> new BigDecimal(r.amount().value()))
                            .reduce(BigDecimal.ZERO, BigDecimal::add);
                    return Optional.of(new PaymentInformation(
                            formatUnit(paidAmount, currency),
                            formatUnit(refund, currency),
                            String.valueOf(unitToCents(gatewayFee, currency)),
    ...
    @Override
    public Optional<PaymentInformation> getInfo(
            alfio.model.transaction.Transaction transaction, PurchaseContext purchaseContext) {
        return getInfo(transaction, purchaseContext, () -> {
            ...
            return FeeCalculator.getCalculator(purchaseContext, configurationManager, transaction.getCurrency())
                    .apply(ticketRepository.countTicketsInReservation(transaction.getReservationId()), (long)
                            transaction.getPriceInCents())
                    .map(String::valueOf)
                    .orElse("0");
        });
    }
    ...
    public boolean refund(
            alfio.model.transaction.Transaction transaction, PurchaseContext purchaseContext, Integer amountToRefund) {
        ...
        try {
            ...
            String currency = transaction.getCurrency();
            String amountOrFull =
                    amount.map(a -> MonetaryUtil.formatCents(a, currency)).orElse("full");
            amount.ifPresent(i -> refundRequest.requestBody(new com.paypal.payments.RefundRequest()
                    .amount(new com.paypal.payments.Money()
                            .currencyCode(currency)
                            .value(formatCents(i, currency)))));
            ...
            if (HttpUtils.statusCodeIsSuccessful(refundResponse.statusCode())) {
                ...
                log.warn(
                        "Paypal: was not able to refund payment with id {} [HTTP {}]",
                        captureId,
    ...
    public PaymentResult doPayment(PaymentSpecification spec) {
        try {
            ...
            long applicationFee = FeeCalculator.getCalculator(
                            spec.getPurchaseContext(), configurationManager, spec.getCurrencyCode())
                    .apply(ticketRepository.countTicketsInReservation(spec.getReservationId()), (long)
                            spec.getPriceWithVAT())
                    .orElse(0L);

            log.info("PayPalManager::: doPayment {} ", spec.getReservationId());
            var existingTransaction = transactionRepository
                    .loadOptionalByReservationIdAndStatus(spec.getReservationId(), Transaction.Status.COMPLETE)
                    .orElse(null);
            if (existingTransaction != null
                    && existingTransaction.getPaymentProxy() == PaymentProxy.PAYPAL
                    && existingTransaction.getTransactionId().equals(chargeDetails.captureId)) {
                ...
                transactionRepository.insert(
                        chargeDetails.captureId,
                        chargeDetails.orderId,
                        ...
                        applicationFee,
                        chargeDetails.payPalFee,
                        alfio.model.transaction.Transaction.Status.COMPLETE,
                        Map.of());
            }
    ...
    public void saveToken(String reservationId, PurchaseContext purchaseContext, PayPalToken token) {
        ...
        transactionRepository.insert(
                ...
                0,
                0,
                alfio.model.transaction.Transaction.Status.PENDING,
                Map.of(PaymentManager.PAYMENT_TOKEN, json.asJsonString(token)));
    }
    ...
    public void removeToken(TicketReservation reservation, String paymentId) {
        ...
        if (optionalTransaction.isPresent()) {
            ...
            if (StringUtils.equals(transaction.getPaymentId(), paymentId)
                    && transaction.getPaymentProxy() == PaymentProxy.PAYPAL
                    && transaction.getStatus() == Transaction.Status.PENDING) {
                PaymentManagerUtils.invalidateExistingTransactions(
                        reservation.getId(), transactionRepository, PaymentProxy.PAYPAL);
            }
        } else {
