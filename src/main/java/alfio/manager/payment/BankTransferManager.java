...
public class BankTransferManager implements PaymentProvider {
    ...
    private static final EnumSet<ConfigurationKeys> OPTIONS_TO_LOAD = EnumSet.of(
    ...
    boolean bankTransferEnabledForMethod(
            PaymentMethod paymentMethod,
            PaymentContext paymentContext,
            Map<ConfigurationKeys, ConfigurationManager.MaybeConfiguration> options) {
    ...
    boolean bankTransferActive(
            PaymentContext paymentContext, Map<ConfigurationKeys, ConfigurationManager.MaybeConfiguration> options) {
        return options.get(BANK_TRANSFER_ENABLED).getValueAsBooleanOrDefault()
                && (paymentContext.getPurchaseContext() == null
                        || getOfflinePaymentWaitingPeriod(
                                                paymentContext.getPurchaseContext(),
                                                options.get(OFFLINE_PAYMENT_DAYS)
    ...
    public Map<String, ?> getModelOptions(PaymentContext context) {
        ...
        boolean recaptchaEnabled = configurationManager.isRecaptchaForOfflinePaymentAndFreeEnabled(
    ...
    void postponePayment(
            PaymentSpecification spec, TicketReservation.TicketReservationStatus status, ZonedDateTime deadline) {
    ...
    public static ZonedDateTime getOfflinePaymentDeadline(
            PaymentContext context, ConfigurationManager configurationManager) {
        PurchaseContext purchaseContext = context.getPurchaseContext();
        var configuration = configurationManager.getFor(
                EnumSet.of(OFFLINE_PAYMENT_DAYS, OFFLINE_PAYMENT_BUSINESS_DAYS),
                purchaseContext.getConfigurationLevel());
        ZonedDateTime now = purchaseContext.now(ClockProvider.clock());
        int waitingPeriod = getOfflinePaymentWaitingPeriod(
                        purchaseContext, configuration.get(OFFLINE_PAYMENT_DAYS).getValueAsIntOrDefault(5))
                .orElse(0);
        if (waitingPeriod == 0) {
            log.warn(
        ...
        if (configuration.get(OFFLINE_PAYMENT_BUSINESS_DAYS).getValueAsBooleanOrDefault()) {
            return ZonedDateTime.from(
                    WorkingDaysAdjusters.addDays(now.truncatedTo(ChronoUnit.HALF_DAYS), waitingPeriod));
        } else {
            return now.truncatedTo(ChronoUnit.HALF_DAYS).plusDays(waitingPeriod);
        }
    }

    public static OptionalInt getOfflinePaymentWaitingPeriod(
            PaymentContext paymentContext, ConfigurationManager configurationManager) {
        PurchaseContext purchaseContext = paymentContext.getPurchaseContext();
        return getOfflinePaymentWaitingPeriod(
                purchaseContext,
                configurationManager
                        .getFor(OFFLINE_PAYMENT_DAYS, purchaseContext.getConfigurationLevel())
                        .getValueAsIntOrDefault(5));
    }

    private static OptionalInt getOfflinePaymentWaitingPeriod(PurchaseContext purchaseContext, int configuredValue) {
        ZonedDateTime now = purchaseContext.now(ClockProvider.clock());
        ZonedDateTime maxDate = purchaseContext
                .event()
