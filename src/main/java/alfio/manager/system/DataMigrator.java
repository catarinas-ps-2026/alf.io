...
public class DataMigrator {
    ...
    static {
        PRICE_UPDATE_BY_KEY.put(
                "event", "update event set src_price_cts = :srcPriceCts, vat_status = :vatStatus where id = :eventId");
        PRICE_UPDATE_BY_KEY.put(
                "category", "update ticket_category set src_price_cts = :srcPriceCts where id = :categoryId");
        PRICE_UPDATE_BY_KEY.put(
                "ticket",
    ...
    public DataMigrator(
            EventMigrationRepository eventMigrationRepository,
    ...
    private void fixVatStatus() {
        transactionTemplate.execute(ts -> {
            int rows = jdbc.update(
                    "update tickets_reservation set vat_status = e.vat_status from event e where tickets_reservation.vat_status is null and tickets_reservation.event_id_fk = e.id",
                    Map.of());
    ...
    private void fixReservationPrice(List<Event> events) {
        transactionTemplate.execute(ts -> {
            Map<Integer, List<String>> candidates = jdbc
                    .queryForList(
                            "select id, event_id_fk from tickets_reservation where event_id_fk is not null and src_price_cts = 0 and payment_method <> 'NONE' and status not in ('CANCELLED', 'CREDIT_NOTE_ISSUED') order by 2",
                            Map.of())
                    .stream()
    ...
    private void fixReservationsForEvent(Event event, List<String> reservations) {
        var byReservationId =
                ticketSearchRepository.loadAllReservationsWithTickets(event.getId(), reservations).stream()
        ...
        if (!byReservationId.isEmpty()) {
            ...
            var reservationsToUpdate = byReservationId.values().stream()
                    .map(ticketsReservationAndTransactions -> {
                        var tickets = ticketsReservationAndTransactions.stream()
                                .map(TicketWithReservationAndTransaction::getTicket)
                                .collect(toList());
                        var ticketReservation =
                                ticketsReservationAndTransactions.get(0).getTicketReservation();
                        var promoCodeDiscountId = ticketReservation.getPromoCodeDiscountId();
                        var discount = promoCodeDiscountId != null
                                ? promoCodeDiscountRepository.findById(promoCodeDiscountId)
                                : null;
                        var additionalServiceItems = additionalServiceItemRepository.findByReservationUuid(
                                event.getId(), ticketReservation.getId());
                        ...
                        return new MapSqlParameterSource("reservationId", calculator.reservation.getId())
                                ...
                                .addValue(
                                        "finalPrice",
                                        MonetaryUtil.unitToCents(calculator.getFinalPrice(), currencyCode))
                                .addValue(
                                        "discount",
                                        MonetaryUtil.unitToCents(calculator.getAppliedDiscount(), currencyCode))
            ...
            int[] results = jdbc.batchUpdate(
                    "update tickets_reservation set src_price_cts = :srcPrice, final_price_cts = :finalPrice, discount_cts = :discount, vat_cts = :vat, currency_code = :currencyCode where id = :reservationId",
                    reservationsToUpdate);
            int sum = IntStream.of(results).sum();
            if (sum != reservationsToUpdate.length) {
                log.warn(
                        "Expected {} reservations to be affected, actual number: {}", reservationsToUpdate.length, sum);
            }
        }
    }
    ...
    private void fillDefaultOptions() {
        transactionTemplate.execute(ts -> {
            Integer count = jdbc.queryForObject(
                    "select count(*) from configuration where c_key = :key",
            ...
            if (count == null || count == 0) {
                configurationRepository.insert(
                        ConfigurationKeys.GOOGLE_ANALYTICS_ANONYMOUS_MODE.getValue(),
                        "true",
    ...
    private void migrateEventToCurrentVersion(Event event) {
        ...
        if (!alreadyDefined || optional.filter(this::needsFixing).isPresent()) {
            transactionTemplate.execute(s -> {
                // optional.ifPresent(eventMigration ->
                ...
                // migrate prices to new structure. This should be done for all events, regardless of the expiration
                // date.
                ...
                if (alreadyDefined) {
                    ...
                    int result = eventMigrationRepository.updateMigrationData(
    ...
    private void createBillingDocuments(Event event) {
        if (event.getEnd().isAfter(event.now(clockProvider))) {
            List<String> reservations = jdbc.queryForList(
                    "select id from tickets_reservation where event_id_fk = :eventId and status in ('OFFLINE_PAYMENT', 'COMPLETE') and invoice_number is not null and id not in(select distinct reservation_id_fk from billing_document where event_id_fk = :eventId)",
            ...
            for (String reservationId : reservations) {
                ...
                billingDocumentManager.getOrCreateBillingDocument(
                        event,
                        reservation,
                        null,
    ...
    void fixStuckTickets(int eventId) {
        List<Integer> ticketIds = jdbc.queryForList(
                "select a.id from ticket a, tickets_reservation b where a.event_id = :eventId and a.status in('PENDING','TO_BE_PAID') and a.tickets_reservation_id = b.id and b.status = 'CANCELLED'",
        ...
        if (!ticketIds.isEmpty()) {
            ...
            Validate.isTrue(
                    result == toBeFixed, "Error while fixing stuck tickets: expected " + toBeFixed + ", got " + result);
        }
    }
        ...
        ticketCategoryRepository.findAllTicketCategories(event.getId()).stream()
                ...
                .forEach(tc -> {
                    Integer result = jdbc.queryForObject(
                            ...
                            new MapSqlParameterSource("eventId", tc.getEventId()).addValue("categoryId", tc.getId()),
                            Integer.class);
                    if (result != null && result != tc.getMaxTickets()) {
                        log.warn(
                                "********* updating category size for {} from {} to {} tickets",
                                tc.getName(),
                                tc.getMaxTickets(),
                                result);
    ...
    void fillReservationsLanguage() {
        transactionTemplate.execute(s -> {
            jdbc.queryForList(
                            "select id from tickets_reservation where user_language is null and event_id_fk is not null",
    ...
    private void fixAvailableSeats(Event event) {
        try {
            ...
            var paymentProxies = event.getAllowedPaymentProxies().stream()
    ...
    boolean needsFixing(EventMigration eventMigration) {
        return eventMigration.getBuildTimestamp().isBefore(buildTimestamp)
                || parseVersion(eventMigration.getCurrentVersion()).compareTo(currentVersion) < 0;
    }
    ...
    private void migratePrices(final int eventId) {
        ...
        Map<String, List<MapSqlParameterSource>> migrationData = jdbc
                .queryForList(
                        "select * from event where type = :type and id = :eventId and regular_price_cts > 0",
                        new MapSqlParameterSource("type", "INTERNAL").addValue("eventId", eventId))
                .stream()
                .flatMap(event -> {
                    // fill the event prices
                    boolean eventVatIncluded = (boolean) event.get("vat_included");
                    BigDecimal vatPercentage =
                            Optional.ofNullable((BigDecimal) event.get("vat")).orElse(BigDecimal.ZERO);
                    ...
                    // ticket categories
                    modifications.addAll(collectTicketCategoryMigrationData(
                            srcPriceCtsParam, eventVatIncluded, vatPercentage, eventIdParam));

                    // tickets
                    modifications.addAll(collectTicketMigrationData(
                            srcPriceCtsParam, eventVatIncluded, vatPercentage, currencyCode, eventId, eventIdParam));

                    // additional_service
                    modifications.addAll(
                            collectASMigrationData(srcPriceCtsParam, eventVatIncluded, vatPercentage, eventIdParam));

                    // additional_service_item
                    modifications.addAll(collectASItemMigrationData(
                            srcPriceCtsParam, eventVatIncluded, vatPercentage, currencyCode, eventIdParam));

                    log.debug(
                            "Price migration: got {} modifications for event {}",
                            modifications.size(),
    ...
    private static void performPriceMigration(
            String updateStatement, List<MapSqlParameterSource> data, NamedParameterJdbcTemplate jdbc) {
        int size = data.size();
        jdbc.batchUpdate(updateStatement, data.toArray(new MapSqlParameterSource[size]));
        log.debug("{} records updated", size);
    }

    private List<Pair<String, MapSqlParameterSource>> collectASItemMigrationData(
            String srcPriceCtsParam,
            final boolean eventVatIncluded,
            final BigDecimal vatPercentage,
            final String currencyCode,
            Map<String, Integer> eventIdParam) {
        return jdbc
                .queryForList(
                        "select ai.id as id, ai.paid_price_cts as paid_price_cts, a.vat_type as vat_type from additional_service_item ai, additional_service a where ai.paid_price_cts > 0 and ai.src_price_cts = 0 and ai.event_id_fk = :eventId",
                        eventIdParam)
                .stream()
                .map(item -> {
                    int oldPrice = (int) item.get("paid_price_cts");
                    AdditionalService.VatType vatType =
                            AdditionalService.VatType.valueOf((String) item.get("vat_type"));
                    return Pair.of((Integer) item.get("id"), new PriceContainer() {
                        ...
                        public VatStatus getVatStatus() {
                            ...
                            return NONE; // no need to check for other VatTypes. At the time of writing VAT override
                ...
                .map(p -> {
                    ...
                    return Pair.of(
                            "additional_service_item",
                            new MapSqlParameterSource(srcPriceCtsParam, priceContainer.getSrcPriceCts())
                                    .addValue(
                                            "finalPriceCts",
    ...
    private List<Pair<String, MapSqlParameterSource>> collectASMigrationData(
            String srcPriceCtsParam,
        ...
        return jdbc
                ...
                .map(as -> {
                    ...
                    int srcPrice = vatType == AdditionalService.VatType.INHERITED && eventVatIncluded
                            ? MonetaryUtil.addVAT(priceCts, vatPercentage)
                            : priceCts;
                    return Pair.of(
                            "additional_service",
                            new MapSqlParameterSource(srcPriceCtsParam, srcPrice)
                                    .addValue("additionalServiceId", as.get("id")));
                })
                .collect(toList());
    }

    private List<Pair<String, MapSqlParameterSource>> collectTicketMigrationData(
            String srcPriceCtsParam,
            final boolean eventVatIncluded,
            final BigDecimal vatPercentage,
            final String currencyCode,
        ...
        return jdbc
                ...
                .map(ticket -> {
                    ...
                    return Pair.of((Integer) ticket.get("id"), new PriceContainer() {
                        @Override
                        public int getSrcPriceCts() {
                            return eventVatIncluded
                                    ? MonetaryUtil.addVAT(oldTicketPrice, vatPercentage)
                                    : oldTicketPrice;
                        }
                        ...
                        public Optional<PromoCodeDiscount> getDiscount() {
                            return Optional.ofNullable(ticket.get("discount_amount"))
                                    .map(amount -> new PromoCodeDiscount(
                                            0,
                                            "",
                                            eventId,
                                            null,
                                            ...
                                            null,
                                            (int) amount,
                                            PromoCodeDiscount.DiscountType.valueOf(
                                                    (String) ticket.get("discount_type")),
                                            "",
                                            null,
                                            null,
                                            null,
    ...
    private List<Pair<String, MapSqlParameterSource>> collectTicketCategoryMigrationData(
            String srcPriceCtsParam,
            boolean eventVatIncluded,
            BigDecimal vatPercentage,
            Map<String, Integer> eventIdParam) {
        return jdbc
                ...
                .map(category -> {
                    ...
                    int categorySrcPrice =
                            eventVatIncluded ? MonetaryUtil.addVAT(oldCategoryPrice, vatPercentage) : oldCategoryPrice;
                    return Pair.of(
                            "category",
                            new MapSqlParameterSource(srcPriceCtsParam, categorySrcPrice)
                                    .addValue("categoryId", category.get("id")));
                })
                .collect(toList());
    }
}
