...
public class WaitingQueueManager {
    ...
    public boolean subscribe(
            Event event, CustomerName customerName, String email, Integer selectedCategoryId, Locale userLanguage) {
        try {
            if (configurationManager
                    .getFor(STOP_WAITING_QUEUE_SUBSCRIPTIONS, ConfigurationLevel.event(event))
            ...
            AffectedRowCountAndKey<Integer> key = waitingQueueRepository.insert(
                    event.getId(),
    ...
    private void validateSelectedCategoryId(int eventId, Integer selectedCategoryId) {
        Optional.ofNullable(selectedCategoryId)
                .ifPresent(id ->
                        Validate.isTrue(ticketCategoryRepository.findUnboundedOrderByExpirationDesc(eventId).stream()
    ...
    private void notifySubscription(
        ...
        notificationManager.sendSimpleEmail(
                ...
                messageSource.getMessage(
                        "email-waiting-queue.subscribed.subject", new Object[] {event.getDisplayName()}, userLanguage),
                () -> templateManager.renderTemplate(
                        event, TemplateResource.WAITING_QUEUE_JOINED, model, userLanguage));
        if (configurationManager
                .getFor(ENABLE_WAITING_QUEUE_NOTIFICATION, ConfigurationLevel.event(event))
                .getValueAsBooleanOrDefault()) {
            String adminTemplate = messageSource.getMessage(
                    "email-waiting-queue.subscribed.admin.text",
                    new Object[] {subscriptionType, event.getDisplayName()},
                    Locale.ENGLISH);
            notificationManager.sendSimpleEmail(
                    event,
                    null,
                    ...
                    messageSource.getMessage(
                            "email-waiting-queue.subscribed.admin.subject",
                            new Object[] {event.getDisplayName()},
                            Locale.ENGLISH),
                    () -> RenderedTemplate.plaintext(
                            templateManager.renderString(
                                    event, adminTemplate, model, Locale.ENGLISH, TemplateManager.TemplateOutput.TEXT),
                            model));
        }
    }
        ...
        if (type == WaitingQueueSubscription.Type.PRE_SALES) {
            Validate.isTrue(
                    configurationManager
                            .getFor(ENABLE_PRE_REGISTRATION, ConfigurationLevel.event(event))
                            .getValueAsBooleanOrDefault(),
                    "PRE_SALES Waiting list is not active");
        } else {
            Validate.isTrue(
                    eventStatisticsManager.noSeatsAvailable().test(event), "SOLD_OUT Waiting list is not active");
        }
    }
    ...
    public Optional<WaitingQueueSubscription> updateSubscriptionStatus(
            int id, WaitingQueueSubscription.Status newStatus, WaitingQueueSubscription.Status currentStatus) {
    ...
    Stream<Triple<WaitingQueueSubscription, TicketReservationWithOptionalCodeModification, ZonedDateTime>>
            distributeSeats(Event event) {
    ...
    private Stream<Triple<WaitingQueueSubscription, TicketReservationWithOptionalCodeModification, ZonedDateTime>>
            handlePreReservation(Event event, int waitingPeople) {
        List<TicketCategory> ticketCategories = ticketCategoryRepository.findAllTicketCategories(event.getId());
        // Given that this Job runs more than once in a minute, in order to ensure that all the waiting list subscribers
        // would get a seat *before*
        ...
        Optional<TicketCategory> categoryWithInceptionInFuture = ticketCategories.stream()
                ...
                .filter(t -> event.now(clockProvider)
    ...
    private void preReserveTickets(Event event, int ticketsNeeded, int eventId, int alreadyReserved) {
        ...
        Map<Integer, TicketCategoryStatisticView> ticketCategoriesStats =
                ticketCategoryRepository.findStatisticsForEventIdByCategoryId(eventId);
        List<Pair<Integer, TicketCategoryStatisticView>> collectedTickets =
    ...
    private Stream<Triple<WaitingQueueSubscription, TicketReservationWithOptionalCodeModification, ZonedDateTime>>
            distributeAvailableSeats(Event event, int waitingPeople, int waitingTickets) {
        return distributeAvailableSeats(
                event, Ticket.TicketStatus.RELEASED, () -> Math.min(waitingPeople, waitingTickets));
    }

    private Stream<Triple<WaitingQueueSubscription, TicketReservationWithOptionalCodeModification, ZonedDateTime>>
            distributeAvailableSeats(Event event, Ticket.TicketStatus status, IntSupplier availableSeatSupplier) {
        ...
        Iterator<Ticket> tickets =
                ticketRepository.selectWaitingTicketsForUpdate(eventId, status.name(), availableSeats).stream()
                        .filter(t -> t.getCategoryId() != null || !unboundedCategories.isEmpty())
                        .iterator();
        int expirationTimeout = configurationManager
                .getFor(WAITING_QUEUE_RESERVATION_TIMEOUT, ConfigurationLevel.event(event))
                .getValueAsIntOrDefault(4);
        ZonedDateTime expiration =
                event.now(clockProvider).plusHours(expirationTimeout).with(WorkingDaysAdjusters.defaultWorkingDays());
    ...
    private Optional<TicketCategory> findBestCategory(
            List<TicketCategory> unboundedCategories, WaitingQueueSubscription subscription) {
