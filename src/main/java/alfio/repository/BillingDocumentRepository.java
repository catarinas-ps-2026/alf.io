...
public interface BillingDocumentRepository {
    ...
    @Query(
            "select * from billing_document where reservation_id_fk = :reservationId and status = 'VALID' order by generation_ts desc limit 1")
    ...
    Optional<BillingDocument> findByIdAndReservationId(
            @Bind("id") long documentId, @Bind("reservationId") String reservationId);
    ...
    int updateStatus(
            @Bind("id") long documentId,
            @Bind("status") BillingDocument.Status status,
            @Bind("reservationId") String reservationId);

    @Query(
            "update billing_document set status = 'NOT_VALID' where type = :type and id < :currentId and reservation_id_fk = :reservationId")
    int invalidateAllPreviousDocumentsOfType(
            @Bind("type") BillingDocument.Type type,
            @Bind("currentId") Long currentId,
            @Bind("reservationId") String reservationId);

    @Query(
            "select * from billing_document where reservation_id_fk = :reservationId order by generation_ts desc, id desc")
    ...
    @Query(
        ...
        """)
    List<Integer> findMatchingInvoiceIds(
            @Bind("eventId") int eventId, @Bind("start") ZonedDateTime start, @Bind("end") ZonedDateTime end);

    @Query(
            "select min(generation_ts) from billing_document where event_id_fk = :eventId and type = 'INVOICE' and status = 'VALID'")
    ...
    Boolean checkBillingDocumentExistsForReservation(
            @Bind("id") long billingDocumentId, @Bind("reservationId") String reservationId);
