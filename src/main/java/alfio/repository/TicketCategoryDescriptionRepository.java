...
public interface TicketCategoryDescriptionRepository {
    ...
    Optional<String> findByTicketCategoryIdAndLocale(
            @Bind("ticketCategoryId") int ticketCategoryId, @Bind("locale") String locale);

    @Query(
            "insert into ticket_category_text(ticket_category_id_fk, locale, description) values (:ticketCategoryId, :locale, :description)")
    int insert(
    ...
    List<TicketCategoryDescription> findByTicketCategoryIds(
            @Bind("ticketCategoryIds") Collection<Integer> ticketCategoryIds);

    default Map<String, String> descriptionForTicketCategory(int ticketCategory) {
        return findByTicketCategoryId(ticketCategory).stream()
                .collect(Collectors.toMap(
                        TicketCategoryDescription::getLocale, TicketCategoryDescription::getDescription));
    }
