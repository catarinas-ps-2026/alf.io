...
public interface AdditionalServiceItemRepository {
    ...
    String UPDATE_STATUS =
            "update additional_service_item set status = :status where event_id_fk = :eventId and tickets_reservation_uuid = :reservationUuid";
    String FIND_BY_RESERVATION_ID =
            "select * from additional_service_item where event_id_fk = :eventId and tickets_reservation_uuid = :reservationUuid";
    ...
    List<AdditionalServiceItem> findByReservationUuid(
            @Bind("eventId") int eventId, @Bind("reservationUuid") String reservationUuid);

    @Query(FIND_BY_RESERVATION_ID + " and ticket_id_fk = :ticketId")
    List<AdditionalServiceItem> findByTicketId(
            @Bind("eventId") int eventId,
            @Bind("reservationUuid") String reservationUuid,
            @Bind("ticketId") int ticketId);

    @Query(UPDATE_STATUS)
    int updateItemsStatusWithReservationUUID(
            @Bind("eventId") int eventId,
            @Bind("reservationUuid") String reservationUuid,
            @Bind("status") AdditionalServiceItemStatus status);

    @Query(UPDATE_STATUS + " and ticket_id_fk = :ticketId")
    int updateItemsStatusWithTicketId(
            @Bind("eventId") int eventId,
            @Bind("reservationUuid") String reservationUuid,
            @Bind("ticketId") int ticketId,
            @Bind("status") AdditionalServiceItemStatus status);
    ...
    @Query(
            "select count(*) from additional_service_item where event_id_fk = :eventId and tickets_reservation_uuid = :reservationId")
    ...
    List<BookedAdditionalService> getAdditionalServicesBookedForReservation(
            @Bind("reservationId") String reservationId,
            @Bind("language") String language,
            @Bind("eventId") int eventId);

    @Query(SELECT_BOOKED_ADDITIONAL_SERVICES + "  and ai.ticket_id_fk = :ticketId " + "  group by ads.id, asd.value")
    List<BookedAdditionalService> getAdditionalServicesBookedForTicket(
            @Bind("reservationId") String reservationId,
            @Bind("ticketId") int ticketId,
            @Bind("language") String language,
            @Bind("eventId") int eventId);
    ...
    @Query(
        ...
        """)
    List<AdditionalServiceItemExport> getAdditionalServicesOfTypeForEvent(
            @Bind("eventId") int eventId,
            @Bind("additionalServiceType") String additionalServiceType,
            @Bind("locale") String locale);

    @Query(
            "select count(*) from additional_service_item where additional_service_id_fk = :serviceId and status <> 'INVALIDATED'")
    ...
    @Query(
            "update additional_service_item set status = 'INVALIDATED' where id in (select id from additional_service_item where additional_service_id_fk = :serviceId and status = 'FREE' limit :count)")
    int invalidateItems(@Bind("serviceId") int additionalServiceId, @Bind("count") int count);

    @Query(
            "select id from additional_service_item where additional_service_id_fk = :serviceId and status = 'FREE' limit :count for update skip locked")
    ...
    @Query(
        ...
        """)
    int deleteAdditionalServiceItemsByReservationId(
    ...
    @Query(
        ...
        """)
    int revertAdditionalServiceItemsByReservationId(
            @Bind("eventId") int eventId, @Bind("reservationId") String reservationId);
    ...
    @Query(
        ...
        """)
    int countMatchingItemsForTicket(
            @Bind("ticketUUID") UUID ticketPublicUUID, @Bind("additionalServiceIds") Collection<Integer> ids);
}
