...
public interface EventRepository {
    ...
    String STATISTICS_QUERY =
            "select count(*) as total_attendees, COALESCE(SUM(CASE WHEN status = 'CHECKED_IN' THEN 1 ELSE 0 END), 0) as checked_in, CURRENT_TIMESTAMP as last_update from ticket where event_id = :eventId and status in("
                    + TicketRepository.CONFIRMED + ")";
    ...
    AffectedRowCountAndKey<Integer> insert(
            @Bind("shortName") String shortName,
    ...
    @Query(
            "select a.* from event a inner join tickets_reservation on event_id_fk = a.id where tickets_reservation.id = :reservationId")
    ...
    Integer countEventsInOrganization(
            @Bind("organizationId") int organizationId, @Bind("eventShortNames") Collection<String> shortNames);

    @Query("select * from event where org_id = :organizationId and short_name in (:eventShortNames)")
    List<Event> findEventsByShortNames(
            @Bind("organizationId") int organizationId, @Bind("eventShortNames") Collection<String> shortNames);

    @Query(
            value =
                    "update event set status = 'DISABLED' where org_id in (select org_id from j_user_organization where user_id in (:userIds)) returning id",
            type = QueryType.MODIFYING_WITH_RETURN)
    List<Integer> disableEventsForUsers(@Bind("userIds") Collection<Integer> userIds);

    @Query(
            value = "update event set status = 'DISABLED' where org_id = :orgId returning id",
            type = QueryType.MODIFYING_WITH_RETURN)
    List<Integer> disableEventsForOrganization(@Bind("orgId") int orgId);

    @Query(
            "select coalesce(sum(final_price_cts),0) from tickets_reservation where event_id_fk = :eventId and status = 'COMPLETE'")
    ...
    @Query(
        ...
        """)
    List<Event> findVisibleBySearchOptions(
