...
public interface TicketCategoryRepository {
    ...
    @Query(
            "select * from ticket_category_with_currency where event_id = :eventId and category_code = :code and tc_status = 'ACTIVE'")
    ...
    @Query("select * from ticket_category_with_currency where " + CHECK_ACTIVE
            + " and bounded = false order by expiration desc")
    List<TicketCategory> findUnboundedOrderByExpirationDesc(@Bind("eventId") int eventId);

    @Query(
            "select * from ticket_category_with_currency where event_id = :eventId  and tc_status = 'ACTIVE' order by ordinal asc, inception asc, expiration asc, id asc")
    ...
    @Query("select id, metadata from ticket_category where " + CHECK_ACTIVE
            + " order by ordinal asc, inception asc, expiration asc, id asc")
    ...
    TicketCategoryStatisticView findStatisticWithId(
            @Bind("ticketCategoryId") int ticketCategoryId, @Bind("eventId") int eventId);
    ...
    default Map<Integer, TicketCategoryStatisticView> findStatisticsForEventIdByCategoryId(int eventId) {
        return findStatisticsForEventId(eventId).stream()
                .collect(Collectors.toMap(TicketCategoryStatisticView::getId, Function.identity()));
    }
    ...
    @Query(
            "update ticket_category set tc_status = 'NOT_ACTIVE' from (select count(*) cnt from ticket where category_id = :categoryId and status in ('PENDING', 'ACQUIRED', 'CHECKED_IN')) tkts where id = :categoryId and tkts.cnt = 0")
    int deleteCategoryIfEmpty(@Bind("categoryId") int categoryId);

    @Query(
            type = QueryType.TEMPLATE,
            value = "update ticket_category set ordinal = :ordinal where id = :id and event_id = :eventId")
    ...
    @Query(
            "select distinct tc.* from ticket_category_with_currency tc join ticket t on t.category_id = tc.id where t.tickets_reservation_id = :reservationId")
    ...
    @Query(
            "update ticket_category set ticket_access_type = :ticketAccessType::ticket_access_type where event_id = :eventId")
    void updateTicketAccessTypeForEvent(
            @Bind("eventId") int eventId, @Bind("ticketAccessType") TicketCategory.TicketAccessType ticketAccessType);
    ...
    @Query(
            "select count(*) from ticket_category tc join event e on tc.event_id = e.id where e.short_name = :shortName and tc.id in (:categoryIds)")
