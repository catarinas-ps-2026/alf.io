...
public interface SpecialPriceRepository {
    ...
    String IS_FREE =
            " status = 'FREE' and recipient_name is null and recipient_email is null and access_code_id_fk is null";
    ...
    @Query("select * from special_price where ticket_category_id = :ticketCategoryId and " + IS_FREE
            + " for update skip locked limit :limitTo")
    List<SpecialPrice> findActiveByCategoryIdForUpdate(
            @Bind("ticketCategoryId") int ticketCategoryId, @Bind("limitTo") int limitTo);

    @Query(SELECT_FREE + " limit :limitTo for update skip locked")
    List<SpecialPrice> findActiveNotAssignedByCategoryId(
            @Bind("ticketCategoryId") int ticketCategoryId, @Bind("limitTo") int limitTo);
    ...
    @Query(
            type = QueryType.MODIFYING_WITH_RETURN,
            value = "update special_price set access_code_id_fk = :accessCodeId where id in ("
                    + "select id from special_price where ticket_category_id = :ticketCategoryId and "
                    + IS_FREE + " and access_code_id_fk is null limit :limitTo" + ") returning *")
    List<SpecialPrice> bindToAccessCode(
            @Bind("ticketCategoryId") int ticketCategoryId,
            @Bind("accessCodeId") int accessCodeId,
            @Bind("limitTo") int limitTo);
    ...
    int markAsSent(
            ...
            @Bind("recipientName") String recipientName,
            @Bind("recipientAddress") String recipientAddress,
            @Bind("code") String code);

    @Query(
            "update special_price set sent_ts = null, recipient_name = null, recipient_email = null where id = :id and ticket_category_id = :ticketCategoryId")
    ...
    @Query(
            "update special_price set status = :status, session_id = null, access_code_id_fk = :accessCodeId where id in (:ids)")
    int batchUpdateStatus(
            @Bind("ids") List<Integer> ids,
            @Bind("status") SpecialPrice.Status status,
            @Bind("accessCodeId") Integer accessCodeId);

    @Query(
            "update special_price set status = :status, session_id = :sessionId, access_code_id_fk = :accessCodeId where id = :id")
    int updateStatus(
            @Bind("id") int id,
            @Bind("status") String status,
            @Bind("sessionId") String sessionIdentifier,
            @Bind("accessCodeId") Integer accessCodeId);
    ...
    @Query(
            "select id, ticket_category_id from special_price where status = 'WAITING' and ticket_category_id in (select id from ticket_category where tc_status = 'ACTIVE') for update skip locked")
    List<SpecialPrice.SpecialPriceTicketCategoryId> findWaitingElements();

    @Query(
            "select id, ticket_category_id from special_price where status = 'WAITING' and ticket_category_id = :categoryId for update skip locked")
    ...
    default Map<Integer, List<SpecialPrice>> findAllByCategoriesIdsMapped(Collection<Integer> ticketCategoriesIds) {
        ...
        return findAllByCategoriesIds(ticketCategoriesIds).stream()
                .collect(Collectors.groupingBy(SpecialPrice::getTicketCategoryId));
    }
}
