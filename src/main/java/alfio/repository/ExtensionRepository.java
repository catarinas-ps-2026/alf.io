...
public interface ExtensionRepository {
    ...
    @Query(
            "delete from extension_event where es_id_fk = (select es_id from extension_support where path = :path and name = :name)")
    ...
    @Query(
        ...
        """)
    List<ExtensionSupport.ScriptPathNameHash> findActive(
    ...
    @Query(
        ...
        """)
    List<ExtensionSupport.ExtensionParameterKeyValue> findExtensionParameterKeyValue(
            @Bind("extensionId") int extensionId);
    ...
    AffectedRowCountAndKey<Integer> registerExtensionConfigurationMetadata(
            @Bind("extensionId") int extensionId,
            @Bind("name") String name,
    ...
    @Query(
            "delete from extension_configuration_metadata_value where fk_ecm_id in (select ecm_id from extension_configuration_metadata where ECM_CONFIGURATION_LEVEL = :confLevel) and conf_path = :confPath")
    ...
    @Query(
            "insert into extension_configuration_metadata_value(fk_ecm_id, conf_path, conf_value) values (:ecmId, :confPath, :value)")
    ...
    @Query(
        ...
        """)
    List<ExtensionSupport.NameAndValue> findParametersForScript(
            @Bind("name") String name, @Bind("path") String path, @Bind("allPaths") Set<String> allPaths);

    @Query(
            "select distinct ecm_name from extension_configuration_metadata where ecm_es_id_fk = (SELECT es_id from extension_support where path = :path and name = :name) and ecm_mandatory is true")
    ...
    List<ExtensionSupport.ExtensionMetadataIdAndName> findAllParametersForExtension(
            @Bind("extensionId") int extensionId);
    ...
    @Query(
        ...
        select count(a3.*) from \
         ...
         (a1.es_id = a2.es_id) and length(a1.path) < length(a2.path) where a2.path is null) a3 \
        """)
    int countScriptsSupportingCapability(
            @Bind("possiblePaths") Set<String> paths, @Bind("capabilities") List<String> capabilities);
    ...
    @Query(
        ...
        select distinct a3.capability, a3.capability_detail from \
         ...
         (a1.es_id = a2.es_id) and length(a1.path) < length(a2.path) where a2.path is null) a3 \
        """)
    List<ExtensionCapabilitySummary> getSupportedCapabilities(
            @Bind("possiblePaths") Set<String> paths, @Bind("capabilities") Collection<String> capabilities);
    ...
    @Query(
        ...
        """)
    Optional<ExtensionSupport.ScriptPathNameHash> getFirstScriptForCapability(
            @Bind("possiblePaths") Set<String> paths, @Bind("capability") String capability);
}
