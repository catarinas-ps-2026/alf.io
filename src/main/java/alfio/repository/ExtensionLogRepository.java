...
public interface ExtensionLogRepository {
    ...
    @Query(
            "insert into extension_log(effective_path, path, name, description, type) values (:effectivePath, :path, :name, :description, :type)")
    int insert(
            @Bind("effectivePath") String effectivePath,
            @Bind("path") String path,
            @Bind("name") String name,
            @Bind("description") String description,
            @Bind("type") ExtensionLog.Type type);

    String FIND_EXTENSION_LOG =
    ...
    int countPages(@Bind("path") String path, @Bind("name") String name, @Bind("type") String type);

    @Query("select * from (" + FIND_EXTENSION_LOG + " limit :pageSize offset :offset) as el_tbl")
    List<ExtensionLog> getPage(
            @Bind("path") String path,
            @Bind("name") String name,
