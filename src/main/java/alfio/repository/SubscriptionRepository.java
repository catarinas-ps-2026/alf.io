...
public interface SubscriptionRepository {
    ...
    @Query(
        ...
        """)
    int createSubscriptionDescriptor(
            @Bind("id") UUID id,
    ...
    @Query(
        ...
        """)
    int updateSubscriptionDescriptor(
            @Bind("title") @JSONData Map<String, String> title,
            @Bind("description") @JSONData Map<String, String> description,
            @Bind("maxAvailable") int maxAvailable,
            @Bind("onSaleFrom") ZonedDateTime onSaleFrom,
    ...
    @Query(
            "update subscription_descriptor set is_public = :isPublic where id = :id and organization_id_fk = :organizationId")
    int setPublicStatus(
            @Bind("id") UUID id, @Bind("organizationId") int organizationId, @Bind("isPublic") boolean isPublic);

    @Query(
            "update subscription_descriptor set status = 'NOT_ACTIVE' where id = :id and organization_id_fk = :organizationId")
    ...
    List<SubscriptionDescriptor> findAllActiveAndPublic(
            @Bind("from") ZonedDateTime from, @Bind("orgSlug") String organizerSlug);

    @Query(
            "select * from subscription_descriptor where status = 'ACTIVE' and id = :id and organization_id_fk = :organizationId")
    ...
    @Query(
            "select count(*) from subscription_descriptor where status = 'ACTIVE' and id in(:ids) and organization_id_fk = :organizationId")
    Integer countDescriptorsBelongingToOrganization(
            @Bind("ids") Collection<UUID> ids, @Bind("organizationId") int organizationId);

    @Query(
            "select * from subscription_descriptor where id = (select subscription_descriptor_fk from subscription where reservation_id_fk = :reservationId) and status = 'ACTIVE'")
    ...
    @Query(
            "select * from subscription_descriptor where id = (select subscription_descriptor_fk from subscription where id = :id) and status = 'ACTIVE'")
    SubscriptionDescriptor findDescriptorBySubscriptionId(@Bind("id") UUID subscriptionId);

    @Query(
            "select organization_id_fk from subscription_descriptor where id = (select subscription_descriptor_fk from subscription where id = :id) and status = 'ACTIVE'")
    ...
    @Query(
            "select * from subscription_descriptor_statistics where sd_organization_id_fk = :organizationId and sd_id = :subscriptionDescriptorId")
    Optional<SubscriptionDescriptorWithStatistics> findOneWithStatistics(
            @Bind("subscriptionDescriptorId") UUID subscriptionDescriptorId,
            @Bind("organizationId") int organizationId);

    @Query(INSERT_SUBSCRIPTION_LINK)
    int linkSubscriptionAndEvent(
    ...
    List<EventSubscriptionLink> findLinkedEvents(
            @Bind("organizationId") int organizationId, @Bind("subscriptionId") UUID id);

    @Query(FETCH_SUBSCRIPTION_LINK + " where se.event_id_fk = :eventId")
    List<EventSubscriptionLink> findLinkedSubscriptions(
            @Bind("organizationId") int organizationId, @Bind("eventId") int eventId);

    @Query(FETCH_SUBSCRIPTION_LINK
            + " where se.event_id_fk = :eventId and se.subscription_descriptor_id_fk = :subscriptionId")
    Optional<EventSubscriptionLink> findLink(
            @Bind("organizationId") int organizationId, @Bind("subscriptionId") UUID id, @Bind("eventId") int eventId);

    @Query(
            "select subscription_descriptor_id_fk from subscription_event where event_id_fk = :eventId and organization_id_fk = :organizationId")
    ...
    int removeAllSubscriptionsForEvent(@Bind("eventId") int eventId, @Bind("organizationId") int organizationId);

    @Query(
            "delete from subscription_event where subscription_descriptor_id_fk = :subscriptionId and organization_id_fk = :organizationId")
    int removeAllLinksForSubscription(
            @Bind("subscriptionId") UUID subscriptionId, @Bind("organizationId") int organizationId);

    default void preGenerateSubscriptions(
            SubscriptionDescriptorModification subscriptionDescriptor, UUID subscriptionDescriptorId, int quantity) {
        var results = getJdbcTemplate()
                .batchUpdate(
                        INSERT_SUBSCRIPTION,
                        Stream.generate(UUID::randomUUID)
                                ...
                                .map(subscriptionId -> new MapSqlParameterSource("id", subscriptionId)
                                        ...
                                        .addValue(
                                                "validFrom", toOffsetDateTime(subscriptionDescriptor.getValidityFrom()))
    ...
    @Query(
            "select id from subscription where subscription_descriptor_fk = :descriptorId and status = 'FREE' limit 1 for update skip locked")
    ...
    @Query(
        ...
        """)
    int bindSubscriptionToReservation(
            @Bind("reservationId") String reservationId,
            @Bind("srcPriceCts") int price,
            @Bind("status") AllocationStatus allocationStatus,
            @Bind("subscriptionId") UUID subscriptionId);
    ...
    @Query(
            "select count(*) from subscription where substring(replace(id::text,'-',''), 0, 11) like concat(:partialUuid, '%') and email_address = :email")
    int countSubscriptionByPartialUuidAndEmail(@Bind("partialUuid") String partialUuid, @Bind("email") String email);

    @Query(
            "select count(*) from subscription where substring(replace(id::text,'-',''), 0, 11) like concat(:partialUuid, '%')")
    int countSubscriptionByPartialUuid(@Bind("partialUuid") String partialUuid);

    @Query(
            "select id from subscription where substring(replace(id::text,'-',''), 0, 11) like concat(:partialUuid, '%') and email_address = :email")
    UUID getSubscriptionIdByPartialUuidAndEmail(@Bind("partialUuid") String partialUuid, @Bind("email") String email);

    @Query(
            "select id from subscription where substring(replace(id::text,'-',''), 0, 11) like concat(:partialUuid, '%')")
    UUID getSubscriptionIdByPartialUuid(@Bind("partialUuid") String partialUuid);

    @Query(
            "select * from subscription where id = (select subscription_id_fk from tickets_reservation where id = :reservationId)")
    ...
    @Query(
            "update subscription set first_name = :firstName, last_name = :lastName, email_address = :email where reservation_id_fk = :reservationId")
    int assignSubscription(
            @Bind("reservationId") String reservationId,
            @Bind("firstName") String firstName,
            @Bind("lastName") String lastName,
            @Bind("email") String email);
    ...
    @Query(
            "update subscription set status = 'INVALIDATED' where id in (select id from subscription where subscription_descriptor_fk = :descriptorId and status = 'FREE' limit :amount)")
    ...
    default Map<Integer, List<AvailableSubscriptionsByEvent>> loadAvailableSubscriptionsByEvent(
            @Bind("eventId") Integer eventId, @Bind("organizationId") Integer organizationId) {
        Map<Integer, List<AvailableSubscriptionsByEvent>> result = new TreeMap<>();
        var paramSource = new MapSqlParameterSource("eventId", eventId).addValue("organizationId", organizationId);
        getJdbcTemplate()
                .query(
                        """
        ...
        """,
                        ...
                        rse -> {
                            ...
                            result.get(eId)
                                    .add(new AvailableSubscriptionsByEvent(
                                            eId,
    ...
    @Query(
            "update subscription set src_price_cts = :price where subscription_descriptor_fk = :descriptorId and status = 'FREE'")
    int updatePriceForSubscriptions(@Bind("descriptorId") UUID subscriptionDescriptorId, @Bind("price") int priceCts);

    @Query(
            "update subscription set max_entries = :maxEntries, max_usage = :maxEntries where subscription_descriptor_fk = :descriptorId")
    int updateMaxEntriesForSubscriptions(
            @Bind("descriptorId") UUID subscriptionDescriptorId, @Bind("maxEntries") int maxEntries);

    @Query("update subscription set metadata = :metadata::jsonb where id = :id")
    int setMetadataForSubscription(
            @Bind("id") UUID subscriptionId, @Bind("metadata") @JSONData SubscriptionMetadata metadata);
