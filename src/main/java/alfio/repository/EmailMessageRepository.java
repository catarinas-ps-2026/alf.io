...
public interface EmailMessageRepository {
    ...
    @Query(
            "select id from email_message where subscription_descriptor_id_fk = :subscriptionDescriptorId and checksum = :checksum limit 1")
    Optional<Integer> findIdBySubscriptionDescriptorAndChecksum(
            @Bind("subscriptionDescriptorId") UUID subscriptionDescriptorId, @Bind("checksum") String checksum);
    ...
    default Optional<Integer> findIdByPurchaseContextAndChecksum(PurchaseContext purchaseContext, String checksum) {
        if (purchaseContext.ofType(PurchaseContext.PurchaseContextType.event)) {
            ...
            return findIdBySubscriptionDescriptorAndChecksum(
                    ((SubscriptionDescriptor) purchaseContext).getId(), checksum);
        }
    }
    ...
    @Query(
            "update email_message set status = :status where id = :id and checksum = :checksum and status in (:expectedStatuses)")
    int updateStatus(
            @Bind("id") int messageId,
            @Bind("checksum") String checksum,
            @Bind("status") String status,
            @Bind("expectedStatuses") List<String> expectedStatuses);
    ...
    int updateStatusAndAttempts(
            @Bind("messageId") int messageId,
            @Bind("status") String status,
            @Bind("nextDate") ZonedDateTime date,
            @Bind("attempts") int attempts,
            @Bind("expectedStatuses") List<String> expectedStatuses);
    ...
    @Query(
            "update email_message set status = 'SENT', sent_ts = :sentTimestamp, html_message = null where id = :id and checksum = :checksum and status in (:expectedStatuses)")
    int updateStatusToSent(
            @Bind("id") int id,
            @Bind("checksum") String checksum,
            @Bind("sentTimestamp") ZonedDateTime sentTimestamp,
            @Bind("expectedStatuses") List<String> expectedStatuses);

    String LIGHTWEIGHT_FIELDS =
            "id, event_id, subscription_descriptor_id_fk, status, recipient, subject, message, checksum, request_ts, sent_ts, attempts, email_cc, organization_id_fk ";
    ...
    String FIND_MAILS_BY_SUBSCRIPTION = "select " + LIGHTWEIGHT_FIELDS
            + " from email_message where subscription_descriptor_id_fk = :subscriptionId and "
            + " (:search is null or (recipient like :search or subject like :search or message like :search)) order by sent_ts desc, id ";

    @Query("select * from (" + FIND_MAILS_BY_EVENT + " limit :pageSize offset :page) as d_tbl")
    List<LightweightMailMessage> findByEventId(
    ...
    @Query(
            "select id, event_id, status, recipient, subject, message, checksum, request_ts, sent_ts, attempts, email_cc, subscription_descriptor_id_fk, organization_id_fk from email_message where event_id = :eventId and reservation_id = :reservationId order by sent_ts desc, id")
    List<LightweightMailMessage> findByEventIdAndReservationId(
    ...
    List<LightweightMailMessage> findBySubscriptionDescriptorId(
    ...
    @Query(
            "select id, event_id, status, recipient, subject, message, checksum, request_ts, sent_ts, attempts, email_cc, subscription_descriptor_id_fk, organization_id_fk from email_message where subscription_descriptor_id_fk = :subscriptionDescriptorId and reservation_id = :reservationId order by sent_ts desc, id")
    List<LightweightMailMessage> findBySubscriptionDescriptorAndReservationId(
            @Bind("subscriptionDescriptorId") UUID subscriptionDescriptorId,
            @Bind("reservationId") String reservationId);

    default List<LightweightMailMessage> findByPurchaseContextAndReservationId(
        ...
        if (purchaseContext.ofType(PurchaseContext.PurchaseContextType.event)) {
            ...
            return findBySubscriptionDescriptorAndReservationId(
                    ((SubscriptionDescriptor) purchaseContext).getId(), reservationId);
        }
    }
    ...
    Integer countFindBySubscriptionDescriptorId(
            @Bind("subscriptionId") UUID subscriptionId, @Bind("search") String search);

    @Query("select " + LIGHTWEIGHT_FIELDS + " from email_message where id = :messageId and event_id = :eventId")
    Optional<LightweightMailMessage> findByEventIdAndMessageId(
            @Bind("eventId") int eventId, @Bind("messageId") int messageId);

    @Query("select " + LIGHTWEIGHT_FIELDS
            + " from email_message where id = :messageId and subscription_descriptor_id_fk = :subscriptionId")
    Optional<LightweightMailMessage> findBySubscriptionDescriptorIdAndMessageId(
            @Bind("subscriptionId") UUID subscriptionId, @Bind("messageId") int messageId);

    @Query(
            "update email_message set status = 'RETRY', attempts = coalesce(attempts, 0) +1 where status = 'IN_PROCESS' and request_ts < :date")
