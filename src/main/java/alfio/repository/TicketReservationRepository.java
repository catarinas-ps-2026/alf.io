...
public interface TicketReservationRepository {
    ...
    @Query(
        ...
        """)
    int createNewReservation(
            @Bind("id") String id,
            @Bind("creationTimestamp") ZonedDateTime creationTimestamp,
            @Bind("validity") Date validity,
            @Bind("promotionCodeDiscountId") Integer promotionCodeDiscountId,
    ...
    @Query(
            "select user_id_fk user_id, organization_id_fk organization_id from tickets_reservation where id = :reservationId and user_id_fk is not null")
    ...
    int updateBillingAddress(
            @Bind("billingAddress") String billingAddress, @Bind("reservationId") String reservationId);
    ...
    @Query(
            "select count(id) from tickets_reservation where status in('OFFLINE_PAYMENT', 'CUSTOM_OFFLINE_PAYMENT', 'DEFERRED_OFFLINE_PAYMENT') and event_id_fk = :eventId")
    Integer findAllReservationsWaitingForPaymentCountInEventId(@Bind("eventId") int eventId);

    @Query(
            "select * from tickets_reservation where status = 'OFFLINE_PAYMENT' and date_trunc('day', validity) <= :expiration and offline_payment_reminder_sent = false for update skip locked")
    List<TicketReservation> findAllOfflinePaymentReservationForNotificationForUpdate(
            @Bind("expiration") Date expiration);

    @Query(
            "select id, full_name, first_name, last_name, email_address, final_price_cts, currency_code, event_id_fk, validity from tickets_reservation where status = 'OFFLINE_PAYMENT' and date_trunc('day', validity) <= :expiration and event_id_fk = :eventId for update skip locked")
    List<TicketReservationInfo> findAllOfflinePaymentReservationWithExpirationBeforeForUpdate(
            @Bind("expiration") ZonedDateTime expiration, @Bind("eventId") int eventId);
    ...
    int updateLatestReminderTimestamp(
            @Bind("reservationId") String reservationId,
            @Bind("latestReminderTimestamp") ZonedDateTime latestReminderTimestamp);
    ...
    int updateRegistrationTimestamp(
            @Bind("reservationId") String id, @Bind("registrationTimestamp") ZonedDateTime registrationTimestamp);
    ...
    @Query(
            "select distinct tr.* from tickets_reservation tr join b_transaction tx on tx.reservation_id = tr.id where tr.id in (:reservationIds) and tr.status = 'PENDING'")
    List<TicketReservation> findReservationsWithPendingTransaction(
            @Bind("reservationIds") Collection<String> reservationIds);

    @Query(
            "select id from tickets_reservation where validity < :date and status = 'OFFLINE_PAYMENT' for update skip locked")
    List<String> findExpiredOfflineReservationsForUpdate(@Bind("date") Date date);

    @Query(
            "select id, event_id_fk from tickets_reservation where validity < :date and status in ('IN_PAYMENT', 'EXTERNAL_PROCESSING_PAYMENT') and event_id_fk is not null for update skip locked")
    ...
    int addReservationInvoiceOrReceiptModel(
            @Bind("reservationId") String reservationId, @Bind("invoiceModel") String invoiceModel);

    @Query(
            "update tickets_reservation set invoice_number = :invoiceNumber where id = :reservationId and invoice_number is null")
    ...
    @Query(
            "select min(confirmation_ts) from tickets_reservation where event_id_fk = :eventId and confirmation_ts is not null")
    ...
    @Query(
        ...
        """)
    List<TicketsByDateStatistic> getSoldStatistic(
            @Bind("eventId") int eventId,
            @Bind("fromDate") ZonedDateTime from,
            @Bind("toDate") ZonedDateTime to,
            @Bind("granularity") String granularity);
    ...
    @Query(
        ...
        select to_char(date_trunc(:granularity, d.day), 'YYYY-MM-DD') as day, count(ticket.id) ticket_count \
         ...
         group by 1 order by 1\
        """)
    List<TicketsByDateStatistic> getReservedStatistic(
            @Bind("eventId") int eventId,
            @Bind("fromDate") ZonedDateTime from,
            @Bind("toDate") ZonedDateTime to,
            @Bind("granularity") String granularity);
    ...
    @Query(
        ...
        """)
    int updateTicketReservationWithValidation(
            @Bind("reservationId") String reservationId,
            @Bind("fullName") String fullName,
            @Bind("firstName") String firstName,
            @Bind("lastName") String lastName,
    ...
    @Query(
        ...
        """)
    Integer countTicketsInReservationForExistingCategories(
            @Bind("reservationId") String reservationId, @Bind("categories") Collection<Integer> categories);
    ...
    @Query(
            "select billing_address_company, billing_address_line1, billing_address_line2, billing_address_zip, billing_address_city, billing_address_state, vat_nr, vat_country, invoicing_additional_information from tickets_reservation where id = :reservationId")
    ...
    @Query(
            "select tr.*, e.short_name from tickets_reservation tr join event e on e.id = tr.event_id_fk where tr.subscription_id_fk = :subscriptionId and tr.status = 'COMPLETE'")
    List<TicketReservationWithEventIdentifier> findConfirmedReservationsBySubscriptionId(
    ...
    int updateVatStatus(
            @Bind("reservationId") String reservationId, @Bind("vatStatus") PriceContainer.VatStatus vatStatus);
    ...
    @Query(
        ...
        """)
    int countReservationWithShortIdsForEvent(
            @Bind("ids") @Array List<String> reservationIds, @Bind("eventId") int eventId);

    @Query("select exists(select id from b_transaction where id = :transactionId and reservation_id = reservationId)")
    boolean hasReservationWithTransactionId(
            @Bind("reservationId") String reservationId, @Bind("transactionId") int transactionId);
}
