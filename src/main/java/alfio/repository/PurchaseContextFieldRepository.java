...
public interface PurchaseContextFieldRepository extends FieldRepository {
    ...
    String INSERT_VALUE =
            "insert into purchase_context_field_value(ticket_id_fk, subscription_id_fk, organization_id_fk, field_configuration_id_fk, field_value, context) values (:ticketId, :subscriptionId::uuid, :organizationId, :fieldConfigurationId, :value, :context::ADDITIONAL_FIELD_CONTEXT)";
    String ADDITIONAL_SERVICE_FIELD_VALUE_COLS =
            "field_name, field_value, additional_service_id_fk, ticket_id_fk, field_configuration_id_fk, additional_service_item_id_fk";
    String FIELD_VALUE_COLUMNS =
            "ticket_id_fk, subscription_id_fk, additional_service_item_id_fk, field_configuration_id_fk, field_name, field_value, context";
    ...
    String PURCHASE_CONTEXT_MATCHER =
            "(:eventId is null or event_id_fk = :eventId) and (:subscriptionId::uuid is null or subscription_descriptor_id_fk = :subscriptionId::uuid)";
    String TICKET_ID_OR_SUBSCRIPTION_ID =
            "((:ticketId::integer is not null or :subscriptionId::uuid is not null) and (:ticketId::integer is null or ticket_id_fk = :ticketId) and (:subscriptionId::uuid is null or subscription_id_fk = :subscriptionId::uuid))";

    @Query(
            "select count(*) from field_value_w_additional where ticket_id_fk = :ticketId and field_value is not null and field_value <> ''")
    ...
    List<PurchaseContextFieldValue> findAllForContextByTicketId(
            @Bind("ticketId") int id, @Bind("context") PurchaseContextFieldConfiguration.Context context);
    ...
    List<FieldValueAndDescription> findValueForTicketId(
            @Bind("ticketId") int id, @Bind("fieldNames") Set<String> fieldNames);
     ...
     */
    @Query(
            "select " + FIELD_VALUE_COLUMNS + ", description from all_ticket_field_values "
    ...
    int updateValue(
            @Bind("ticketId") Integer ticketId,
    ...
    @Query("delete from purchase_context_field_value where " + TICKET_ID_OR_SUBSCRIPTION_ID
            + " and field_configuration_id_fk = :fieldConfigurationId")
    int deleteValue(
            @Bind("ticketId") Integer ticketId,
            @Bind("subscriptionId") UUID subscriptionId,
    ...
    @Query(
        ...
        """)
    int deleteAllValuesForAdditionalItems(
            @Bind("ticketIds") Collection<Integer> ticketIds, @Bind("eventId") int eventId);

    @Query(
            "delete from purchase_context_field_value fv using ticket t where t.id = fv.ticket_id_fk and t.tickets_reservation_id in(:reservationIds)")
    int deleteAllTicketValuesForReservations(@Bind("reservationIds") List<String> reservationIds);

    @Query(
            "delete from purchase_context_field_value fv using additional_service_item asi where asi.id = fv.additional_service_item_id_fk and asi.tickets_reservation_uuid in(:reservationIds)")
    int deleteAllAdditionalItemsValuesForReservations(@Bind("reservationIds") List<String> reservationIds);

    @Query(
            "delete from purchase_context_field_value fv using subscription s where s.id = fv.subscription_id_fk and s.reservation_id_fk in(:reservationIds)")
    ...
    default int deleteAllValuesForReservations(List<String> reservationIds) {
        return deleteAllTicketValuesForReservations(reservationIds)
                + deleteAllAdditionalItemsValuesForReservations(reservationIds)
                + deleteAllSubscriptionValuesForReservations(reservationIds);
    }
    ...
    @Query(
        ...
        """)
    List<PurchaseContextFieldDescription> findDescriptionsForLocale(
            @Bind("eventId") int eventId, @Bind("locale") String locale);

    @Query(
            "select field_configuration_id_fk, field_locale, description, purchase_context_field_configuration.field_name as field_name from purchase_context_field_description"
                    + " inner join purchase_context_field_configuration on field_configuration_id_fk = id"
                    + " where "
                    + PURCHASE_CONTEXT_MATCHER)
    List<PurchaseContextFieldDescription> findAllDescriptions(
            @Bind("eventId") Integer eventId, @Bind("subscriptionId") UUID subscriptionDescriptorId);
    ...
    @Query(FIND_ALL
            + " where ticket_id_fk in (select id from ticket where event_id = :eventId and status = 'ACQUIRED')")
    ...
    List<PurchaseContextFieldValue> findAllValuesBySubscriptionIds(
            @Bind("subscriptionIds") Collection<UUID> subscriptionIds);

    default void updateOrInsert(
            Map<String, List<String>> values, PurchaseContext purchaseContext, Integer ticketId, UUID subscriptionId) {
        ...
        if (purchaseContext instanceof Event event) {
            ...
            additionalFields =
                    findAdditionalFieldsForSubscriptionDescriptor(((SubscriptionDescriptor) purchaseContext).getId());
            toUpdate = collectByName(findAllValuesBySubscriptionIds(List.of(subscriptionId)));
        }
        var readOnlyFields = additionalFields.stream()
                .filter(PurchaseContextFieldConfiguration::isReadOnly)
                .map(PurchaseContextFieldConfiguration::getName)
                .collect(Collectors.toSet());
        Map<String, Long> fieldNameToId = additionalFields.stream()
                .collect(Collectors.toMap(
                        PurchaseContextFieldConfiguration::getName, PurchaseContextFieldConfiguration::getId));
            ...
            if (toUpdate.containsKey(fieldName)) {
                if (!readOnlyFields.contains(fieldName)) {
                    ...
                    if (isNotBlank) {
                        updateValue(
                                field.getTicketId(),
                                field.getSubscriptionId(),
                                field.getFieldConfigurationId(),
                                fieldValue);
                    } else {
                ...
                var context = ticketId != null
                        ? PurchaseContextFieldConfiguration.Context.ATTENDEE
                        : PurchaseContextFieldConfiguration.Context.SUBSCRIPTION;
                insertValue(
                        ticketId,
                        subscriptionId,
                        purchaseContext.getOrganizationId(),
                        fieldNameToId.get(fieldName),
                        fieldValue,
                        context);
            }
        });
    ...
    default Map<String, PurchaseContextFieldValue> findAllByTicketIdGroupedByName(
            Integer id, boolean eventSupportsLink) {
    ...
    default Map<Integer, Set<String>> findAdditionalFieldNamesForEvents(Collection<Integer> eventIds) {
        return getJdbcTemplate()
                .query(
                        "select event_id_fk, field_name from purchase_context_field_configuration where event_id_fk = :eventIds",
    ...
    @Query(
            "select * from purchase_context_field_configuration where subscription_descriptor_id_fk = :subscriptionId::uuid order by field_order asc")
    List<PurchaseContextFieldConfiguration> findAdditionalFieldsForSubscriptionDescriptor(
    ...
    @Query(
        ...
        """)
    List<PurchaseContextFieldConfiguration> findAdditionalFieldsOfTypeForEvent(
            @Bind("eventId") int eventId, @Bind("type") String type);
    ...
    default Map<Long, PurchaseContextFieldDescription> findTranslationsFor(Locale locale, int eventId) {
        return findDescriptionsForLocale(eventId, locale.getLanguage()).stream()
                .collect(Collectors.toMap(
                        PurchaseContextFieldDescription::getFieldConfigurationId, Function.identity()));
    }

    default Map<String, List<String>> findAllValuesForTicketId(int ticketId) {
        return findNameAndValue(ticketId).stream()
    ...
    @Query(
            "select field_value as name, count(*) as count from field_value_w_additional where field_configuration_id_fk = :configurationId group by field_value")
    ...
    default List<RestrictedValueStats> retrieveStats(long configurationId) {
        ...
        Map<String, Integer> valueStats = getValueStats(configurationId).stream()
                .collect(Collectors.toMap(
                        RestrictedValueStats.RestrictedValueCount::getName,
                        RestrictedValueStats.RestrictedValueCount::getCount));
        ...
        if (configuration.isCountryField()) {
            ...
            return valueStats.entrySet().stream()
                    ...
                    .map(entry -> {
                        ...
                        return new RestrictedValueStats(
                                entry.getKey(),
                                count,
    ...
    @Query("select " + ADDITIONAL_SERVICE_FIELD_VALUE_COLS + " from additional_item_field_value_with_ticket_id"
            + "  where ticket_id_fk = :ticketId"
            + "  and additional_service_id_fk in (:additionalServiceIds)")
    List<AdditionalServiceFieldValue> loadTicketFieldsForAdditionalService(
            @Bind("ticketId") int ticketId, @Bind("additionalServiceIds") List<Integer> additionalServiceIds);

    @Query("select " + ADDITIONAL_SERVICE_FIELD_VALUE_COLS
            + " from additional_item_field_value_with_ticket_id where ticket_id_fk in(:ticketIds)")
    List<AdditionalServiceFieldValue> findAdditionalServicesValueByTicketIds(
            @Bind("ticketIds") List<Integer> ticketIds);

    @Query("select " + ADDITIONAL_SERVICE_FIELD_VALUE_COLS
            + " from additional_item_field_value_with_ticket_id where additional_service_item_id_fk in (:itemIds)")
    List<AdditionalServiceFieldValue> findAdditionalServicesValueByItemIds(@Bind("itemIds") List<Integer> itemIds);

    @Query("select count(*) from purchase_context_field_configuration where id in (:additionalFieldIds) and "
            + PURCHASE_CONTEXT_MATCHER)
    int countMatchingAdditionalFieldsForPurchaseContext(
