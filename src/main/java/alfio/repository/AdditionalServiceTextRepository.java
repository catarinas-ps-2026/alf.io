...
public interface AdditionalServiceTextRepository {
    ...
    List<AdditionalServiceText> findAllByAdditionalServiceIdAndType(
            @Bind("additionalServiceId") int additionalServiceId, @Bind("type") AdditionalServiceText.TextType type);
    ...
    Optional<AdditionalServiceText> findByLocaleAndType(
            @Bind("additionalServiceId") int additionalServiceId,
            @Bind("locale") String locale,
            @Bind("type") AdditionalServiceText.TextType type);

    @Query(
            "insert into additional_service_description(additional_service_id_fk, locale, type, value) values(:additionalServiceId, :locale, :type, :value)")
    int insert(
            @Bind("additionalServiceId") int additionalServiceId,
            @Bind("locale") String locale,
            @Bind("type") AdditionalServiceText.TextType type,
    ...
    @Query(
            "update additional_service_description set locale = :locale, type = :type, value = :value where id = :id and additional_service_id_fk = :additionalServiceId")
    int update(
            @Bind("id") int id,
            @Bind("locale") String locale,
            @Bind("type") AdditionalServiceText.TextType type,
    ...
    @Query(
            "select id, additional_service_id_fk, locale, type, value from additional_service_description where additional_service_id_fk in (:additionalServiceIds)")
    List<AdditionalServiceText> findAllByAdditionalServiceIds(
            @Bind("additionalServiceIds") Collection<Integer> additionalServiceIds);

    default Map<Integer, Map<AdditionalServiceText.TextType, Map<String, String>>>
            getDescriptionsByAdditionalServiceIds(Collection<Integer> additionalServiceIds) {
    ...
    default AdditionalServiceText findBestMatchByLocaleAndType(
            int additionalServiceId, String locale, AdditionalServiceText.TextType type) {
        return findByLocaleAndType(additionalServiceId, locale, type).orElseGet(() -> {
            List<AdditionalServiceText> texts = findAllByAdditionalServiceIdAndType(additionalServiceId, type);
            return !texts.isEmpty()
                    ? texts.get(0)
                    : new AdditionalServiceText(-1, additionalServiceId, locale, type, "N/A");
        });
    }
}
