...
public interface PromoCodeDiscountRepository {
    ...
    @Query(
            "select * from promo_code where organization_id_fk = :organizationId and event_id_fk is null order by promo_code asc")
    ...
    @Query(
        ...
        """)
    int addPromoCode(
            @Bind("promoCode") String promoCode,
    ...
    @Query(
        ...
        """)
    Optional<PromoCodeDiscount> findPromoCodeInEventOrOrganization(
            @Bind("eventId") int eventId, @Bind("promoCode") String promoCode);
    ...
    @Query(
        ...
        select * from promo_code where promo_code = :promoCode and code_type <> 'DYNAMIC' and (\
         ...
         order by event_id_fk is null limit 1\
        """)
    Optional<PromoCodeDiscount> findPublicPromoCodeInEventOrOrganization(
            @Bind("eventId") int eventId, @Bind("promoCode") String promoCode);

    @Query(
            "select count(*) from promo_code where event_id_fk = :eventId or (event_id_fk is null and organization_id_fk = :organizationId)")
    ...
    @Query(
            "update promo_code set valid_from = :start, valid_to = :end, max_usage = :maxUsage, categories = :categories, description = :description, email_reference = :emailReference, hidden_category_id = :hiddenCategoryId where id = :id")
    int updateEventPromoCode(
            @Bind("id") int id,
            @Bind("start") ZonedDateTime start,
            @Bind("end") ZonedDateTime end,
            @Bind("maxUsage") Integer maxUsage,
    ...
    @Query(
        ...
        """)
    List<PromoCodeUsageResult> findDetailedUsage(@Bind("promoCode") String promoCode, @Bind("eventId") Integer eventId);
