...
public interface AdditionalServiceRepository {
    ...
    default Map<Integer, Map<AdditionalServiceItemStatus, Integer>> getCount(int eventId) {
        ...
        getJdbcTemplate()
                .query(
                        "select count(*) as cnt, additional_service_id_fk, status from additional_service_item where event_id_fk = :eventId group by additional_service_id_fk, status",
                        Collections.singletonMap("eventId", eventId),
                        rse -> {
                            var additionalServiceId = rse.getInt("additional_service_id_fk");
                            res.putIfAbsent(
                                    additionalServiceId,
                                    new EnumMap<>(AdditionalServiceItem.AdditionalServiceItemStatus.class));
                            var statusCount = res.get(additionalServiceId);
                            var status =
                                    AdditionalServiceItem.AdditionalServiceItemStatus.valueOf(rse.getString("status"));
    ...
    default Map<Integer, AdditionalService.AdditionalServiceType> getTypeByIds(
            Collection<Integer> additionalServiceIds) {
        Map<Integer, AdditionalService.AdditionalServiceType> res = new HashMap<>();
        getJdbcTemplate()
                .query(
                        "select id, service_type from additional_service where id in (:additionalServiceIds)",
                        Map.of("additionalServiceIds", additionalServiceIds),
                        row -> {
                            res.put(
                                    row.getInt("id"),
                                    AdditionalService.AdditionalServiceType.valueOf(row.getString("service_type")));
                        });
        return res;
    }
