...
public interface AdminJobQueueRepository {
    ...
    @Query(
            "select * from admin_job_queue where status = 'SCHEDULED' and job_name in (:jobNames) and request_ts < :now::timestamp for update skip locked")
    List<AdminJobSchedule> loadPendingSchedules(
            @Bind("jobNames") Collection<String> jobNames, @Bind("now") ZonedDateTime now);

    @Query("select * from admin_job_queue")
    List<AdminJobSchedule> loadAll();

    @Query(
            "update admin_job_queue set status = :status, execution_ts = :executionDate, metadata = to_json(:metadata::json) where id = :id")
    int updateSchedule(
            @Bind("id") long id,
    ...
    int scheduleRetry(@Bind("id") long id, @Bind("requestTs") ZonedDateTime requestTs);
