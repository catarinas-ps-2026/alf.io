...
public interface GroupRepository {
    ...
    AffectedRowCountAndKey<Integer> insert(
            @Bind("name") String name, @Bind("description") String description, @Bind("orgId") int organizationId);
    ...
    @Query(
            "select exists (select id from group_link where id = :id and organization_id_fk = :organizationId and event_id_fk = :eventId and (:categoryId is null or ticket_category_id_fk = :categoryId))")
    Boolean checkGroupLinkExists(
            @Bind("id") int id,
    ...
    @Query(
        ...
        """)
    @AutoGeneratedKey("id")
    AffectedRowCountAndKey<Integer> createConfiguration(
    ...
    @Query(
        ...
        """)
    int insertWhitelistedTicket(
            @Bind("itemId") int itemId,
            @Bind("configurationId") int configurationId,
            @Bind("ticketId") int ticketId,
            @Bind("requiresUniqueValue") Boolean requiresUniqueValue);

    @Query(
            BY_EVENT_ID + " and ticket_category_id_fk = :categoryId"
    ...
    @Query(
            "update group_link set a_group_id_fk = :groupId, ticket_category_id_fk = :categoryId, type = :type, match_type = :matchType, max_allocation = :maxAllocation where id = :id and event_id_fk = :eventId")
    int updateConfiguration(
            @Bind("id") int configurationId,
            @Bind("groupId") int groupId,
            @Bind("eventId") int eventId,
            @Bind("categoryId") Integer categoryId,
            @Bind("type") LinkedGroup.Type type,
    ...
    Optional<GroupMember> findItemEndsWith(
            @Bind("configurationId") int configurationId, @Bind("groupId") int groupId, @Bind("value") String value);

    @Query(
            "select count(*) from whitelisted_ticket where group_member_id_fk = :itemId and group_link_id_fk = :configurationId")
    int countExistingWhitelistedTickets(@Bind("itemId") int itemId, @Bind("configurationId") int configurationId);
    ...
    default void deactivateGroupMember(List<Integer> memberIds, int groupId) {
        MapSqlParameterSource[] params = memberIds.stream()
                .map(memberId -> new MapSqlParameterSource("groupId", groupId)
