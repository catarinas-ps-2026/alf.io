...
public interface TicketRepository {
    ...
    String FIND_TICKETS_IN_RESERVATION =
            "select * from ticket where tickets_reservation_id = :reservationId " + SORT_TICKETS;

    String RESET_TICKET =
            " TICKETS_RESERVATION_ID = null, FULL_NAME = null, EMAIL_ADDRESS = null, SPECIAL_PRICE_ID_FK = null, LOCKED_ASSIGNMENT = false, USER_LANGUAGE = null, REMINDER_SENT = false, SRC_PRICE_CTS = 0, FINAL_PRICE_CTS = 0, VAT_CTS = 0, DISCOUNT_CTS = 0, FIRST_NAME = null, LAST_NAME = null, EXT_REFERENCE = null, TAGS = array[]::text[], VAT_STATUS = null, METADATA = '{}'::jsonb ";
    String RELEASE_TICKET_QUERY =
            "update ticket set status = 'RELEASED', uuid = :newUuid, public_uuid = :newPublicUuid, " + RESET_TICKET
                    + " where id = :ticketId and status in('ACQUIRED', 'PENDING', 'TO_BE_PAID') and tickets_reservation_id = :reservationId and event_id = :eventId";
    String FIND_BASIC_TICKET_INFO_BY_EVENT_ID =
            "select t.id t_id, t.uuid t_uuid, t.public_uuid t_public_uuid, tc.id tc_id, tc.bounded tc_bounded, t.vat_status t_vat_status from ticket t inner join ticket_category tc on t.category_id = tc.id where t.event_id = :eventId";
    String UPDATE_TICKET_PRICE =
            "update ticket set src_price_cts = :srcPriceCts, final_price_cts = :finalPriceCts, vat_cts = :vatCts, discount_cts = :discountCts, currency_code = :currencyCode, vat_status = :vatStatus::VAT_STATUS where event_id = :eventId and category_id = :categoryId";
    ...
    List<Integer> selectTicketInCategoryForUpdate(
            @Bind("eventId") int eventId,
            @Bind("categoryId") int categoryId,
            @Bind("amount") int amount,
            @Bind("requiredStatuses") List<String> requiredStatus);

    @Query(
            "select id from ticket where status in (:requiredStatuses) and category_id = :categoryId and event_id = :eventId and tickets_reservation_id is null order by id limit :amount for update skip locked")
    List<Integer> selectTicketInCategoryForUpdateSkipLocked(
            @Bind("eventId") int eventId,
            @Bind("categoryId") int categoryId,
            @Bind("amount") int amount,
            @Bind("requiredStatuses") List<String> requiredStatus);
    ...
    List<Integer> selectNotAllocatedTicketsForUpdate(
            @Bind("eventId") int eventId,
            @Bind("amount") int amount,
            @Bind("requiredStatuses") List<String> requiredStatuses);

    @Query(
            "select id from ticket where status in(:requiredStatuses) and category_id is null and event_id = :eventId and tickets_reservation_id is null order by id limit :amount for update skip locked")
    List<Integer> selectNotAllocatedTicketsForUpdateSkipLocked(
            @Bind("eventId") int eventId,
            @Bind("amount") int amount,
            @Bind("requiredStatuses") List<String> requiredStatuses);
    ...
    List<Integer> lockTicketsToInvalidate(
            @Bind("eventId") int eventId, @Bind("categoryId") int categoryId, @Bind("amount") int amount);

    @Query(
            "select count(*) from ticket where status in (" + CONFIRMED
                    + ") and category_id = :categoryId and event_id = :eventId and full_name is not null and email_address is not null")
    Integer countAssignedTickets(@Bind("eventId") int eventId, @Bind("categoryId") int categoryId);

    @Query("select count(*) from ticket where status in (" + CONFIRMED
            + ", 'PENDING') and category_id = :categoryId and event_id = :eventId")
    Integer countConfirmedAndPendingTickets(@Bind("eventId") int eventId, @Bind("categoryId") int categoryId);

    @Query("select count(*) from ticket where status in (" + CONFIRMED
            + ") and category_id = :categoryId and event_id = :eventId")
    ...
    @Query(
        ...
        """)
    CategoryAvailability getUnboundedCategoryAvailability(
            @Bind("eventId") int eventId, @Bind("categoryId") int categoryId);
    ...
    @Query(
            "select case(show_public_statistics) when true then dynamic_allocation else 0 end from events_statistics where id = :eventId")
    ...
    default int reserveTickets(
        ...
        var batchReserveParameters = ticketIds.stream()
                .map(id -> {
                    var attendee = Objects.requireNonNullElse(
                            attendeeDataSupplier.apply(idx.getAndIncrement()), AttendeeData.empty());
                    String metadata = null;
                    if (attendee.hasMetadata()) {
                        metadata = Json.toJson(TicketMetadataContainer.fromMetadata(
        ...
        return (int) Arrays.stream(
                        getNamedParameterJdbcTemplate().batchUpdate(batchReserveTickets(), batchReserveParameters))
                .asLongStream()
                .sum();
    }
    ...
    @Query(
        ...
        """)
    void reserveTicket(
            @Bind("reservationId") String transactionId,
    ...
    int updateTicketsStatusWithReservationId(
            @Bind("reservationId") String reservationId, @Bind("status") String status);
    ...
    @Query(type = QueryType.TEMPLATE, value = UPDATE_TICKET_PRICE + " and tickets_reservation_id = :reservationId")
    ...
    @Query(
            "update ticket set category_id = null where event_id = :eventId and category_id = :categoryId and id in (:ticketIds)")
    int unbindTicketsFromCategory(
            @Bind("eventId") int eventId, @Bind("categoryId") int categoryId, @Bind("ticketIds") List<Integer> ids);
    ...
    List<Ticket> findByEventIdAndPartialUUIDForUpdate(
            @Bind("eventId") int eventId, @Bind("uuid") String partialUUID, @Bind("status") Ticket.TicketStatus status);
    ...
    int updateTicketOwner(
            @Bind("ticketIdentifier") String ticketIdentifier,
    ...
    int toggleTicketLocking(
            @Bind("id") int ticketId, @Bind("categoryId") int categoryId, @Bind("lockedAssignment") boolean locked);
    ...
    @Query(
            "update ticket set ext_reference = :extReference, locked_assignment = :lockedAssignment where id = :id and category_id = :categoryId")
    int updateExternalReferenceAndLocking(
            @Bind("id") int ticketId,
            @Bind("categoryId") int categoryId,
            @Bind("extReference") String extReference,
            @Bind("lockedAssignment") boolean locked);

    @Query("update ticket set user_language = :userLanguage where uuid = :ticketIdentifier")
    int updateOptionalTicketInfo(
            @Bind("ticketIdentifier") String ticketIdentifier, @Bind("userLanguage") String userLanguage);
    ...
    int moveToAnotherCategory(
            @Bind("ticketIds") List<Integer> ticketIds,
    ...
    @Query(
            FIND_FULL_TICKET_INFO
                    + " where t.event_id = :eventId and t.full_name is not null and t.email_address is not null and t.id in (:ids) order by t.id asc")
    List<FullTicketInfo> findAllFullTicketInfoAssignedByEventId(
            @Bind("eventId") int eventId, @Bind("ids") List<Integer> ids);
    ...
    @Query(
            ...
            """)
    List<Integer> findAllAssignedByEventIdForCheckIn(
            @Bind("eventId") int eventId, @Bind("changedSince") Date changedSince);

    @Query("select * from reservation_and_ticket_and_tx where t_id is not null and t_status in (" + CONFIRMED
            + ") and t_event_id = :eventId order by tr_confirmation_ts, t_id")
    List<TicketWithReservationAndTransaction> findAllConfirmedForCSV(@Bind("eventId") int eventId);

    @Query(
            "select a.*, b.confirmation_ts from ticket a, tickets_reservation b where a.event_id = :eventId and a.status in("
                    + CONFIRMED + ") and a.tickets_reservation_id = b.id order by b.confirmation_ts")
    ...
    @Query("select count(*) from ticket where event_id = :eventId and status in(" + CONFIRMED
            + ") and full_name is not null and email_address is not null")
    Integer countAllAssigned(@Bind("eventId") int eventId);

    //
    @Query(
            "select tickets_reservation_id from ticket where event_id = :eventId and status in('ACQUIRED', 'TO_BE_PAID') and (full_name is null or email_address is null) for update skip locked")
    ...
    @Query(
            "select * from ticket where event_id = :eventId  and status in('ACQUIRED', 'TO_BE_PAID') and full_name is not null and email_address is not null and reminder_sent = false for update skip locked")
    ...
    int releaseTicket(
            ...
            @Bind("newUuid") String newUuid,
            @Bind("newPublicUuid") UUID newPublicUuid,
            @Bind("eventId") int eventId,
            @Bind("ticketId") int ticketId);

    default int[] batchReleaseTickets(String reservationId, List<Integer> ticketIds, Event event) {
        MapSqlParameterSource[] args = ticketIds.stream()
    ...
    @Query(
            type = QueryType.TEMPLATE,
            value =
                    "update ticket set status = 'RELEASED', uuid = :newUuid, public_uuid = :newPublicUuid, "
                            + RESET_TICKET
                            + " where id = :ticketId and status = 'PENDING' and tickets_reservation_id = :reservationId and event_id = :eventId")
    ...
    default void resetTickets(List<Integer> ticketIds) {
        ...
        getNamedParameterJdbcTemplate()
                .batchUpdate(
                        "update ticket set status = 'RELEASED', uuid = :newUuid, public_uuid = :newPublicUuid, "
                                + RESET_TICKET + " where id = :ticketId",
                        params);
    }
    ...
    List<TicketInfo> findReleasedBelongingToExpiredCategories(
            @Bind("eventId") int eventId, @Bind("currentTs") ZonedDateTime now);

    @Query(FIND_BASIC_TICKET_INFO_BY_EVENT_ID + " and t.tickets_reservation_id = :reservationId")
    List<TicketInfo> findBasicTicketInfoForReservation(
            @Bind("eventId") int eventId, @Bind("reservationId") String reservationId);
    ...
    List<Ticket> selectWaitingTicketsForUpdate(
            @Bind("eventId") int eventId, @Bind("status") String status, @Bind("amount") int amount);

    @Query(
            "select id from ticket where status = 'FREE' and event_id = :eventId and category_id = :categoryId order by id limit :amount for update skip locked")
    List<Integer> selectFreeTicketsForPreReservation(
            @Bind("eventId") int eventId, @Bind("amount") int amount, @Bind("categoryId") int categoryId);

    @Query(
            "select id from ticket where status = 'FREE' and event_id = :eventId and category_id is null order by id limit :amount for update skip locked")
    List<Integer> selectNotAllocatedFreeTicketsForPreReservation(
            @Bind("eventId") int eventId, @Bind("amount") int amount);
    ...
    @Query("update ticket set status = 'FREE' where event_id = :eventId and category_id in(:categoryId) and status = '"
            + RELEASED + "'")
    int revertToFreeForRestrictedCategories(@Bind("eventId") int eventId, @Bind("categoryId") List<Integer> categoryId);

    @Query(
            "select distinct category_id from ticket where tickets_reservation_id = :reservationId and src_price_cts > 0")
    List<Integer> getCategoriesIdToPayInReservation(@Bind("reservationId") String reservationId);

    @Query(
            "select * from checkin_ticket_event_and_category_info where t_public_uuid = :publicUUID and e_short_name = :eventShortName and (e_format = 'ONLINE' or tc_ticket_access_type = 'ONLINE') ")
    Optional<CheckInFullInfo> getFullInfoForOnlineCheckin(
            @Bind("eventShortName") String eventShortName, @Bind("publicUUID") UUID publicUUID);

    @Query("select * from checkin_ticket_event_and_category_info where e_id = :eventId " + "and ("
            + TicketSearchRepository.BASE_FILTER + " or lower(tc_name) like lower(:search)) "
    ...
    List<CheckInFullInfo> searchAttendees(
            @Bind("eventId") int eventId,
    ...
    @Query("select count(*) as total, count(*) filter (where t_status = 'CHECKED_IN') as checked_in "
            ...
            + TicketSearchRepository.BASE_FILTER + " or lower(tc_name) like lower(:search)) "
            + "and (e_format = 'IN_PERSON' or tc_ticket_access_type = 'IN_PERSON') ")
    AttendeeSearchResultsCount countSearchResults(@Bind("eventId") int eventId, @Bind("search") String search);

    @Query(
            "update ticket set status = 'CHECKED_IN', locked_assignment = true where uuid = :uuid and event_id = :eventId and status = 'ACQUIRED'")
    ...
    @Query(
            "select count(*) from ticket t where t.event_id = :eventId and t.status in (:statuses) and t.tags @> ARRAY[ :tags ]::text[]")
    Integer countTicketsMatchingTagsAndStatus(
            @Bind("eventId") int eventId,
            @Bind("tags") List<String> tags,
            @Bind("statuses") Collection<String> statuses);
    ...
    @Query(
        ...
        """)
    int applySubscriptionToTicketsInReservation(
            @Bind("reservationId") String reservationId,
    ...
    @Query(
            "select count(*) from ticket where subscription_id_fk = :subscriptionId and (:eventId is null or event_id = :eventId)")
    ...
    int updateTicketMetadata(
            @Bind("ticketId") int ticketId,
            @JSONData @Bind("metadata") TicketMetadataContainer ticketMetadataContainer);

    @Query("update ticket set vat_status = :vatStatus::VAT_STATUS where tickets_reservation_id = :reservationId")
    int updateVatStatusForReservation(
            @Bind("reservationId") String reservationId,
            @Bind("vatStatus") @EnumTypeAsString PriceContainer.VatStatus vatStatus);

    @Query(FIND_TICKETS_IN_RESERVATION)
    List<TicketWithMetadataAttributes> findTicketsInReservationWithMetadata(
            @Bind("reservationId") String reservationId);
    ...
    @Query(
           ...
           """)
    List<Integer> findTicketsWithAdditionalData(
            @Bind("reservationId") String reservationId, @Bind("eventPublicIdentifier") String eventPublicIdentifier);
    ...
    @Query(
            "select count(*) from ticket where category_id = :categoryId and lower(email_address) = lower(:emailAddress)")
    Integer countByEmailAddressAndCategory(
            @Bind("emailAddress") String emailAddress, @Bind("categoryId") int categoryId);
}
