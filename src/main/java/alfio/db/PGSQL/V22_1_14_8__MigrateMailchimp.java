...
public class V22_1_14_8__MigrateMailchimp extends BaseJavaMigration {
    ...
    public void migrate(Context context) throws Exception {
        ...
        ExtensionLogRepository extensionLogRepository =
                QueryFactory.from(ExtensionLogRepository.class, "PGSQL", dataSource);
        ...
        for (ConfValue cv : confValues) {
            if (cv.value != null) {
                optionally(() -> jdbcTemplate.queryForObject(
                                "select org_id from event where id = " + cv.eventId, Integer.class))
                        .ifPresent(orgId -> extensionRepository.insertSettingValue(
                                "apiKey".equals(cv.name) ? apiKeyId : listIdId,
                                "-" + orgId + "-" + cv.eventId,
                                cv.value));
            }
        }
    ...
    public interface PluginRepository {
        ...
        int getConfigurationMetadataIdFor(
                @Bind("extensionId") int extensionId, @Bind("name") String name, @Bind("confLevel") String confLevel);
    }
    ...
    public static class ConfValue {
        ...
        public ConfValue(
                @ConstructorAnnotationRowMapper.Column("conf_name") String name,
