...
abstract class AbstractFormBasedWebSecurity {
    ...
    public SecurityFilterChain publicOpenId(HttpSecurity http) throws Exception {
        ...
        http.securityMatcher(antStyleMatcher("/openid/**"))
                ...
                // this allows us to sync between spring session and spring security, thus saving the principal name in
                // the session table
        ...
        http.oauth2Login(oauth -> oauth.loginPage(OPENID_AUTHENTICATION_PATH)
                .loginProcessingUrl(OPENID_CALLBACK_PATH)
                .clientRegistrationRepository(registrationRepository)
                .authorizationEndpoint(auth ->
                        auth.authorizationRequestResolver(new PublicOpenIdRequestResolver(registrationRepository)))
        ...
        http.addFilterBefore(
                new PreAuthCookieWriterFilter(cookieSerializer, antStyleMatcher(OPENID_AUTHENTICATION_PATH)),
    ...
    public SecurityFilterChain filterChain(
            HttpSecurity http, StripeConnectManager stripeConnectManager, MollieConnectManager mollieConnectManager)
            throws Exception {
        if (configurationManager
                .getForSystem(ConfigurationKeys.HTTPS_FORCE_REDIRECT)
                .getValue()
                .map(Boolean::parseBoolean)
                .orElseGet(() -> environment.acceptsProfiles(Profiles.of(Initializer.PROFILE_LIVE)))) {
            http.redirectToHttps(
                    customizer -> customizer.requestMatchers(new NegatedRequestMatcher(antStyleMatcher("/healthz"))));
        }
        ...
        //
        http.addFilterBefore(
                        new RecaptchaLoginFilter(
                                recaptchaService,
                                AUTHENTICATE,
                                "/authentication?recaptchaFailed",
                                configurationManager),
                        UsernamePasswordAuthenticationFilter.class)
                .addFilterBefore(
                        new CFTurnstileVerificationFilter(
                                configurationManager,
                ...
                .addFilterAfter(
                        new PaymentProviderConnectFilter(
                                templateManager, userManager, stripeConnectManager, mollieConnectManager),
                        CFTurnstileVerificationFilter.class);

        if (environment.acceptsProfiles(Profiles.of(Initializer.PROFILE_DEMO))) {
            http.addFilterAfter(
                    new UserCreatorBeforeLoginFilter(userManager, AUTHENTICATE), RecaptchaLoginFilter.class);
        }
    ...
    private OAuth2UserService<OidcUserRequest, OidcUser> publicOidcUserService(
            ConfigurationManager configurationManager) {
        final OidcUserService delegate = new OidcUserService();
        return userRequest -> {
            OidcUser oidcUser = delegate.loadUser(userRequest);
            var alfioUser = new OpenIdAlfioUser(
                    null, oidcUser.getSubject(), oidcUser.getEmail(), User.Type.PUBLIC, Set.of(), Map.of());
            var configuration = Json.fromJson(
                    configurationManager
            ...
            return new OpenIdPrincipal(
                    List.of(),
                    oidcUser.getIdToken(),
                    oidcUser.getUserInfo(),
                    alfioUser,
                    buildLogoutUrl(configuration),
                    signedUp);
        };
    }
    ...
    protected void setupAuthenticationEndpoint(HttpSecurity http) throws Exception {
        http.authenticationManager(createAuthenticationManager())
                .formLogin(login -> login.loginPage("/authentication")
    ...
    private static void authorizeRequests(
        ...
        auth.requestMatchers(antStyleMatcher(ADMIN_PUBLIC_API + "/**"))
                .denyAll() // Admin public API requests must be authenticated using API-Keys
                .requestMatchers(antStyleMatcher(HttpMethod.GET, ADMIN_API + "/users/current"))
                .hasAnyRole(ADMIN, OWNER, SUPERVISOR)
                .requestMatchers(
                        antStyleMatcher(HttpMethod.POST, ADMIN_API + "/users/check"),
                        antStyleMatcher(HttpMethod.POST, ADMIN_API + "/users/current/edit"),
                        antStyleMatcher(HttpMethod.POST, ADMIN_API + "/users/current/update-password"))
                .hasAnyRole(ADMIN, OWNER, SUPERVISOR)
                .requestMatchers(
                        antStyleMatcher(ADMIN_API + "/configuration/**"), antStyleMatcher(ADMIN_API + "/users/**"))
                .hasAnyRole(ADMIN, OWNER)
                .requestMatchers(
                        antStyleMatcher(ADMIN_API + "/organizations/new"), antStyleMatcher(ADMIN_API + "/system/**"))
                .hasRole(ADMIN)
                .requestMatchers(antStyleMatcher(ADMIN_API + "/check-in/**"))
                .hasAnyRole(ADMIN, OWNER, SUPERVISOR)
                .requestMatchers(OWNERSHIP_REQUIRED.stream()
                        .map(u -> antStyleMatcher(HttpMethod.GET, u))
                        .toArray(RequestMatcher[]::new))
                .hasAnyRole(ADMIN, OWNER)
                .requestMatchers(antStyleMatcher(HttpMethod.GET, ADMIN_API + "/**"))
                .hasAnyRole(ADMIN, OWNER, SUPERVISOR)
                .requestMatchers(
                        antStyleMatcher(HttpMethod.POST, ADMIN_API + "/reservation/event/*/new"),
                        antStyleMatcher(HttpMethod.POST, ADMIN_API + "/reservation/event/*/*"))
                .hasAnyRole(ADMIN, OWNER, SUPERVISOR)
                .requestMatchers(
                        antStyleMatcher(HttpMethod.PUT, ADMIN_API + "/reservation/event/*/*/notify"),
                        antStyleMatcher(HttpMethod.PUT, ADMIN_API + "/reservation/event/*/*/notify-attendees"),
                        antStyleMatcher(HttpMethod.PUT, ADMIN_API + "/reservation/event/*/*/confirm"))
                .hasAnyRole(ADMIN, OWNER, SUPERVISOR)
                .requestMatchers(antStyleMatcher(ADMIN_API + "/**"))
                .hasAnyRole(ADMIN, OWNER)
                .requestMatchers(antStyleMatcher("/admin/**"))
                .hasAnyRole(ADMIN, OWNER, SUPERVISOR)
                .requestMatchers(antStyleMatcher("/api/attendees/**"))
                .denyAll()
                .requestMatchers(antStyleMatcher("/callback"))
                .permitAll()
                .requestMatchers(antStyleMatcher("/**"))
                .permitAll();
    }
    ...
    private static void configureExceptionHandling(HttpSecurity http) throws Exception {
        http.exceptionHandling(handling -> handling.accessDeniedHandler((request, response, accessDeniedException) -> {
                            if (!response.isCommitted()) {
                                if ("XMLHttpRequest"
                                        .equals(request.getHeader(AuthenticationConstants.X_REQUESTED_WITH))) {
                        ...
                        .defaultAuthenticationEntryPointFor(
                                ...
                                new RequestHeaderRequestMatcher(
                                        AuthenticationConstants.X_REQUESTED_WITH, "XMLHttpRequest")))
                .headers(headers -> headers.cacheControl(HeadersConfigurer.CacheControlConfig::disable));
    }

    private static void configureCsrf(HttpSecurity http, Consumer<CsrfConfigurer<HttpSecurity>> additionalConfiguration)
            throws Exception {
        http.csrf(c -> {
            Pattern methodsPattern = Pattern.compile("^(GET|HEAD|TRACE|OPTIONS)$");
            Predicate<HttpServletRequest> csrfAllowListPredicate =
                    r -> r.getRequestURI().startsWith("/api/webhook/")
                            || r.getRequestURI().startsWith("/api/payment/webhook/")
                            || methodsPattern.matcher(r.getMethod()).matches();
            csrfAllowListPredicate =
                    csrfAllowListPredicate.or(r -> r.getRequestURI().equals("/report-csp-violation"));
