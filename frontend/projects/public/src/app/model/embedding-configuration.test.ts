...
import { ReservationStatusChanged, EmbeddingConfiguration } from './embedding-configuration';

describe('embedding-configuration.ts', () => {
    describe('ReservationStatusChanged', () => {
        it('should create instance with status and id', () => {
            const statusChanged = new ReservationStatusChanged('COMPLETED', 'res-123');
        ...
        it('should create instance with status, id and error', () => {
            const statusChanged = new ReservationStatusChanged('FAILED', 'res-456', 'Payment failed');
        ...
        it('should accept different reservation statuses', () => {
            const statuses: ReservationStatus[] = ['PENDING', 'COMPLETED', 'FAILED', 'CANCELLED', 'EXPIRED'];

            statuses.forEach((status) => {
                const statusChanged = new ReservationStatusChanged(status, 'test-id');
                expect(statusChanged.status).toBe(status);
            });
        });

        it('should allow undefined error', () => {
            const statusChanged = new ReservationStatusChanged('COMPLETED', 'res-789');
            expect(statusChanged.error).toBeUndefined();
        });
