...
describe('ProcessingPaymentComponent', () => {
    ...
    const mockReservationService = {
        ...
        getReservationStatusInfo: vi.fn(() =>
            of({ status: 'IN_PAYMENT', validatedBookingInformation: true }),
        ),
        forcePaymentStatusCheck: vi.fn(() =>
            of({ success: false, failure: false, redirect: null }),
        ),
    };
    ...
    describe('forceCheck', () => {
        ...
        it('should navigate to success when status changes', async () => {
            mockReservationService.forcePaymentStatusCheck.mockReturnValue(
                of({ success: true, failure: false, redirect: false }),
            );
