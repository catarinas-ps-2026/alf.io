...
## 7.B Pruebas de Integración de Controladores REST (Contrato de API)

> Las siguientes pruebas aplican técnicas de **prueba de contrato de API** y **validación de respuestas HTTP** sobre los controladores REST del sistema. Se verifican códigos de respuesta HTTP, schemas JSON (conformidad con la especificación OpenAPI), validación de entradas y manejo correcto de errores. Nivel de prueba: **Integración**.

### Contrato de API – Creación de Evento (Admin)
| ID | CPF-API-01 |
| :--- | :--- |
| **Funcionalidad** | Creación de evento vía API de administración |
| **Descripción** | Valida que el endpoint `POST /admin/api/events/new` retorne los códigos HTTP correctos y un cuerpo JSON conforme al schema OpenAPI según las distintas condiciones de entrada (datos válidos, campos faltantes, usuario no autorizado). |
| **Requisito Asociado** | RF-MAN-01 / Endpoint A1 del [[Plan-de-Pruebas-de-Integración]] |
| **Componente** | `EventApiController` |
| **Precondiciones** | Servidor alf.io en ejecución con PostgreSQL. Usuario administrador autenticado (token/sesión válida). |
| **Datos de Entrada** | Cuerpo JSON con campos: `shortName`, `organizationId`, `begin`, `end`, `availableSeats`, `currency`. |
| **Técnicas de Pruebas** | Prueba de Contrato de API, Tabla de Decisión, Partición de Equivalencia. |
| **Prioridad** | Crítica |

**Tabla de Decisión: Códigos de Respuesta HTTP**
| Condición | C1 | C2 | C3 | C4 |
| :--- | :--- | :--- | :--- | :--- |
| ¿Usuario autenticado como admin? | SÍ | SÍ | NO | SÍ |
| ¿Todos los campos obligatorios presentes? | SÍ | NO | — | SÍ |
| ¿`shortName` único (no duplicado)? | SÍ | — | — | NO |
| **Código HTTP Esperado** | **200 OK** | **400 Bad Request** | **401 Unauthorized** | **422 / 400** |
| **Schema JSON Esperado** | `EventWithStatistics` | `ValidationResult` | Error genérico | `ValidationResult` |

**Catálogo de Pruebas**
| #CP | Datos de Entrada | Código HTTP Esperado | Schema JSON | Obs |
| :--- | :--- | :---: | :--- | :--- |
| CPF-API-01-001 | Todos los campos válidos, admin autenticado | 200 | `EventWithStatistics` (contiene `id`, `shortName`, `status`) | f+ |
| CPF-API-01-002 | Falta el campo `currency` en el body | 400 | `ValidationResult` con campo `errorDescriptor` | f- |
| CPF-API-01-003 | `availableSeats` = -1 | 400 | `ValidationResult` indicando valor inválido | f- |
| CPF-API-01-004 | Sin sesión / token inválido | 401 | Redirección o error de autenticación | f- |
| CPF-API-01-005 | `shortName` duplicado (ya existe un evento con ese nombre) | 400 | `ValidationResult` con error de unicidad | f- |

---

### Contrato de API – Listado de Eventos Públicos
| ID | CPF-API-02 |
| :--- | :--- |
| **Funcionalidad** | Consulta de evento público por nombre corto |
| **Descripción** | Valida que el endpoint `GET /api/v2/public/event/{name}` retorne la información del evento en formato JSON conformante al schema OpenAPI, y que gestione correctamente los casos de evento inexistente y evento no publicado. |
| **Requisito Asociado** | RF-RES-01 / Endpoint R1 del [[Plan-de-Pruebas-de-Integración]] |
| **Componente** | `EventApiV2Controller` |
| **Precondiciones** | Evento publicado disponible. |
| **Datos de Entrada** | Parámetro de ruta `{name}`: nombre corto del evento. |
| **Técnicas de Pruebas** | Prueba de Contrato de API, Partición de Equivalencia. |
| **Prioridad** | Alta |

**Tabla de Decisión: Códigos de Respuesta HTTP**
| Condición | C1 | C2 | C3 |
| :--- | :--- | :--- | :--- |
| ¿El evento existe? | SÍ | NO | SÍ |
| ¿El evento está publicado? | SÍ | — | NO |
| **Código HTTP Esperado** | **200 OK** | **404 Not Found** | **404 Not Found** |
| **Schema JSON Esperado** | `EventPublic` | Error JSON | Error JSON |

**Catálogo de Pruebas**
| #CP | Datos de Entrada | Código HTTP Esperado | Schema JSON | Obs |
| :--- | :--- | :---: | :--- | :--- |
| CPF-API-02-001 | `name` = nombre de evento publicado existente | 200 | `EventPublic` (contiene `shortName`, `title`, `currency`, `availableTicketsCount`) | f+ |
| CPF-API-02-002 | `name` = "evento-inexistente-xyz" | 404 | JSON con campo `type: NOT_FOUND` | f- |
| CPF-API-02-003 | `name` = nombre de evento en borrador (no publicado) | 404 | JSON con error de no disponibilidad | f- |

---

### Contrato de API – Reserva de Entradas (Pública)
| ID | CPF-API-03 |
| :--- | :--- |
| **Funcionalidad** | Iniciar reserva de entradas vía API pública |
| **Descripción** | Valida que el endpoint `POST /api/v2/public/event/{name}/reserve-tickets` procese correctamente la reserva, retorne el ID de reserva en el schema esperado y gestione los errores de cantidad inválida, categoría inexistente y cupo agotado. |
| **Requisito Asociado** | RF-RES-08 / Endpoint R3 del [[Plan-de-Pruebas-de-Integración]] |
| **Componente** | `EventApiV2Controller` |
| **Precondiciones** | Evento publicado con categorías y cupo disponible. |
| **Datos de Entrada** | Body JSON: `[{ "ticketCategoryId": N, "quantity": M }]` |
| **Técnicas de Pruebas** | Prueba de Contrato de API, Tabla de Decisión, Análisis de Valores Límite. |
| **Prioridad** | Crítica |

**Tabla de Decisión: Códigos de Respuesta HTTP**
| Condición | C1 | C2 | C3 | C4 |
| :--- | :--- | :--- | :--- | :--- |
| ¿`quantity` > 0? | SÍ | NO | SÍ | SÍ |
| ¿`ticketCategoryId` existe? | SÍ | — | NO | SÍ |
| ¿Hay cupo disponible suficiente? | SÍ | — | — | NO |
| **Código HTTP Esperado** | **200 OK** | **400 Bad Request** | **400 Bad Request** | **400 Bad Request** |
| **Cuerpo Esperado** | `{ reservationId, status }` | Error de validación | Error de categoría | Error de cupo insuficiente |

**Catálogo de Pruebas**
| #CP | Datos de Entrada | Código HTTP Esperado | Cuerpo JSON | Obs |
| :--- | :--- | :---: | :--- | :--- |
| CPF-API-03-001 | `quantity` = 1, `ticketCategoryId` válido, cupo disponible | 200 | `{ "reservationId": "...", "status": "SUCCESS" }` | f+ |
| CPF-API-03-002 | `quantity` = 0 | 400 | Error de validación | f- |
| CPF-API-03-003 | `quantity` = -1 | 400 | Error de validación | f- |
| CPF-API-03-004 | `ticketCategoryId` = 99999 (no existe) | 400 | Error indicando categoría no encontrada | f- |
| CPF-API-03-005 | `quantity` mayor al cupo disponible de la categoría | 400 | Error de cupo insuficiente | f- |

---

### Contrato de API – Check-In de Ticket (Admin)
| ID | CPF-API-04 |
| :--- | :--- |
| **Funcionalidad** | Registro de check-in de ticket por parte del operador |
| **Descripción** | Valida que el endpoint `POST /admin/api/check-in/event/{name}/ticket/{id}` retorne el resultado del check-in con el código HTTP y schema JSON correctos según el estado actual del ticket (válido, duplicado, no encontrado, no pagado). |
| **Requisito Asociado** | RF-010 / Endpoints C2 y E8 del [[Plan-de-Pruebas-de-Integración]] |
| **Componente** | `CheckInApiController` |
| **Precondiciones** | Evento activo. Operador de check-in autenticado. |
| **Datos de Entrada** | Parámetros de ruta: `{name}` (evento), `{id}` (UUID del ticket). Body: `{ "code": "...", "secret": "..." }`. |
| **Técnicas de Pruebas** | Prueba de Contrato de API, Tabla de Decisión. |
| **Prioridad** | Crítica |

**Tabla de Decisión: Códigos de Respuesta HTTP según estado del ticket**
| Condición | C1 | C2 | C3 | C4 |
| :--- | :--- | :--- | :--- | :--- |
| ¿El ticket existe? | SÍ | NO | SÍ | SÍ |
| ¿El ticket está pagado? | SÍ | — | NO | SÍ |
| ¿El ticket ya fue registrado (check-in previo)? | NO | — | — | SÍ |
| **Código HTTP Esperado** | **200 OK** | **404** | **200 OK** | **200 OK** |
| **Campo `result` en JSON** | `"OK"` | Error | `"INVALID_TICKET_STATE"` | `"ALREADY_CHECK_IN"` |

**Catálogo de Pruebas**
| #CP | Datos de Entrada | Código HTTP Esperado | Campo `result` en respuesta | Obs |
| :--- | :--- | :---: | :--- | :--- |
| CPF-API-04-001 | Ticket válido, pagado, sin check-in previo | 200 | `"OK"` | f+ |
| CPF-API-04-002 | UUID de ticket inexistente | 404 | — | f- |
| CPF-API-04-003 | Ticket en estado `PENDING` (no pagado) | 200 | `"INVALID_TICKET_STATE"` | f- |
| CPF-API-04-004 | Ticket ya registrado (segundo intento de check-in) | 200 | `"ALREADY_CHECK_IN"` | f- |

---

### Contrato de API – Estado de Reserva
| ID | CPF-API-05 |
| :--- | :--- |
| **Funcionalidad** | Consulta de estado de una reserva activa |
| **Descripción** | Valida que el endpoint `GET /api/v2/public/reservation/{id}/status` retorne el schema JSON correcto con el estado actual de la reserva, y que gestione errores para IDs inexistentes o inválidos. |
| **Requisito Asociado** | RF-RES-08 / Endpoint E1 del [[Plan-de-Pruebas-de-Integración]] |
| **Componente** | `ReservationApiV2Controller` |
| **Precondiciones** | Al menos una reserva creada. |
| **Datos de Entrada** | Parámetro de ruta `{id}`: UUID de la reserva. |
| **Técnicas de Pruebas** | Prueba de Contrato de API, Partición de Equivalencia. |
| **Prioridad** | Alta |
...
| Condición | C1 | C2 | C3 |
| :--- | :--- | :--- | :--- |
| ¿El ID de reserva existe? | SÍ | NO | — |
| ¿El formato del UUID es válido? | SÍ | SÍ | NO |
| **Código HTTP Esperado** | **200 OK** | **404 Not Found** | **400 Bad Request** |
| **Schema JSON Esperado** | `{ status, paymentProxy, validatedBooking }` | Error JSON | Error JSON |

**Catálogo de Pruebas**
| #CP | Datos de Entrada | Código HTTP Esperado | Schema JSON | Obs |
| :--- | :--- | :---: | :--- | :--- |
| CPF-API-05-001 | UUID de reserva existente en estado `PENDING` | 200 | `{ "status": "PENDING", "validatedBooking": false }` | f+ |
| CPF-API-05-002 | UUID de reserva existente en estado `COMPLETE` | 200 | `{ "status": "COMPLETE", "validatedBooking": true }` | f+ |
| CPF-API-05-003 | UUID inexistente (correctamente formado) | 404 | JSON con tipo `NOT_FOUND` | f- |
| CPF-API-05-004 | Cadena no-UUID (ej. `"abc"`) | 400 | JSON de error de formato | f- |

---

### Contrato de API – Validación de Entrada: Aplicar Código Promocional
| ID | CPF-API-06 |
| :--- | :--- |
| **Funcionalidad** | Aplicar código de descuento a una reserva activa |
| **Descripción** | Valida que el endpoint `POST /api/v2/public/reservation/{id}/apply-code` aplique correctamente un código promocional válido, y rechace con el código HTTP y mensaje apropiados los códigos inválidos, ya usados o expirados. |
| **Requisito Asociado** | RF-RES-12 / Endpoint E3 del [[Plan-de-Pruebas-de-Integración]] |
| **Componente** | `ReservationApiV2Controller` |
| **Precondiciones** | Reserva en estado `PENDING`. Código promocional configurado en el evento. |
| **Datos de Entrada** | Body JSON: `{ "code": "PROMO2026" }` |
| **Técnicas de Pruebas** | Prueba de Contrato de API, Tabla de Decisión, Partición de Equivalencia. |
| **Prioridad** | Alta |
...
| Condición | C1 | C2 | C3 | C4 |
| :--- | :--- | :--- | :--- | :--- |
| ¿Código existe en el sistema? | SÍ | NO | SÍ | SÍ |
| ¿Código dentro del período de validez? | SÍ | — | NO | SÍ |
| ¿Código no ha superado su límite de usos? | SÍ | — | — | NO |
| **Código HTTP Esperado** | **200 OK** | **400 Bad Request** | **400 Bad Request** | **400 Bad Request** |
| **Campo en respuesta** | `{ result: "OK", discountApplied: true }` | Error `INVALID_CODE` | Error `CODE_EXPIRED` | Error `MAX_USAGE_REACHED` |

**Catálogo de Pruebas**
| #CP | Datos de Entrada | Código HTTP Esperado | Cuerpo JSON | Obs |
| :--- | :--- | :---: | :--- | :--- |
| CPF-API-06-001 | Código válido, vigente, con usos disponibles | 200 | `{ "result": "OK", "discountApplied": true }` | f+ |
| CPF-API-06-002 | Código inexistente ("XXXXXXXX") | 400 | Error con descripción `INVALID_CODE` | f- |
| CPF-API-06-003 | Body vacío (sin campo `code`) | 400 | Error de validación de campo obligatorio | f- |
| CPF-API-06-004 | Código correcto pero expirado | 400 | Error con descripción `CODE_EXPIRED` | f- |
| CPF-API-06-005 | Código correcto pero con usos agotados | 400 | Error con descripción `MAX_USAGE_REACHED` | f- |

---

### Contrato de API – Webhook de Confirmación de Pago Stripe
| ID | CPF-API-07 |
| :--- | :--- |
| **Funcionalidad** | Procesamiento de webhook de pago confirmado de Stripe |
| **Descripción** | Valida que el endpoint `POST /api/payment/webhook/stripe/payment` procese correctamente los eventos de Stripe (`payment_intent.succeeded`) y rechace llamadas con firma inválida o payload malformado, retornando los códigos HTTP definidos en el contrato. |
| **Requisito Asociado** | RF-006 / Endpoint P3 del [[Plan-de-Pruebas-de-Integración]] |
| **Componente** | `StripePaymentWebhookController` |
| **Precondiciones** | Reserva en estado `PENDING_PAYMENT` asociada a un PaymentIntent de Stripe. Clave `stripe-webhook-secret` configurada. |
| **Datos de Entrada** | Header `Stripe-Signature`, body JSON del evento Stripe. |
| **Técnicas de Pruebas** | Prueba de Contrato de API, Tabla de Decisión. |
| **Prioridad** | Crítica |
...
| Condición | C1 | C2 | C3 | C4 |
| :--- | :--- | :--- | :--- | :--- |
| ¿Firma `Stripe-Signature` es válida? | SÍ | NO | SÍ | SÍ |
| ¿Tipo de evento reconocido por el sistema? | SÍ | — | NO | SÍ |
| ¿Reserva asociada al PaymentIntent existe? | SÍ | — | — | NO |
| **Código HTTP Esperado** | **200 OK** | **400 Bad Request** | **200 OK** | **200 OK** |
| **Efecto en el sistema** | Reserva pasa a `COMPLETE` | Rechazado | Sin efecto (ignorado) | Sin efecto (ignorado) |

**Catálogo de Pruebas**
| #CP | Datos de Entrada | Código HTTP Esperado | Efecto Esperado en Sistema | Obs |
| :--- | :--- | :---: | :--- | :--- |
| CPF-API-07-001 | Evento `payment_intent.succeeded` con firma válida y PaymentIntent vinculado | 200 | La reserva asociada cambia a estado `COMPLETE` | f+ |
| CPF-API-07-002 | Firma `Stripe-Signature` ausente o inválida | 400 | La reserva permanece sin cambios | f- |
| CPF-API-07-003 | Tipo de evento desconocido (`payment_intent.created`) con firma válida | 200 | El evento se ignora, sin cambio de estado | f+ |
| CPF-API-07-004 | Evento `payment_intent.succeeded` con PaymentIntent no vinculado a ninguna reserva | 200 | Sin efectos secundarios, respuesta vacía `{}` | f- |
...
| **RF-MAN-01 / A1:** Contrato API – Creación de Evento (Admin) | CPF-API-01 (001-005) |
| **RF-RES-01 / R1:** Contrato API – Listado de Eventos Públicos | CPF-API-02 (001-003) |
| **RF-RES-08 / R3:** Contrato API – Reserva de Entradas | CPF-API-03 (001-005) |
| **RF-010 / C2-E8:** Contrato API – Check-In de Ticket | CPF-API-04 (001-004) |
| **RF-RES-08 / E1:** Contrato API – Estado de Reserva | CPF-API-05 (001-004) |
| **RF-RES-12 / E3:** Contrato API – Aplicar Código Promocional | CPF-API-06 (001-005) |
| **RF-006 / P3:** Contrato API – Webhook Stripe | CPF-API-07 (001-004) |
