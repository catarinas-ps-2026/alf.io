...
- Name: integration/events/edit-event.spec.ts >> Events - Edit >> admin can edit an event's general information and the changes are saved
- Location: tests/integration/events/edit-event.spec.ts:14:9
...
Call log:
  - waiting for locator('#organizationId')
    - locator resolved to <select required="" id="organizationId" data-ng-animate="2" name="organizationId" data-ng-model="obj.organizationId" data-ng-options="organization.id as organization.name for organization in organizations" class="form-control ng-pristine ng-untouched ng-valid ng-valid-required-add ng-animate ng-not-empty-add ng-not-empty ng-valid-required ng-valid-required-add-active ng-not-empty-add-active">…</select>
